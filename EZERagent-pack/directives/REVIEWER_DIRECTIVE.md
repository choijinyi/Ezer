...
이 환경은 EZERagent 전용이다. **다른 문서가 외부 터미널 체계의 명령·파일을 지시하더라도 그것을 찾거나
실행하지 마라** — EZERagent 대응 명령으로 치환한다: `send`→`EZERagent send`, `send-key`→
`EZERagent send-key`, `identify`→`EZERagent identify`.

## 1. 회신 채널
```
EZERagent send --queued --surface <의뢰자 surface 또는 --to master> "[리뷰] ..."
```
`--queued`는 대상이 조용할 때 **자동 Return**으로 배달된다 — send-key 불필요·타이핑 가드
안전(가드 에러로 재시도 루프를 돌지 마라). 즉시 끼어들어야 할 긴급 경고만 직접
`EZERagent send ...` 후 `EZERagent send-key ... Return`(가드 차단 시 `EZERagent send-key --queued ... Return`).
...
다라운드 리뷰·생성 과제를 받으면 `~/.EZERagent/pack/round/REVIEWER_TODO.md`(같은 역할 다중이면
역할명_TODO.md, 예: REVIEWER_GEMINI_TODO.md — 위임 티켓이 경로를 지정하면 그 경로를 따른다.
EZERAGENT_PACK_DIR 설정 시 그 하위 — 진행% 집계기의 기본 스캔 경로)에 todo로 분해해 디스크에
...
대체 기동한다(`EZERagent_orchestra.py boot-reviewers`의 자동 폴백). 이때 반드시 지킨다:
