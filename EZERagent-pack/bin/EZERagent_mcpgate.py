#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""EZERagent_mcpgate — MCP 거버넌스 결정론 게이트 (stdlib 전용·SkillSpector MCP 규칙 포트).
...
least-privilege(LP1~4)는 EZERagent_skillscan이 담당(중복 구현 금지) — 여기선 안 한다.
stdlib만(re·json·unicodedata·hashlib). 점수 미노출 — enum verdict(REVIEWER_VERDICT §1).

사용:
  EZERagent_mcpgate.py scan <skill>                      # tool-poisoning(TP1~3) → verdict
  EZERagent_mcpgate.py snapshot <skill> [--store DIR]    # 승인본 manifest 스냅샷 저장
  EZERagent_mcpgate.py diff <skill> [--store DIR]        # rug-pull: 승인본 vs 현재 diff
  EZERagent_mcpgate.py --self-test
...
#   재발 방지). 관례 = $EZERAGENT_ROOT(env) 또는 CWD (EZERagent_wakeup.py:33 등과 동일).
DEFAULT_STORE = os.path.join(os.environ.get("EZERAGENT_ROOT") or os.getcwd(), "_round", "mcp_approved")
...
def _parse_manifest(text):
    """name/description/triggers/permissions 추출 (stdlib YAML-lite — EZERagent_skillscan 정합)."""
