#!/usr/bin/env python3
"""EZERagent_serena_eval.py — Serena crossover eval 하베스터 (S7 · 독립-노드 채점).

설계서: _research/Serena기반_EZERagent_업그레이드_구현설계서.md §S7.
역할: Serena 토큰절감 주장을 EZERagent repo **측정 사실**로 전환하고, 작은-편집 crossover rule 을
...
producer ≠ evaluator (file-level):
  ...
  - EVALUATOR = master 가 LOCKED 루브릭(EZERagent attest pin)으로 이 분석기를 직접 실행해 채점한다.
...
def self_test():
    ...
    print("EZERagent_serena_eval self-test OK "
