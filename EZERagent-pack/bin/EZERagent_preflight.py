#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""EZERagent_preflight — EZERagent 부트 결정론 프리플라이트 (절대지침의 기계 검증부).
...
사용:
    python3 EZERagent_preflight.py [--fix] [--json] [--skip <ID> ...]
...
CONTENT_PINS = {
    "MASTER_DIRECTIVE.md": [
        ...
        ("EZERagent_preflight", "부트 ⓪ 결정론 프리플라이트 편입"),
        ...
        ("EZERAGENT_TYPING_GUARD_SECS", "타이핑 가드 초수 명시(앵커1-c)"),
        ("양방향 소켓통신", "양방향 소켓 절대규칙(앵커3-A)"),
        ("5분 주기 진행% 보고", "5분 주기 주인님 보고(앵커3-A6)"),
        ("EZERagent_report.py", "진행% 결정론 산출기 편입(앵커3-A6)"),
        ("주기적 능동 점검", "능동 모니터링 강제(앵커3-B1)"),
        ("라운드 사이클 의무 단계", "라운드마다 master 주기 점검(앵커3-B5)"),
        ("EZERAGENT_IDLE_SECONDS", "idle 5분 임계 명시(앵커3-B3)"),
        ("EZERagent_route.py", "3단 사고 라우팅 결정론 엔진 편입(사고 모드 §1)"),
        ("기억 증류", "slow 종료 게이트 증류 의무(§10)"),
        ("EZERagent_memory.py", "증류 결정론 도구 편입(§10)"),
        ("4종 의무 노드", "LLM orchestrating 4노드 부트 의무(앵커4-1)"),
        ("EZERagent_orchestra.py", "LLM 오케스트레이션 결정론 도구 편입(앵커4)"),
        ...
        ("EZERagent schedule add", "축3 자기 웨이크업 고유 핀(앵커6 — next-action 다중출현 보완)"),
    ...
    "WORKER_DIRECTIVE.md": [
        ...
        ("EZERagent_memory.py", "slow 종료 게이트 증류 도구(§10)"),
...
# 6MB+ 개발 저장소라 클론 설치한다. 해석 순서: $EZERAGENT_HARNESS_HOME → ~/.EZERagent/harness-creator
# → ~/Desktop/EZERagent/EZERagent-harness-creator(로컬 원본).
HARNESS_REPO = "https://github.com/choijinyi/EZERagent-harness-creator"
...
SERENA_PROJECT = os.environ.get("EZERAGENT_SERENA_PROJECT") or os.path.expanduser("~/Desktop/EZERagent")  # 절대경로(노드 spawn·cwd 미보장)·env override·배포 SOT 개인경로 0
...
SERENA_STDIO_ARGS = [
    ...
    "--mode", "no-memories",              # S8: 메모리 tool 전부 drop(EZERagent_memory FileLock SOT 보호)
    "--enable-web-dashboard", "false",    # S4: stray 24282 리스너 제거(stdio 라이브니스=노드 자체)
    "--open-web-dashboard", "false",      # S4: 브라우저 spawn 방지
]

# EZERagent-video-creator 영상 자동제작 스킬(오너 제작 32종) — pack 임베드로 배포되고, C26이
...
# 충돌 정의: EZERagent가 배선된 프로필(우리 SessionStart hook 등록)과 **같은 프로필**에
...
FOREIGN_AGENT_OS = {
    "gstack": {
        ...
        "claude_md_markers": ("skills/gstack", "/gstack-upgrade", "/land-and-deploy",
                              "open-gstack-browser"),
        "hook_marker": "gstack",
        "guide": ("격리 수칙: ①EZERagent 프로필이 아닌 전용 CLAUDE_CONFIG_DIR로 이동 "
...
BINPIN_MARKERS = {
    "EZERagent_task.py": (
        ...
        ("EZERAGENT_TASK_EVIDENCE_GATE", "evidence env 안전밸브"),
        ("TERMINAL_FROM", "W2-1 전이표"),
        ("transition denied(2)", "전이표 집행 메시지"),
    ),
    "EZERagent_orchestra.py": (
...
def _EZERagent_hook_cmd(script_name):
    """Claude settings.json hook 명령 문자열(단일 진실 — 모든 등록부 공용).
    Windows: git-bash `bash`로 명시 호출 + **정슬래시 + 따옴표**. 미따옴표 역슬래시 경로는 bash가
    escape로 먹어 경로가 파괴되고(C:\\Users\\...\\hooks\\EZERagent-hook.sh → C:UsersEZERagent.EZERagent/packhooksEZERagent-hook.sh
...
def _prune_stale_hook_entries(arr, script_name, desired):
    ...
    엔트리를 제거한다. return (정리된 리스트, desired 존재 여부). 비-EZERagent 엔트리·타 스크립트·정상
...
def is_dept_pack():
    """부서/CEO pack 컨텍스트인가 — pack_dir이 기본(~/.EZERagent/pack)이 아니면 부서/CEO 데몬이다.
    ...
    default = os.path.join(os.path.expanduser("~"), ".EZERagent/pack")
    try:
...
def discover_claude_settings():
    """$HOME 직하 .claude*/settings.json 전부(존재 파일만·사전순) + EZERagent 계정 config dir.

    home-glob 부분은 EZERagent.rs와 동일 규칙(unchanged·isfile 게이트·사전순). 추가로 master 및
    ~/.EZERagent/claude 를 공유하는 claude-adapter 리뷰어가 기동하는 EZERagent 전용 config dir
    (${EZERAGENT_ACCOUNT_DIR:-dirname(pack_dir())/claude})의 settings.json을 마지막에 append한다 —
    ...
    found = []
    # 축A 근본복원(2026-06-30): 부서 데몬 컨텍스트(EZERAGENT_ACCOUNT_DIR이 부서 전용 dir·basename에
    ...
    _acct = os.environ.get("EZERAGENT_ACCOUNT_DIR")
    _acct_is_dept = bool(_acct and "dept-" in os.path.basename(os.path.normpath(_acct)))
    # R2 강화(2026-06-30): EZERAGENT_ACCOUNT_DIR 누락 엣지에서도 pack_dir이 pack-dept-* 면 부서로 판별 →
    ...
    if _acct_is_dept or _pack_is_dept:
        ...
        return []  # account_dir 미상 시: 글로벌 등록 절대 금지(누수방지 우선·부서 settings는 EZERagent-dept가 생성)
    # 2026-07-02 근본복원(temp-pack 누수): grill embed/스냅샷 하네스가 EZERAGENT_PACK_DIR=/tmp/snap_grill_* 로
    ...
    for n in sorted(names):
        if n == ".claude" or n.startswith(".claude-"):
            ...
            if os.path.isfile(p):
                found.append(p)
    # EZERagent 계정 config dir(master + ~/.EZERagent/claude 공유 claude-adapter 리뷰어) 포함.
    # env-first: dept 데몬은 EZERAGENT_ACCOUNT_DIR=~/.EZERagent/claude-<key>로 기동되므로 자기 dir이 정답
    # (하드코딩 ~/.EZERagent/claude는 dept 오타깃). 디렉터리 존재 시에만 포함(미기동 노드 config dir
    # 생성 방지). 파일 부재여도 포함 — _register_event_hook이 makedirs+create.
    try:
        account_dir = os.environ.get("EZERAGENT_ACCOUNT_DIR") or os.path.join(
...
class Preflight:
    ...
    # ── 공용 수리: EZERagent init-pack — 누락 항목 재설치 + 비수정 파일 신버전 갱신 + **수정된
    ...
    def repair_via_init_pack(self):
        ...
        EZERagent = shutil.which("EZERagent")
        if not EZERagent:
            self._init_pack_ran = False
            return False
        try:
            r = subprocess.run(
                [EZERagent, "init-pack", "--no-install-hook"],
    ...
    def c01_pack_dir(self):
        ...
        if self.fix and self.repair_via_init_pack() and os.path.isdir(d):
            self.add(cid, FIXED, "%s (EZERagent init-pack로 생성)" % d)
            return
        self.add(cid, FAIL, "%s 없음 — `EZERagent init-pack` 실행 필요" % d)
    ...
    def c03_content_pins(self):
        ...
        for f, pins in CONTENT_PINS.items():
            ...
            if self.skipped(cid):
                continue
            p = os.path.join(pack_dir(), "directives", f)
            # CEO 승격 상태(F1 보완 · 오너 A안 2026-07-14): EZERagent-dept ceo_promote가 기본 pack의
            ...
            if f == "MASTER_DIRECTIVE.md" and os.path.isfile(marker):
                ...
                if tpl is None or live != tpl:
                    self.add(cid, FAIL,
                             ...
                             "불일치(수동 편집 의심 · 정상화는 EZERagent-dept 조정 또는 ceo_demote)")
                    continue
            ...
            if lost:
                self.add(
                    cid, FAIL,
                    "%s에서 소실된 조항: %s — 템플릿 복원은 `EZERagent init-pack --force`"
    ...
    def _hook_registered(self, settings_path):
        ...
        desired = _EZERagent_hook_cmd("session-start.sh")
    ...
    def _register_hook(self, settings_path):
        ...
        cmd = _EZERagent_hook_cmd("session-start.sh")
    ...
    # stdin JSON이다. EZERagent-statusline.sh가 매 메시지마다 usage.report로 push해 pane 배지에
    ...
    def _statusline_registered(self, settings_path):
        ...
        if not (isinstance(sl, dict) and "EZERagent-statusline.sh" in sl.get("command", "")):
            return False
    ...
    def _register_statusline(self, settings_path):
        """statusLine 등록. 성공=None, 실패=사유 문자열. 기존 statusLine은 EZERAGENT_PREV_STATUSLINE로
        래핑해 체인 보존(덮어쓰기 금지) — _register_hook과 동일한 symlink 거부·파싱 거부·최초
        백업·원자적 쓰기 철학."""
        if os.path.islink(settings_path):
            return "symlink 거부(실파일만 허용): %s" % settings_path
        base = _EZERagent_hook_cmd("EZERagent-statusline.sh")   # 정슬래시+따옴표(Windows) / sh <abs>(unix)
        if os.path.isfile(settings_path):
            try:
                data = json.load(open(settings_path, encoding="utf-8"))
            except (OSError, ValueError) as e:
            ...
            if d:
                os.makedirs(d, exist_ok=True)
        # 기존 statusLine(우리 것이 아니면) → EZERAGENT_PREV_STATUSLINE로 보존 체인(사람용 줄 위임).
        prev = data.get("statusLine")
        prev_cmd = prev.get("command", "") if isinstance(prev, dict) else ""
        if prev_cmd and "EZERagent-statusline.sh" not in prev_cmd:
            cmd = "EZERAGENT_PREV_STATUSLINE=%s %s" % (shlex.quote(prev_cmd), base)
        else:
            cmd = base
    ...
    def c32_statusline(self):
        ...
        if self.skipped(cid):
            return
        script = os.path.join(pack_dir(), "hooks", "EZERagent-statusline.sh")
        if not os.path.isfile(script):
            if not (self.fix and self.repair_via_init_pack() and os.path.isfile(script)):
                self.add(cid, FAIL, "hooks/EZERagent-statusline.sh 없음 — `EZERagent init-pack` 또는 --fix")
                return
        targets = discover_claude_settings()
        if not targets:
    ...
    # PreToolUse/PostToolUse에 hooks/EZERagent-hook.sh 등록 → 툴·스킬·에이전트 호출·exit_code를
    # EZERagentd events 테이블에 적재(E3 스킬 TOP·반복실패 토대). hook은 fail-open(에이전트 무차단)이라
    # 무관 작업 불간섭. C32와 동일 규약(체인보존·symlink/파손 거부·원자적). FAIL 없음(미등록=WARN).
    EVENT_HOOK = "EZERagent-hook.sh"
    ...
    def c33_event_hooks(self):
        ...
        if not os.path.isfile(script):
            if not (self.fix and self.repair_via_init_pack() and os.path.isfile(script)):
                self.add(cid, WARN, "hooks/%s 없음 — `EZERagent init-pack` 또는 --fix" % self.EVENT_HOOK)
                return
        targets = discover_claude_settings()
        if not targets:
            self.add(cid, WARN, "~/.claude*/settings.json 미발견 — claude 노드 기동 후 재실행")
    ...
    # ── C11 EZERagent 바이너리 ──
    def c11_EZERagent_binary(self):
        cid = "C11.EZERagent-binary"
        if self.skipped(cid):
            return
        p = shutil.which("EZERagent")
        if p:
            self.add(cid, PASS, p)
        else:
            self.add(cid, FAIL, "PATH에 EZERagent 없음 — EZERagent 터미널 설치/PATH 확인 필요")

    # ── C11b EZERagent-dept PATH 노출 (Fix3' — CEO fan-out `EZERagent-dept list`가 풀경로 없이 동작) ──
    def c11b_EZERagent_dept_path(self):
        cid = "C11b.EZERagent-dept-path"
        if self.skipped(cid):
            return
        # EZERagent·EZERagentd는 컴파일 바이너리(scripts/deploy_gate.py가 target/release→/opt/homebrew/bin 복사)지만
        # EZERagent-dept는 팩 bash 스크립트(~/.EZERagent/pack/bin/EZERagent-dept)라 PATH에 없다 → CEO 디렉티브의
        # `EZERagent-dept list` fan-out이 풀경로 없이 실패한다(GUI/백엔드는 pack_dir()/bin 풀경로라 무관).
        # EZERagent 옆(=같은 PATH dir)에 팩 스크립트로의 심링크를 둬 노출한다 — 가역·멱등·자가치유(스킬 심링크와
        # 동일 규약: 실파일은 덮지 않음). 비가역 외부설치가 아니므로 may_mutate 불요(reversible local).
        src = os.path.join(pack_dir(), "bin", "EZERagent-dept")
        if not os.path.isfile(src):
            self.add(cid, WARN, "팩에 bin/EZERagent-dept 없음 — `EZERagent init-pack` 재실행")
            return
        existing = shutil.which("EZERagent-dept")
        if existing and os.path.realpath(existing) == os.path.realpath(src):
            self.add(cid, PASS, existing)
            return
        EZERagent = shutil.which("EZERagent")
        if not EZERagent:
            self.add(cid, SKIP, "EZERagent 부재로 PATH dir 판정 불가 (C11 먼저)")
            return
        link = os.path.join(os.path.dirname(EZERagent), "EZERagent-dept")  # EZERagent와 같은 PATH dir
        ...
        if self.fix:
            try:
                # PATH 해소(which/셸)는 대상이 실행가능해야 한다 — 팩 EZERagent-dept가 0644면 심링크해도
                # `EZERagent-dept`가 안 잡힌다. 실행비트를 보강(가역·멱등)한 뒤 심링크한다(Fix1' 실행비트 의존 해소).
            ...
            self.add(cid, WARN, "EZERagent-dept PATH 미노출 — --fix로 %s 심링크 생성" % link)

    # ── C12 EZERagentd 데몬 생존 ──
    def c12_daemon(self):
        cid = "C12.daemon"
        if self.skipped(cid):
            return
        EZERagent = shutil.which("EZERagent")
        if not EZERagent:
            self.add(cid, SKIP, "EZERagent 부재로 판정 불가 (C11 먼저)")
            return

        def ping():
            try:
                return subprocess.run(
                    [EZERagent, "ping"], capture_output=True, timeout=5
                ).returncode == 0
            except Exception:
                return False

        if ping():
            self.add(cid, PASS, "EZERagent ping OK")
            return
        EZERagentd = shutil.which("EZERagentd")
        if self.fix and EZERagentd:
            log = open("/tmp/EZERagentd-preflight.log", "ab") if os.name == "posix" else subprocess.DEVNULL
            subprocess.Popen(
                [EZERagentd], stdout=log, stderr=subprocess.STDOUT,
                start_new_session=True,
            )
            for _ in range(10):
                time.sleep(0.5)
                if ping():
                    self.add(cid, FIXED, "EZERagentd 기동 후 ping OK")
                    return
            self.add(cid, FAIL, "EZERagentd 기동 시도했으나 ping 실패 — /tmp/EZERagentd-preflight.log 확인")
        else:
            self.add(cid, FAIL, "데몬 다운 — `EZERagentd > /tmp/EZERagentd.log 2>&1 &` 후 재실행 (--fix로 자동 기동 가능)")
    ...
    def c14_self(self):
        ...
        if self.skipped(cid):
            return
        p = os.path.join(pack_dir(), "bin", "EZERagent_preflight.py")
        if os.path.isfile(p):
            self.add(cid, PASS, p)
            return
        if self.fix:
            ...
            self.add(cid, FAIL, "pack/bin/EZERagent_preflight.py 없음 — `EZERagent init-pack` 또는 --fix")

    # ── C15 진행% 보고기 EZERagent_report.py (앵커3-A6) ──
    def c15_report_tool(self):
        cid = "C15.report-tool"
        if self.skipped(cid):
            return
        p = os.path.join(pack_dir(), "bin", "EZERagent_report.py")
        if not os.path.isfile(p):
            if self.fix and self.repair_via_init_pack() and os.path.isfile(p):
                self.add(cid, FIXED, "EZERagent_report.py 재설치")
            else:
                self.add(cid, FAIL, "pack/bin/EZERagent_report.py 없음 — `EZERagent init-pack` 또는 --fix")
            return
        self.add(cid, PASS, p)
    ...
    def c16_report_schedule(self):
        ...
        # 결정론 환원: text_command(데몬이 EZERagent_report 실행)가 권장이나, text도 허용한다.
        def is_report(j):
        ...
        if self.fix:
            jobs.append({
                ...
                "text_command": ('printf \'[heartbeat] 5분 보고 — 아래 진행%%는 결정론 산출값이다. '
                                 ...
                                 'python3 "${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_report.py"'),
                "if_absent": "skip",
            })
            data["jobs"] = jobs
    ...
    def _check_bin_tool(self, cid, fname, extra_files=()):
        ...
        if missing and self.fix and self.repair_via_init_pack():
            missing = [f for f in missing
                       if not os.path.isfile(os.path.join(pack_dir(), "bin", f))]
        if missing:
            self.add(cid, FAIL, "pack/bin 누락: %s — `EZERagent init-pack` 또는 --fix"
                     % ", ".join(missing))
            return None
    ...
    def c17_route_engine(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_route.py",
    ...
    def c19_orchestra_engine(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_orchestra.py")
        if p:
    ...
    def c41_skillscan(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_skillscan.py",
    ...
    def c42_mcpgate(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_mcpgate.py")
        if p:
    ...
    def c34_registry(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_registry.py")
        if p:
    ...
    def c35_select(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_select.py")
        if p:
    ...
    def c36_verdict(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_verdict.py")
        if p:
    ...
    def c37_adr_engine(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_adr.py")
        if p:
    ...
    # 단일 source-of-record를 보존하려 렌더/드리프트 판정은 EZERagent_orchestra에 위임(subprocess).
    def c38_silent_failure_catalog(self):
        cid = "C38.silent-failure-catalog"
        if self.skipped(cid):
            return
        orch = os.path.join(pack_dir(), "bin", "EZERagent_orchestra.py")
        cat = os.path.join(pack_dir(), "round", "SILENT_FAILURE_CATALOG.md")
        if not os.path.isfile(orch):
            self.add(cid, WARN, "EZERagent_orchestra.py 부재 — 무음실패 카탈로그 검사 보류")
            return
        try:
            if self.fix:
            ...
            if r.returncode == 0:
                ...
                self.add(cid, WARN, "무음실패 카탈로그 드리프트/부재 — `EZERagent_preflight.py --fix` 또는 "
                         "`EZERagent_orchestra.py silent-failure-catalog`로 재생성")
        except Exception as e:
    ...
    def c39_prereq_orphan_lint(self):
        ...
        if self.skipped(cid):
            return
        reg = os.path.join(pack_dir(), "bin", "EZERagent_registry.py")
        if not os.path.isfile(reg):
            self.add(cid, WARN, "EZERagent_registry.py 부재 — 전제지식 고아 lint 보류")
            return
        try:
    ...
    def c40_workflow_manifest(self):
        ...
        if self.skipped(cid):
            return
        p = os.path.join(pack_dir(), "bin", "EZERagent_manifest.py")
        if not os.path.isfile(p):
            self.add(cid, WARN, "EZERagent_manifest.py 부재 — 타입드 워크플로우 매니페스트 미설치(opt-in·README 폴백)")
            return
        try:
            r = subprocess.run([sys.executable, p, "--self-test"],
                               capture_output=True, timeout=30, env=_utf8_env())
        except Exception as e:
            self.add(cid, WARN, "EZERagent_manifest.py --self-test 실행 불가 — 보류: %s" % e)
            return
        if r.returncode == 0:
            self.add(cid, PASS, "EZERagent_manifest.py self-test OK (매니페스트 계약·무점수·콘텐츠 checks·exit 4 폴백)")
        else:
            tail = (r.stdout or r.stderr or b"").decode("utf-8", "replace").strip()
            self.add(cid, WARN, "EZERagent_manifest.py self-test 실패(도구 점검 필요) — %s" % tail[-200:])
    ...
    def c18_memory_engine(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_memory.py")
        if not p:
            return
        ...
        try:
            ...
            self.add(cid, FAIL, "EZERagent_memory verify 실행 불가: %s" % e)
            return
        if r.returncode == 0:
    ...
    def _serena_nodes():
        ...
        # $EZERAGENT_WORKER_PROFILE_DIR → ~/.EZERagent/worker-profile-dir 파일(1줄) → ~/.claude-worker 기본
        wp = os.environ.get("EZERAGENT_WORKER_PROFILE_DIR", "")
        if not wp:
            try:
                wp = open(os.path.expanduser("~/.EZERagent/worker-profile-dir"), encoding="utf-8").read().strip()
            except OSError:
                wp = ""
        wp = os.path.expanduser(wp or "~/.claude-worker")
        return [("master", os.path.expanduser("~/.EZERagent/claude/.claude.json"), False),
    ...
    def _serena_enable_approved():
        ...
        if os.environ.get("EZERAGENT_SERENA_ENABLE_APPROVED") == "1":
            return True
        return os.path.isfile(os.path.expanduser("~/.EZERagent/state/serena-enable-approved"))
    ...
    def _serena_reviewer_gap():
        ...
        yml = os.path.join(pack_dir(), "resources", "contexts", "EZERagent-codex-readonly.yml")
        miss = []
        if not (os.path.isfile(toml)
                and "serena-ro" in open(toml, encoding="utf-8", errors="replace").read()):
            miss.append("codex serena-ro 미등록")
        if not os.path.isfile(yml):
            miss.append("EZERagent-codex-readonly.yml 부재")
    ...
    def _serena_governance_gap(self):
        ...
        miss = []
        probe = os.path.join(pack_dir(), "bin", "EZERagent_serena_probe.py")
        ...
        try:
            ...
            if not any(j.get("id") == "serena-heartbeat" for j in jobs):
                miss.append("serena-heartbeat job 부재(EZERagent schedule add — 사람단계)")
        except (OSError, ValueError):
    ...
    def c43_serena(self):
        ...
        if not uvx:
            ...
            return
        # (b) MCP 등록 — 등록 .mcp.json 디렉터리 = enable-key root 일치 필수(§1.4). EZERagent는
        ...
        if gaps:
            self.add(cid, WARN,
                     "serena 등록됨%s · 미활성: %s — 사람 단계(노드 .claude.json 편집·EZERagent feed push 승인)"
                     % (tail, "; ".join(gaps)))
            return
        self.add(cid, FIXED if fixed else PASS, "serena(uvx) · 등록+활성 OK%s" % tail)

    # ── C44 Serena crossover eval 하베스터 게이트 (S7) ──
    # 분석기 EZERagent_serena_eval.py 의 --self-test(기계 게이트)는 FAIL 할 수 있으나(코드 결함),
    ...
    def c44_serena_eval(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_serena_eval.py")
        if not p:
        ...
        if unpinned:
            self.add(cid, WARN,
                     ...
                     "— 측정 선행=worker serena 마운트(human-hold) + master EZERagent attest pin" % len(unpinned))
            return
    ...
    # _check_bin_tool 아님: 그건 부재·self-test 실패를 FAIL로 만든다. EZERagent_semver.py 는 순수
    ...
    def c45_semver_selftest(self):
        ...
        if self.skipped(cid):
            return
        p = os.path.join(pack_dir(), "bin", "EZERagent_semver.py")
        if not os.path.isfile(p):
            self.add(cid, WARN, "EZERagent_semver.py 부재 — strictly-newer 버전 비교 advisory 미설치"
        ...
        try:
            ...
            self.add(cid, WARN, "EZERagent_semver.py --self-test 실행 불가 — 보류: %s" % e)
            return
        if r.returncode == 0:
            self.add(cid, PASS, "EZERagent_semver.py self-test OK (strictly-newer 불변식 박제·"
                     "main-ahead 거부·fail-safe·무점수·advisory only)")
        else:
            tail = (r.stdout or r.stderr or b"").decode("utf-8", "replace").strip()
            self.add(cid, WARN, "EZERagent_semver.py self-test 실패(도구 점검 필요) — %s" % tail[-200:])
    ...
    # EZERagent_channels.py 가 배선됐고 self-test(네트워크0·집계/verdict/permutation/429비종결/tier
    ...
    def c49_channel_health(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_channels.py")
        if p:
    ...
    # EZERagent_channel_watch.py 배선·self-test(네트워크0·diff/2-strike/silence-first/snapshot
    # round-trip 박제) 결정론 검증. 채널건강≠노드건강(EZERagent_report 와 별개 층위·중복 회피).
    def c50_channel_watch(self):
        cid = "C50.channel-watch"
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_channel_watch.py")
        if p:
    ...
    # EZERagent_cleanroom.py 의 self-test 가 vendor 변이검증(1바이트 tamper→DRIFTED·삭제→MISSING·
    ...
    def c51_cleanroom_vendor(self):
        ...
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_cleanroom.py")
        if p:
    ...
    # 동일 EZERagent_cleanroom.py self-test 가 라이선스 변이검증(MIT vendored=ACCEPT·AGPL embed=
    ...
    def c52_license_gate(self):
        ...
        if self.skipped(cid):
            return
        p = os.path.join(pack_dir(), "bin", "EZERagent_cleanroom.py")
        if not os.path.isfile(p):
            self.add(cid, FAIL, "EZERagent_cleanroom.py 부재 — C51 과 동일 도구(`EZERagent init-pack`)")
            return
        # C51 이 이미 self-test 를 돌렸으므로 여기선 존재만 재확인(중복 subprocess 회피·외과적).
        self.add(cid, PASS, "EZERagent_cleanroom.py license self-test 박제 OK (MIT=ACCEPT·AGPL embed="
                 "ESCALATE·unknown SPDX=BLOCK·정규화) — AGPL copyleft 추적(오너 승인·ESCALATE 큐잉)")

    # ── C53 관찰 명령 부작용 금지 멱등성 봉인 (AGENTREACH OPP-21) ──
    # EZERagent_idempotency.py self-test 가 spy/AST 배터리를 결정론 실행: cmd_check 관찰멱등
    # (calls∩MUTATE=∅ negative assertion)·C12.daemon fix=False Popen 0·coverage_battery
    # 관찰전용(POST/--cookies/yt-dlp 다운로드 토큰 AST 부재)·표면커버리지(EZERagent actions⊆OBSERVE∪MUTATE).
    def c53_idempotency(self):
        cid = "C53.idempotency"
        if self.skipped(cid):
            return
        p = self._check_bin_tool(cid, "EZERagent_idempotency.py")
        if p:
    ...
    def _harness_root():
        cands = []
        env = os.environ.get("EZERAGENT_HARNESS_HOME", "")
        if env:
            cands.append(env)
        home = os.path.expanduser("~")
        cands.append(os.path.join(home, ".EZERagent/harness-creator"))
        cands.append(os.path.join(home, "Desktop/EZERagent/EZERagent-harness-creator"))
        for d in cands:
    ...
    def c21_harness_creator(self):
        ...
        if root:
            ...
            return
        dst = os.path.join(os.path.expanduser("~"), ".EZERagent/harness-creator")
    ...
    def _skill_indexable(self, name):
        ...
        if not (os.path.isfile(p) and os.path.getsize(p) > 0):
            return False
        # 실파서(EZERagent.rs compose_directive)는 read_to_string이라 전 파일 UTF-8 유효 +
    ...
    def c22_work_skills(self):
        ...
        if problems:
            self.add(cid, FAIL,
                     "work 스킬 결함: %s — `EZERagent init-pack` 또는 --fix"
                     "(파일이 존재하되 깨진/약화된 경우 init-pack은 보존한다 — "
                     "`EZERagent init-pack --force`로 템플릿 복원, 사용자 수정 덮어씀 주의)"
                     % "; ".join(problems))
        elif repaired:
    ...
    def c23_governance_conflict(self):
        ...
        for settings_path in discover_claude_settings():
            ...
            # 충돌 조건 = EZERagent 배선 프로필(우리 hook 등록)과의 '동거'만
        ...
        if findings:
            ...
            self.add(cid, PASS, "EZERagent 배선 프로필에 외부 운영체계 점유 신호 없음")
    ...
    def _index_registered(itext):
        """색인 등재 판정 — EZERagent_memory verify의 index_links와 동일 기준(링크 타깃,
    ...
    def c25_autopilot_memory(self):
        ...
        if not os.path.isfile(fpath):
            if self.fix and self.repair_via_init_pack() and os.path.isfile(fpath):
                ...
                self.add(cid, FAIL, "memory/%s 없음 — `EZERagent init-pack` 또는 --fix"
        ...
        if lost:
            self.add(cid, FAIL, "자율주행 메모리 본문 핀 소실: %s — `EZERagent init-pack --force`"
        ...
        if not self._index_registered(itext):
            if not self.fix:
                ...
                return
            # EZERagent_memory와 동일한 lock 규약(O_CREAT|O_EXCL + stale 회수)으로 색인 1줄
            ...
            for _ in range(2):
                try:
                    ...
                    try:  # 죽은 프로세스의 만료 잠금(30초+)은 회수한다 — EZERagent_memory와 동일
    ...
    # ── C26 영상 자동제작 스킬(EZERagent-video-creator) 기본 탑재 ──
    ...
    def _appbuild_hook_registered(settings_path):
        ...
        if not isinstance(data, dict):
            return False
        desired = _EZERagent_hook_cmd(APPBUILD_HOOK)
    ...
    def _register_appbuild_hook(self, settings_path):
        ...
        cmd = _EZERagent_hook_cmd(APPBUILD_HOOK)
        data = {}
        if os.path.isfile(settings_path):
            try:
                data = json.load(open(settings_path, encoding="utf-8"))
    ...
    def _event_hook_registered(settings_path, event, script_name):
        ...
        if not isinstance(data, dict):
            return False
        desired = _EZERagent_hook_cmd(script_name)
    ...
    def _register_event_hook(self, settings_path, event, script_name, matcher=None):
        ...
        if os.path.islink(settings_path):
            return "symlink 거부: %s" % settings_path
        cmd = _EZERagent_hook_cmd(script_name)
        data = {}
        if os.path.isfile(settings_path):
            try:
                data = json.load(open(settings_path, encoding="utf-8"))
    ...
    def c28_self_correction(self):
        ...
        if self.skipped(cid):
            return
        fixed, warns = [], []
        # (a) hook 스크립트 4종 + EZERagent_reflect.py 존재·실행권한
        rels = [os.path.join("hooks", s) for s, _ in SELFCORR_HOOKS]
        rels.append(os.path.join("bin", "EZERagent_reflect.py"))
        for rel in rels:
    ...
    # EZERagent 마스터는 전용 CLAUDE_CONFIG_DIR(~/.EZERagent/claude)로 격리 기동돼 사용자 ~/.claude 의
    ...
    def c31_config_isolation(self):
        ...
        if not os.path.isfile(router):
            self.add(cid, WARN,
                     "EZERagent 전용 config dir 라우터 부재(%s) — `EZERagent init-pack` 재실행 권장 "
        ...
        for n in entries:
            for fn in ("CLAUDE.md", "soul.md", "CSO_DIRECTIVE.md", "MASTER_DIRECTIVE.md"):
                ...
                try:
                    ...
                    continue
                # EZERagent 치환 선언("cmux 아님"/"치환")이 있으면 신체계 — 오염 아님
                if cmux_cmd.search(t) and ("cmux 아님" not in t) and ("치환" not in t):
                    contaminated.append(p)
        if contaminated:
            self.add(cid, WARN,
                     "사용자 프로필에 외부 터미널 체계/구 지침 %d건 감지 — EZERagent는 전용 config dir로 격리돼 "
                     "영향 없으나, 정리하려면 **백업 후 직접 제거**(EZERagent는 자동삭제 안 함): %s"
                     % (len(contaminated), ", ".join(contaminated[:3])))
            return
        self.add(cid, PASS, "격리 config dir 라우터 설치됨 · 사용자 프로필 외부 체계 오염 없음")

    # ── C54 god-file 회귀 방지 LOC-cap (EZERagentd 코어 파일 줄수 ceiling 감시) ──
    def c54_loc_cap(self):
        cid = "C54.loc-cap"
        if self.skipped(cid):
            return
        # EZERagent 소스 경로 추정: EZERAGENT_REPO_DIR env 우선, 없으면 관용 경로 후보.
        # 경로 부재면 SKIP(이 preflight는 pack 부트용 — repo 부재 환경에서 FAIL 금지).
        repo = os.environ.get("EZERAGENT_REPO_DIR")
        candidates = [repo] if repo else []
        candidates += [os.path.join(os.path.expanduser("~"), "dev", "EZERagent")]
        root = next((c for c in candidates if c and os.path.isdir(
            os.path.join(c, "src", "bin", "EZERagentd"))), None)
        if not root:
            self.add(cid, SKIP, "EZERagent 소스 경로 미발견(EZERAGENT_REPO_DIR 미설정·repo 부재) — pack 단독 부트 정상")
            return
        # 대상 파일별 ceiling — 회귀 경보용 초기 상한(현재값+여유). 순수화로 줄면 따라 낮춰
        # god-file을 한 방향으로 압박(후행: ceiling 점진 강화). 형식: (모듈상대경로, ceiling).
        caps = [
            ("src/bin/EZERagentd/handlers.rs", 5300),
            ("src/bin/EZERagentd/governance.rs", 2000),
            ("src/bin/EZERagentd/state.rs", 2700),
        ]
        over = []
        for rel, cap in caps:
        ...
        if over:
            ...
            self.add(cid, PASS, "EZERagentd 코어 파일 LOC-cap 이내(%d개 감시)" % len(caps))
    ...
    def c55_grill_gate(self):
        ...
        if not os.path.isfile(engine):
            fails.append("엔진 %s 미설치 — `EZERagent init-pack`" % GRILL_ENGINE)
        else:
            try:
        ...
        for hname, hevent, hmatcher in GRILL_HOOKS:
            ...
            if not os.path.isfile(hook):
                fails.append("hook %s 미설치 — `EZERagent init-pack`" % hname)
                continue
            if os.name == "posix":
    ...
    # ── C58 트러스트 하드닝 (개인 alias 프로필 config가 EZERagent 워크스페이스를 자동 신뢰) ──
    # 배경(실측): 개인 alias(claude-<profile> 등·config=~/.claude-<profile>)로 Claude 기동 시
    #   그 config의 .claude.json에서 EZERagent 워크스페이스 hasTrustDialogAccepted=False/부재면
    #   시작 시 "Ignoring N permissions.allow entries … workspace has not been trusted" 경고가
    #   flash하고 permissions.allow가 무시된다. 패키지 표준 경로(launch-agent→~/.EZERagent/claude)는
    ...
    # ★보안 스코프(절대·티켓 §②): EZERagent가 관리하는 워크스페이스에만 신뢰 세팅 — 임의
    #   워크스페이스 blanket 신뢰 금지(신뢰 다이얼로그는 악성 .claude/settings.local.json 방어
    #   장치라 남용 시 보안구멍). 2중 스코프: ①대상 config = EZERagent 배선 프로필(우리 hook
    #   등록)의 .claude.json만 ②대상 워크스페이스 = 결정론 EZERagent 마커(_round/ 시스템 폴더 +
    #   CLAUDE.md의 EZERagent 마커 토큰) 보유 project 항목만. 마커 없는 항목·stale 경로는 무변경.
    ...
    EZERAGENT_WS_CLAUDEMD_MARKERS = ("EZERagent ", "EZERagent", "EZERagent", "claim-role")

    def _is_EZERagent_workspace(self, path):
        """결정론 판정 — 이 워크스페이스 경로가 EZERagent 관리 대상인가.
        마커: ①_round/ 시스템 폴더 존재 AND ②CLAUDE.md 존재 + EZERagent 마커 토큰 포함.
        ...
        try:
            ...
            return any(m in text for m in self.EZERAGENT_WS_CLAUDEMD_MARKERS)
        except OSError:
            return False

    def _trust_gap_workspaces(self, config_path):
        """읽기전용 탐지 — .claude.json 미변경. 신뢰 갭(EZERagent 워크스페이스인데
        ...
        for ws, ent in (data.get("projects", {}) or {}).items():
            if not isinstance(ent, dict):
                continue
            if not self._is_EZERagent_workspace(ws):
                continue
    ...
    def _set_workspace_trust(self, config_path, workspaces):
        """denylist write — 지정 EZERagent 워크스페이스 항목의 hasTrustDialogAccepted만 True.
    ...
    def c58_trust_harden(self):
        ...
        if self.skipped(cid):
            return
        # 대상 config = EZERagent 배선 프로필(우리 hook 등록)의 .claude.json만(스코프 ①).
        targets = []
        for settings_path in discover_claude_settings():
            if not self._hook_registered(settings_path):
                continue
            ...
            if os.path.isfile(cfg):
                targets.append(cfg)
        if not targets:
            self.add(cid, PASS, "EZERagent 배선 프로필 .claude.json 없음 — 트러스트 대상 없음")
            return
        set_lines = []
        gap_lines = []
        for cfg in targets:
        ...
        if set_lines:
            detail = "EZERagent 워크스페이스 트러스트 세팅 — " + " | ".join(set_lines)
            if gap_lines:
            ...
            self.add(cid, PASS,
                     "EZERagent 워크스페이스 트러스트 OK(갭 없음 · %d config 점검)" % len(targets))
    ...
    #   공급 경로: ①env EZERAGENT_GUARD_ROLE_PROFILES(콤마구분) ②<pack>/guard-profiles.txt(로컬 파일 —
    #   리포·임베드 미포함, 오너 머신 전용) ③둘 다 없으면 검증 대상 없음 skip(PASS) — 소비자
    #   설치본엔 역할 프로필 자체가 없어 의미 동일.
    @staticmethod
    def _guard_role_profiles():
        env = os.environ.get("EZERAGENT_GUARD_ROLE_PROFILES", "")
    ...
    def c60_gate_wiring(self):
        ...
        r = subprocess.run(
            [sys.executable, "-c",
             "import sys; sys.path.insert(0, %r); import EZERagent_memory as m; "
        ...
        acked_note = ""
        try:
            scan_tool = os.path.join(pack_dir(), "bin", "EZERagent_skillscan.py")
            r = subprocess.run([sys.executable, scan_tool, "all", "--json"],
                               capture_output=True, text=True, timeout=120)
            data = json.loads(r.stdout or "{}")
            blocked = data.get("blocked") or []
            if blocked:
                ack_p = os.path.join(os.environ.get("EZERAGENT_ROOT") or os.getcwd(),
        ...
        store = os.path.join(os.environ.get("EZERAGENT_ROOT") or os.getcwd(), "_round", "mcp_approved")
        ...
        if not snaps:
            ...
            for f in snaps[:10]:
                ...
                r = subprocess.run(
                    [sys.executable, os.path.join(pack_dir(), "bin", "EZERagent_mcpgate.py"),
    ...
    def c61_doc_code_sot(self):
        ...
        if self.skipped(cid):
            return
        root = os.environ.get("EZERAGENT_ROOT") or os.getcwd()
    ...
    def c62_pack_heal_ledger(self):
        ...
        self.add(cid, WARN, "; ".join(parts) + " — `EZERagent pack-merge`로 검토(가치 있는 수정은 vendor 승격 제보)")

    # ── C63 팩 bin 기능 마커 트립와이어 (attention-p0 · 2026-07-14 박사님 승인) ──
    # 승격·자가치유가 파일을 구버전으로 덮어 기능이 '무음 소실'되는 실패 계급(2회 실증:
    # EZERagent_task W2 소실 2026-07-06 · pack-heal 원복)의 외부 수호자. self-test는 피검증
    ...
    # EZERagent_directive_bench.py(07-06 도입)가 어디에도 배선되지 않아 회귀(CEO 교체로 인한
    ...
    def c64_directive_bench(self):
        ...
        if self.skipped(cid):
            return
        tool = os.path.join(pack_dir(), "bin", "EZERagent_directive_bench.py")
        if not os.path.isfile(tool):
            self.add(cid, WARN, "EZERagent_directive_bench.py 부재 — 회귀 트립와이어 비활성")
            return
    ...
    def run(self):
        ...
        checks = [
            ...
            self.c10_todo_files, self.c11_EZERagent_binary, self.c11b_EZERagent_dept_path,
...
def main():
    ap = argparse.ArgumentParser(description="EZERagent 결정론 부트 프리플라이트")
