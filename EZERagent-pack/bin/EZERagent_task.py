#!/usr/bin/env python3
"""EZERagent_task.py — P0-1 원자적 태스크 체크아웃 (Paperclip 계약의 클린룸 파일 포트)
...
- 태스크가 done이 되면, 그 태스크를 기다리던 태스크들 중 blocker가 전부 해소된 것을 보고한다
  (wakeup 큐 연동은 EZERagent_wakeup.py 몫 — 여기선 unblocked 목록 출력만).

상태 저장: $EZERAGENT_ROOT/_round/tasks/<id>.json (쓰기는 temp+os.replace 원자적)
락:        $EZERAGENT_ROOT/_round/tasks/<id>.lock/ (디렉터리 = mkdir 원자성) + owner.json
...
ROOT = os.environ.get("EZERAGENT_ROOT") or os.getcwd()  # 개인경로 하드코딩 금지(pack scan gate) — env 또는 CWD(워크스페이스 루트에서 호출)
...
def _pid_alive(pid):
    """run-liveness: pid 생존 확인. 테스트 주입용 EZERAGENT_TASK_LIVENESS=alive|dead 지원."""
    override = os.environ.get("EZERAGENT_TASK_LIVENESS")
    if override == "alive":
        return True
    if override == "dead":
        return False
...
def _settle_probe(owner):
    """1회 관측 → (idle_secs|None, todo_mtime|None). EZERAGENT_SETTLE_PROBE로 결정론 주입 가능
    (형식 "idle:mtime;idle:mtime" — 호출마다 하나 소비 · wakeup EZERAGENT_WAKEUP_LIVENESS 관례)."""
    global _PROBE_IDX
    seq = os.environ.get("EZERAGENT_SETTLE_PROBE")
    if seq is not None:
        parts = seq.split(";")
        ...
        try:
            ...
            return None, None
    try:
        import EZERagent_boot_node as _bn  # 형제 모듈(orchestra:194 관례) — 부재 시 관측불가=fail-closed
        st = _bn.EZERagent_status()
...
def _completion_settled(owner):
    ...
    반환 (ok, 사유)."""
    settle = float(os.environ.get("EZERAGENT_SETTLE_SEC", "5"))
    idle_min = float(os.environ.get("EZERAGENT_SETTLE_IDLE_MIN", "4"))
    last_mt = None
...
def cmd_set_status(a):
    ...
    # 기본 strict, EZERAGENT_TASK_EVIDENCE_GATE=warn|off 안전밸브.
    ev_text = (getattr(a, "evidence", None) or "").strip()
    skip_text = (getattr(a, "skip_reason", None) or "").strip()
    if a.status == "done":
        mode = os.environ.get("EZERAGENT_TASK_EVIDENCE_GATE", "strict")
    ...
    with _wlock(a.id):  # ★WP-8(P-ORCH): JSON read-modify-write 직렬화 — 동시 mutator lost-update 차단
        ...
        # 어떤 변이보다 먼저 검사(거부 시 무변이 반환). EZERAGENT_TASK_TRANSITION_GATE=warn|off 안전밸브.
        if a.status in TERMINAL_STATUSES and task.get("status") not in TERMINAL_FROM:
            tmode = os.environ.get("EZERAGENT_TASK_TRANSITION_GATE", "strict")
            if tmode == "strict":
        ...
        if a.status == "done":
            result["unblocked"] = _find_unblocked_dependents(a.id)  # → EZERagent_wakeup enqueue 대상
            ...
            print("handoff: _round/handoffs/%s-done.md 5필드 기록 권장(HANDOFF_CONTRACT)" % a.id,
                  file=sys.stderr)
            print("evidence에 전사 통계 첨부 권장: EZERagent_transcript_stats.py --latest --oneline "
