...
# -*- coding: utf-8 -*-
"""
EZERagent_phoenix.py — 불사조(무손실 복원) Phase 2: 부활 저널 상태머신 MVP (M1 단일 게이트)

설계 근거: _round/ZERO_LOSS_RESTORE_DESIGN.md §9.4-2 · §11(M1/M5/M9) · §10(운영 수칙)
원칙(P2 재사용 제1원칙): 신규 부활 엔진을 만들지 않는다 — 기존 프리미티브
  (EZERagent restore · node-recover · watch · reinject · attest · EZERagent_state_snapshot)를
...
spawn(생성) 백엔드 2종:
  · production(기본): `EZERagent restore` — 실제 죽은 역할을 topology에서 일괄 재기동(실 프리미티브 재사용).
  · surrogate(--stub / 하네스): `EZERagent new-surface` + 경량 stub 에이전트. 실 claude/agy/codex를
...
def _live_state_dir():
    """라이브 상태 디렉터리 — unix: ~/.local/state/EZERagent · Windows: %LOCALAPPDATA%\\EZERagent
    (Rust EZERagent::socket_path / state.rs state_dir 의 기본 데몬 규약과 정합)."""
    if IS_WINDOWS:
        base = os.environ.get("LOCALAPPDATA") or os.path.join(HOME, "AppData", "Local")
        return os.path.realpath(os.path.join(base, "EZERagent"))
    return os.path.realpath(os.path.join(HOME, ".local", "state", "EZERagent"))
...
_ACTIVE_EPOCH = None  # cmd_restore 시작 시 EZERagent status의 daemon.started_at로 취득
...
#   과거: cmd_restore 는 FAILED 여도 dict 만 return → 프로세스는 항상 exit 0 → EZERagentd 가 완전실패와 성공을
#   구분 불가(exit 1 은 오히려 미포착 예외 crash 때만 나와 신호가 뒤집혀 있었다). 이제 판정을 결정론 exit 로 방출한다.
#   EZERagentd 재시도 정책과 정합: 5(BREAKER)·6(손상/identity)=재시도 금지, 1·3=1회 지연 재시도, 0=성공(재시도 없음).
...
PHOENIX_EXIT_CORRUPT = 6   # 손상 감지 / EZERagent identity 불일치 — 재시도 금지(사람 개입)
...
EZERAGENT = None  # lazy resolve

# ★phoenix protocol version — Rust EZERagent::pack::PHOENIX_PROTOCOL_VERSION 과 동기(identity 3중 대조·B1 self-test 표기).
...
# ★W1/B3·§5-1: 표준 설치 경로 폴백 후보(GUI/데몬 최소 PATH 에 EZERagent 가 없을 때). 라이브 실증(2026-07-06):
#   GUI 기동 데몬 PATH=/usr/bin:/bin:/usr/sbin:/sbin 뿐 → /opt/homebrew/bin/EZERagent 미탐색 → 리터럴 'EZERagent'
#   FileNotFoundError → auto-restore exit 1 침묵사. 폴백은 identity-check 통과 시에만 채택한다.
_EZERAGENT_STD_PATHS = ["/opt/homebrew/bin/EZERagent", "/usr/local/bin/EZERagent"]


def _extract_version(text):
    """`EZERagent --version`('EZERagent 0.12.20') / daemon.version('0.12.20') 에서 x.y.z 를 추출(진단 로깅용)."""
...
# ★W1(gate2): 해석된 EZERagent 의 identity 검증 상태(결과 JSON `EZERagent_identity` 로 노출).
...
def _EZERagent_self_identity(candidate):
    """후보 EZERagent 자신의 3필드 self-report(`EZERagent phoenix-identity` — 데몬 불요·컴파일타임 상수). 실패=None."""
    try:
...
def _daemon_identity(candidate, socket):
    """대상 데몬의 3필드(`EZERagent status --json`.daemon). 서브프로세스 실패/미도달/파싱실패=None."""
    cmd = [candidate]
    if socket:
        cmd += ["--socket", socket]
    cmd += ["status", "--json"]
...
def _EZERagent_identity_check(candidate, socket):
    ...
    self_id = _EZERagent_self_identity(candidate)
    if not self_id:
        return ("inconclusive", "self-report(phoenix-identity) 실패 — 구버전 EZERagent(커맨드 부재) 의심", "")
...
def _identity_with_retry(candidate, socket):
    ...
    status, field, detail = _EZERagent_identity_check(candidate, socket)
    tries = 0
...
def _accept_with_identity(socket, candidate, source_label, allow_inconclusive):
    """★gate2 fix 1·2: 후보 EZERagent 를 3중 identity 계약으로 채택/거부한다(PHOENIX_EZERAGENT·PATH·표준경로 공통 경로).
      · match        → 채택(_EZERAGENT_IDENTITY='verified')
      ...
      · inconclusive → allow_inconclusive 면 채택하되 **'검증 통과'가 아니라 degraded availability exception**으로
                       분리 기록(_EZERAGENT_IDENTITY='degraded-unverified'·저널+stderr). 아니면 exit 6.
    ...
    global _EZERAGENT_IDENTITY
    status, field, detail = _identity_with_retry(candidate, socket)
    if status == "match":
        _EZERAGENT_IDENTITY = "verified"
        sys.stderr.write("[phoenix] EZERagent 채택(%s): %s — 3중 identity match(%s)\n" % (source_label, candidate, detail))
        sys.stderr.flush()
        return candidate
    if status == "mismatch":
        _resolve_die(socket, "EZERagent(%s=%s) identity 3중 대조 불일치 — 불일치 필드=%s (%s). "
                     "다른 빌드/팩/프로토콜 스큐 차단·exit 6." % (source_label, candidate, field, detail), 6)
    # inconclusive
    if not allow_inconclusive:
        _resolve_die(socket, "EZERagent(%s=%s) identity inconclusive(%s) — positive proof 요구 경로라 채택 거부·exit 6. "
                     "PHOENIX_EZERAGENT 로 이 데몬과 동일 빌드의 EZERagent 를 명시하라." % (source_label, candidate, field), 6)
    _EZERAGENT_IDENTITY = "degraded-unverified"
    _resolve_journal_mark(socket, "degraded",
                          "EZERagent(%s=%s) identity inconclusive(%s) — availability 위해 채택하되 '검증 통과' 아님"
                          "(degraded-unverified 분리 보고)." % (source_label, candidate, field))
    sys.stderr.write("[phoenix] ★degraded availability exception: EZERagent(%s=%s) identity **unverified(degraded)** — "
...
def _resolve_journal_start(socket):
    """★gemini major: _resolve_EZERagent die 전에 저널 뼈대(ticket=resolve·타임스탬프·stage=resolve_EZERagent)를 먼저
    생성한다 — die 로 무단 종료해도 저널에 시도 이력이 남게 하는 관측 사각 제거. best-effort."""
    try:
        j = load_journal(socket, "resolve")
        jevent(j, "*", "resolve_EZERagent", "start", "EZERagent 실행 경로 해석 시작")
...
def _resolve_journal_mark(socket, status, msg):
    ...
    try:
        j = load_journal(socket, "resolve")
        jevent(j, "*", "resolve_EZERagent", status, msg[:280])
        save_journal(socket, "resolve", j)
    except Exception:
        pass
...
def _resolve_die(socket, msg, code):
    ...
    try:
        j = load_journal(socket, "resolve")
        jevent(j, "*", "resolve_EZERagent", "fail", msg[:280])
...
def _resolve_EZERagent(socket):
    """EZERagent 실행 경로 해석(§5-1 W1). 우선순위: PHOENIX_EZERAGENT > which('EZERagent') > 표준경로 폴백.
    ★리터럴 'EZERagent' 최종 폴백은 제거됨 — PATH 미해석 침묵 통과가 FileNotFoundError→exit 1 침묵사 근원(2026-07-06).
    ★gate2: 모든 실행 경로(PHOENIX_EZERAGENT·_which PATH·표준경로)에 3중 identity 계약을 적용한다 — 어느 경로든
       mismatch=exit 6. inconclusive 정책만 경로별 차등:
      · PHOENIX_EZERAGENT(명시)·_which PATH 후보 → X_OK + identity. inconclusive 는 재시도 후 degraded-unverified 로
        채택(분리 보고) — 하네스·격리·비데몬 서브커맨드에서 데몬 미도달 inconclusive 가 정상 존재하므로.
      · 표준경로 폴백(PATH·PHOENIX_EZERAGENT 모두 미해석) → positive proof 요구: **match 만** 채택(inconclusive=exit 6).
      · PHOENIX_STRICT_EZERAGENT=1 / 하네스(--socket·PHOENIX_FORBID_LIVE) → 표준경로 폴백 자체 금지(미해석=exit 6).
    실패·채택·degraded 전부 저널·stderr(→EZERagentd phoenix-restore.log)에 기록한다."""
    _resolve_journal_start(socket)
    explicit = os.environ.get("PHOENIX_EZERAGENT")
    if explicit:
        if not (os.path.isfile(explicit) and os.access(explicit, os.X_OK)):
            _resolve_die(socket, "PHOENIX_EZERAGENT(%s) 실행 불가(파일 부재/실행권한 없음) — 무검증 수용 금지·exit 6." % explicit, 6)
        return _accept_with_identity(socket, explicit, "PHOENIX_EZERAGENT", allow_inconclusive=True)
    found = _which("EZERagent")
    if found:
    ...
    # 여기부터 표준경로 폴백 — PATH·PHOENIX_EZERAGENT 모두 EZERagent 미해석.
    strict = os.environ.get("PHOENIX_STRICT_EZERAGENT") == "1"
    harness = bool(socket) or os.environ.get("PHOENIX_FORBID_LIVE") == "1"
    if strict:
        _resolve_die(socket, "PHOENIX_STRICT_EZERAGENT=1 이나 EZERagent 를 PHOENIX_EZERAGENT/PATH 로 해석하지 못했다 — 표준경로 폴백 "
                     "금지(exit 6). Rust auto-restore 의 PHOENIX_EZERAGENT 주입이 누락됐을 수 있다(B3).", 6)
    if harness:
        _resolve_die(socket, "하네스 모드(--socket/PHOENIX_FORBID_LIVE)에서 EZERagent 미해석 — 표준경로 폴백 금지"
                     "(테스트 독립성·exit 6). 격리 실행은 PHOENIX_EZERAGENT 로 대상 EZERagent 를 명시하라.", 6)
    for c in _EZERAGENT_STD_PATHS:
    ...
    _resolve_die(socket, "EZERagent 실행 경로 미해석(PHOENIX_EZERAGENT·PATH·표준경로 %s 모두 실패) — 리터럴 폴백 제거됨"
                 "(침묵사 방지·exit 6)." % _EZERAGENT_STD_PATHS, 6)
...
def _emit_evt(evt_type, **fields):
    """★C2/C3(W3): 구조화 이벤트를 HUD·음성 버스로 best-effort 방출(EVENT_CONTRACT v2·형제 EZERagent_event.py).
    ...
    mod = os.path.join(os.path.dirname(os.path.abspath(__file__)), "EZERagent_event.py")
...
def _snap_mod():
    """형제 파일 EZERagent_state_snapshot 모듈 로드·캐시. Windows 파이프→state_dir 매핑 규칙의 단일 소스이며
    (Rust state.rs 정합) phoenix 는 여기서 재사용한다(중복 구현 금지). mac 경로는 이 함수를 호출하지 않는다."""
    global _SNAP_MOD
    if _SNAP_MOD is None:
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        import EZERagent_state_snapshot as _s
        _SNAP_MOD = _s
    return _SNAP_MOD


def _win_pipe_slug(socket):
    """Windows named pipe 슬러그 — 규칙 단일 소스는 EZERagent_state_snapshot(중복 구현 금지)."""
    return _snap_mod()._win_pipe_slug(socket)


def _win_state_dir_for_socket(socket):
    """Windows 소켓(named pipe)→상태 디렉터리 — 규칙 단일 소스는 EZERagent_state_snapshot(중복 구현 금지)."""
...
def _run_capture(cmd, env, timeout):
    ...
    받고 '직접 자식(EZERagent.exe) 종료'만 기다린다 — 데몬을 스폰하지 않는 명령에도 안전하고, 스폰하는 명령(EZERagent list)에서
    치명적이다: `EZERagent list` 는 detached EZERagentd 를 스폰하는데 Rust spawn 이 bInheritHandles=TRUE 라, 파이썬 subprocess 가
    EZERagent.exe 에 물려준 inheritable 파이프 write 핸들을 EZERagentd 가 상속해 계속 연다. 그러면 subprocess.run(capture_output=
    True)+communicate() 는 EOF 를 영영 못 받아 hang 하고, timeout 후 정리 communicate() 는 timeout 이 없어 무한 대기한다
    (=CI 12분 스텝 타임아웃). 임시파일 리다이렉트는 파이프 EOF 문제를 없애고, p.wait() 는 데몬이 아니라 EZERagent.exe 종료만
    기다린다(EZERagent.exe 는 lazy-spawn 후 곧 종료). 크로스플랫폼(테스트는 mac 에서도 가능). 반환=_CapR."""
    import tempfile
    ...
    try:
        try:
            ...
            # ★codex major(Windows 대칭): EZERagent.exe 미해석/실행불가 → 비구조화 crash 대신 구조화 실패(rc=127).
            r.returncode = 127
            r.stderr = "EZERagent 실행 불가(%s: %s) cmd=%r" % (type(e).__name__, e, cmd)
            return r
        try:
...
def EZERagent(*args, socket=None, timeout=25):
    cmd = [EZERAGENT]
    if socket:
        cmd += ["--socket", socket]
    ...
    # ★Windows: 임시파일 캡처(_run_capture)로 detached EZERagentd 파이프 상속 hang 회피. mac 은 기존 경로 유지(무회귀).
    if IS_WINDOWS:
    ...
    try:
        ...
        # ★codex major: EZERAGENT 해석 후에도 파일이 사라지거나 실행 불가면 여기서 비구조화 exit 1 crash 가 났다.
        #   구조화 실패(rc=127)로 강등해 상위 판정(스폰 실패→INCOMPLETE 등)이 정직히 흐르게 한다.
        class _R:
            returncode = 127
            stdout = ""
            stderr = "EZERagent 실행 불가(%s: %s) cmd=%r" % (type(e).__name__, e, cmd)
        return _R()


def get_boot_epoch(socket):
    """boot-epoch = 데몬 기동 세대 식별자. EZERagent status --json 의 daemon.started_at 실측(재시작마다 변경).
    이 값이 저널 완료 마킹의 세대 유효성 기준이다(재부팅을 넘긴 마킹 = stale). 획득 실패 시 None
    → 호출측(stage_done)이 보수적으로 stale 취급(=재spawn, 잘못-skip 아님)."""
    r = EZERagent("status", "--json", socket=socket, timeout=12)
...
def _atomic_write_json(path, obj, keep_bak=False):
    """tmp+fsync+replace+dir fsync — EZERagent_state_snapshot 과 동일한 원자성 규약.
...
def _snapshot_roster_entries(socket):
    """직전 세대 스냅샷(EZERagent_state_snapshot)의 topology.json 로스터 — §12가 실증한 안전망.
    라이브 상태 디렉터리 대상일 때만 유효(격리 하네스는 자기 topology만 씀). best-effort."""
    if state_dir_for(socket) != LIVE_STATE:
        return {}
    snap = os.path.join(os.path.dirname(os.path.abspath(__file__)), "EZERagent_state_snapshot.py")
    gen_root = os.path.join(HOME, ".EZERagent", "state-generations")
    try:
...
def _recover_retention_file(socket, path, kind):
    ...
    if kind == "desired_roster":
        ...
        # 부서는 glob(EZERagent-dept-*)∪registry 로 재발견 가능(discover_depts) — 빈 상태로 복원하면 observe 가 roster 를
...
def _resync_intents_if_daemon_up(socket, intents):
    """★A-S3: 데몬 복귀 시 미소화 intent 를 RPC(EZERagent tombstone) 로 재동기하고, topology.json 을 다시 로드해
    ...
    for it in intents:
        ...
        if it.get("op") == "remove":
            args.append("--remove")
        r = EZERagent(*args, socket=socket, timeout=12)
        if getattr(r, "returncode", 1) != 0:
...
#   수리: 부서를 glob(state_root/EZERagent-dept-*) ∪ depts.json 으로 동적 발견(파일시스템 truth)해 phoenix 소유
...
def _dept_discovery_roots():
    ...
    depts_json = os.environ.get("PHOENIX_DEPTS_JSON") or os.path.join(HOME, ".EZERagent", "depts.json")
    return state_root, depts_json


def discover_depts():
    """현재 존재하는 부서를 동적 발견 — glob(state_root/EZERagent-dept-*) ∪ depts.json 레지스트리.
    ...
    try:
        for name in os.listdir(state_root):
            if name.startswith("EZERagent-dept-"):
                p = os.path.join(state_root, name)
                if os.path.isdir(p):
                    found[name[len("EZERagent-dept-"):]] = {"state_dir": os.path.realpath(p)}
    except OSError:
        pass
...
def _status_json(socket):
    """★C5(W3): `EZERagent status --json` 파싱 결과(dict) — 구조화 liveness/readiness 단일 소스. 실패=None."""
    r = EZERagent("status", "--json", socket=socket, timeout=12)
    if getattr(r, "returncode", 1) != 0:
        return None
    try:
...
def _live_role_surfaces_from_list(socket):
    """폴백: `EZERagent list` 화면 정규식 파싱(status --json 미도달 시). 반환 (dict, known:bool).
    ...
    r = EZERagent("list", socket=socket, timeout=12)
    txt = r.stdout or ""
    out = {}
...
def _live_surfaces_raw(socket):
    ...
    for s in st.get("surfaces", []):
        # 미claim surface 는 status --json 에서 role=null — 구 `EZERagent list` 규약(미claim="-")과 일치시켜 보존한다.
...
def live_role_surfaces(socket):
    ...
    ★C5/P1-6(W3): liveness 근원을 화면 정규식(`EZERagent list`)에서 **구조화 소스**(`EZERagent status --json`.surfaces)로
...
def rollback_proposal(socket):
    """직전 GREEN 세대로의 롤백 제안(제안만 — 실행 금지). EZERagent_state_snapshot list 재사용."""
    snap = os.path.join(os.path.dirname(os.path.abspath(__file__)), "EZERagent_state_snapshot.py")
...
def spawn_production(socket, pending_roles, include_master=False):
    """실 프리미티브 재사용: EZERagent restore 로 죽은 역할 일괄 재기동(세션핀 resume 경로)."""
    args = ["restore"]
    if include_master:
        args.append("--include-master")
    r = EZERagent(*args, socket=socket, timeout=90)
    return {"backend": "production(EZERagent restore)", "rc": r.returncode,
            "out": (r.stdout or r.stderr or "").strip()[:800]}


def spawn_fresh_production(socket, role, agent):
    """★Phase11 독약세션 fresh-fallback(prod): 무 resume 로 새 세션 기동(EZERagent launch-agent).
    EZERagent restore 는 topology 의 session_id 를 resume 하므로 독약 세션이면 계속 실패한다 → 세션핀을 버리고
    ...
    r = EZERagent("launch-agent", "--role", role, "--agent", agent or "claude", socket=socket, timeout=60)
...
def spawn_surrogate(socket, role, observed_sid, attempt=0, mode="resume"):
    ...
    r = EZERagent("new-surface", socket=socket, timeout=15)
    ...
    EZERagent("send", "--surface", ref, cmdline, socket=socket, timeout=10)
    EZERagent("send-key", "--surface", ref, "Return", socket=socket, timeout=10)
...
def stage_ready(socket, role, surface, stub):
    ...
    r0 = EZERagent("read-screen", "--surface", surface, socket=socket, timeout=10)
    if marker in (r0.stdout or ""):
        return True, "ready marker present on screen (read-screen)"
    r = EZERagent("watch", "--surface", surface, "--until", marker, "--timeout", "12",
...
def stage_observe_session(socket, surface, stub, role=None):
    ...
    for attempt in range(tries):
        r = EZERagent("read-screen", "--surface", surface, socket=socket, timeout=12)
        txt = r.stdout or ""
        last_txt = txt
...
def _surface_agent_present(socket, surface):
    ...
    try:
        r = EZERagent("status", "--json", socket=socket, timeout=8)
        if r.returncode != 0:
            return None
...
def stage_reinject(socket, role, surface, stub):
    ...
    r = EZERagent("reinject", "--check", "--role", role, "--surface", surface, "--timeout", "6",
...
def stage_g2_ack(socket, role, surface, stub):
    ...
    r = EZERagent("reinject", "--check", "--role", role, "--surface", surface, "--timeout", "4",
...
def c6_reap_stale_surfaces(socket):
    ...
    (`EZERagent close-surface <ref> --reap` → CloseCause::Reap → 묘비 미생성·부활 대상 유지). 고정 OwnerClose 경로
    ...
    for item in detected:
        ...
        if not ref:
            continue
        r = EZERagent("close-surface", ref, "--reap", socket=socket, timeout=12)
        if getattr(r, "returncode", 1) == 0:
            reaped.append(ref)
        else:
...
def run_restore(socket, ticket="default", stub=False, no_breaker=False, roles=None,
    ...
    # ★C6 S0 실 회수(W2 — P0-6 cause): exited=true 잔재를 Reap 사유로만 회수(EZERagent close-surface --reap →
    ...
    #   따라서 복구본이 유효 JSON 이 된 뒤 도는 EZERagentd auto-retry 2차 실행도 동일하게 보류가 유지된다(휘발 방지).
    _c2_degraded = []
    for _p, _kind in ((desired_roster_path(socket), "desired_roster"),
                      (dept_roster_path(socket), "dept_roster")):
    ...
    #   (--ignore-tombstones 류)는 추가하지 않는다 — 정당한 재편입은 untomb RPC(EZERagent tombstone --remove)가 정도(正道)다.
    ...
    # ── spawn 단계(공유): production=EZERagent restore 1회 / surrogate=역할별 stub ──
    role_surface = {}
    forced_sids = {}
    if stub_sids:
        try:
    ...
    #    prod: EZERagent restore 는 idempotent(죽은 역할만 재스폰)이라 재호출로 미스폰 역할만 다시 시도된다.
        ...
        if stub:
            ...
            for role in need:
                ...
                if alive:
                    ...
                    mark_stage(j, role, "spawn", True, "EZERagent restore → %s (attempt %d)" % (ref, attempt))
                else:
                    still.append(role)
        if not still:
            need = []
    ...
    if need and POISON_FRESH_FALLBACK:
        ...
        for role in need:
            ...
            if stub:
                ...
                msg = "EZERagent launch-agent(fresh·rc=%s) → %s" % (res["rc"], ref or res["out"])
            if ref:
                role_surface[role] = ref
                rr = j["roles"].setdefault(role, {"stages": {}})
                rr["surface"] = ref
    ...
    result = {
        ...
        "backend": "surrogate(stub)" if stub else "production(EZERagent restore)",
        ...
        "EZERagent_identity": _EZERAGENT_IDENTITY,     # ★gate2: 해석된 EZERagent 의 3중 identity 검증 상태(verified|degraded-unverified|None)
...
def cmd_restore(args):
    ...
    #   (1급 원칙: 의도삭제>강제부활). raw `EZERagent restore --include-master`도 묘비를 skip(심층방어).
...
def cmd_tombstone(args):
    ...
    r = EZERagent(*rpc_args, socket=socket, timeout=12)
    if getattr(r, "returncode", 1) == 0:
...
def cmd_inherit(args):
    ...
    ※구현 계층 권고: EZERagentd 무변경. 이 primitive 를 (a)launch-agent 후행 훅 또는 (b)`EZERagent schedule` 주기 reconciler 로
...
def _read_worker_todo():
    ...
    cand = os.path.join(os.environ.get("EZERAGENT_PACK_DIR", os.path.join(HOME, ".EZERagent", "pack")),
...
def _protection_grade():
    """★Phase 8: 정직한 보호등급(GREEN/AMBER/RED)을 EZERagent_backup 에서 가져온다(앵커 불요·RED 기본).
    백업 도구가 없거나 실패해도 status 는 죽지 않는다 — 보호 미상은 정직하게 RED 로 보고."""
    try:
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        import EZERagent_backup
        return EZERagent_backup.protection_status()
    except Exception as e:
...
# 자동 부활(EZERagent phoenix/restore)이 불능일 때, 사람이 이 세대 스냅샷 안에서 직접 조직을 재건한다.
# 의존: EZERagent 바이너리 + 같은 폴더의 topology.json 사본. 그 외 어떤 데몬 상태·hook·팩 로직에도 의존하지 않는다.
...
for e in t.get('entries',[]):
    ...
    print("EZERagent launch-agent --role %s --agent %s   # 기동 후 각성 확인, 필요시 EZERagent reinject --role %s" % (role, agent, role))
PY
echo ""
...
def cmd_gen_manual(args):
    ...
    out = {"manual_restore_script": sp, "topology_copy": os.path.join(dest, "topology.json"),
           "self_contained": True,
           "note": "데몬/hook 비의존 — EZERagent 바이너리 + 이 폴더의 topology.json 만 있으면 사람이 재건 가능(§11.1 하한2 독립성)."}
    print(json.dumps(out, ensure_ascii=False, indent=2))
    return out
...
def protected_paths():
    ...
    사용자 config soul 은 env(EZERAGENT_SOUL_PATH 우선, 없으면 CLAUDE_CONFIG_DIR/soul.md)에서 파생한다 — 없으면
    pack soul 만 보호(누구의 개인 config 경로도 소스에 박지 않는다). Phase9 하드코딩 감사 수리."""
    hp = "$HOME/.EZERagent/pack"
    paths = [
        hp + "/agents.json",
        hp + "/bin/EZERagent_phoenix.py",
        hp + "/bin/EZERagent_state_snapshot.py",
        hp + "/bin/EZERagent_backup.py",
        hp + "/directives",
        hp + "/soul.md",
    ]
    soul = os.environ.get("EZERAGENT_SOUL_PATH")
    if not soul:
...
#   · Windows: schtasks(작업 스케줄러) 기반. supervisor_status=schtasks /Query, supervisor_ensure=`EZERagent daemon install`
...
#   managed 면 `schtasks /Run`, 비관리/실패면 `EZERagent list`(CLI lazy-spawn 보완). 진짜 KeepAlive 패리티는 후속 사이클.
...
def _schtasks_has_restart_on_failure(task):
    """schtasks /Query /XML 에 RestartOnFailure 존재 여부(=진짜 KeepAlive 켜짐 · Rust EZERagent daemon install 이 심는 XML).
...
def _schtasks_ensure(task, running_pid=None):
    """미관리/고아면 `EZERagent daemon install`(기존 Rust schtasks 등록 로직 재사용)로 managed 복원. 멱등(이미 managed=noop).
    ★schtasks XML 직접 조립 금지 — Rust 단일 소스(EZERagent daemon install)만 사용."""
    before = _schtasks_status(task, running_pid=running_pid)
    if before["state"] == "managed":
        return {"ensured": True, "action": "noop(이미 등록)", "before": before, "after": before}
    r = EZERagent("daemon", "install", socket=None, timeout=30)
    after = _schtasks_status(task, running_pid=running_pid)
    return {"ensured": after["state"] == "managed",
            "action": "EZERagent daemon install rc=%s" % getattr(r, "returncode", None),
            "out": (getattr(r, "stdout", "") or getattr(r, "stderr", "") or "").strip()[:300],
            "before": before, "after": after}


def _win_identify_daemon_pid(socket):
    """EZERagent identify 결과 최상위 daemon_pid(handlers.rs system.identify) 획득 — taskkill 대상 pid.
    ★데몬 생존 중에만 호출한다(kill 전). 획득 실패 시 None(호출측이 kill 생략하고 재기동만 시도)."""
    r = EZERagent("identify", socket=socket, timeout=8)
...
def _win_restart_daemon(socket, timeout):
    ...
    ★파이프 해제 폴링(socket_death): named pipe first_pipe_instance(true) 단일인스턴스 경합 회피 —
      새 EZERagentd 가 bind 하기 전에 기존 데몬이 파이프를 놓을 때(ping 무응답)까지 기다린 뒤 재기동을 유발한다
    ...
    if pid:
        ...
        res["taskkill_out"] = "EZERagent identify 로 daemon_pid 미획득 — kill 생략(재기동만 시도)"
    # ★파이프 해제(소켓 소멸) 폴링 — 재기동 유발 전 필수(단일인스턴스 경합 회피). ping 은 autostart 안 함(안전 프로브).
    death = False
    t0 = time.time()
    while time.time() - t0 < min(8, timeout):
        pr = EZERagent("ping", socket=socket, timeout=4)
        if not (getattr(pr, "returncode", 1) == 0 and "pong" in (getattr(pr, "stdout", "") or "")):
            death = True
            break
        time.sleep(0.3)
    res["socket_death_observed"] = death
    # ★재기동 유발: managed=schtasks /Run(등록 태스크 기동) · 비관리/실패=EZERagent list(형제 EZERagentd lazy-spawn 보완).
    if sdst.get("state") == "managed":
        ...
        if getattr(rr, "returncode", 1) != 0:  # /Run 실패 시 CLI lazy-spawn 폴백
            lr = EZERagent("list", socket=socket, timeout=15)
            res["retrigger"] += " → 폴백 EZERagent list rc=%s" % getattr(lr, "returncode", None)
    else:
        lr = EZERagent("list", socket=socket, timeout=15)   # list=lazy-spawn(형제 EZERagentd 자동기동)
        res["retrigger"] = "EZERagent list(lazy-spawn) rc=%s" % getattr(lr, "returncode", None)
    return res
...
def supervisor_ensure(label, plist=None, running_pid=None):
    """미관리/고아면 managed 복원(멱등). mac=launchd_ensure(bootstrap) · win=_schtasks_ensure(EZERagent daemon install)."""
    if IS_WINDOWS:
...
LAUNCHD_LABEL = "com.EZERagent.EZERagentd"
# supervisor 레이블 — mac=launchd label · win=schtasks 태스크명 'EZERagentd'(Rust EZERagent.rs DaemonAction TASK 와 정합).
#   PHOENIX_SUPERVISOR_LABEL 오버라이드: 격리 스모크가 글로벌 'EZERagentd' 태스크와 분리(예: 존재하지 않는 테스트 태스크명
#   → schtasks 미등록=unmanaged → 재기동 유발이 커스텀 파이프의 EZERagent list lazy-spawn 경로를 타게 함·교차오염 0).
SUPERVISOR_LABEL = os.environ.get("PHOENIX_SUPERVISOR_LABEL") or ("EZERagentd" if IS_WINDOWS else LAUNCHD_LABEL)


def _launchd_plist_path():
    """~/Library/LaunchAgents/com.EZERagent.EZERagentd.plist — src/launchd.rs plist_path() 와 동일 규약.
...
def _render_deploy_runbook(roster):
    ...
    EZERagent 바이너리 + launchctl 만으로 사람이 그대로 복사-실행. 현재 로스터를 반영해 launchd 재기동 토대 복원 +
    ...
    lines = [
        ...
        "# 자동 부활(EZERagent phoenix)이 불능일 때, 사람이 이 스냅샷 세대 안에서 직접 조직을 재건한다.",
        "# 의존: EZERagent 바이너리 + launchctl 만 — phoenix/데몬/hook 등 집행 계층 로직에 의존하지 않는다(자기완결 평문).",
        ...
        "else",
        '  echo "  !! plist 부재($PLIST) — EZERagent daemon install 로 재생성 후 재시도"',
        "fi",
        'echo "데몬 소켓 응답 대기..."; for i in $(seq 1 40); do EZERagent ping 2>/dev/null | grep -q pong && { echo "  pong OK"; break; }; sleep 0.5; done',
        "",
    ...
    for role in sorted(roster.keys()):
        ...
        lines.append('EZERagent launch-agent --role %s --agent %s   # 각성 확인 후: EZERagent reinject --role %s'
                     % (role, agent, role))
    lines += [
        "",
...
def _render_deploy_runbook_ps1(roster):
    ...
    EZERagent.exe + schtasks(작업 스케줄러)만으로 사람이 그대로 실행: 로그온 자동기동 토대 재등록 → EZERagent list 로 데몬 기동
    → EZERagent ping 대기 → 역할별 순차 launch-agent + reinject 나열(§11.1 하한2 독립성 · §10.4 첫 행동=원장 대조).
    ★Windows 데몬은 사망 시 자동 respawn 이 없다 — schtasks 등록은 '로그온 자동기동' 토대이고, CLI lazy-spawn(EZERagent list)이 보완."""
    lines = [
        "# MANUAL_RESTORE.ps1 — 불사조 deploy '독립 수동 복원' 경로 (Windows · M1 출하 조건 · §11.1 하한2 독립성)",
        "# 자동 부활(EZERagent phoenix)이 불능일 때, 사람이 이 스냅샷 세대 안에서 직접 조직을 재건한다.",
        "# 의존: EZERagent.exe + schtasks 만 — phoenix/데몬/hook 등 집행 계층 로직에 의존하지 않는다(자기완결 평문).",
        "# ★참석(attended) 경로 — 사람이 한 줄씩 확인하며 실행(§11.1 하한1: 유인 복구는 어떤 상태에서도 잠기지 않는다).",
        ...
        '  Write-Host "  작업 스케줄러 미등록 → EZERagent daemon install"; EZERagent daemon install',
        "} else {",
        '  Write-Host "  (이미 등록됨) schtasks /Run 으로 기동 시도"; schtasks /Run /TN $Task 2>$null | Out-Null',
        "}",
        "",
        "# 2) 데몬 기동 유발(EZERagent list = 형제 EZERagentd lazy-spawn) + named pipe 응답 대기:",
        "EZERagent list 2>$null | Out-Null",
        'Write-Host "데몬 named pipe 응답 대기..."',
        "for ($i = 0; $i -lt 40; $i++) { if ((EZERagent ping 2>$null) -match 'pong') { Write-Host '  pong OK'; break }; Start-Sleep -Milliseconds 500 }",
        "",
...
def _deploy_snapshot(socket, roster):
    """세대 스냅샷 + ★하한2① 독립 수동 runbook 동봉. 라이브=~/.EZERagent/state-generations + default_sources(전 L1 선언상태),
    격리(--socket 하네스)=<phoenix_home>/state-generations + 소켓 상태 디렉터리의 L1 파일(자동 격리·hermetic)."""
    import io
    import contextlib
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    try:
        import EZERagent_state_snapshot as _snap
    except Exception as e:
        return {"ok": False, "error": "EZERagent_state_snapshot import 실패: %s" % e}
    sd = state_dir_for(socket)
    if sd == LIVE_STATE:
        gen_root = os.path.join(HOME, ".EZERagent", "state-generations")
...
def cmd_deploy(args):
    ...
    if not post_restart:
            ...
            if opened:
                ...
                _finish("BREAKER_OPEN", 5, {"rollback_proposal": rollback_proposal(socket),
                        "alert": "%ss 내 %d회 배포 시도(임계 %d) — 자동 배포 정지. 사람 승인 필요."
                                 % (BREAKER_T, len(attempts), BREAKER_N)})
        pong = EZERagent("ping", socket=socket, timeout=8)
        ...
        #   (mac=launchd bootstrap · win=`EZERagent daemon install` — 기존 로직 재사용·코드 복제 금지), 여전히 비관리면
        ...
        if not restart_hook:  # hook 경로(격리)는 supervisor 무관 — 라이브 경로에만 적용
            ...
            if sdst.get("state") != "managed":
                ...
                if not ens.get("ensured"):
                    ...
                    _finish("SUPERVISOR_UNMANAGED", 8, {"supervisor_status": sdst, "ensure_attempt": ens,
                            "alert": "대상 데몬이 감독자 관리 밖(state=%s)이라 재시작 자동기동 토대가 없다. "
                                     "재등록 실패 — mac: `launchctl bootstrap gui/$(id -u) %s` · win: `EZERagent daemon install` "
                ...
                #   사망 시 감시자 자동 재기동이 없어 CLI lazy-spawn/사람 개입에 의존. `EZERagent daemon install` 재실행 권장.
                _dmark(j, "preflight", "keepalive_warn",
                       "supervisor managed 이나 KeepAlive(자동 재기동) 부재(구버전 install 의심). 배포는 계속(경고) — "
                       "`EZERagent daemon install` 재실행으로 RestartOnFailure 갱신 권장.",
        ...
        # ── quiescent (EZERagent drain · best-effort · 성공 단정 금지) ──
        if not _same_gen("quiescent"):
            dr = EZERagent("drain", socket=socket, timeout=30)
            _dmark(j, "quiescent", "ok",
                   "EZERagent drain rc=%s · %s" % (getattr(dr, "returncode", None),
        ...
        if not restart_confirmed:
            ...
            _finish("RESTART_FAILED", 4, {
                ...
                "alert": "데몬 재시작 실패/미확증(%s). 독립 수동 복원 runbook 을 실행하라: %s (§11.1 하한2 독립 경로). "
                         "launchd 비관리(고아) 의심 시 `EZERagent_phoenix.py --socket <s> launchd-ensure --label %s "
...
def main():
    global EZERAGENT
    # ★B1 self-test(임베드 추출 직후 EZERagentd 가 호출): 추출된 phoenix 가 실행가능한지만 확인한다 — 데몬·EZERagent 해석·
    #   상태파일 무접촉. argparse(서브커맨드 required)·_resolve_EZERagent 이전에 조기 종료해 순수 실행성만 검증.
    ...
    # ★W2 콜드부트: EZERagentd가 소켓 바인드 직후 detached 스폰. master 포함 강제(부활할 상위가 없으므로)
    ...
    args = ap.parse_args()
    # ★W1/B3·§5-1: EZERagent 실행 경로를 여기서(소켓 인지 후) 해석한다 — PHOENIX_EZERAGENT > which('EZERagent') > 표준경로
    #   폴백(identity-check·STRICT/하네스 금지). 리터럴 'EZERagent' 최종 폴백 제거로 FileNotFoundError 침묵사를 차단.
    #   미해석/불일치는 _resolve_EZERagent 내부에서 die(exit 6)로 정직 정지한다(라이브 실증 2026-07-06 근원 수리).
    # ★W5(⑥ 수리·CI run 28778120380 실증): `deploy --plan` 은 계획만 출력하고 sys.exit(0) 하는 **무실행** 경로라
    #   EZERagent 를 전혀 호출하지 않는다. 그런데 _resolve_EZERagent 는 identity 프로브(subprocess `EZERagent status/phoenix-identity`)를
    #   돌리는데, 데몬이 없는 환경(스모크 ⑥)에서 Windows 는 이 프로브가 12s×재시도로 지연돼 --plan 이 30s 타임아웃
    #   (exit 124)됐다. 무실행 서브커맨드는 EZERAGENT 해석을 lazy 로 건너뛴다(실 subprocess 필요 경로는 모두 EZERAGENT 해석 후에
    #   실행되므로 무영향). die 경로(EZERagent 부재)도 --plan 에선 부적절(계획 출력에 EZERagent 불요)이라 함께 해소된다.
    _no_exec_plan = args.cmd == "deploy" and getattr(args, "plan", False)
    EZERAGENT = None if _no_exec_plan else _resolve_EZERagent(args.socket)
    {
