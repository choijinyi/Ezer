#!/usr/bin/env python3
"""관찰 명령 부작용 금지 — EZERagent 헬스 probe 멱등성 계약 (AGENTREACH OPP-21).

AR PHIL-04('관찰이 상태를 바꾸지 않는다')의 EZERagent 일반화. AR 의 python pytest spy 를
...
세 가지 계약을 실행 가능 테스트(`--self-test`)로 잠근다:
  ...
  2. preflight C12.daemon 관찰 단계(fix=False)는 비멱등 0 — EZERagentd Popen 미발생.
...
Run:  python3 EZERagent_idempotency.py --self-test     # 결정론 배터리(preflight C53 이 호출)
      python3 EZERagent_idempotency.py classify 'EZERagent send --to worker x'   # verb 분류 디버그
"""
...
# ── A. 멱등 verb 분류기 (단일 진실) ──────────────────────────────────────────
# 시드셋은 실측 `EZERagent actions`(clap 정의 = self-describing SOT) 표면에 동기화한다.
# self-test 가 `EZERagent actions ⊆ OBSERVE ∪ MUTATE ∪ UNCLASSIFIED_ALLOWED` 를 검증해
...
MUTATE_VERBS = frozenset({           # 상태 변경 — repair/실행 의도 뒤에서만
    ...
    # 'new-split' 부재 — 실제 EZERagent CLI 에 없음(cmux 잔재·CLAUDE.md 치환표상 폐기 verb).
})

# 현재 EZERagent 표면에 존재하나 멱등 분류가 모호해 의도적으로 미분류로 둔 verb(정직한 미커버).
...
# 별도 분류로 면제. EZERagent_session.py:5-7 'well-known idempotent upsert 는 보호 대상 아님' 근거.
# 면제 자체를 박제해 무한 확장(계약 형해화) 차단 — self-test 가 상한(<=3) 검증.
SELF_HEAL_EXEMPT = ("todo-path",)    # 부재 시 결정론 생성하는 자기수복 upsert


def classify_EZERagent_verb(argv):
    """'EZERagent <verb> ...' 에서 verb 추출 → "observe" | "mutate" | "unknown".
...
def _extract_verb(argv):
    """argv 리스트에서 EZERagent 서브커맨드(verb)를 추출. EZERagent 바이너리/옵션을 건너뛴다.
...
def is_observe_only(argv):
    ...
    return classify_EZERagent_verb(argv) == "observe"


def assert_observe_phase(calls):
    """관찰 단계 호출 로그(argv 리스트들)에서 mutate/unknown EZERagent verb 부재 단언.

    AR 의 "['doctor'] not in calls" 의 EZERagent 일반화: calls 중 EZERagent 호출은 전부 observe 여야 한다.
    EZERagent 가 아닌 호출(python·yt-dlp 등)은 무시한다. 반환: (ok, violations[{verb, argv}])."""
    violations = []
    for argv in calls:
        verb = _extract_verb(argv)
        if verb is None:               # EZERagent 호출 아님 — 본 계약 범위 밖
            continue
        kind = classify_EZERagent_verb(argv)
...
class _SubprocessSpy:
    ...
    네트워크·프로세스 0 — 완전 격리·결정론(EZERagent 미설치/CI 무데몬 환경에서도 통과)."""
...
def _spy_cmd_check():
    ...
    import EZERagent_orchestra as orch
    spy = _SubprocessSpy()
    ...
    # 실측: cmd_check 의 유일한 EZERagent 호출은 'EZERagent status --json'(관찰) 이어야 한다.
    EZERagent_calls = [c for c in spy.calls if _extract_verb(c) is not None]
    assert EZERagent_calls, "cmd_check 가 EZERagent status 조차 호출하지 않음(spy 미작동 의심)"
    assert all(_extract_verb(c) == "status" for c in EZERagent_calls), \
        "cmd_check 의 EZERagent 호출이 status 외 verb 포함: %s" % EZERagent_calls


def _spy_negative_assertion_self_attack():
    """자기공격 변이검증(defensive-security-gate): 금지 verb 를 calls 에 주입하면 RED 인가."""
    poisoned = [["EZERagent", "status", "--json"], ["EZERagent", "launch-agent", "--role", "worker"]]
...
def _spy_preflight_daemon_observe_phase():
    """preflight C12.daemon 관찰 단계(fix=False)는 EZERagentd Popen 0(비멱등 0) 박제.

    AR 'doctor 자동기동 회피' 의 EZERagent 판. fix=True 일 때만 Popen≤1 허용(의도된 수리)."""
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    import EZERagent_preflight as pf
    spy = _SubprocessSpy(stdout=b"")          # ping 성공 모킹(returncode 0)
    # fix=False: 관찰 단계 — EZERagentd Popen 은 절대 발생하면 안 된다.
...
def _EZERagent_actions_surface():
    """실측 `EZERagent actions` verb 표면 frozenset. EZERagent 부재 시 None(검사 skip)."""
    import shutil
    EZERagent = shutil.which("EZERagent")
    if not EZERagent:
        return None
    try:
        r = subprocess.run([EZERagent, "actions"], capture_output=True, timeout=10)
        if r.returncode != 0:
            return None
    except Exception:
        return None
...
def _invariants():
    ...
    assert not is_observe_only(["EZERagent", "totally-unknown-verb"]), \
        "미등록 verb 가 관찰 경로 통과(default-deny 위반)"
    assert is_observe_only(["EZERagent", "status", "--json"]), "status 가 observe 가 아님"
    assert not is_observe_only(["EZERagent", "launch-agent"]), "launch-agent 가 거부 안 됨"
    assert _extract_verb(["yt-dlp", "--dump-json"]) is None, "EZERagent 아닌 호출이 verb 로 추출됨"
    # 면제 상한(계약 형해화 차단).
    assert len(SELF_HEAL_EXEMPT) <= 3, "SELF_HEAL_EXEMPT 무한 확장(계약 형해화)"
    # 표면 커버리지(과장 차단): EZERagent actions ⊆ OBSERVE ∪ MUTATE ∪ UNCLASSIFIED_ALLOWED.
    surface = _EZERagent_actions_surface()
    if surface is None:
        return "SKIP(EZERagent 부재 — 표면 커버리지 검증 생략)"
    ...
    return "FULL(EZERagent 표면 전부 분류됨)"
...
def self_test():
    ...
    print("EZERagent_idempotency self-test OK "
...
def main(argv):
    ...
    if argv and argv[0] == "classify":
        if len(argv) < 2:
            print("usage: EZERagent_idempotency.py classify '<argv...>'", file=sys.stderr)
            return 2
        toks = argv[1].split()
        print("%s → %s" % (toks, classify_EZERagent_verb(toks)))
        return 0
    print(__doc__)
    return 0
