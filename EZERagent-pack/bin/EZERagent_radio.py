...
VALID_NAME = re.compile(r"^[A-Za-z0-9가-힣_-]{1,64}$")
...
def _unread(role, all_msgs=False):
    ...
    for thread in _threads():
        last = int(cur.get(thread, 0))
        for m in _read_msgs(thread):
            if m["id"] <= last or m.get("from") == role:
                continue
            if all_msgs or role in (m.get("mentions") or []):
                out.append((thread, m))
    return out, cur
...
def self_test():
    ...
    print("EZERagent_radio self-test OK (create·@멘션 파싱·dirty 플래그·ack 커서·기본 스레드·이름 검증)")
    return 0
...
def main():
    ...
    fn = {"create": cmd_create, "send": cmd_send, "inbox": cmd_inbox,
          "read": cmd_read, "list": cmd_list}.get(a.cmd)
    if not fn:
        ap.print_help()
        return 1
    return fn(a)
