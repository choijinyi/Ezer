#!/usr/bin/env python3
"""test_ceo_pending_gate.py — EZERagent-dept CEO 부트 게이트·PENDING 상태 기계 핀 (WP-2).

가짜 HOME(팩 directives·registry)+스텁 EZERagent($HOME/.local/bin — EZERagent-dept PATH prepend 1순위)로
실 데몬 무접촉 검증:
...
def setup(tmp, ndepts=1):
    ...
    pack = os.path.join(home, ".EZERagent", "pack", "directives")
    bindir = os.path.join(home, ".local", "bin")  # EZERagent-dept PATH prepend 1순위
    ...
    # 스텁 EZERagent: feed push=승인(exit 0)·status=실패(reinject skip 경로)·전 호출 기록
    stub = os.path.join(bindir, "EZERagent")
    ...
    for k in ("EZERAGENT_ROLE", "EZERAGENT_SOCKET", "EZERAGENT_PACK_DIR"):
        env.pop(k, None)
    return env, home
...
def paths(home):
    d = os.path.join(home, ".EZERagent", "pack", "directives", "MASTER_DIRECTIVE.md")
    return (d, d + ".pre-ceo",
            os.path.join(home, ".EZERagent", "state", "ceo-pending"),
            os.path.join(home, ".EZERagent", ".master-bootstrapped"))
