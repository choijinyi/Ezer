...
데몬 불요: EZERAGENT 를 더미로 두거나 m.EZERagent/헬퍼를 몽키패치해 파일·순수 로직만 검증한다(라이브 무접촉).

실행: python3 EZERagent-pack/bin/tests/test_phoenix_w3_corruption.py  (0=전건 PASS)
"""
...
def _ph_home(td, name):
    ...
    return os.path.join(sd, "EZERagent.sock"), home
...
def c2_bak_fallback(td):
    ...
    m.EZERAGENT = os.path.join(td, "nonexistent-EZERagent")  # EZERagent 전부 rc127 강등 → live 없음
...
def c2_snapshot_fallback(td):
    ...
    os.makedirs(home, exist_ok=True)
    sock = os.path.join(live, "EZERagent.sock")
    dp = os.path.join(home, "desired_roster.json")
    _write(dp, "]]] corrupt no bak")
    gen = os.path.join(td, ".EZERagent", "state-generations", "20260706T120000Z")
    ...
    try:
        m.LIVE_STATE = live
        m.HOME = td
        m.EZERAGENT = os.path.join(td, "nonexistent-EZERagent")
...
def c5_liveness_from_status_json(td):
    """live_role_surfaces 가 `EZERagent status --json`.surfaces(구조화)에서 role/exited 를 읽는다."""
    saved = m.EZERagent
    status = {"surfaces": [
...
def c5_liveness_fallback_to_list(td):
    """status --json 미도달(rc≠0) 시 `EZERagent list` 정규식 폴백(가용성 하한)."""
    saved = m.EZERagent
    def fake_EZERagent(*args, **kw):
        if args[:2] == ("status", "--json"):
...
def c5_harness_allow_live_gate():
    """하네스 guard_isolation: LIVE 타깃은 EZERAGENT_PHOENIX_ALLOW_LIVE=1 없으면 거부, 있으면 통과."""
    HPH = os.path.normpath(os.path.join(HERE, "..", "EZERagent_phoenix_harness.py"))
    hspec = importlib.util.spec_from_file_location("EZERagent_phoenix_harness", HPH)
    h = importlib.util.module_from_spec(hspec)
    hspec.loader.exec_module(h)
    saved_hd, saved_sock = h.HARN_DIR, h.HARN_SOCK
    saved_env = os.environ.get("EZERAGENT_PHOENIX_ALLOW_LIVE")
    try:
        h.HARN_DIR = h.LIVE_STATE          # 강제 LIVE 타깃(사고/오설정 시뮬)
        h.HARN_SOCK = h.LIVE_SOCK
        os.environ.pop("EZERAGENT_PHOENIX_ALLOW_LIVE", None)
        refused = False
        try:
            h.guard_isolation()
        except SystemExit:
        ...
        os.environ["EZERAGENT_PHOENIX_ALLOW_LIVE"] = "1"
        allowed = True
        try:
        ...
        if saved_env is None:
            ...
            os.environ["EZERAGENT_PHOENIX_ALLOW_LIVE"] = saved_env
...
def c2_degraded_persists_across_retry(td):
    """★codex BLOCKING(1): 손상 복구→DEGRADED 후, EZERagentd auto-retry 상당 2차 실행에서도 보류 유지(휘발 금지).
