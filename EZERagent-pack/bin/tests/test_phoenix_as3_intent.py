...
(TOCTOU: topology.json 디스크 영속 확인 후에만 절단)을 결정론 검증. 데몬 불요(EZERagent/read_topology mock).

실행: python3 EZERagent-pack/bin/tests/test_phoenix_as3_intent.py  (0=전건 PASS)
"""
...
_ORIG_EZERAGENT = m.EZERagent  # 원본 EZERagent() 저장(시나리오 E 에서 복원 — 더미 EZERAGENT 로 rc127 강등)
...
def main():
    ...
    sock = os.path.join(sd, "EZERagent.sock")
    ijp = os.path.join(ph, "tombstone-intents.jsonl")

    # ── A. 데몬 다운타임: cmd_tombstone RPC 실패 → intent 저널 append(desired 직접 쓰기 아님) ──
    m.EZERAGENT = os.path.join(td, "nonexistent-EZERagent")  # EZERagent() rc127 → RPC 실패
    ...
    m.EZERagent = lambda *a, **k: _R(0, "tombstone ghost set")  # RPC 성공
    ...
    m.EZERagent = _ORIG_EZERAGENT  # 원본 EZERagent() 복원 — 더미 EZERAGENT 로 rc127(데몬 down·RPC 재동기 실패)
    sd2 = os.path.join(td, "e"); ph2 = os.path.join(sd2, "phoenix"); os.makedirs(ph2, exist_ok=True)
    sock2 = os.path.join(sd2, "EZERagent.sock")
