...
EZERagentd loop_auto_restore 의 지연 재시도가 '중복 스폰 0'을 시스템 속성으로 내는 근거는 **재시도가 phoenix
...
증명하고, E1 E2E 는 실 EZERagentd 재시도 loop 발화를 관측한다 — 본 테스트는 그 사이 'python 재산정' 층을 봉인.)

데몬 불요(더미 EZERAGENT 로 EZERagent() rc127 강등 → 스폰 실패 결정론). 라이브 무접촉.
실행: python3 EZERagent-pack/bin/tests/test_phoenix_e2e_crosslayer.py
"""
...
# lease 핸들 캡처 — 프로덕션은 attempt0·1 이 별도 EZERagentd 스폰(=별도 프로세스·별도 lease, atexit 로 각자 해제)이나
...
def main():
    ...
    m.EZERAGENT = os.path.join(td, "nonexistent-EZERagent")   # EZERagent() rc127 → 스폰 실패 결정론
    ...
    # attempt1: 재실행(EZERagentd 지연 재시도 상당) → live 재산정으로 worker-x alive → target 제외 → NOOP·spawn 0.
