#!/usr/bin/env python3
"""W6 수용 게이트 7시나리오 하네스 골격(리포 커밋). 데몬 불요(더미 EZERAGENT 로 EZERagent() rc127 정직 강등) —
...
실행: python3 EZERagent-pack/bin/tests/test_phoenix_w6_scenarios.py  (0=전건 PASS)
"""
...
def _sock(td, name):
    ...
    return os.path.join(d, "EZERagent.sock")


def main():
    td = tempfile.mkdtemp(prefix="phoenix-w6-")
    m.EZERAGENT = os.path.join(td, "nonexistent-EZERagent")  # EZERagent() → rc127 정직 강등(데몬 무접촉)
