...
이제 팩 schedule.json 배달이 아니라 **EZERagentd 코드(schedule.rs builtin_jobs)** 가 소유하고 데몬 부트 시
idempotent ensure 한다 — schedule.json 이 user-owned 로 전환돼(사용자 `EZERagent schedule add` 잡 보존) 팩
...
실행: python3 EZERagent-pack/bin/tests/test_phoenix_e2_schedule.py  (0=전건 PASS)
"""
...
SCHED_RS = os.path.join(ROOT, "src", "bin", "EZERagentd", "schedule.rs")
MAIN_RS = os.path.join(ROOT, "src", "bin", "EZERagentd", "main.rs")
...
def main():
    ...
    check("② phoenix-snapshot-6h 정의(6h·snapshot)",
          ...
          and "EZERagent_state_snapshot.py" in rs
