#!/usr/bin/env python3
"""EZERagent_briefing.py — W0-B 음성 브리핑 배달층 (EVT v1 → 발화 정책 → 3중 억제 → TTS)
...
- 3중 억제(Paperclip LiveUpdatesProvider의 클린룸 번안):
    ① rate limit — 타입당 10초 창 최대 3건 (상태: $EZERAGENT_ROOT/_round/briefing/rate.json)
...
- 모든 판정은 원장에 기록: $EZERAGENT_ROOT/_round/briefing/ledger.jsonl (spoken|suppressed + 사유)

사용:  echo '<EVT v1 line>' | EZERagent_briefing.py deliver [--speak] [--suppress-actor m] [--focus w]
       EZERagent_briefing.py deliver --line '<EVT v1 line>' ...
...
import EZERagent_event  # noqa: E402  (동일 폴더 계약 구현 재사용 — parse/speak)
...
def _voice_mode():
    ...
    테스트 주입: EZERAGENT_BRIEFING_VOICE_MODE=vm|plain|text
    """
    override = os.environ.get("EZERAGENT_BRIEFING_VOICE_MODE")
...
def deliver(line, speak_flag, suppress_actors, focus, voice=None):
    """반환: (exit_code, 설명 문자열)"""
    try:
        evt_type, payload = EZERagent_event.parse_wire(line)
    ...
    sentence = EZERagent_event.speak(evt_type, payload)
    spoken_mode = "dryrun"
    if speak_flag:
        mode = _voice_mode()
        if mode == "vm":
