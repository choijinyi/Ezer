#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""EZERagent_channels — 콘텐츠 채널 per-channel 헬스 doctor (AGENTREACH OPP-02).

EZERagent는 "부트 노드가 살아있나"(preflight C01~C48)는 결정론 검증하나 "지금
...
★coverage_battery 함정 봉인(보고서 OPP-02 self-검출):
  ...
  제외하므로 배포 머신(~/.EZERagent/pack)에 **존재하지 않는다**. 따라서:
...
서브커맨드:
    EZERagent_channels.py --self-test              # 네트워크0 — 집계/verdict/permutation/429비종결 박제, exit 0=ok
    EZERagent_channels.py --json [채널...]          # 라이브 타격(엔진 있을 때)·[{platform,verdict,evidence,routes}]
    EZERagent_channels.py --silence-first [채널...]  # 전부 STRONG_OK면 1줄, 아니면 실패 채널만 상세
...
def _battery_path():
    ...
    for base in (os.environ.get("EZERAGENT_PACK_DIR", ""),
                 os.path.join(os.path.expanduser("~"), ".EZERagent/pack"),
                 os.path.join(os.path.expanduser("~"), "dev/EZERagent/EZERagent-pack")):
        if not base:
            continue
