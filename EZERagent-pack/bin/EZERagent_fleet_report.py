#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""EZERagent_fleet_report — read-only fleet 채택 + 비용 digest (스케줄 발화, 핫패스 밖).
...
를 집계하고, events에 명령 컬럼이 없으므로 `EZERagent recall` FTS로 고빈도 수동 idiom(U1/U2 필터의
missed-savings 후보)을 마이닝한다.

★불변: analytics.db·데몬 테이블에 **절대 쓰지 않는다**(WAL 소유=EZERagentd). file:...?mode=ro 만.
한 DB 불량에 hard-fail 금지(skip+기록+계속). 데이터 문제에 절대 non-zero exit 안 함.

사용: python3 EZERagent_fleet_report.py [--json] [--push] [--days N=7] [--limit N=200] [--round-dir D]
  (무플래그) -> 단일화면 digest stdout(스케줄러 action:push 가 캡처·배달)
  --json     -> 풀 머신리더블 JSON stdout
  --push     -> digest stdout + `EZERagent send --queued --to master`(CLI --command 경로용 자체 배달)
의존성: 파이썬 표준 라이브러리(sqlite3 포함) + PATH의 `EZERagent`(recall/send; 없으면 그 단계 skip).

CORE-DEPENDENT(부재 EZERagent.app/EZERagentd 소스 필요 — 본 스크립트는 정직 디스코프):
...
LABEL_RE = re.compile(r"^(EZERagent|EZERagent-ceo|EZERagent-dept-[A-Za-z0-9-]+|aiterm)$")
...
def round_dir(arg):
    return (arg or os.environ.get("EZERAGENT_ROUND_DIR")
...
# ── missed-savings 마이닝 (EZERagent recall FTS — events엔 명령 컬럼 없음) ─────────────────────
def mine_missed_savings(days, limit):
    ...
    for idiom in MANUAL_IDIOMS:
        try:
            r = subprocess.run([EZERagent, "recall", idiom, "--days", str(days),
...
# ── PACK/버전 advisory (EZERagent_semver.py 위임 — 순수 advisory·db쓰기0·exit 영향0) ─────────────
def pack_version_advisory():
    """installed `EZERagent --version` vs source Cargo.toml version 을 결정론으로 판정(advisory only).

    ★불변 보존: analytics.db 미접근·non-zero exit 미발생·어떤 비가역/외부발행 행동도 0.
    EZERagent_semver.py(부재 가능)·EZERagent(부재 가능)·소스 트리(부재 가능) 중 하나라도 없으면
    조용히 None 반환(skip+기록). 실패는 절대 fleet_report 를 죽이지 않는다(라인 11 불변).
    """
    try:
        bindir = os.path.join(pack_dir(), "bin")
        semver = os.path.join(bindir, "EZERagent_semver.py")
        EZERagent = shutil.which("EZERagent")
        if not (os.path.isfile(semver) and EZERagent):
            return None
        out = subprocess.run([EZERagent, "--version"], capture_output=True, text=True,
                             timeout=15).stdout
        m = re.search(r"(\d+\.\d+\.\d+\S*)", out or "")
        if not m:
            return None
        installed = m.group(1)
        # 소스 후보 = EZERagent Cargo.toml(있으면). 없으면 advisory 불가 → skip.
        cargo = os.path.expanduser("~/dev/EZERagent/Cargo.toml")
        ...
        return {"verdict": data.get("verdict"), "installed_EZERagent": installed,
...
def render_digest(rep):
    ...
    if ms:
        ...
        L.append("• 고빈도 명령: EZERagent recall 불가(스캔 skip)")
    pv = f.get("pack_version_advisory")
    if pv and pv.get("verdict"):
        L.append("• PACK/버전 advisory(EZERagent_semver·결정론·비행동): installed EZERagent %s vs source %s → %s"
                 % (pv.get("installed_EZERagent"), pv.get("source_cargo"), pv["verdict"]))
...
def push_to_master(text):
    ...
    if not EZERagent:
        return False
    try:
        subprocess.run([EZERagent, "send", "--queued", "--to", "master", text],
...
def main():
    ...
    ap.add_argument("--push", action="store_true",
                    help="digest 를 EZERagent send --queued --to master 로 자체 배달(CLI --command 경로)")
