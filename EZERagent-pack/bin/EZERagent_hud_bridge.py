#!/usr/bin/env python3
"""EZERagent_hud_bridge.py — 메타버스 오피스 수집·정규화 브리지 (DESIGN.md §3)

EZERagent 데몬의 이벤트 스트림(`EZERagent events`)과 스냅샷(`EZERagent fleet/status --json`)을
WorldState(§4)로 정규화해 127.0.0.1 전용 HTTP(+SSE)로 내보낸다.

원칙:
  · 파이썬 stdlib만 사용 (외부 의존 0)
  · 읽기 전용 — 쓰기 API 없음, EZERagent에 대한 호출도 관측 명령만 (R4)
  · 소음 필터·코얼레싱 (§6.3) — watchdog 무시, fx 초당 상한, 알림류 중복 병합
  · 이벤트 유실 방어 — --cursor-file 로 시퀀스 영속, 재시작 시 gap 없이 재개

기동:  EZERagent run --scoped -- python3 bin/EZERagent_hud_bridge.py
...
# Windows: 이 브리지는 콘솔 없는 EZERagentd가 NO_WINDOW로 띄운다 — 콘솔 자식(EZERagent.exe)을 그냥
...
EZERAGENT_ROOT = os.environ.get("EZERAGENT_ROOT") or os.path.expanduser("~/Desktop/EZERagent")
TASKS_DIR = os.path.join(EZERAGENT_ROOT, "_round", "tasks")
ROUND_DIR = os.path.join(EZERAGENT_ROOT, "_round")
TRANSCRIPTS_DB = os.path.expanduser("~/.EZERagent/transcripts.db")
...
#   이미 EZERagent를 직접 호출 가능하므로 경계 밖 — 이 게이트의 목적이 아니다).
...
class World:
    ...
    def _annotate_depts(self, new_deps):
        ...
        dept 필드 부재(구 EZERagent 조합) 시 display_name 정규화 폴백 + 1회 경고(계약 §4b·§5).
        """
        used = set()
        sockets = {}
        for d in new_deps:
            raw_dept = d.get("dept")
            if raw_dept is None:   # 구 EZERagent: dept 필드 없음 → display_name 정규화 폴백 + 1회 경고
...
def maybe_rotate_spool(path, offset):
    ...
    0(드레인)일 때만 호출한다. 방출자(EZERagent_event._spool_append)는 open→write(1줄 원자)→close
...
def scan_kanban(tasks_dir, now=None):
    """`$EZERAGENT_ROOT/_round/tasks/*.json`을 읽어 칸반 뷰로 정규화 (읽기 전용)."""
    now = time.time() if now is None else now
    tasks = []
    try:
...
def peek_surface(key, socket=None, lines=12):
    ...
    부서 소켓(socket 인자)이 있으면 `EZERagent --socket <path>` 전역 옵션으로 해당 데몬에 라우팅한다.
    """
...
def run_json(args, timeout=10):
    try:
        out = subprocess.run([EZERAGENT] + args, capture_output=True, text=True, timeout=timeout, **NOWIN)
        if out.returncode != 0:
            return None
...
class SubscriptionSupervisor:
    ...
    cursor 는 slug 별 cursor-<slug>.seq. 각 구독은 독립 리더 스레드 + `EZERagent [--socket S] events`.
    """
    ...
    def _reader(self, slug, socket, stop, holder):
        ...
        args_base = [EZERAGENT] + (["--socket", socket] if socket else []) + \
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # `EZERagent --socket <path>` 로 해당 데몬에 라우팅 — 본부(socket=None)는 --socket 생략.
        found, socket = self.world.socket_for(key)
        if not found:
            return self._cmd_result(403, False, "unknown_target", cleaned)
        bare = key.split("@", 1)[1]
        pre = [EZERAGENT] + (["--socket", socket] if socket else [])
        # 동일화 경로: EZERagent send(타이핑) → send-key Return(확정). argv 배열 직접
