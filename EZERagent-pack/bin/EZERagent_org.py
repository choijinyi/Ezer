#!/usr/bin/env python3
"""EZERagent_org.py — 부서 자동 편성 브리지 (org-manifest → 검증·적용·착수확인·삭제).
설계: multi-master-ceo/2026-06-26-org-provisioning-design.md (v2)
하우스스타일: EZERagent_manifest.py (--self-test 밀폐 검증)
...
# EZERagent_org 전체 불능이던 P0 차단). 락 실패해도 최종 원자교체(os.replace)가 일관성 보장 → best-effort.
try:
...
CATALOG = os.environ.get("EZERAGENT_DEPT_CATALOG", f"{HOME}/.EZERagent/dept-catalog.json")
DEPTS = os.environ.get("EZERAGENT_DEPTS_JSON", f"{HOME}/.EZERagent/depts.json")
MISSIONS = os.environ.get("EZERAGENT_DEPT_MISSIONS", f"{HOME}/.EZERagent/dept-missions")
...
def require_cso():
    if os.environ.get("EZERAGENT_ROLE") != "cso":
        sys.stderr.write("[EZERagent_org] ★CSO 전용: apply/destroy는 EZERAGENT_ROLE=cso에서만(부서 mutation 단일소유). CSO에 위임하라.\n")
        sys.exit(3)
...
def backfill_mission_key(depts_path, key, mission_key, display=None):
    """레거시 부서에 mission_key 소급(F6). EZERagent-dept와 같은 .lock으로 직렬화, 다른 필드 무접촉.
...
def v_quote_binding(depts, doc_text, catalog, dept_level=True):
    ...
    for i, d in enumerate(depts):
        ...
        if dept_level:
            ...
            # cwd 규약 경로
            cwd = d.get("cwd", "")
            if disp and not cwd.replace("\\", "/").endswith(f"Desktop/EZERagent/{disp}"):
                errs.append(f"{tag}: cwd가 규약 경로($HOME/Desktop/EZERagent/{disp}) 불일치")
    return errs
...
def dept_status(socket):
    """부서 소켓의 EZERagent status --json 회수."""
    r = subprocess.run(["EZERagent", "--socket", socket, "status", "--json"],
                       capture_output=True, text=True, env={**os.environ, "EZERAGENT_NO_AUTOSTART": "1"})
...
def tar_snapshot(key, workdir, dest_dir=None):
    ...
    dest_dir = dest_dir or f"{HOME}/.EZERagent/dept-snapshots"
    os.makedirs(dest_dir, exist_ok=True)
    stamp = os.environ.get("EZERAGENT_SNAP_STAMP", str(int(os.path.getmtime(workdir))))
...
def destroy_dept(name, mission_key, purge=False, purge_workdir=False):
    ...
    # 2) EZERagent-dept down 위임 — 부모 role 상속(require_cso로 이미 cso 보장·하류 가드 무력화 방지·R1 REVISE-1)
    r = subprocess.run(["EZERagent-dept", "down", name], capture_output=True, text=True,
                       env={**os.environ})
    actions.append(("down", r.returncode))
    # 3) pack-dept rm (결정론) — forwarder는 self-reap(직접 회수 안 함)
    if purge:
        pack = f"{HOME}/.EZERagent/pack-dept-{name}"
...
def classify_dept(alive, intake):
    if not alive: return "redeploy"   # 소켓 死 → 멱등 apply 재실행(EZERagent-dept REUSE_DEAD 재spawn)
...
def create_dept(key):
    """EZERagent-dept create 위임. 부서장 각성·미션주입·격리·멱등 전부 EZERagent-dept 책임."""
    require_cso()  # 게이트를 효과 함수에 (R1 REVISE-1) — import 직접호출 우회 차단
    r = subprocess.run(["EZERagent-dept", "create", key], capture_output=True, text=True,
                       env={**os.environ})  # 부모 role 상속(require_cso로 cso 보장·하류 가드 유지)
    return r.returncode, (r.stdout or "") + (r.stderr or "")

def dispatch_task(t):
    """첫 프로젝트 = 유일 실행 티켓(task-prompt). 미션 md의 첫프로젝트와 중복 금지(DUP-4)."""
    cmd = ["python3", f"{HOME}/.EZERagent/pack/bin/EZERagent_orchestra.py", "task-prompt",
...
def self_test():
    ...
    # Task별로 케이스가 여기 누적된다.
    # --- Task2: v_schema ---
    good_dept = {"key":"future-research","display":"미래연구부","account":"ezerinsight",
                 "cwd":"$HOME/Desktop/EZERagent/미래연구부","mission_md":"# m","source_quote":"x"}
    ...
    cat = {"accounts":{"ezerinsight":"x","owner":"y"},
           "departments":{
             "future-research":{"display":"미래연구부","account":"ezerinsight"},
             "authoring":{"display":"저술콘텐츠부","account":"owner"}}}
    d_ok = {"key":"future-research","display":"미래연구부","account":"ezerinsight",
            "cwd":"$HOME/Desktop/EZERagent/미래연구부",
            "source_quote":"미래연구부는 모든 통찰의 원천 엔진이다."}
    chk("f1-ok", v_quote_binding([d_ok], doc, cat) == [], f"errs={v_quote_binding([d_ok],doc,cat)}")
    # 오귀속: 실재(미래연구부) 문장을 엉뚱한 key/account(authoring/ezerinsight)에 붙임 → FAIL
    d_mis = {**d_ok, "key":"authoring", "display":"미래연구부", "account":"ezerinsight"}
    chk("f1-misattr", v_quote_binding([d_mis], doc, cat) != [], "오귀속 미검출")
    # key↔account 불일치 (authoring은 owner여야)
    d_acct = {"key":"authoring","display":"저술콘텐츠부","account":"ezerinsight",
              "cwd":"$HOME/Desktop/EZERagent/저술콘텐츠부",
    ...
    empty_cat = {"accounts":{"ezerinsight":"x","owner":"y"},"departments":{}}
    # greenfield(empty catalog) fabricated 신규key + 실재 quote + display 위장 → 승인플래그 없으면 FAIL
    d_mis_empty = {**d_ok, "key":"shadow-ops", "display":"미래연구부", "account":"ezerinsight",
                   "source_quote":"미래연구부는 모든 통찰의 원천 엔진이다."}
    ...
    d_keybind = {**d_ok, "key":"shadow-ops", "display":"미래연구부", "account":"ezerinsight",
    ...
    json.dump({"version":1,"accounts":{"ezerinsight":"x","owner":"y"},"departments":{}}, open(cpath,"w"))
    catalog_upsert(cpath, {"key":"authoring","display":"저술콘텐츠부","account":"owner",
                           "cwd":"$HOME/Desktop/EZERagent/저술콘텐츠부"})
    catalog_upsert(cpath, {"key":"authoring","display":"저술콘텐츠부","account":"owner",
                           "cwd":"$HOME/Desktop/EZERagent/저술콘텐츠부"})  # 멱등
    ...
    json.dump({"depts":{"d3":{"cwd":"$HOME/Desktop/EZERagent/미래연구부","socket":"s3"}}}, open(dpath,"w"))
    ...
    json.dump({"depts":{"d4":{"cwd":"$HOME/Desktop/EZERagent/구미래연구부","socket":"s4"}}}, open(dpath,"w"))
    backfill_mission_key(dpath, "future-research", "future-research", "미래연구부")
    ...
    cat_drift = {"accounts":{"ezerinsight":"x"},
                 "departments":{"future-research":{"display":"미래연구부","account":"ezerinsight","cwd":"$HOME/Desktop/EZERagent/미래연구부"}}}
    d_drift = [{"key":"future-research","display":"위장된딴부서","account":"ezerinsight","cwd":"$HOME/Desktop/EZERagent/미래연구부"}]
    chk("catalog-drift-display", any("display" in e for e in v_catalog_consistency(d_drift, cat_drift)), "display drift 미검출")
    d_cwd_drift = [{"key":"future-research","display":"미래연구부","account":"ezerinsight","cwd":"$HOME/Desktop/EZERagent/딴경로"}]
    chk("catalog-drift-cwd", any("cwd" in e for e in v_catalog_consistency(d_cwd_drift, cat_drift)), "cwd drift 미검출")
    d_consistent = [{"key":"future-research","display":"미래연구부","account":"ezerinsight","cwd":"$HOME/Desktop/EZERagent/미래연구부"}]
