...
set -euo pipefail
T=$(mktemp -d)
export EZERAGENT_DEPT_CATALOG="$T/catalog.json"
export EZERAGENT_DEPTS_JSON="$T/depts.json"
export EZERAGENT_DEPT_MISSIONS="$T/missions"
BIN="$(cd "$(dirname "$0")" && pwd)/EZERagent_org.py"
...
echo "== apply CSO 게이트 (exit3 기대·EZERAGENT_ROLE 없음) =="
if EZERAGENT_ROLE= python3 "$BIN" apply "$T/m.json"; then echo "E2E FAIL: 비-CSO apply 통과"; exit 1; else echo "  → 기대대로 차단"; fi

echo "== destroy workdir 부재 (rc0·skip 기대·R1 REVISE-2) =="
# ★EZERagent-dept 스텁: 실제 down은 격리 reg_count==0 시 라이브 ceo_demote(MASTER_DIRECTIVE.md.pre-ceo)를
#   건드림 → 무접촉 보장 위해 스텁으로 대체(라이브 EZERagent-dept 무호출).
mkdir -p "$T/bin"; printf '#!/bin/bash\necho "[stub EZERagent-dept] $*"\nexit 0\n' > "$T/bin/EZERagent-dept"; chmod +x "$T/bin/EZERagent-dept"
echo '{"depts":{"e2e-dept":{"cwd":"'"$T"'/Desktop/EZERagent/없는부서","socket":"'"$T"'/fake.sock","mission_key":"e2e-dept"}}}' > "$EZERAGENT_DEPTS_JSON"
OUT=$(PATH="$T/bin:$PATH" EZERAGENT_ROLE=cso python3 "$BIN" destroy --dept e2e-dept --purge-workdir); RC=$?
echo "$OUT"
...
BINDIR="$(dirname "$BIN")"
if PATH="$T/bin:$PATH" EZERAGENT_ROLE=worker python3 -c "
import sys; sys.path.insert(0,'$BINDIR'); import EZERagent_org
ok=True
for fn,args in [('apply_manifest',({},)),('destroy_dept',('x','x')),('create_dept',('x',))]:
    try: getattr(EZERagent_org,fn)(*args); print('FAIL',fn,'no exit'); ok=False
