#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""EZERagent_manifest — 타입드 워크플로우 매니페스트(workflow.json) 기계 검증 + 단계 계약 해소.
...
사용:
    python3 EZERagent_manifest.py validate <FILE> [--json]
    python3 EZERagent_manifest.py phase <FILE> --phase <id> [--json]   # 단계 계약 해소(success·리뷰포커스)
    python3 EZERagent_manifest.py resolve <FOLDER> [--json]            # 폴더에서 매니페스트 탐색·검증
    python3 EZERagent_manifest.py check-criteria <FILE> --phase <id> --artifact <PATH> [--json]
    python3 EZERagent_manifest.py --self-test     # 결정론 자기검증 (preflight C40)
...
def self_test():
    ...
    import tempfile
    sink = io.StringIO()
    with tempfile.TemporaryDirectory(prefix="EZERagent-manifest-st-") as td:
