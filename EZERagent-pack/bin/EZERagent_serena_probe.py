#!/usr/bin/env python3
"""EZERagent_serena_probe.py — Serena MCP/LSP 생명주기 라이브니스 프로버 (S4 · CSO heartbeat).

설계서: _research/Serena기반_EZERagent_업그레이드_구현설계서.md §S4.
역할: 장수 Serena(HTTP/공유 regime, dashboard ON) 의 /heartbeat 라이브니스와 LSP 자식-트리
      fan-out 을 주기적(EZERagent schedule)으로 관측해 ALERT만 낸다. **절대 kill 하지 않는다** —
      실제 reap(close-surface/EZERagent kill)은 CSO 가 alert 를 받고 수행한다.
...
핵심 불변(설계서 §S4):
  ...
  - foreground poll 금지 — EZERagent schedule cadence 로만 호출된다.
...
STATE_PATH = os.path.expanduser("~/.EZERagent/state/serena-probe.json")
...
def send_alert(role, msg):
    """EZERagent send --queued --to <role> 로 OOB push (자동 Return 배달). 실패는 stderr."""
    import shutil
    import subprocess
    EZERagent = shutil.which("EZERagent")
    if not EZERagent:
        sys.stderr.write("[serena-probe] EZERagent 미발견 — alert 미전송: %s\n" % msg)
        return False
    try:
        subprocess.run([EZERagent, "send", "--queued", "--to", role, msg],
        ...
        if int(p) in live:
            ...
            if ps["miss"] >= miss_threshold:
                alerts.append("[serena-probe] heartbeat MISS on port %s (%d consecutive) — "
                              "CSO: close-surface/EZERagent kill 로 트리 reap 검토" % (p, ps["miss"]))
                ps["alive_seen"] = False
                ps["miss"] = 0
    # 새로 alive 인 포트 등록.
    for p in live:
...
def self_test():
    ...
    print("EZERagent_serena_probe self-test OK (psutil=%s)" % _HAS_PSUTIL)
    return 0
