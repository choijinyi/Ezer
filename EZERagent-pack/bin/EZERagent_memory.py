#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""EZERagent_memory — 장기기억 증류 결정론 도구 (slow 종료 게이트의 기계 검증부).
...
사용:
    python3 EZERagent_memory.py add --type feedback --name <kebab-slug> \
        --desc "<한 줄 요약>" --body "<사실 본문>"        # 증류 1건 (원자적)
    python3 EZERagent_memory.py verify [--json]               # 색인↔파일 정합 기계검증
    python3 EZERagent_memory.py recent --minutes 1440 [--json] # 최근 증류 목록 (게이트 증거)
    python3 EZERagent_memory.py --self-test                    # 결정론 자기검증 (preflight C18)

공통 옵션: --dir <memory 디렉터리> (기본: $EZERAGENT_PACK_DIR/memory 또는 ~/.EZERagent/pack/memory)
...
# P0.2 메모리 포이즌 WARN 게이트(구현설계서 v2 §3) — EZERagent_skillscan(같은 bin) 있으면 활성.
...
try:
    import EZERagent_skillscan as _skillscan
except Exception:
    _skillscan = None
    print("⚠ [memory] 포이즌 스캐너(EZERagent_skillscan) 로드 실패 — WARN 게이트 다운(무검사 fail-open) "
...
def default_memory_dir():
    ...
    return os.path.join(os.path.expanduser("~"), ".EZERagent/pack", "memory")
...
def collect_problems(mdir):
    ...
    if not os.path.isfile(index_path):
        return ["%s 없음 — EZERagent init-pack 또는 골격 복원 필요" % INDEX_FILE]
...
def cmd_health(mdir, as_json):
    """건강도 대시보드 — 분석 전용(수정 0). 외부 /memory-health의 EZERagent판.
...
def cmd_audit(mdir, threshold, as_json):
    ...
    외부 /memory-consolidate(memory-auditor가 분석만)의 EZERagent판."""
...
def cmd_scan_memory(mdir, native_dirs, as_json):
    ...
    if as_json:
        ...
        if _skillscan is None:
            print("memory scan: EZERagent_skillscan 미가용 — 스캔 불가(graceful)")
...
def self_test():
    ...
    failures = []
    with tempfile.TemporaryDirectory(prefix="EZERagent-memory-selftest-") as td:
