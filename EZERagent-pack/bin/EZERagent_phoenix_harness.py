...
# -*- coding: utf-8 -*-
"""
EZERagent_phoenix_harness.py — 불사조(무손실 복원) Phase 1 격리 하네스

설계 근거: _round/ZERO_LOSS_RESTORE_DESIGN.md §9.2 P3 · §9.4-1
목적: 라이브 데몬·소켓·~/.local/state/EZERagent/ 를 절대 건드리지 않고, 별도 소켓·별도
      상태 디렉터리(~/.EZERagent/state-harness/)에서 격리된 테스트 데몬을 띄워
      크래시 주입·부활 drill·프리미티브 자격시험을 안전하게 반복하는 "증명 장소".

격리 계약(실측 확정):
  · 데몬 bind:  EZERAGENT_SOCKET=<harness>/EZERagent.sock  EZERagentd
                → 데몬이 그 소켓에 listen하고, 상태 디렉터리는 소켓의 부모 디렉터리로 격리됨
                  (analytics.db·transcripts.db·event.seq·schedule_state.json·EZERagent.lock 전부 격리 dir)
  · 클라이언트: EZERagent --socket <harness>/EZERagent.sock <cmd>
  · 함정: `EZERagent --socket <X> list` 는 데몬이 없으면 자동으로 EZERagentd를 autostart한다(고아 위험).
          `EZERagent --socket <X> ping` 은 autostart하지 않는다 → 데몬 down 감지는 반드시 ping으로 한다.
...
자원 거버넌스(잔여 프로세스 0 절대 준수):
  ...
  · teardown은 (1) 살아있으면 모든 격리 surface를 close-surface로 닫고 (2) 격리 dir에 bind한
    모든 EZERagentd를 lsof로 찾아 kill -9 (autostart 고아 청소) (3) 추적 데몬 그룹 killpg 순으로 동작한다.

사용:
  EZERagent_phoenix_harness.py up            # 격리 데몬 기동 + 라이브 무영향 불변식 실측
  EZERagent_phoenix_harness.py down          # 전면 teardown + 잔여 0 실측
  EZERagent_phoenix_harness.py status        # 격리/라이브 상태 스냅샷
  EZERagent_phoenix_harness.py crash-daemon  # 격리 데몬 kill -9 → 재기동(fixture 재부팅) 성공 검증
  EZERagent_phoenix_harness.py crash-agent   # 격리 surface 에이전트 kill -9 시나리오
  EZERagent_phoenix_harness.py record-cli    # 실 claude CLI 기동→ready_marker→resume PTY 1회 녹화(토큰0)
  EZERagent_phoenix_harness.py qualify       # 기존 프리미티브 자격시험 4건(--queued/restore/watch/lock경합)
  EZERagent_phoenix_harness.py drill         # 위 전 과정 일괄 + 증거 JSON 방출(HARNESS_REPORT 원천)
"""
...
# --- 라이브(절대 무접촉) ---
LIVE_STATE = os.path.join(HOME, ".local", "state", "EZERagent")
LIVE_SOCK = os.path.join(LIVE_STATE, "EZERagent.sock")

# --- 격리(하네스 전용) ---
HARN_DIR = os.path.join(HOME, ".EZERagent", "state-harness")
HARN_SOCK = os.path.join(HARN_DIR, "EZERagent.sock")
...
_tracked_daemon = None  # subprocess.Popen of the isolated EZERagentd we own


# ------------------------------------------------------------------ 유틸

def _resolve_EZERagentd():
    """EZERagentd 절대경로 결정 — PHOENIX_HARNESS_EZERAGENTD 오버라이드 우선(Phase3 신 바이너리 자격시험),
    없으면 app 번들, 없으면 PATH."""
    override = os.environ.get("PHOENIX_HARNESS_EZERAGENTD")
    if override and os.path.exists(override):
        return os.path.realpath(override)
    cand = "/Applications/EZERagent.app/Contents/MacOS/EZERagentd"
    if os.path.exists(cand):
        return cand
    p = shutil_which("EZERagentd")
    if p:
        return os.path.realpath(p)
    die("EZERagentd 바이너리를 찾을 수 없다.")
...
def _resolve_EZERagent():
    p = shutil_which("EZERagent")
    return p or "EZERagent"


EZERAGENTD = None  # lazy
EZERAGENT = None
...
def guard_isolation():
    ...
    ★C5/P1-10(W3): 라이브 상태/소켓을 타깃하는 것은 **EZERAGENT_PHOENIX_ALLOW_LIVE=1 명시 opt-in** 이 있을 때만
    ...
    allow_live = os.environ.get("EZERAGENT_PHOENIX_ALLOW_LIVE") == "1"
    ...
    if overlap or sock_same:
        if not allow_live:
            die("★C5/P1-10: 하네스가 라이브 상태(%s)/소켓을 타깃 — EZERAGENT_PHOENIX_ALLOW_LIVE=1 명시 opt-in "
                "없으면 LIVE write 거부(격리가 기본)." % ls)
        log("★C5 경고: EZERAGENT_PHOENIX_ALLOW_LIVE=1 — 하네스의 라이브 상태 쓰기 허용(명시 opt-in·위험 작업).")


def _daemon_env():
    env = dict(os.environ)
    env["EZERAGENT_SOCKET"] = HARN_SOCK       # ← 데몬 bind 경로 오버라이드(실측 확정)
    # ★W2: 하네스 데몬은 phoenix 로직을 직접 테스트한다 — EZERagentd 콜드부트 auto-restore(W6 --socket 이후 하네스
    #   소켓 대상)가 restore.lease 를 잡으면 드릴의 직접 restore 가 LEASE_HELD 로 경합한다. 기본 비활성화하되
    #   (드릴 결정론화·EZERagentd auto-restore 는 E1 e2e_replacement 가 별도 검증), 호출자가 os.environ 으로 명시
    #   설정(예: E1 이 "0"으로 auto-restore 활성화)하면 존중한다.
    env.setdefault("EZERAGENT_NO_AUTORESTORE", "1")
    for k in LEAKY_ENV:
        env.pop(k, None)
    return env


def EZERagent(*args, timeout=20, socket=True):
    """격리 데몬 대상 EZERagent 호출. socket=False면 라이브 대상(무접촉 관측용 list/ping만)."""
    cmd = [EZERAGENT]
    if socket:
...
def live_surfaces():
    ...
    try:
        r = subprocess.run([EZERAGENT, "list"], capture_output=True, text=True, timeout=10)
    except Exception:
        return []
...
def harness_daemon_pids():
    """격리 dir(state-harness)에 bind한 모든 EZERagentd pid — lsof 실측(autostart 고아 포함).
    ...
    수리: (a) 격리 소켓(HARN_SOCK)의 realpath 를 lsof 출력에서 정확히 매칭(socket 기반 — 어떤 EZERagentd 가 우리 소켓을
    ...
    try:
        out = subprocess.run(["pgrep", "-x", "EZERagentd"], capture_output=True, text=True, timeout=10).stdout
    except Exception:
        return pids
...
def close_all_surfaces():
    ...
    closed = []
    for ref in refs:
        EZERagent("close-surface", ref, timeout=10)
        closed.append(ref)
    return closed
...
def teardown(verbose=False):
    ...
    try:
        ...
        pass
    # 2) 격리 dir에 bind한 모든 EZERagentd(우리 것 + autostart 고아) kill -9
    ...
    if verbose:
        log("teardown 후 격리 dir bind EZERagentd 잔여: %s" % (remain or "0(없음)"))
    return remain
...
def _wipe_daemon_state():
    ...
    for name in ("topology.json", "analytics.db", "analytics.db-shm", "analytics.db-wal",
                 "event.seq", "queue-state.json", "schedule_state.json", "EZERagent.lock",
...
def cmd_crash_daemon(args):
    ...
    ev["state_files_survived"] = sorted(
        ...
        if f.endswith(".db") or f in ("event.seq", "schedule_state.json", "EZERagent.lock")
...
def cmd_crash_agent(args):
    ...
    guard_isolation()
    _fresh_harness()
    r = EZERagent("new-surface", timeout=15)
    ...
    time.sleep(0.5)
    lst = EZERagent("list", timeout=10).stdout or ""
    pid = None
    ...
    lst2 = EZERagent("list", timeout=10).stdout or ""
...
def _q_queued_durability():
    ...
    ev["pause_out"] = (EZERagent("pause", timeout=10).stdout or "").strip()[:160]
    snd = EZERagent("send", "--queued", "--surface", ref, "PHOENIX_PROBE_MSG_A1", timeout=10)
    ev["send_queued_rc"] = snd.returncode
    ev["send_queued_out"] = (snd.stdout or snd.stderr or "").strip()[:200]
    q1 = EZERagent("queue", "list", timeout=10)
    ...
    time.sleep(0.6)
    start_daemon()
    q2 = EZERagent("queue", "list", timeout=10)
    ev["queue_after_restart"] = (q2.stdout or q2.stderr or "").strip()[:400]
    EZERagent("resume", timeout=10)
...
def _q_restore_idempotency():
    ...
    n0 = len([l for l in (EZERagent("list", timeout=10).stdout or "").splitlines() if l.startswith("surface:")])
    ev["surfaces_before"] = n0
    r1 = EZERagent("restore", timeout=45)
    ev["restore1_rc"] = r1.returncode
    ev["restore1_out"] = (r1.stdout or r1.stderr or "").strip()[:500]
    time.sleep(1.0)
    n1 = len([l for l in (EZERagent("list", timeout=10).stdout or "").splitlines() if l.startswith("surface:")])
    ev["surfaces_after_restore1"] = n1
    r2 = EZERagent("restore", timeout=45)
    ev["restore2_rc"] = r2.returncode
    ev["restore2_out"] = (r2.stdout or r2.stderr or "").strip()[:500]
    time.sleep(1.0)
    n2 = len([l for l in (EZERagent("list", timeout=10).stdout or "").splitlines() if l.startswith("surface:")])
...
def _q_watch():
    ...
    box = {}
    def _run_watch():
        w = EZERagent("watch", "--surface", ref, "--until", marker, "--timeout", "12", timeout=16)
        box["rc"] = w.returncode
        box["out"] = (w.stdout or w.stderr or "").strip()[:200]
    th = threading.Thread(target=_run_watch, daemon=True)
    th.start()
    time.sleep(1.0)
    EZERagent("send", "--surface", ref, "echo %s" % marker, timeout=10)
    EZERagent("send-key", "--surface", ref, "Return", timeout=10)
    th.join(timeout=14)
...
def _q_startup_lock_contention():
    """d) launchd KeepAlive vs 앱 내장 데몬 startup lock 경합 재현 — 같은 소켓에 EZERagentd 2개 동시 기동."""
    ev = {"test": "startup_lock_contention"}
    _fresh_harness()
    ev["primary_alive"] = harness_ping()
    # 같은 격리 소켓에 두 번째 EZERagentd 기동 시도 → startup lock 경합 재현
    p = subprocess.run([EZERAGENTD], env=_daemon_env(), capture_output=True, text=True,
                       cwd=HARN_DIR, timeout=8)
    combined = (p.stdout or "") + (p.stderr or "")
    ev["second_EZERagentd_rc"] = p.returncode
    ev["second_EZERagentd_output"] = combined.strip()[:400]
    ev["lock_contention_reproduced"] = ("holds the startup lock" in combined) or ("EZERagent.lock" in combined)
    ev["verdict"] = ("REPRODUCED(두 번째 EZERagentd가 startup lock에 막혀 종료 — EZERagentd.log 경합과 동일)"
...
# 불사조 부활 저널 상태머신(EZERagent_phoenix.py)을 격리 하네스에서 완료 기준 drill로 검증한다.

PHOENIX = os.path.join(os.path.dirname(os.path.abspath(__file__)), "EZERagent_phoenix.py")
...
def _phoenix(*args, extra_env=None, timeout=60):
    """EZERagent_phoenix.py 를 격리 소켓 대상으로 실 CLI 호출(하위 프로세스 — 실제 게이트 검증)."""
...
def cmd_phoenix_drill(args):
    ...
    rns = EZERagent("new-surface", timeout=15)
    m = re.search(r"(surface:\d+)", rns.stdout or "")
    ref = m.group(1) if m else None
    EZERagent("send", "--surface", ref, "echo PHOENIX_STUB_READY role=worker SESSION=SID-W-1 ENDMARK; exec sleep 3600", timeout=10)
    EZERagent("send-key", "--surface", ref, "Return", timeout=10)
    time.sleep(1.0)
    ...
    rns2 = EZERagent("new-surface", timeout=15)
    m2 = re.search(r"(surface:\d+)", rns2.stdout or "")
    ref2 = m2.group(1) if m2 else None
    EZERagent("send", "--surface", ref2, "EZERagent --socket %s claim-role worker; exec sleep 3600" % HARN_SOCK, timeout=10)
    EZERagent("send-key", "--surface", ref2, "Return", timeout=10)
    time.sleep(1.5)
    _seed_topology([{"agent": "stub", "agent_bin": "stub", "cwd": HARN_DIR,
                     "role": "worker", "session_id": "SID-W-1", "title": "w"}])
...
def cmd_phoenix_p5_redelivery(args):
    ...
    def _mk_worker():
        ...
        claimed = False
        for _ in range(8):
            EZERagent("send", "--surface", ref, "EZERagent --socket %s claim-role worker" % HARN_SOCK, timeout=10)
            EZERagent("send-key", "--surface", ref, "Return", timeout=10)
            time.sleep(1.2)
            if "worker" in live_role_surfaces_local():
                claimed = True
                break
        EZERagent("send", "--surface", ref, "exec sleep 600", timeout=10)
        EZERagent("send-key", "--surface", ref, "Return", timeout=10)
        time.sleep(0.5)
        return ref, claimed
    ...
    ev["s1"] = s1
    # ★Phase10 정직 SKIP: send-key Return 이 harness surface 명령을 실행하지 못하는 환경(app-bundle EZERagentd PTY)에서는
    ...
    if not claimed1:
        ...
        print(json.dumps(ev, ensure_ascii=False, indent=2))
        return ev
    EZERagent("pause", timeout=10)
    snd = EZERagent("send", "--queued", "--to", "worker", "P5_REDELIV_MSG", timeout=10)
    ...
    time.sleep(0.6)
    start_daemon()
    q = EZERagent("queue", "list", timeout=10)
    ...
    scr = EZERagent("read-screen", "--surface", s2, timeout=12) if s2 else None
...
def cmd_phoenix_p6_bootepoch(args):
    ...
    def _started_at():
        r = EZERagent("status", "--json", timeout=10)
        try:
    ...
    # 실 EZERagentd 의 topology 영속을 시뮬레이션(재기동 시 actual-state 재구성) — desired 로스터는 이미 worker 박제
    seed()
...
def cmd_phoenix_p7_inherit(args):
    ...
    for dep in ("EZERagent-dept-alpha", "EZERagent-dept-beta"):
    ...
    with open(depts_json, "w") as f:
        json.dump({"depts": {"alpha": {"socket": os.path.join(dept_root, "EZERagent-dept-alpha", "EZERagent.sock")}}}, f)
    ...
    snap = os.path.join(os.path.dirname(PHOENIX), "EZERagent_state_snapshot.py")
    probe = subprocess.run([sys.executable, "-c",
        "import sys; sys.path.insert(0, %r); import EZERagent_state_snapshot as m; "
    ...
    ev["snapshot_covers_alpha"] = ("EZERagent-dept-alpha" in src_lines and "schedule_state.json" in src_lines)
    ev["snapshot_covers_beta"] = ("EZERagent-dept-beta" in src_lines)
    ...
    subprocess.run(["pkill", "-9", "-f", "sleep 600"], capture_output=True)
    time.sleep(0.6)
    _wipe_daemon_state()  # ★EZERagentd 자동복원 차단(phoenix roster 보존) → dead_by_desired 결정론
    start_daemon()
...
def cmd_phoenix_p8_backup(args):
    """★Phase 8: 일반 백업 능력 + 정직 보호 상태 회귀 drill. EZERagent_backup 함수 self-test(단위) +
    ...
    live_before = len(live_surfaces())
    BACKUP = os.path.join(os.path.dirname(PHOENIX), "EZERagent_backup.py")
    import shutil as _sh
    import tempfile as _tf
    scratch = _tf.mkdtemp(prefix="EZERagent-p8-", dir="/tmp")
    try:
        ...
        pack = os.path.join(home, ".EZERagent", "pack")
        ...
        soul_r = os.path.join(dest, ".EZERagent", "pack", "soul.md")
        ev["restored_soul_matches"] = os.path.isfile(soul_r) and open(soul_r).read() == "SOUL\n"
        ev["token_not_restored"] = not os.path.exists(os.path.join(dest, ".EZERagent", "pack", "api.token"))
        ...
        red = json.loads(cli("status", "--home", home, env={"EZERAGENT_BACKUP_DIR": os.path.join(scratch, "none")}).stdout)
        ev["status_red_no_backup"] = red["grade"] == "RED"
        green = json.loads(cli("status", "--home", home,
                               env={"EZERAGENT_BACKUP_DIR": out, "EZERAGENT_BACKUP_OFFSITE": "scratch://remote"}).stdout)
...
def cmd_phoenix_p9_catastrophe(args):
    ...
    import tempfile as _tf
    _sh.rmtree(ph, ignore_errors=True)
    scratch = _tf.mkdtemp(prefix="EZERagent-p9cat-", dir="/tmp")
    BACKUP = os.path.join(os.path.dirname(PHOENIX), "EZERagent_backup.py")
    try:
        # 합성 부서(env 격리) + 백업 대상 정체성 home(스크래치)
        dept_root = os.path.join(HARN_DIR, "dept-root")
        _sh.rmtree(dept_root, ignore_errors=True)
        dd = os.path.join(dept_root, "EZERagent-dept-alpha")
        os.makedirs(dd)
        open(os.path.join(dd, "schedule_state.json"), "w").write('{"jobs":[]}')
        depts_json = os.path.join(dept_root, "depts.json")
        json.dump({"depts": {"alpha": {"socket": os.path.join(dd, "EZERagent.sock")}}}, open(depts_json, "w"))
        dept_env = {"PHOENIX_DEPT_STATE_ROOT": dept_root, "PHOENIX_DEPTS_JSON": depts_json}
        bhome = os.path.join(scratch, "home")
        pack = os.path.join(bhome, ".EZERagent", "pack")
        os.makedirs(os.path.join(pack, "memory"))
        ...
        # 신 데몬 부활 — ★topology 소거 후 기동해 EZERagentd 자동복원(죽은 role 재등장)을 차단한다.
...
def cmd_phoenix_p9_failsafe(args):
    ...
    _sh.rmtree(ph, ignore_errors=True)
    BACKUP = os.path.join(os.path.dirname(PHOENIX), "EZERagent_backup.py")
    scratch = _tf.mkdtemp(prefix="EZERagent-p9fs-", dir="/tmp")
    ...
    try:
        ...
        st = subprocess.run([sys.executable, BACKUP, "status", "--home", scratch], capture_output=True, text=True,
                            timeout=20, env=dict(clean_env, EZERAGENT_BACKUP_DIR=os.path.join(scratch, "none")))
        ...
        pk = os.path.join(bhome, ".EZERagent", "pack")
        os.makedirs(pk)
        ...
        nokey = subprocess.run([sys.executable, BACKUP, "restore", "--in", os.path.join(scratch, "bk"),
                               ...
                               env={kk: vv for kk, vv in clean_env.items() if kk != "EZERAGENT_BACKUP_KEY"})
...
def cmd_phoenix_p10_populated(args):
    ...
    import tempfile as _tf
    _sh.rmtree(ph, ignore_errors=True)
    scratch = _tf.mkdtemp(prefix="EZERagent-p10-", dir="/tmp")
    BACKUP = os.path.join(os.path.dirname(PHOENIX), "EZERagent_backup.py")
    ...
    try:
        ...
        _sh.rmtree(dept_root, ignore_errors=True)
        for d in depts:
            dd = os.path.join(dept_root, "EZERagent-dept-" + d)
            os.makedirs(dd)
            open(os.path.join(dd, "schedule_state.json"), "w").write('{"jobs":[]}')
        depts_json = os.path.join(dept_root, "depts.json")
        json.dump({"depts": {d: {"socket": os.path.join(dept_root, "EZERagent-dept-" + d, "EZERagent.sock")} for d in depts}},
                  open(depts_json, "w"))
        dept_env = {"PHOENIX_DEPT_STATE_ROOT": dept_root, "PHOENIX_DEPTS_JSON": depts_json}
        ...
        #   (EZERagentd 자동복원 차단·phoenix roster 보존).
        global _tracked_daemon
        ev["annihilated_pids"] = _annihilate()
        _tracked_daemon = None
        time.sleep(0.8)
...
def _launchd_subverify(scratch):
    ...
    out = {}
    label = "com.EZERagent.harness.p11"
...
def cmd_phoenix_p11_poison(args):
    ...
    ph = os.path.join(HARN_DIR, "phoenix")
    _sh.rmtree(ph, ignore_errors=True)
    scratch = _tf.mkdtemp(prefix="EZERagent-p11-", dir="/tmp")
    healthy = ["cso", "analyst"]
    ...
    try:
        ...
        _sh.rmtree(dept_root, ignore_errors=True)
        for d in depts:
            os.makedirs(os.path.join(dept_root, "EZERagent-dept-" + d))
        depts_json = os.path.join(dept_root, "depts.json")
        json.dump({"depts": {d: {"socket": os.path.join(dept_root, "EZERagent-dept-" + d, "EZERagent.sock")} for d in depts}},
                  open(depts_json, "w"))
        dept_env = {"PHOENIX_DEPT_STATE_ROOT": dept_root, "PHOENIX_DEPTS_JSON": depts_json}
        ...
        # ★전멸(realpath/pid) → 신 데몬 부활(topology 소거로 EZERagentd 자동복원 차단·phoenix roster 보존)
        ...
        surf_pid = {}
        for line in (EZERagent("list", timeout=10).stdout or "").splitlines():
...
def cmd_deploy_restart_fixture(args):
    ...
    _annihilate=realpath/pid 전멸 §15발견2 · _wipe_daemon_state=topology 소거로 EZERagentd 자동복원 차단·phoenix roster 는
    ...
    _tracked_daemon = None
    time.sleep(0.5)
    _wipe_daemon_state()   # topology 소거(EZERagentd 자동복원 차단) — phoenix roster(desired) 는 phoenix/ 에 보존
    pid = start_daemon()
...
def main():
    global EZERAGENTD, EZERAGENT
    EZERAGENT = _resolve_EZERagent()
    EZERAGENTD = _resolve_EZERagentd()
