...
description: >
  ...
  Python·아키텍처를 검증하고, 데이터셋 메모리 fit을 추정한다. EZERagent 규약: 수치는 스크립트
...
- `timesfm_forecast.py`는 장시간 추론 시 `EZERagent run --scoped`로 생명주기 강제종료 권장.
