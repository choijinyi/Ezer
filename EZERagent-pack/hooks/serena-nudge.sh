...
#
# EZERagent 불변(절대 위반 금지):
#   - ALWAYS exit 0 (never exit 2) — EZERagent hook-never-block 불변.
#   - additionalContext 만 출력 — NEVER updatedInput (RTK EZERagent-rewrite.sh 가 PreToolUse:Bash
...
#
# 설치(human-hold/denylist): 노드 settings.json PreToolUse 에 RTK EZERagent-rewrite tuple **뒤** 등록.
#   {"matcher":"","hooks":[{"type":"command","command":"$HOME/.EZERagent/pack/hooks/serena-nudge.sh"}]}
...
state_dir = os.path.expanduser("~/.EZERagent/state")
