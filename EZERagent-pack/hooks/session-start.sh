...
if [ ! -t 0 ] && command -v EZERagent >/dev/null 2>&1 && [ -n "$EZERAGENT_PY" ]; then
  # readline 한정 — stdin 전량 소비로 같은 stdin을 보는 후속 처리를 굶기지 않는다
  # (hook 입력 JSON은 단일 라인)
  TP=$("$EZERAGENT_PY" -c 'import sys,json
try:
    print(json.loads(sys.stdin.readline()).get("transcript_path",""))
except Exception:
    print("")' 2>/dev/null)
  [ -n "$TP" ] && EZERagent usage-register --transcript "$TP" >/dev/null 2>&1
fi
...
3) 마스터 외 역할은 \`EZERagent claim-role <worker|cso|reviewer>\` 로 자기 surface를 역할 주소로 등록한다.
...
# 재시작·/clear 후 EZERAGENT_ROLE env는 남는데 레지스트리 role이 다른 surface로 이동한 드리프트를
# 매 세션 시작마다 조정한다(레지스트리가 항상 우위). 3상태:
#  ⓐ성공(자기 재점유 포함)→현행 주입  ⓑ명시적 거부→디렉티브 대신 self-demote 지시
#  ⓒ데몬-불가(EZERagent 부재·미응답·timeout — EZERagent 밖 정당 사용 포함)→fail-open: 현행 주입+1줄 고지.
case "$EZERAGENT_ROLE" in
  master|cso)
    if command -v EZERagent >/dev/null 2>&1; then
      if command -v timeout >/dev/null 2>&1; then
        CLAIM_OUT=$(timeout 2 EZERagent claim-role "$EZERAGENT_ROLE" 2>&1); CLAIM_RC=$?
      else
        CLAIM_OUT=$(EZERagent claim-role "$EZERAGENT_ROLE" 2>&1); CLAIM_RC=$?
      fi
      if [ "$CLAIM_RC" -ne 0 ] && printf '%s' "$CLAIM_OUT" | grep -qi 'claim_denied\|privileged role held'; then
        echo "■ 역할 주소 상실 (EZERAGENT_ROLE=$EZERAGENT_ROLE — 레지스트리의 살아있는 보유자가 우위)"
        echo "이 surface는 더 이상 $EZERAGENT_ROLE 역할이 아니다. 역할 지휘·역할 행동을 중단하고,"
        echo "레지스트리의 $EZERAGENT_ROLE 노드에 인계하라(\`EZERagent send --to $EZERAGENT_ROLE\`). 이 세션은 일반 세션으로 동작한다."
        exit 0
      fi
      if [ "$CLAIM_RC" -ne 0 ]; then
        echo "■ 고지: 역할 재확인 불가(데몬 미응답 — EZERagent 밖 실행일 수 있음). 현행 각성 유지(fail-open)."
      fi
    fi
    ;;
esac
echo "■ EZERagent 역할 각성 (EZERAGENT_ROLE=$EZERAGENT_ROLE)"
cat "$D"
...
LD="${EZERAGENT_LOCAL_DIR:-$HOME/.EZERagent/local}/directives/$(basename "$D" .md).local.md"
if [ -f "$LD" ]; then
