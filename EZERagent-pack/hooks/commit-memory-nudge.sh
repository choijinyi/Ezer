#!/bin/bash
# EZERagent 자기교정 — PostToolUse(Bash) 시 git commit 감지 → 장기기억 증류 넛지(shadow)
...
MSG='방금 git commit 했다. 이 커밋이 아키텍처 결정·중요 피드백·비자명 접근법이면 장기기억 증류를 고려하라(작성은 master 판단 — 자동작성 0): python3 "${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_memory.py" add --type feedback|project --name <slug> --desc "<한 줄>" --outcome success|failure|neutral --body "<사실>". 단순 수정·리팩터·문서오타면 무시. (shadow 넛지 — 강제 아님)'
