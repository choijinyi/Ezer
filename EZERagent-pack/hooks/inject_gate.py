...
stdin 텍스트를 EZERagent_skillscan.memory_poison_scan(add 시점과 동일 규칙·자기발화 doc-context
...
def main():
    text = sys.stdin.read()
    try:
        pack = os.path.expanduser(os.environ.get("EZERAGENT_PACK_DIR", "") or "~/.EZERagent/pack")
        sys.path.insert(0, os.path.join(pack, "bin"))
        import EZERagent_skillscan
        findings = EZERagent_skillscan.memory_poison_scan(text, "inject")
    except Exception:
