#!/bin/bash
# EZERagent 영속성 — SessionStart 컨텍스트 주입 hook
...
# 경로: SOUL·ROOT는 환경변수(EZERAGENT_SOUL·EZERAGENT_ROOT)로 오버라이드 가능. 미설정 시 portable 기본값(아래).
set +e

INPUT=$(cat 2>/dev/null)
[ -z "$INPUT" ] && exit 0
# 인터프리터 해소 — Windows는 python3 명령이 없고 python/py만 있는 경우가 흔하다(미해소 시 graceful degrade).
EZERAGENT_PY="$(command -v python3 || command -v python || command -v py || echo python3)"

# JSON stdin 을 python 1회 스폰으로 source·cwd 동시 파싱(콜드스타트 절감 — 기존 2회 스폰 병합).
# __EZERAGENT_END__ sentinel 로 cwd 공백 시에도 필드 경계를 결정론 보존($()가 후행 개행을 삭제해도
# 마지막 줄이 sentinel 이라 두 read 가 정확히 source·cwd 를 집는다). 어떤 예외든 graceful(startup/'').
_PARSED=$(printf '%s' "$INPUT" | "$EZERAGENT_PY" -c "import json,sys
try:
    d=json.load(sys.stdin)
    print(d.get('source','startup')); print(d.get('cwd',''))
except Exception:
    print('startup'); print('')
print('__EZERAGENT_END__')" 2>/dev/null)
{ IFS= read -r SOURCE; IFS= read -r CWD; } <<< "$_PARSED"
[ -z "$SOURCE" ] && SOURCE="startup"
case "$CWD" in /*) ;; *) CWD="" ;; esac  # 절대경로만 상향탐색 (상대·빈값은 fallback으로 — 무한루프 방지)

SOUL="${EZERAGENT_SOUL:-$HOME/.claude/soul.md}"
[ -f "$SOUL" ] || SOUL="$HOME/.EZERagent/pack/soul.md"   # 배포 기본 soul (일반 사용자)
ROOT="${EZERAGENT_ROOT:-$HOME}"
OUT=""
...
case "$EZERAGENT_PACK_DIR" in */pack-dept-dept-*) DEPT_CTX=1 ;; esac
case "$EZERAGENT_SOCKET"   in */EZERagent-dept-dept-*)  DEPT_CTX=1 ;; esac
if [ -n "$DEPT_CTX" ]; then
  case "$EZERAGENT_PACK_DIR" in
    */pack-dept-dept-*) DEPT_ROUND="$EZERAGENT_PACK_DIR/round" ;;
    *)                  DEPT_NO_STATE=1 ;;
  esac
...
if [ -z "$STATE" ] && [ -n "$DEPT_NO_STATE" ]; then
  OUT="${OUT}⚠ 부서 pack round SESSION_STATE 부재/이상환경 — 작업기억 주입 생략(메인 레인 미참조·R2/R3). 부서 정본 생성: EZERagent todo-path.
"
fi
if [ -n "$STATE" ]; then
...
# 출력은 EZERagent_checklist가 6줄·1KB 이내로 자체 절단, 머리에 P0.2 방어 문구 내장.
if [ "$SOURCE" != "compact" ] && [ -n "$STATE" ]; then
  CHK="${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_checklist.py"
  ...
  if [ -f "$CHK" ]; then
    if command -v timeout >/dev/null 2>&1; then
      ...
      CHK_OUT=$(timeout 40 "$EZERAGENT_PY" "$CHK" --state "$STATE" --round-dir "$(dirname "$STATE")" 2>/dev/null)
    else
      CHK_OUT=$("$EZERAGENT_PY" "$CHK" --state "$STATE" --round-dir "$(dirname "$STATE")" 2>/dev/null)
    fi
