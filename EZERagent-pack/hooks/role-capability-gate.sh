...
#
# ★두 hook 클래스 (EZERagent-hook.sh:6 불변 narrowing — 위반이 아니라 정밀화):
#   (a) OBSERVABILITY hook (EZERagent-hook.sh) = **절대 차단 금지·항상 exit 0** — 텔레메트리가
#       에이전트를 깨뜨려선 안 된다(관측은 무해 통과가 불변).
#   (b) GATE hook (appbuild-gate.sh, role-capability-gate.sh) = **설계상 deny 가능**(deny-by-default
#       act tier) — 이 클래스는 차단이 목적이다. EZERagent-hook.sh:6의 "막지 않는다"는 (a) 관측 전용
...
#   EZERagentd는 이 내부 도구를 직접 못 막는다(에이전트 프로세스 내부) — EZERagentd는 EZERagentd-매개 경로
#   (send/scoped run write-shell)만 게이트한다(handlers.rs check_caps_gate). 이 둘이 합쳐 가드다.
#
# 신원 = EZERagentd 권위: 자기 surface 역할은 `EZERagent surface-role`(EZERAGENT_SURFACE_ID→데몬 roles 맵)로 읽는다.
#   self-declared가 아니라 claim_role/launch-agent가 신원검증 후 등록한 값. EZERAGENT_SURFACE_ID는
...
#   write-shell의 *대표* 위험 동사(파일 리다이렉트·tee·rm·mv·cp·설치자)만 deny. ② EZERagentd 인증
...
if [ "${1:-}" = "--self-test" ]; then
  ...
  # 자기 surface 역할 조회(EZERagentd 권위). 실패 시 빈 문자열 → '무역할'로 통과(사람/일반 셸 과차단 방지).
  ...
  CAPGATE_CACHE="${TMPDIR:-/tmp}/EZERagent-capgate-role-${EZERAGENT_SURFACE_ID:-none}"
  if [ -z "${EZERAGENT_SURFACE_ROLE:-}" ] && command -v EZERagent >/dev/null 2>&1; then
    EZERAGENT_SURFACE_ROLE="$(EZERagent surface-role 2>/dev/null | head -n1)"
    if [ -n "${EZERAGENT_SURFACE_ID:-}" ]; then
      case "$EZERAGENT_SURFACE_ROLE" in
        reviewer*|planner|planner-*) printf '%s\n' "$EZERAGENT_SURFACE_ROLE" > "$CAPGATE_CACHE" 2>/dev/null ;;
        "") [ -f "$CAPGATE_CACHE" ] && EZERAGENT_SURFACE_ROLE="$(head -n1 "$CAPGATE_CACHE" 2>/dev/null)" ;;
      esac
    fi
  fi
  export EZERAGENT_SURFACE_ROLE
fi

exec "$EZERAGENT_PY" - <<'PYEOF'
...
ALLOW_PATH_SUBSTRS = ("/.EZERagent/", "/logs/", "/log/", "/_round/")
...
def main():
    ...
    role = os.environ.get("EZERAGENT_SURFACE_ROLE", "")
    try:
        data = json.loads(raw)
    except ValueError:
...
def self_test():
    ...
    cases_allow = [
        ...
        ("reviewer-codex", "Edit", {"file_path": "/Users/x/.EZERagent/scratch.txt"}),
