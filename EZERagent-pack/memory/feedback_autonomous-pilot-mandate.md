...
**축1** 게이트 4자 수렴(agy+codex+master+기계검증, `EZERagent_orchestra.py gate-status`로
...
checksum)·"준비 완료" ack ④CSO 재독·검증 후 `EZERagent cycle-agent --role master --verifier <cso>`로
주인 대신 `/clear` ⑤master 자동복구. 무응답 시 CSO 독립검증 후 조건부 집행(신선=집행·낡음=오너
escalation). **축3** 작업 단위 종료→`EZERagent_orchestra.py next-action`으로 다음
미완 작업 자가 착수(완료 push/`EZERagent schedule add --in` 원샷 웨이크업 트리거).
