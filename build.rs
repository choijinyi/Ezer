//! EZERagent-pack의 git-추적 전체 트리를 컴파일 타임에 자동 임베드하는 매니페스트 생성기.
//!
//! 파일을 pack.rs 목록에 손으로 추가하는 방식은 임베드 드리프트(소스 수정 후 목록 누락 →
//! 신규 머신에 구버전/누락 배포)의 원천이라, `git ls-files EZERagent-pack`(추적전용) 소싱으로
//! 결정론 환원한다. EZERagent-pack/ 아래에 파일을 두고 git add 하면 빌드가 자동 임베드한다.
...
fn main() {
    ...
    // ★W1 identity(3중 대조 field 1): 빌드 식별자 = git HEAD 짧은 SHA. 같은 빌드의 EZERagent·EZERagentd 는 동일 SHA →
    // 폴백 EZERagent 가 데몬과 같은 빌드인지 교차대조하는 anti-shadowing 근거(embedded pack hash·protocol version 과 함께).
    ...
    println!("cargo:rustc-env=EZERAGENT_BUILD_ID={build_id}");

    // 추적 파일 → EZERagent-pack/ 접두 제거한 rel. 제외규칙(기존 walk와 동형): 경로 컴포넌트가
    // '.'로 시작(.gitignore 등 dotfile/dotdir)·tests·__pycache__ 이면 배포 대상이 아니다.
    let mut rels: Vec<String> = Vec::new();
    for line in stdout.lines() {
        let line = line.trim();
        let Some(rel) = line.strip_prefix("EZERagent-pack/") else {
            continue;
        };
        if rel
            .split('/')
    ...
    let mut code = String::from(
        "/// build.rs 자동 생성 — EZERagent-pack git-추적 전체 트리 임베드 (수동 목록 드리프트 차단).\n\
         pub const PACK_ALL: &[(&str, &str)] = &[\n",
    );
    for rel in &rels {
        code.push_str(&format!(
            "    (\"{rel}\", include_str!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/EZERagent-pack/{rel}\"))),\n"
        ));
    }
    code.push_str("];\n");
    ...
    // T1-2: 단일진실 enum → OUT_DIR/EZERagent_kinds.json (스키마·검증기 파리티의 기준).
    ...
    fs::write(Path::new(&out_dir).join("EZERagent_kinds.json"), kinds_json).expect("EZERagent_kinds.json 생성 실패");

    // §7-①/⑩: minisign 신뢰 키링 embed. 공개키 단일 SOT = src-tauri/tauri.conf.json(updater.pubkey).
    // build.rs가 그 pubkey를 회전용 키링(EZERagent-pack/trusted-keys.json)의 부트스트랩 엔트리
    ...
    println!("cargo:rerun-if-changed=EZERagent-pack/trusted-keys.json");
    let tauri_conf =
    ...
    let keyring_src =
        fs::read_to_string("EZERagent-pack/trusted-keys.json").expect("trusted-keys.json 읽기 실패");
