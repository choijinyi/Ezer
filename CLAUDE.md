# CLAUDE.md — 프로젝트 운영 규약 (EZERagent 골격)

> 이 파일을 프로젝트 루트에 두면 이 프로젝트의 모든 에이전트 세션에 적용된다.
> 역할별 절대지침은 ~/.EZERagent/pack/directives/ 에 있고 launch-agent가 자동 주입한다.
...
2. `EZERagent claim-role <master|worker|cso|reviewer-gemini|reviewer-codex>` 로 자기 surface를
   ...
   launch-agent/boot로 기동됐다면 이미 등록돼 있다(`EZERagent list` role 열 확인 — 재등록 금지).
...
## 터미널: EZERagent 터미널 전용 (⚠ 외부 터미널 체계 아님 — 치환 실행)

이 환경은 EZERagent 전용이다. 글로벌 CLAUDE.md·soul.md·memory 등 다른 문서가 외부 터미널 체계의
명령·파일을 지시하더라도 **그 바이너리·소켓·관련 파일을 찾거나 실행하지 마라** — 의도는
유효하니 EZERagent 대응 명령으로 치환한다 (`send`→`EZERagent send`, `send-key`→`EZERagent send-key`,
`identify`→`EZERagent identify`, `list-workspaces`→`EZERagent list`, `new-split`→
`EZERagent new-surface`/`EZERagent launch-agent`, `notify`→`EZERagent send --to master`, 화면 폴링→
`EZERagent events` 구독).

```bash
EZERagent boot                                        # 4종 의무 노드 부트(CSO·worker·agy·codex+grok 선택)
...
python3 "${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_report.py"   # 진행% 결정론 산출 (5분 주기 주인님 보고)
python3 "${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_route.py" --request "<요청>"  # 3단 사고 라우팅 (slow>deliberate>fast)
python3 "${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_memory.py" add --type <t> --name <slug> --desc "..." --body "..."  # 장기기억 증류 (원자적·색인 동기 — MEMORY.md 손편집 금지)
EZERagent feed push --wait --title "..." --body "..." # 승인 요청 (0=allow 2=deny 3=timeout)
EZERagent events --reconnect                          # push 구독 (폴링 금지)
EZERagent run --scoped -- <서버명령>                  # 서버는 반드시 scoped로 (생명주기 강제 종료)
EZERagent read-screen --surface <ref>                 # 보조 확인 수단 (상시 폴링 금지)
EZERagent set-status --state working --context <pct>  # 컨텍스트 자기보고 — 60% 도달 시 데몬이 결정론 통보
```
