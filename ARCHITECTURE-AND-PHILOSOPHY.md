...
> EZERagent + EZERagent 팩이 **무엇을, 왜, 어떻게** 이렇게 만들었는지를 설명하는 문서입니다.
...
**EZERagent은 "AI 에이전트 함대"를 하나의 회사처럼 굴리기 위한 터미널·데몬·관제탑이고,
EZERagent 팩은 그 회사의 취업규칙·운영도구·기억 골격이다.** 둘은 따로 설치하는 별개 제품이
아니라, 하나의 저장소에서 함께 빌드·서명·배포되어 **한 몸으로 동작**한다
(`EZERagent-pack/README.md`: "터미널의 기계 기능과 역할별 절대지침 문서가 한 몸으로 동작한다").
...
EZERagent은 이 세 가지를 **1급 기능**으로 해결하기 위해 처음부터 새로 작성한 독자
구현이다. 그리고 네 번째 벽 — **"에이전트를 어떻게 조직으로 묶을 것인가"** — 를 EZERagent
...
시스템 전체는 세 층으로 분리된다 (`EZERagent-pack/README.md` §3층 구조).

| 층 | 내용 | 출처 |
|---|---|---|
| **코어 (기계 기능)** | 양방향 소켓·승인 Feed·watchdog/프로세스 원장·이벤트 push·세션 영속·서명 검증 | EZERagent 바이너리 (`src/`) |
| **EZERagent 팩 (운영체계)** | 역할별 절대지침·결정론 운영 도구·훅·스킬·어댑터 | `EZERagent init-pack` (`EZERagent-pack/`) |
...
> 왜 일하는가'다." — `EZERagent-pack/soul.md`
>
> "장기기억은 빌리는 것이 아니라 사용하며 축적하는 것이다." — `EZERagent-pack/memory/MEMORY.md`
...
부트 검증은 `EZERagent_preflight.py`의 exit code가 사실이고, 진행률은 `EZERagent_report.py`의
체크박스 산술이 사실이며, 이벤트는 `EZERagent_event.py`의 닫힌 enum만 통과한다.
...
학습 루프(`EZERagent_learn.py`)는 citation 없는 입력을 hard fail 시키고, 자기개선 봉쇄
...
(`EZERagent_rsi.py checkpoint`), baseline을 못 이기면 rollback 한다 — 이때도 콘텐츠 삭제로
...
fail-open**(항상 exit 0 — 관측이 에이전트를 깨뜨리면 안 된다, `hooks/EZERagent-hook.sh`),
...
외부 대시보드·클라우드 의존 0. PII는 `EZERAGENT_CONTROL_REDACT=1`로 가린 채 집계만 보존할 수 있다.
...
allowlist·fail-closed)과 데몬 kill-switch(`EZERagent pause` — 큐 배달·스케줄 발화 동결)가 맡는다.

---

## 4. 역할 체계 — 에이전트를 회사로 묶는 법

EZERagent 팩은 에이전트를 다섯 역할로 조직한다. 각 역할은 기동 시 절대지침
...
- 데몬은 **커널 peer pid**로 발신자를 검증하고(자기신고 role 불신), `acl.json`의
  ...
  (중재는 master 경유).
- 능력 모델(`src/bin/EZERagentd/caps.rs`)은 deny-by-default — reviewer/planner는 읽기·검색만.
...
┌─────────────────────────────── 한 저장소, 한 배포 ───────────────────────────────┐
│                                                                                │
│  EZERagent.app   Tauri 2 데스크톱 앱 — 터미널 UI(xterm.js) + Control Center.          │
│            데몬의 thin client. PTY를 소유하지 않으므로 앱이 죽어도 세션 생존.       │
│                                                                                │
│  EZERagentd      헤드리스 코어 데몬 — NDJSON 소켓 서버(UDS / named pipe), PTY 소유      │
│            (portable-pty: openpty·ConPTY), vt100 화면 재구성, 이벤트 버스,        │
│            watchdog·프로세스 원장, 사용량/비용 수집, SQLite 영속 분석, 스케줄러     │
│                                                                                │
│  EZERagent       CLI — pane 안의 AI가 쓰는 동등 노드 클라이언트 (60+ 서브커맨드)          │
│                                                                                │
...
모든 pane 프로세스에 `EZERAGENT_SURFACE_ID`·`EZERAGENT_SURFACE_REF`·`EZERAGENT_SOCKET`이 자동 주입된다 —
pane 안의 AI는 `EZERagent identify` 한 번으로 자기 주소를 알고, 그 순간부터 소켓의 **동등
노드**다. "사람만 조종석에 앉는 터미널"과의 근본적 차이가 여기다.

### 5.2 EZERagentd 데몬 내부

| 모듈 (`src/bin/EZERagentd/`) | 역할 |
|---|---|
...
```
부트      EZERagent_preflight --fix (존재·매핑·훅 검증 — exit code만이 사실)
          → EZERagent_orchestra check (필수 노드 생존 판정)
라우팅    EZERagent_route --request "…" (fast/deliberate/slow 3단 판정)
위임      EZERagent_task checkout (원자적 체크아웃 — 충돌=exit 9, 선행 미해소=exit 4)
착수 게이트 EZERagent_resource_gate check (서버·노드·load·컨텍스트 사전 차단)
실행 중   EZERagent_event emit (닫힌 enum 이벤트) · EZERagent_wakeup (코얼레싱 웨이크업 큐)
진행 보고  EZERagent_report (todo 체크박스 산술 → 주기 push)
검증      EZERagent_manifest check-criteria (기계 FLOOR) + EZERagent_verdict (리뷰어 판정 스키마)
자율 전진  EZERagent_orchestra gate-status (수렴 판정) → next-action (다음 액션 큐)
종료 게이트 EZERagent_memory add (장기기억 증류 — 색인과 원자적 동기) · EZERagent_adr add (결정 기록)
복원      EZERagent_state_snapshot (세대 보관) · EZERagent_phoenix (부활 저널·정직 상태 enum)
```

훅(`hooks/`, 18종)은 두 계급으로 명시 분리된다 — **OBSERVABILITY**(절대 차단 금지·항상
exit 0: 사용량 관측, statusline)와 **GATE**(deny-by-default·차단이 목적: 자율주행 guard,
역할 능력 게이트, 기획 선행 게이트). 보안 스캐너(`EZERagent_skillscan.py` — 46 규칙, 스킬
정적 스캔 + 복원 주입 경로의 메모리 포이즌 스캔)와 MCP 거버넌스 게이트(`EZERagent_mcpgate.py`
...
4. **승인은 사람 또는 서명** — 화면의 승인 프롬프트에 자동 응답하는 코드는 없다(HITL).
   ...
   (`EZERagent approval sign` — master surface 전용, 상수시간 비교, 시크릿 0600 파일).
...
7. **PII** — `EZERAGENT_CONTROL_REDACT=1`이면 세션 식별자를 해시로 가리고 집계만 보존.
...
- **변조 증거성** — 전사 기록은 해시체인으로 이어지고, `EZERagent attest pin/verify`로 외부
  보관·사후 대조가 된다(producer≠evaluator의 기록 버전).
- **부활(phoenix)** — 상태 스냅샷은 세대 보관되고(`EZERagent_state_snapshot`), 복원은 부활
  저널 상태머신(`EZERagent_phoenix`)이 수행한다. 복원 상태는 VERIFIED/UNVERIFIED/FAILED 정직
  enum으로 보고된다 — "무출력=성공" 해석은 금지되어 있다. 크래시 루프에는 회로차단기가
  걸린다. 복원 시 재주입되는 텍스트는 메모리 포이즌 스캔을 거친다(`hooks/inject_gate.py`).
- **역할 조직 복원** — `EZERagent restore`가 토폴로지 스냅샷의 죽은 역할들을 일괄 재기동·지침
...
2. **학습 5단계** — 제안 → 검색(citation 강제) → 추출(관찰 가능한 행동 주장으로 변환) →
   평가(외부 채점·baseline 대비 실측) → 저장/하네스화 (`EZERagent_learn.py`).
3. **게이트** — `rsi-gate.sh`(fail-closed) + 디렉티브 회귀 트립와이어
   (`EZERagent_directive_bench.py` — 지침이 결함 행동을 실제로 금지하는지 결정론 채점).
4. **가시화** — Control Center "학습" 탭에서 라운드 타임라인·채택/롤백·발견 누적을 본다.
5. **영속** — 통과한 것만 스킬(`EZERagent skill`)·장기기억(`EZERagent_memory`)·결정기록(`EZERagent_adr`)
...
- **클린룸 흡수** — 외부 오픈소스에서 배울 때 코드를 복사하지 않고 **규칙·패턴을 표준
  ...
  `EZERagent_cleanroom.py`가 헤더·해시핀·카피레프트 게이트를 기계 검증).
