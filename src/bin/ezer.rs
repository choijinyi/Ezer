//! ezer — the Ezer terminal CLI client. 모든 pane 안의 AI가 이 CLI로 동등 노드가 된다.
//! 예: ezer send --surface surface:31 "..." ; ezer send-key --surface surface:31 Return

use clap::{Parser, Subcommand};
use ezer::{key_to_bytes, parse_surface_ref, socket_path, surface_ref, ENV_SURFACE_ID};
...
#[command(
    name = "ezer",
    version,
    about = "ezer — the Ezer terminal CLI (bidirectional socket, multi-agent OS)"
)]
struct Cli {
...
enum Command {
    ...
    Identify,
    /// ★W1: 이 ezer 바이너리 자신의 identity 3필드(build_id·embedded_pack_hash·protocol_version) JSON 출력.
    ...
    /// T5 Phase 2-A: claude statusline stdin JSON을 읽어 usage.report로 push (ezer-statusline.sh 전용 plumbing)
    UsageReportStdin {
        #[arg(long)]
        surface: Option<String>,
    ...
    /// T7 E1-4: PreToolUse/PostToolUse hook stdin을 읽어 usage.event로 push (ezer-hook.sh 전용 plumbing)
    UsageEventStdin {
        #[arg(long)]
        surface: Option<String>,
    },
    ...
    Tombstone {
        ...
        /// (BOOTSTRAP_HARDENING WP-3 · dept_tombstone.set RPC). ezer-dept가 삭제/재생성 시
    ...
    /// Install the Ezer Pack (multi-agent operating system templates) to ~/.ezer/pack
    ...
    PackMerge {
        ...
        #[arg(long)]
        ai: bool,
        /// (healed system 파일 전용) 보존본(.user)을 ~/.ezer/local 오버레이로 이동(스킬 shadowing)
        #[arg(long)]
        to_local: bool,
        /// 확인 프롬프트 없이 적용
        #[arg(long)]
    ...
    TodoPath,
    /// Print this surface's ezerd-authoritative role (one word) — PreToolUse capability-gate hook용.
    /// EZER_SURFACE_ID로 자기 surface를 찾아 데몬 roles 맵의 role을 출력(미등록 시 빈 줄·exit 0).
    SurfaceRole,
...
#[derive(Subcommand)]
enum ChannelAction {
    /// 브리지 스폰(ezerd 자식·1회용 토큰) + desired-state enabled=1. 첫 기동엔 --cmd 필수.
    Start {
        channel: String,
    ...
    /// 토큰은 --token 대신 **env `EZER_CHANNEL_TOKEN`**(스폰 시 이미 주입됨)에서 읽는 것을 권장한다
    ...
    /// 인바운드 메시지 제출(브리지→ezerd). inbox-first 퍼널 판정. kind=interaction이면 승인 버튼 검증.
    Inbound {
        channel: String,
        #[arg(long = "sender-id")]
        sender_id: String,
    ...
    /// 아웃바운드 receipt 보고(브리지→ezerd). 단조 전이·terminal 후는 late_receipt 화해.
    Receipt {
...
enum SkillAction {
    ...
    Run {
        /// 카탈로그의 skill name
        name: String,
        /// task-prompt 티켓 본문(ezer_orchestra가 생성). 빈 값이면 거부(무계약 차단)
        #[arg(long)]
        ticket: String,
...
enum CostBaselineAction {
    /// 현재 7d 분포를 ~/.ezer/_round/cost_baseline.json에 박제(sha256 핀·locked_at)
    Lock,
...
enum ScheduleAction {
    /// Add a job to ~/.ezer/pack/schedule.json (daemon hot-reloads)
    Add {
        #[arg(long)]
        id: String,
...
fn main() {
    ...
    if let Some(s) = &cli.socket {
        std::env::set_var(ezer::ENV_SOCKET, s);
    }
...
fn target_surface(explicit: &Option<String>, to_role: &Option<String>) -> Result<u64, String> {
    ...
    if let Ok(env) = ezer::env_compat(ENV_SURFACE_ID).ok_or(std::env::VarError::NotPresent) {
        if let Some(id) = parse_surface_ref(&env) {
            return Ok(id);
        }
    }
    Err("no --surface/--to given and EZER_SURFACE_ID is not set".into())
}
...
fn connect_raw() -> Result<std::os::unix::net::UnixStream, String> {
    ...
    std::os::unix::net::UnixStream::connect(&path)
        .map_err(|e| format!("cannot connect to ezerd at {}: {e}", path.display()))
}
...
/// 231을 데몬 다운으로 오판하면 connect()의 sibling ezerd autostart 까지 헛발동한다
/// (2026-07-10 Windows 실사고). 정책 상수는 GUI(ezer-app)와 공용 단일 진실인 lib(ezer::PIPE_BUSY_*)
...
fn open_pipe_busy_retry(path: &std::path::Path) -> std::io::Result<std::fs::File> {
    ...
    loop {
        match std::fs::OpenOptions::new().read(true).write(true).open(path) {
            ...
            {
                std::thread::sleep(ezer::PIPE_BUSY_RETRY_INTERVAL);
            }
            other => return other,
        }
    }
...
fn connect_raw() -> Result<std::fs::File, String> {
    ...
    open_pipe_busy_retry(&path)
        .map_err(|e| format!("cannot connect to ezerd pipe {}: {e}", path.display()))
}
...
fn sibling_daemon_path() -> Option<std::path::PathBuf> {
    let name = if cfg!(windows) { "ezerd.exe" } else { "ezerd" };
    std::env::current_exe()
        .ok()?
        .parent()
        .map(|d| d.join(name))
...
/// ezerd 작업 스케줄러 태스크 XML. LogonTrigger(현재 사용자) + RestartOnFailure(PT1M×10) +
...
#[cfg(windows)]
fn ezerd_task_xml(daemon: &std::path::Path, user_id: &str) -> String {
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>Ezer terminal daemon (ezerd) — 로그온 자동기동 + 사망 시 자동 재기동(RestartOnFailure)</Description>
  </RegistrationInfo>
  <Triggers>
    <LogonTrigger>
...
/// 온보딩④: 연결 실패 시 형제 ezerd를 자동 기동 후 재시도 — 신규 머신 zero-setup.
/// 옵트아웃: EZER_NO_AUTOSTART=1. (데몬 중복 기동은 ezerd 자체의 flock이 차단)
/// ★W3: macOS에서 launchd가 ezerd를 소유(적재)하면 sibling spawn 대신 launchctl kickstart로
/// 위임한다 — 구형 CLI가 자기 옆 구형 ezerd를 띄워 startup lock을 선점하고 launchd 신형과
/// crashloop 하는 경로를 원천 차단. kickstart 실패·폴링 타임아웃 시에만 sibling fallback(개발 환경).
fn connect() -> Result<ConnStream, String> {
    match connect_raw() {
        Ok(s) => Ok(s),
        Err(first) => {
            let opted_out = ezer::env_compat("EZER_NO_AUTOSTART")
                .map(|v| v == "1")
                .unwrap_or(false);
            if opted_out
            ...
            #[cfg(target_os = "macos")]
            {
                if ezer::launchd::should_delegate_autostart(ezer::launchd::is_loaded()) {
                    eprintln!("[ezer] ezerd not serving — delegating to launchd (launchctl kickstart)");
                    if ezer::launchd::kickstart() {
                        if let Some(s) = poll_socket_ready() {
                            return Ok(s);
                        }
                        eprintln!(
                            "[ezer] launchd kickstart did not yield a socket within 4s — falling back to sibling spawn"
                        );
                    } else {
                        eprintln!("[ezer] launchctl kickstart failed — falling back to sibling spawn");
                    }
                }
            }
            let Some(daemon) = sibling_daemon_path() else {
                return Err(format!("{first} (no sibling ezerd to autostart)"));
            };
            eprintln!("[ezer] ezerd not running — autostarting {}", daemon.display());
            if spawn_detached_daemon(&daemon).is_err() {
                return Err(first);
            }
            poll_socket_ready()
                .ok_or_else(|| format!("{first} (autostarted ezerd did not come up within 4s)"))
        }
    }
}
...
fn request(method: &str, params: Value) -> Result<Value, String> {
    ...
    let resp: Value = ezer::wire::parse_frame(resp_line.trim()).map_err(|e| format!("abi: {e:?}"))?;
...
fn run(command: Command) -> i32 {
    let result = match command {
        ...
        Command::PhoenixIdentity => {
            ...
            println!(
                ...
                json!({
                    ...
                    "build_id": ezer::pack::build_id(),
                    "embedded_pack_hash": ezer::pack::embedded_pack_hash(),
                    "protocol_version": ezer::pack::PHOENIX_PROTOCOL_VERSION,
                })
            );
            return 0;
        }
        ...
        Command::Identify => {
            let caller = ezer::env_compat(ENV_SURFACE_ID).ok_or(std::env::VarError::NotPresent)
                .ok()
        ...
        Command::Send { surface, to, queued, clear_first, text } => {
            resolve_targets(&surface, &to).and_then(|sids| {
                let from = ezer::env_compat(ENV_SURFACE_ID).and_then(|s| parse_surface_ref(&s));
        ...
        Command::Pause { reason } => request("system.pause", json!({"reason": reason}))
            .map(|_| println!("PAUSED — 큐 배달·스케줄 발화 동결 (이미 실행 중인 에이전트 행동은 계속된다; ezer resume로 해제)")),
        ...
        Command::Approval { action } => {
            return match action {
                // exit 0 = 서명됨(통과) / 비0 = 미서명·차단. ezerd 미가용 시 fail-closed(비0).
                ApprovalAction::Check { command, cwd } => {
                    ...
                    match request(
                        ...
                        // ezerd 미가용·RPC 실패 = fail-closed(미서명 취급, 자동 통과 금지)
                        Err(e) => {
        ...
        Command::License { action } => {
            ...
            match action {
                LicenseAction::Install { path } => {
                    match ezer::license::install(std::path::Path::new(&path), now) {
                        Ok(msg) => {
                            println!("{msg}");
                            return 0;
                        }
                ...
                LicenseAction::Status => {
                    println!("{}", ezer::license::render_status(now));
                    return 0;
                }
            }
        }
        ...
        Command::Learn { topic, status } => {
            if status {
                ...
                Err("usage: ezer learn <topic> | ezer learn --status".to_string())
            }
        }
    ...
    let result: Result<i32, String> = match action {
        FeedAction::Push { kind, title, body, surface, request_id, wait, timeout_secs, tier } => {
            parse_explicit_surface(&surface)
                .and_then(|explicit| {
                    let sid = explicit
                        .or_else(|| ezer::env_compat(ENV_SURFACE_ID).and_then(|s| parse_surface_ref(&s)));
                    request(
                        "feed.push",
...
/// ★실측 정렬: ezer connect()는 `cannot connect to ezerd at {path}: {e}`를 반환하고 {e}는 OS 에러
...
fn is_transient_event_error(msg: &str) -> bool {
    ...
    const MARKERS: &[&str] = &[
        "no such file or directory", // ezer connect_raw: 누락 소켓(ENOENT) — 데몬 재기동 중
        ...
        "cannot connect to ezerd",    // connect_raw 래퍼 문구(autostart 실패 포함)
        "os error 32",
        "os error 35",
        "os error 54",
        "os error 57",
...
fn run_channel(action: ChannelAction) -> i32 {
    ...
    // M10: register 토큰은 argv 대신 env `EZER_CHANNEL_TOKEN`(스폰 시 주입) 우선 — ps 노출 회피.
    // --token 있으면 그것을, 없으면 env로 폴백. 둘 다 없으면 명확히 보고.
    if let ChannelAction::Register { channel, token, caps, bridge_ver } = &action {
        let token = token
            .clone()
            .or_else(|| std::env::var("EZER_CHANNEL_TOKEN").ok().filter(|s| !s.is_empty()));
        let Some(token) = token else {
            println!("{}", json!({"ok": false,
                "error": "no token — pass --token or set EZER_CHANNEL_TOKEN env"}));
            return 1;
        };
...
fn run_cost_baseline(action: CostBaselineAction) -> i32 {
    // baseline 박제 위치 — pack 밖·로컬·gitignore(~/.ezer는 repo 밖). _round 컨벤션.
    let path = match dirs::home_dir() {
        Some(h) => h.join(".ezer/_round/cost_baseline.json"),
        None => {
    ...
    match action {
        ...
        CostBaselineAction::Diff => {
            let locked_raw = match std::fs::read_to_string(&path) {
                Ok(s) => s,
                Err(_) => {
                    eprintln!("박제본 없음 — 먼저 `ezer cost-baseline lock` 실행: {}", path.display());
                    return 2;
                }
            };
...
fn run_skill(action: SkillAction) -> i32 {
    let skills_dir = ezer::pack::pack_dir().join("skills");
    let result: Result<(), String> = match action {
        ...
        SkillAction::List => (|| {
            if !skills_dir.exists() {
                return Err(format!(
                    "no skills dir: {} (run ezer init-pack)",
                    skills_dir.display()
                ));
            }
            // ① 오버레이 shadowing: 팩 스킬 위에 ~/.ezer/local/skills 동명 스킬이 이긴다(업데이트 불가침).
            let mut merged: std::collections::BTreeMap<String, (String, bool)> = Default::default();
            for (root, local) in [(skills_dir.clone(), false), (ezer::pack::local_dir().join("skills"), true)] {
            ...
            if merged.is_empty() {
                println!("(no skills yet — `ezer skill new <name> --description \"...\"`)");
            }
            Ok(())
        })(),
        SkillAction::Show { name } => (|| {
            ...
            let local = ezer::pack::local_dir().join("skills").join(&name).join("SKILL.md");
        ...
        SkillAction::Run { name, ticket, agent, close_after } => (|| {
            ...
            // B1 교정: now_epoch()는 ezerd 전용 → ezer.rs는 chrono로 epoch 취득.
...
fn run_schedule(action: ScheduleAction) -> i32 {
    let path = ezer::pack::pack_dir().join("schedule.json");
    let result: Result<(), String> = match action {
        ScheduleAction::Add {
            id,
            time,
...
/// Ezer Pack 설치: 임베드된 템플릿을 ~/.ezer/pack 에 기록 (기존 파일 보존이 기본).
...
fn run_init_pack(force: bool, no_install_hook: bool, claude_settings: Option<String>) -> i32 {
    ...
    println!(
        "Ezer Pack installed at {} ({} written, {} preserved{})",
        dir.display(),
        written,
        kept,
        if force { ", forced" } else { "" }
    );
    println!("다음: ezer launch-agent --role master --agent claude  (역할 지침 자동 주입)");
...
fn hook_command(pack_dir: &std::path::Path) -> String {
    ...
    ezer::pack::session_start_hook_command(pack_dir)
}
...
fn install_claude_hook(settings_path: &str, pack_dir: &std::path::Path) -> Result<String, String> {
    ...
    // 백업하면 온보딩이 매 기동 init-pack을 호출할 때(멱등) 정상 상태 .bak-ezer가 매번 클로버돼
    // "정상 백업"이 소실된다(적대검증 serious). already→skip 경로는 백업을 건드리지 않는다.
    if std::path::Path::new(settings_path).exists() {
        let backup = format!("{settings_path}.bak-ezer");
    ...
    std::fs::write(settings_path, serde_json::to_string_pretty(&root).unwrap())
        .map_err(|e| e.to_string())?;
    Ok(format!(
        "SessionStart hook registered in {settings_path} (backup: .bak-ezer)"
    ))
}

// ─────────────────────────────────────────────────────────────────────────────
// ezer doctor (§3.4) — 시스템 자기진단·수리. 진단은 읽기전용, --fix는 안전 항목만
...
struct DoctorCtx {
    ...
    /// pack_dir 부모(~/.ezer) — apply lock·.pack-staging 잔재 루트.
...
fn diag_pack_version(ctx: &DoctorCtx) -> DiagItem {
    ...
    match std::fs::read_to_string(&vf) {
        ...
        Ok(s) => {
            ...
            let note = match (
                ezer::pack::parse_semver(&disk),
                ezer::pack::parse_semver(&ctx.binary_version),
            ) {
                (Some(d), Some(b)) if d < b => "팩이 바이너리보다 구버전 — ezer init-pack 권장",
...
fn diag_pack_state(ctx: &DoctorCtx) -> DiagItem {
    match ezer::pack::read_pack_state(&ctx.pack_dir) {
        ezer::pack::PackStateRead::Absent => DiagItem {
            name: "pack-state",
            status: DiagStatus::Ok,
            detail: "채널 상태 미기록(free 기본)".into(),
            action: String::new(),
        },
        ezer::pack::PackStateRead::Valid(st) => DiagItem {
            name: "pack-state",
            status: DiagStatus::Ok,
            detail: format!(
        ...
        ezer::pack::PackStateRead::Corrupt(e) => DiagItem {
            name: "pack-state",
            status: DiagStatus::Fail,
            detail: format!(".pack-state.json 손상: {e}"),
            action: "ezer pack-repair-channel (doctor는 상태파일을 자동 수정하지 않음)".into(),
        },
    }
}
...
fn diag_install_manifest(ctx: &DoctorCtx) -> DiagItem {
    ...
    if !mf.exists() {
        ...
        return if installed {
            DiagItem {
                ...
                action: "ezer init-pack --force 로 매니페스트 재생성 권장".into(),
            }
        } else {
            DiagItem {
                name: "install-manifest",
        ...
        .and_then(|s| serde_json::from_str::<Value>(&s).ok())
    {
        ...
        None => DiagItem {
            ...
            action: "ezer init-pack --force 로 재생성".into(),
        },
    }
}

fn diag_hook(ctx: &DoctorCtx, fix: bool) -> DiagItem {
    let hook_cmd = ezer::pack::session_start_hook_command(&ctx.pack_dir);
    ...
    DiagItem {
        ...
        action: "ezer doctor --fix 또는 ezer init-pack 로 등록".into(),
    }
}
...
fn diag_orphan_socket(ctx: &DoctorCtx, fix: bool) -> DiagItem {
    ...
    if fix {
        ...
        DiagItem {
            ...
            action: "ezer doctor --fix 로 제거".into(),
        }
    }
}
...
fn staging_protect_secs() -> u64 {
    std::env::var("EZER_DOCTOR_STAGING_MIN_IDLE_SECS")
        .ok()
...
fn diag_staging_residue(ctx: &DoctorCtx, fix: bool) -> DiagItem {
    ...
    if fix {
        ...
        DiagItem {
            ...
            status: DiagStatus::Warn,
            detail: format!("staging 잔재 {}건", residue.len()),
            action: "ezer doctor --fix 로 정리".into(),
        }
    }
}
...
fn diag_legacy_config(_ctx: &DoctorCtx) -> DiagItem {
    ...
    // 진단한다(런타임은 canonical EZER_* 우선). 백업·재작성은 대상 파일이 없어 해당 없음(진단만).
    let legacy_keys = [
        "EZER_PACK_DIR",
        "AITERM_JARVIS_DIR",
        "AITERM_PACK_DIR",
    ...
    if set.is_empty() {
        DiagItem {
            name: "legacy-config",
            status: DiagStatus::Ok,
            detail: "레거시 env 미사용(canonical EZER_*)".into(),
            action: String::new(),
        }
    } else {
        DiagItem {
            ...
            action: "EZER_* 키로 이관 권장(런타임은 canonical 우선)".into(),
        }
    }
}
...
fn run_doctor(fix: bool, json_out: bool) -> i32 {
    let pack_dir = ezer::pack::pack_dir();
    let state_base = pack_dir
        .parent()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| std::path::PathBuf::from("."));
    let socket_path = ezer::socket_path();
...
fn run_boot(cwd: Option<String>) -> i32 {
    ...
    let agents: Value = std::fs::read_to_string(ezer::pack::pack_dir().join("agents.json"))
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
        .unwrap_or_else(|| json!({}));
    ...
    let mut launched = 0;
    let mut failed = 0;
    println!("ezer boot — LLM orchestrating 편성 점검 (CSO·worker·agy·codex 4종 의무 + grok 선택)");
    ...
    println!(
        "boot 완료: 신규 기동 {launched} · 실패 {failed} · 현황은 `ezer list`로 확인 (role 열)"
    );
    if failed > 0 {
        1
    } else {
...
fn load_agent_spec(agent: &str) -> Result<Value, String> {
    let agents_path = ezer::pack::pack_dir().join("agents.json");
    let agents: Value = std::fs::read_to_string(&agents_path)
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
        .ok_or_else(|| {
            format!(
                "agents.json not found at {} — run `ezer init-pack` first",
                agents_path.display()
            )
        })?;
    agents
...
fn compose_directive(role: &str) -> Result<String, String> {
    ...
    let directive_path = ezer::pack::role_directive_path(role).unwrap_or_else(|| {
    ...
    if let Ok(memory) = std::fs::read_to_string(&memory_path) {
        directive.push_str(&format!(
            "\n\n■ 장기메모리 색인 ({} — 노드 공유 의미 기억 · 증류는 bin/ezer_memory.py add)\n",
            memory_path.display()
        ));
        directive.push_str(&memory);
    }
    // 스킬 색인(표지) 동봉 — 본문은 필요 시 `ezer skill show <name>`으로 점진 로드.
    // ① 오버레이: ~/.ezer/local/skills 가 동명 팩 스킬을 shadowing(업데이트 불가침 사용자 커스텀).
    ...
    for (name, desc) in scan_skills(&ezer::pack::local_dir().join("skills")) {
        skill_index.insert(name, (desc, true)); // 동명 → local 이 이긴다(shadowing)
    }
    if !skill_index.is_empty() {
        directive.push_str("\n\n■ 보유 스킬 색인 (본문: `ezer skill show <name>`)\n");
    ...
    // ① 사용자 로컬 디렉티브 오버레이(~/.ezer/local/directives/<ROLE>_DIRECTIVE.local.md) —
    ...
    if let Some(stem) = directive_path.file_name().and_then(|n| n.to_str()) {
        ...
        let local_path = ezer::pack::local_dir().join("directives").join(&local_name);
        if let Ok(raw) = std::fs::read_to_string(&local_path) {
            let (clean, warnings) = ezer::overrides::sanitize_local_directive(&raw);
            for w in &warnings {
    ...
    let expert = std::env::var("EZER_OVERRIDE_EXPERT").map(|v| v == "1").unwrap_or(false);
    let ov = ezer::overrides::load_overrides(role, expert);
    let ov_block = ezer::overrides::render_block(&ov);
    if ov_block.is_empty() && local_appended {
        // 오버라이드 파일이 없어도 로컬 지침이 붙었으면 안전핵 재선언이 최후(last-word)여야 한다(⑥).
        directive.push_str(ezer::overrides::SAFETY_CORE_REASSERT);
    } else {
    ...
    let directive = ezer::action_catalog::substitute_catalog(&directive);
    Ok(directive)
}
...
fn resolve_resume_suffix(
    ...
    let cfg = config_dir
        .map(String::from)
        .unwrap_or_else(ezer::resolve_claude_config_dir);
    let comp = ezer::claude_project_component(cwd.unwrap_or(""));
...
/// (W1-4) restore 시 agents.json env 템플릿(`${EZER_ACCOUNT_DIR:-...}`) 대신 topology에 기록된 원
...
fn boot_agent_on_surface(
    ...
    // config_dir=None이면 게이트가 ezer::resolve_claude_config_dir()로 best-effort 해소한다.
    cwd: Option<&str>,
    config_dir: Option<&str>,
) -> Result<(), String> {
    ...
    if !ready {
        ...
        return Err(format!(
            "agent '{agent}' readiness not confirmed in {max_wait_secs}s — directive injection \
             ...
             → agents.json의 cmd를 점검하고 `ezer launch-agent --role <role> --agent {agent}`로 \
             재시도하라"
        ));
    }
    ...
    if flat.contains("ABSOLUTEDIRECTIVE") || flat.contains("절대지침") {
        ...
        eprintln!("[launch-agent] warning: directive not visible on screen — verify with `ezer read-screen --surface {}`", surface_ref(sid));
    }
...
/// 자기 surface의 ezerd-권위 역할 한 단어를 stdout으로 출력 (PreToolUse capability-gate hook 전용).
/// EZER_SURFACE_ID(데몬이 PTY에 주입·상속)로 자기 surface를 surface.list에서 찾아 데몬 roles 맵의
...
fn run_surface_role() -> i32 {
    let Some(my_sid) = ezer::env_compat(ENV_SURFACE_ID).and_then(|s| parse_surface_ref(&s)) else {
        println!();
        return 0;
    };
...
fn run_todo_path() -> i32 {
    let Some(sref) = ezer::env_compat(ENV_SURFACE_ID) else {
        eprintln!("EZER_SURFACE_ID 없음 — 데몬이 띄운 pane 안에서만 동작한다");
        return 1;
    };
    let Some(my_sid) = parse_surface_ref(&sref) else {
        eprintln!("EZER_SURFACE_ID 파싱 실패: {sref}");
        return 1;
    };
    ...
    let pack = ezer::env_compat("EZER_PACK_DIR")
        .map(std::path::PathBuf::from)
        .or_else(|| dirs::home_dir().map(|h| h.join(".ezer/pack")))
        .unwrap_or_else(|| std::path::PathBuf::from(".ezer/pack"));
...
fn sanitize_launch_cwd(cwd: String) -> String {
    ...
    if trimmed.is_empty() || (trimmed.len() == 2 && trimmed.ends_with(':')) {
        return ezer::home_dir().to_string_lossy().into_owned();
    }
    cwd
}
...
fn run_launch_agent_opts(
    ...
    // ★루트 cwd 교정(오너 2026-07-15 실사고): Finder 런칭 GUI·launchd 소유 ezerd의 cwd는 "/"라
    // ▶CEO/▶부서장 버튼·ezer boot 경유 노드가 루트에서 태어나고, 그 값이 phoenix roster에
                ...
                match request("surface.close", json!({"surface_id": sid, "cause": "reap"})) {
                    ...
                    Err(e) => eprintln!(
                        "[launch-agent] failed surface {} close 실패: {e} — \
                         `ezer close-surface {}`로 수동 정리 필요(role 점유 잔존 가능)",
                        surface_ref(sid),
                        surface_ref(sid)
                    ),
                }
...
// plist 포맷·경로·LABEL은 `ezer::launchd`(앱 자동등록과 단일 소스) 위임 — 드리프트 방지.
...
fn run_daemon_cmd(action: DaemonAction) -> i32 {
    let result: Result<(), String> = (|| {
            ...
            match action {
                DaemonAction::Install { takeover } => {
                    let daemon = sibling_daemon_path()
                        .ok_or("ezerd binary not found next to ezer (같은 폴더에 동봉 필요)")?;
                    let running = connect_raw().is_ok();
                    if running && !takeover {
                        return Err(
                            "데몬이 이미 가동 중 — 등록만 하면 launchd 인스턴스가 flock에 막혀 재시도 루프가 된다.\n\
                             기존 데몬을 정지하고 소유권을 이관하려면: ezer daemon install --takeover\n\
                             (주의: 가동 중인 세션이 소멸한다 — `ezer list`로 먼저 확인)"
                                .into(),
                        );
                    }
                    let plist = ezer::launchd::render_plist(&daemon, &ezer::launchd::log_path());
                    let path = ezer::launchd::plist_path();
                    if let Some(parent) = path.parent() {
                        std::fs::create_dir_all(parent).map_err(|e| e.to_string())?;
                    }
                    ...
                    if running && takeover {
                        ...
                        if ezer::launchd::is_loaded() {
                            let _ = std::process::Command::new("launchctl")
                                .args(["unload", "-w"])
                                .arg(&path)
                                .output();
                        }
                        // ⚠ `pkill -x ezerd`는 macOS comm이 15자로 잘려(/Applications/cy…)
                ...
                DaemonAction::Status => {
                    ...
                    let loaded = std::process::Command::new("launchctl")
                        .args(["list", ezer::launchd::LAUNCHD_LABEL])
                        .output()
                        .map(|o| o.status.success())
                        .unwrap_or(false);
        ...
        #[cfg(windows)]
        {
            const TASK: &str = "ezerd";
            match action {
                DaemonAction::Install { takeover: _ } => {
                    let daemon = sibling_daemon_path()
                        .ok_or("ezerd.exe not found next to ezer.exe")?;
                    ...
                    // ─ 종료코드 의미론(ezerd/main.rs 소스 확인): graceful shutdown(콘솔 이벤트 → shutdown_cleanup →
                    ...
                    //   스케줄러가 PT1M 뒤 재기동을 시도할 수 있으나, 그 무렵 앱 ensure_daemon 이 새 ezerd 를
                    //   이미 띄워 파이프를 점유했으면 스케줄러 ezerd 는 first_pipe_instance(true) 가드에 막혀 즉시
                    ...
                    let xml = ezerd_task_xml(&daemon, &user);
                    let xml_path = std::env::temp_dir().join("ezerd-task.xml");
...
/// ezer-statusline.sh 래퍼 전용 — stdin의 claude statusline JSON을 읽어 usage.report로 push하고,
...
/// ezer-hook.sh 전용 — hook stdin을 읽어 usage.event로 push. ★불변: 절대 에이전트를 막지 않는다
...
fn run_fleet(as_json: bool) -> i32 {
    ...
    let home = ezer::home_dir().to_string_lossy().into_owned();
    ...
    let reg = std::env::var("EZER_DEPTS_JSON")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| std::path::PathBuf::from(&home).join(".ezer/depts.json"));
            ...
            if let Some(depts) = v["depts"].as_object() {
                for (name, meta) in depts {
                    ...
                    let sock = meta["socket"]
                        .as_str()
                        .map(std::path::PathBuf::from)
                        .unwrap_or_else(|| ezer::dept_socket_path(name));
                    let disp = meta["display_name"].as_str().unwrap_or(name).to_string();
                    // 방출 socket = 실제 도달 소켓과 동일 경로 문자열(브리지가 ezer --socket 로 재사용).
...
fn run_status(as_json: bool) -> i32 {
    ...
    if r["paused"].as_bool() == Some(true) {
        println!(
            "⛔ PAUSED — {} (ezer resume로 해제; 큐·스케줄 동결 중, 실행 중 에이전트 행동은 계속)",
    ...
    if pending > 0 {
        println!(
            "feed: {pending} pending (oldest {}) — `ezer feed list --status pending`",
...
fn run_cycle_agent(
    ...
    let result = (|| -> Result<(), String> {
        ...
        let files: Vec<String> = if !save_files.is_empty() {
            ...
            let pack_round = ezer::pack::pack_dir().join("round");
        ...
        inject_text(sid, "[CYCLE] 컨텍스트 순환 절차 개시. 지금 즉시: ① 자기 TODO 파일(~/.ezer/pack/round/<역할>_TODO.md)과 SESSION_STATE(_round/ 또는 pack round/ 정본)에 현재 작업 상태·미해결 게이트·다음 액션을 저장하라. ② 저장 완료 후 다른 출력 없이 plain 한 줄로 CYCLE-SAVED 를 출력하라.")?;
        ...
        if let Some(v) = &verifier {
            ...
            inject_text(vsid, &format!("[CYCLE-VERIFY] role '{role_name}'(surface:{sid})의 컨텍스트 순환 전 저장 검증 요청. SESSION_STATE/TODO 파일이 방금 갱신되었는지 확인하고 `ezer feed reply {req_id} allow` 또는 `ezer feed reply {req_id} deny`로 판정하라."))?;
            let deadline = std::time::Instant::now() + std::time::Duration::from_secs(timeout);
            let decision = loop {
...
fn run_node_recover(surface: Option<String>, role: Option<String>) -> i32 {
    let result = (|| -> Result<(), String> {
        ...
        if entry["exited"].as_bool() == Some(true) {
            return Err(format!(
                "surface:{sid} 셸 자체가 종료됨 — `ezer restore`로 재기동하라"
            ));
        }
        ...
        if entry["env_injected"].as_bool() != Some(true) {
            return Err(format!(
                "surface:{sid}는 env 미주입 pane(수동 생성·구세션) — Windows에선 순수 cmd 재기동 시 \
                 CLAUDE_CONFIG_DIR 등이 실리지 않아 안전하지 않다. `ezer restore` 또는 \
                 `ezer close-surface {sid}` 후 `ezer launch-agent`로 재기동하라"
            ));
        }
        let role_name = entry["role"].as_str().unwrap_or("worker").to_string();
        let spec = load_agent_spec(&agent)?;
...
fn run_restore(cwd: Option<String>, include_master: bool, no_resume: bool) -> i32 {
    ...
    match result {
        Ok((ok, fail)) => {
            println!("restore 완료: 재기동 {ok} · 실패 {fail} · 현황 `ezer status`");
            if fail > 0 {
                1
            } else {
                0
...
// ─────────────────────────────────────────────────────────────────────────────
// 무중단 팩 업데이트 (ezer pack-update, DESIGN-noshutdown-pack-update §2-②/§7)
// ─────────────────────────────────────────────────────────────────────────────
...
fn version_gates(
    ...
    if !ezer::pack::remote_is_newer_tuple(remote_pack, disk_pack) {
...
/// deferred reinject 대상 영속 경로 — pack_state_base(=~/.ezer) 아래 .pack-reinject-pending.json.
...
fn reinject_exit_code(failed: usize) -> i32 {
    if failed > 0 {
        ezer::pack::EXIT_REINJECT_DEGRADED
    } else {
        0
    }
}
...
fn reinject_decision(
    ...
    if let Some(m) = marker {
        ...
        if let (Some(mv), Some(nv)) =
            (ezer::pack::parse_semver(&m.pack_version), ezer::pack::parse_semver(new_ver))
        {
            if mv >= nv {
...
/// manifest의 단일 SOT다(같은 ezer-pack/ 소스 → tree와 일치 보장). key_id/signed_at/expires_at는
...
fn build_pack_manifest_value(
    ...
    for (rel, content) in ezer::pack::PACK.iter().chain(ezer::pack::PACK_SKILLS.iter()) {
...
/// `ezer pack-manifest` 진입점 — 권위 manifest를 stdout으로 방출(§2-①). CI가 standalone 팩
...
fn run_pack_manifest(
    ...
    if let Some(ref pv) = pack_version {
        if ezer::pack::parse_semver(pv).is_none() {
...
fn with_apply_lock<T>(lock_path: &std::path::Path, f: impl FnOnce() -> T) -> Result<T, String> {
    ...
    {
        use std::os::unix::io::AsRawFd;
        // CI fresh 환경엔 ~/.ezer/ 가 없어 lock 파일 open이 ENOENT로 실패한다.
...
fn pack_update_from_dir(
    ...
    let manifest = ezer::packsig::verify_with_keyring(
        &manifest_bytes,
        &sig_bytes,
        now_unix,
        accepted_path,
    ...
    if let Err(e) = ezer::packsig::verify_files(&manifest, staging) {
        let _ = std::fs::remove_dir_all(staging);
    ...
    if let Err(e) = ezer::packsig::verify_no_extra_files(&manifest, staging) {
        let _ = std::fs::remove_dir_all(staging);
    ...
    let disk_version = std::fs::read_to_string(pack_dir.join(".pack-version"))
        ...
        .unwrap_or_default();
    let (disk_channel, disk_pro_rev) = match ezer::pack::read_pack_state(&pack_dir) {
        ezer::pack::PackStateRead::Absent => ("free".to_string(), 0u32),
        ezer::pack::PackStateRead::Corrupt(e) => {
            // 손상 상태의 튜플은 신뢰 불가 — typed 거부, repair 선행 요구(v4 §5).
            return Err(format!(
                "[pack-state-corrupt] .pack-state.json 손상({e}) — pack-update 거부. \
                 ezer pack-repair-channel 로 복구 후 재시도하라"
            ));
        }
        ezer::pack::PackStateRead::Valid(st) => {
            if st.base_version != disk_version {
                // 정합 불일치 = 손상 간주(v4 §3). ezerd 기동/init-pack의 제한적 자가치유가
                // 선행 경로 — pack-update는 보수적으로 거부한다.
                return Err(format!(
                    "[pack-state-mismatch] state.base {:?} ≠ .pack-version {:?} — pack-update 거부. \
                     ezer init-pack(자가치유) 또는 ezer pack-repair-channel 후 재시도하라",
    ...
    if disk_channel == "pro" && manifest.channel == "free" {
        return Err(
            "[pack-channel-refused] pro 설치에 free 번들 — pro→free 전환은 \
             ezer pack-downgrade-to-free 전용 명령만 허용된다"
                .to_string(),
        );
    }
    ...
    if gate == VersionGate::Apply && do_apply {
        ...
        let new_state = ezer::pack::PackState {
        ...
        let res = with_apply_lock(lock_path, move || {
            ...
            ezer::pack::apply_pack_transactional(&items, &pv, &new_state, || {
                ezer::packsig::record_accepted(&acc_path, &manifest_acc)
            })
        })?;
        let (w, k, post_ok) = res?;
        written = w;
        ...
        with_apply_lock(lock_path, move || {
            ...
            match installed {
                Some(m) if m == manifest_acc.files => {
                    match ezer::packsig::record_accepted(&acc_path, &manifest_acc) {
                        Ok(()) => eprintln!(
...
/// ~/.ezer (pack_dir의 부모) — 무중단 채널 상태파일(.pack-staging·.pack-apply.lock·.pack-accepted.json) 루트.
...
/// `ezer pack-downgrade-to-free`(free/pro v3 §5) — 유일한 pro→free 전환 경로. license-aware:
...
fn run_pack_downgrade_to_free(yes: bool, override_valid_license: bool) -> i32 {
    let dir = ezer::pack::pack_dir();
    let now = chrono::Utc::now().timestamp();
    let license_line = ezer::license::render_status(now);
    println!("라이선스: {license_line}");
    let st = match ezer::pack::read_pack_state(&dir) {
        ezer::pack::PackStateRead::Absent => {
            println!("팩 상태: state 부재(=free) — 강등 대상 없음. no-op.");
            return 0;
        }
        ezer::pack::PackStateRead::Valid(st) if st.channel == "free" => {
            println!("팩 상태: 이미 channel=free (base {}) — no-op.", st.base_version);
            return 0;
        }
        ezer::pack::PackStateRead::Valid(st) => st,
        ezer::pack::PackStateRead::Corrupt(e) => {
            eprintln!("팩 상태 손상({e}) — 먼저 ezer pack-repair-channel 로 복구하라.");
            return 1;
        }
    };
    println!(
    ...
    if ezer::license::is_pro(now) && !override_valid_license {
        eprintln!(
    ...
    let disk_v = std::fs::read_to_string(dir.join(".pack-version"))
        .map(|s| s.trim().to_string())
        .unwrap_or_default();
    let free_state = ezer::pack::PackState {
        channel: "free".to_string(),
        base_version: disk_v,
        pro_revision: 0,
    };
    if let Err(e) = ezer::pack::write_pack_state(&dir, &free_state) {
        eprintln!("error: state 전환 실패 — {e}");
        return 1;
    }
    match ezer::pack::install(false) {
        Ok((written, kept)) => {
        ...
        Err(e) => {
            eprintln!(
                "[downgrade] ⚠ state는 free로 전환됐으나 내장 재설치 실패: {e} — ezer init-pack 으로 재시도하라."
            );
            1
        }
    }
...
/// `ezer pack-repair-channel`(free/pro v4 §5) — 채널 상태 진단·복구. 재기록 권위 =
...
fn run_pack_repair_channel(to: Option<String>, yes: bool, expert_override: bool) -> i32 {
    ...
    let disk_v = std::fs::read_to_string(dir.join(".pack-version"))
        .map(|s| s.trim().to_string())
        .unwrap_or_default();
    let state_desc = match ezer::pack::read_pack_state(&dir) {
        ezer::pack::PackStateRead::Absent => "부재(=free/0)".to_string(),
        ezer::pack::PackStateRead::Valid(st) => format!(
            "channel={} base={} pro.{}{}",
            st.channel,
            st.base_version,
            st.pro_revision,
    ...
    println!("라이선스      : {}", ezer::license::render_status(now));
    ...
    if to == "free" {
        if ezer::license::is_pro(now) && !expert_override {
            eprintln!(
                "거부 — 유효 pro 라이선스 실재 중 free 재기록은 downgrade와 동일한 위험 \
                 (다음 내장 install이 pro 파일을 prune). 강등은 ezer pack-downgrade-to-free, \
                 강행은 --expert-override."
            );
            return 1;
        }
    ...
    let st = ezer::pack::PackState {
        channel: to.clone(),
        base_version: disk_v,
        pro_revision: pro_rev,
    };
    match ezer::pack::write_pack_state(&dir, &st) {
        Ok(()) => {
...
/// `ezer pack-update` 진입점(§2-② 전체 흐름). --from(핵심)·--manifest-url(부차).
...
fn run_pack_plan(force: bool) -> i32 {
    let dir = ezer::pack::pack_dir();
    let items: Vec<(&str, &str)> = ezer::pack::PACK_ALL.iter().map(|(r, c)| (*r, *c)).collect();
    let plan = ezer::pack::plan_install(&dir, &items, force, env!("CARGO_PKG_VERSION"));
    ...
    section("⏸ 보존+병합 대기", &plan.merge_new, "user-owned 수정본 유지 + 신버전 <파일>.new 병치 → ezer pack-merge");
    ...
    let pending = ezer::pack::load_merge_pending(&dir);
    if !pending.is_empty() {
        println!("※ 기존 병합 대기 {}건 — `ezer pack-merge` 로 검토", pending.len());
    }
    println!("※ 사용자 전용 오버레이(~/.ezer/local — 디렉티브 append·스킬 shadowing·훅 후행)는 업데이트가 절대 건드리지 않음");
    0
}
...
fn run_pack_merge(
    ...
    let dir = ezer::pack::pack_dir();
    let mut pending = ezer::pack::load_merge_pending(&dir);
    let Some(rel) = file else {
        ...
        for (rel, e) in pending.iter() {
            ...
            match kind {
                "new-pending" => println!(
                    "  ⏸ {rel} — 내 수정본 유지 중, vendor {ver} 신버전이 {side} 에 대기\n     → ezer pack-merge --file {rel} [--take-new|--keep-mine|--ai]"
                ),
                "healed" => println!(
                    "  🛠 {rel} — vendor {ver} 로 치유됨, 내 수정본은 {side} 에 보존\n     → ezer pack-merge --file {rel} [--to-local|--keep-mine(보존본 정리)]"
                ),
                _ => println!("  ? {rel} ({kind})"),
            }
        }
    ...
    let Some(entry) = pending.get(&rel).cloned() else {
        eprintln!("'{rel}' 은 병합 대기 목록에 없음 — `ezer pack-merge` 로 목록 확인");
        return 1;
    };
    let kind = entry.get("kind").and_then(|v| v.as_str()).unwrap_or("?");
    let target = dir.join(&rel);
    let embed_now: Option<&str> = ezer::pack::PACK_ALL
        .iter()
    ...
    let resolve = |pending: &mut serde_json::Map<String, serde_json::Value>, side_suffix: &str| {
        pending.remove(&rel);
        ezer::pack::save_merge_pending(&dir, pending);
    ...
    let advance_manifest_base = |content: &str| {
        let mpath = dir.join(ezer::pack::INSTALL_MANIFEST);
        let mut m: std::collections::BTreeMap<String, String> = std::fs::read_to_string(&mpath)
            .ok()
            .and_then(|s| serde_json::from_str(&s).ok())
            .unwrap_or_default();
        m.insert(rel.clone(), ezer::pack::content_hash_pub(content));
        if let Ok(json) = serde_json::to_string_pretty(&m) {
            let _ = ezer::pack::write_atomic(&mpath, json.as_bytes());
        }
    };
    match kind {
        "new-pending" => {
            ...
            if take_new {
                if confirm(&format!("'{rel}' 을 vendor 신버전으로 교체(내 수정 폐기)?")) {
                    if let Err(e) = ezer::pack::write_atomic(&target, theirs.as_bytes()) {
                        eprintln!("쓰기 실패: {e}");
                        return 1;
                    }
            ...
            let base_path = dir.join(ezer::pack::PRISTINE_DIR).join(&rel);
            ...
            match merged {
                ...
                Some(m) => {
                    ...
                    if confirm(&format!("'{rel}' 에 병합본 적용?")) {
                        if let Err(e) = ezer::pack::write_atomic(&target, m.as_bytes()) {
                            eprintln!("쓰기 실패: {e}");
                            return 1;
                        }
                        advance_manifest_base(&theirs);
                        ...
                        let _ = ezer::pack::write_atomic(&base_path, theirs.as_bytes());
                        resolve(&mut pending, ".new");
                ...
                None => {
                    println!(
                        "자동 병합 불가(충돌 또는 도구 부재). 선택지:\n\
                         \x20 ezer pack-merge --file {rel} --take-new   # vendor 신버전 채택\n\
                         \x20 ezer pack-merge --file {rel} --keep-mine # 내 수정 유지\n\
                         \x20 ezer pack-merge --file {rel} --ai        # AI 3-way 병합 제안\n\
        ...
        "healed" => {
            ...
            if to_local {
                ...
                let local_root = ezer::pack::local_dir();
                ...
                match std::fs::read_to_string(&user_path) {
                    Ok(content) => {
                        if let Err(e) = ezer::pack::write_atomic(&dest, content.as_bytes()) {
                ...
                println!(
                    "'{rel}' 은 system 파일 — 직접 되쓰기는 다음 기동 때 다시 치유(P0-4)되므로 지원하지 않음.\n\
                     \x20 ezer pack-merge --file {rel} --to-local  # 사용자본을 ~/.ezer/local 오버레이로(스킬 shadowing)\n\
                     \x20 ezer pack-merge --file {rel} --keep-mine # vendor 본 유지 확정(보존본 정리)\n\
                     보존본 위치: {}",
                    user_path.display()
                );
                0
...
fn diff3_merge(base: Option<&str>, ours: &str, theirs: &str) -> Option<String> {
    let base = base?;
    let tmp = std::env::temp_dir().join(format!("ezer-merge-{}", std::process::id()));
...
fn ai_three_way_merge(rel: &str, base: Option<&str>, ours: &str, theirs: &str) -> Option<String> {
    ...
    let prompt = format!(
        "다음은 ezer 팩 파일 '{rel}' 의 3-way 병합 요청이다.\n\
         {base_block}\
         <<<내 수정본(의도를 보존해야 할 대상)>>>\n{ours}\n<<<끝>>>\n\
         <<<vendor 신버전(새 베이스라인)>>>\n{theirs}\n<<<끝>>>\n\
    ...
    // ★세션 env 스크럽(ezerd scrub_claude_session_env 동형): claude 세션 안에서 실행하면 자식이
...
/// ezer_skillscan.py(`scan <스킬 디렉토리>`)가 팩에 있으면 스캔해 발견사항을 경고로만 출력한다.
/// 사용자 오버레이는 사용자 책임 영역 — 차단하면 자기발화·커스터마이즈가 막힌다(WARN-not-BLOCK).
fn skillscan_warn(skill_dir: &std::path::Path) {
    let scanner = ezer::pack::pack_dir().join("bin/ezer_skillscan.py");
    if !scanner.exists() {
        return;
    }
...
fn run_pack_update(from: Option<String>, manifest_url: Option<String>, dry_run: bool) -> i32 {
    ...
    let result = (|| -> Result<i32, String> {
        ...
        with_apply_lock(&lock_path, ezer::pack::recover_pack_journal)??;
        ...
        let keyring = ezer::packsig::embedded_keyring()?;
        ...
        match run_pack_reinject(&outcome.pack_version) {
            Ok(rep) => {
                ...
                println!(
                    ...
                    ezer::pack::REINJECT_RESULT_PREFIX,
                    outcome.pack_version,
                    rep.injected,
                    rep.skipped,
                    rep.deferred,
                ...
                if accepted_degraded {
                    return Ok(ezer::pack::EXIT_ACCEPTED_DEGRADED);
                }
...
mod tests {
    ...
    // 두 그룹이 병렬 교차하면 None 복원 시 remove_var가 실행 중 테스트를 실 ~/.ezer/pack으로
    ...
    fn test_keyring(key_id: &str, pubkey: &str) -> ezer::packsig::Keyring {
        ezer::packsig::Keyring {
            keys: vec![ezer::packsig::TrustedKey {
    ...
    fn pack_manifest_emits_embedded_files_with_content_hash() {
        ...
        let embedded: std::collections::BTreeSet<&str> = ezer::pack::PACK
            .iter()
            .chain(ezer::pack::PACK_SKILLS.iter())
            .map(|(r, _)| *r)
            .collect();
    ...
    fn reinject_failed_signals_degraded_exit() {
        ...
        assert_eq!(reinject_exit_code(5), ezer::pack::EXIT_REINJECT_DEGRADED);
        // 0(완전 성공)·1(일반 실패)과 구분되는 신호여야 호출자가 디스크 반영+미각성을 분간한다.
        assert_ne!(ezer::pack::EXIT_REINJECT_DEGRADED, 0);
        assert_ne!(ezer::pack::EXIT_REINJECT_DEGRADED, 1);
    }
    ...
    fn reinject_pending_persists_and_clears() {
        let base = std::env::temp_dir().join(format!("ezer-reinject-pending-{}", std::process::id()));
    ...
    fn pending_consume_retries_idle_keeps_busy() {
        let base = std::env::temp_dir().join(format!("ezer-pending-c1-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let path = reinject_pending_path(&base);
    ...
    fn pending_consume_clears_file_when_all_resolved() {
        let base = std::env::temp_dir().join(format!("ezer-pending-c2-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let path = reinject_pending_path(&base);
    ...
    fn pending_consume_noop_when_absent() {
        let base = std::env::temp_dir().join(format!("ezer-pending-c3-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
    ...
    fn pending_consume_graceful_when_daemon_absent() {
        ...
        let saved_sock = std::env::var(ezer::ENV_SOCKET).ok();
        let saved_noauto = std::env::var("EZER_NO_AUTOSTART").ok();
        let base = std::env::temp_dir().join(format!("ezer-pending-c4-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        std::env::set_var(ezer::ENV_SOCKET, base.join("nonexistent.sock"));
        std::env::set_var("EZER_NO_AUTOSTART", "1");
        ...
        // env 복원(assert 전).
        match saved_sock {
            Some(v) => std::env::set_var(ezer::ENV_SOCKET, v),
            None => std::env::remove_var(ezer::ENV_SOCKET),
        }
        match saved_noauto {
            Some(v) => std::env::set_var("EZER_NO_AUTOSTART", v),
            None => std::env::remove_var("EZER_NO_AUTOSTART"),
        }
    ...
    fn pack_update_from_dir_applies_signed_pack() {
        ...
        let saved_cfg = std::env::var(ezer::pack::ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("ezer-pu-apply-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        let pack_dir = td.join("pack");
        std::fs::create_dir_all(&pack_dir).unwrap();
        std::env::set_var(ezer::pack::ENV_PACK_DIR, &pack_dir);
        std::env::set_var(ezer::pack::ENV_CONFIG_DIR, td.join("ezerclaude"));
        ...
        let restore = || {
            ...
            match &saved_cfg {
                Some(v) => std::env::set_var(ezer::pack::ENV_CONFIG_DIR, v),
                None => std::env::remove_var(ezer::pack::ENV_CONFIG_DIR),
            }
        };
    ...
    fn pack_update_from_dir_rejects_invalid() {
        ...
        let saved = std::env::var(ezer::pack::ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ezer::pack::ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("ezer-pu-reject-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        let pack_dir = td.join("pack");
        std::fs::create_dir_all(&pack_dir).unwrap();
        std::env::set_var(ezer::pack::ENV_PACK_DIR, &pack_dir);
    ...
    fn pack_update_pro_channel_e2e() {
        ...
        let saved = std::env::var(ezer::pack::ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ezer::pack::ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("ezer-pu-pro-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        let pack_dir = td.join("pack");
        std::fs::create_dir_all(&pack_dir).unwrap();
        std::env::set_var(ezer::pack::ENV_PACK_DIR, &pack_dir);
        ...
        let state = ezer::pack::read_pack_state(&pack_dir);
        let acc_ev = ezer::packsig::read_accepted_evidence(&accepted);
        restore();
        let _ = std::fs::remove_dir_all(&td);
        ...
        assert!(
            matches!(state, ezer::pack::PackStateRead::Valid(ref st)
    ...
    fn pack_update_from_dir_rejects_extra_unlisted_file() {
        ...
        let saved = std::env::var(ezer::pack::ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ezer::pack::ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("ezer-pu-extra-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        let pack_dir = td.join("pack");
        std::fs::create_dir_all(&pack_dir).unwrap();
        std::env::set_var(ezer::pack::ENV_PACK_DIR, &pack_dir);
    ...
    /// (2c) 회귀 박제: transient 화이트리스트가 ezer connect()의 실제 에러 문자열과 정렬돼야
    /// (2a) slow_consumer return 후 재연결이 작동한다. ezer connect_raw는 누락 소켓에
    ...
    fn transient_event_error_matches_real_connect_strings() {
        // ezer connect_raw가 실제로 내는 형태
        assert!(is_transient_event_error(
            "cannot connect to ezerd at /tmp/x.sock: No such file or directory (os error 2)"
        ));
        assert!(is_transient_event_error(
            "cannot connect to ezerd at /tmp/x.sock: Connection refused (os error 61)"
        ));
        // half-open read 단절
    ...
    /// 멀티 노드 동시 RPC 에서 상시 실패하고("cannot connect to ezerd pipe … os error 231" —
    /// 2026-07-10 Windows 실사고), 다운 오판이 sibling ezerd autostart 헛발동까지 부른다.
    /// 간격이 0이면 busy spin, 마감 ≤ 간격이면 사실상 무재시도 — 정책 상수로 의도를 박제한다
    /// (Windows arm 은 이 호스트에서 컴파일/실행 불가 — ezerd PIPE_ACCEPT_ERROR_BACKOFF 와 같은 방식).
    #[test]
    fn pipe_busy_retry_policy_is_bounded_and_nonzero() {
        assert_eq!(
            ezer::PIPE_BUSY_ERROR, 231,
            "ERROR_PIPE_BUSY 는 Win32 상수 231 — 바뀌면 busy 분기가 영영 안 탄다"
        );
        assert!(
            !ezer::PIPE_BUSY_RETRY_INTERVAL.is_zero(),
            "busy-retry 간격이 0이면 100% CPU busy spin: {:?}",
            ezer::PIPE_BUSY_RETRY_INTERVAL
        );
        assert!(
            ezer::PIPE_BUSY_RETRY_DEADLINE > ezer::PIPE_BUSY_RETRY_INTERVAL,
            "마감({:?}) ≤ 간격({:?})이면 사실상 재시도 없는 1회 open 으로 회귀한다",
            ezer::PIPE_BUSY_RETRY_DEADLINE,
            ezer::PIPE_BUSY_RETRY_INTERVAL
        );
    }
    ...
    fn event_cursor_roundtrip_and_missing() {
        let dir = std::env::temp_dir().join(format!("ezer-cursor-test-{}", std::process::id()));
    ...
    fn boot_bin_skips_env_prefix_tokens() {
        assert!(is_env_assignment("CLAUDE_CONFIG_DIR=\"$HOME/.ezer/claude\""));
        ...
        assert_eq!(
            extract_bin(
                "CLAUDE_CONFIG_DIR=\"$HOME/.ezer/claude\" claude --dangerously-skip-permissions",
                "claude"
            ),
            "claude"
        );
    ...
    fn compose_directive_includes_memory_index_after_soul() {
        ...
        let td = std::env::temp_dir().join(format!("ezer-compose-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
    ...
    fn compose_directive_injects_rsi_only_for_master_worker() {
        let _env = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let td = std::env::temp_dir().join(format!("ezer-rsi-inject-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
    ...
    fn render_launch_os_aware_unix_byte_identical() {
        ...
        let env = vec![(
            ...
            "${EZER_ACCOUNT_DIR:-$HOME/.ezer/claude}".to_string(),
        )];
        ...
        #[cfg(not(windows))]
        {
            ...
            assert_eq!(
                send,
                "CLAUDE_CONFIG_DIR=\"${EZER_ACCOUNT_DIR:-$HOME/.ezer/claude}\" claude --dangerously-skip-permissions"
            );
    ...
    fn resolve_env_value_expands_default_branch() {
        ...
        std::env::remove_var("EZER_TEST_ACCT_X");
        let r = resolve_env_value("${EZER_TEST_ACCT_X:-$HOME/.ezer/claude}");
        assert!(r.ends_with("/.ezer/claude"), "default+HOME 전개: {r}");
        assert!(!r.contains("${") && !r.contains("$HOME"), "잔여 미전개 없음: {r}");
        std::env::set_var("EZER_TEST_ACCT_X", "/acct/dir");
        assert_eq!(resolve_env_value("${EZER_TEST_ACCT_X:-$HOME/.ezer/claude}"), "/acct/dir");
        std::env::remove_var("EZER_TEST_ACCT_X");
    }
    ...
    fn install_claude_hook_skips_backup_when_already_installed() {
        ...
        // hook이 이미 있으면 `.bak-ezer` 정상 백업이 클로버되면 안 된다(백업은 실제 write 시에만).
        let base =
            std::env::temp_dir().join(format!("ezer-hookbak-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        ...
        //    안 된다(D2 master 조건: 기존 hook 존재 시 .bak-ezer 무변경). mtime보다 견고한 내용 비교.
        ...
        assert_eq!(
            ...
            sentinel,
            "already-installed skip must NOT clobber existing .bak-ezer (정상 백업 무변경)"
        );
        let _ = std::fs::remove_dir_all(&base);
    }
    ...
    fn persona_set_normalizes_non_object_params() {
        let _env = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let td = std::env::temp_dir().join(format!("ezer-persona-bad-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        ...
        let body = std::fs::read_to_string(ezer::overrides::override_path("master")).unwrap();
    ...
    /// 회귀: ~/.ezer/ 가 없는 CI fresh 환경에서 with_apply_lock이 락 파일 부모 디렉토리를
    ...
    fn apply_lock_creates_missing_parent_dir() {
        // 존재하지 않는 부모(~/.ezer/ 부재 모사): base/<없는 .ezer>/.pack-apply.lock
        let base =
            std::env::temp_dir().join(format!("ezer-applylock-fresh-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        let missing_ezer = base.join("nonexistent-dot-ezer");
        let lock_path = missing_ezer.join(".pack-apply.lock");
        assert!(!missing_ezer.exists(), "사전조건: 부모 디렉토리가 없어야 함");

        let ran = with_apply_lock(&lock_path, || 42).expect("부모 부재여도 lock 성공해야 함");
        assert_eq!(ran, 42, "클로저가 실행돼 반환값이 전달돼야 함");
        assert!(missing_ezer.exists(), "lock이 부모 디렉토리를 생성했어야 함");
    ...
    fn doctor_ctx_at(base: &std::path::Path) -> DoctorCtx {
        DoctorCtx {
            ...
            socket_path: base.join("ezer.sock"),
    ...
    fn doctor_pack_version_ok_and_skew() {
        let base = std::env::temp_dir().join(format!("ezer-doc-ver-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn doctor_orphan_socket_detect_and_fix() {
        let base = std::env::temp_dir().join(format!("ezer-doc-sock-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let ctx = doctor_ctx_at(&base);
        // 소켓 없음 → OK
    ...
    fn doctor_stale_lock_detect_and_fix() {
        let base = std::env::temp_dir().join(format!("ezer-doc-lock-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let ctx = doctor_ctx_at(&base);
        let lock = ctx.socket_path.with_extension("lock");
    ...
    fn doctor_staging_residue_fix_keeps_prev() {
        ...
        std::env::set_var("EZER_DOCTOR_STAGING_MIN_IDLE_SECS", "0");
        let base = std::env::temp_dir().join(format!("ezer-doc-stg-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let ctx = doctor_ctx_at(&base);
        ...
        let _ = std::fs::remove_dir_all(&base);
        std::env::remove_var("EZER_DOCTOR_STAGING_MIN_IDLE_SECS");
    }

    // L5: 진행중(최근 수정) staging은 doctor --fix가 삭제하지 않고 보호한다.
    #[test]
    fn doctor_staging_residue_protects_in_progress() {
        std::env::set_var("EZER_DOCTOR_STAGING_MIN_IDLE_SECS", "3600"); // 1시간 보호창
        let base = std::env::temp_dir().join(format!("ezer-doc-stg-prot-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let ctx = doctor_ctx_at(&base);
        std::fs::create_dir_all(base.join(".pack-staging")).unwrap();
    ...
    fn doctor_channels_db_ok_and_corrupt() {
        let base = std::env::temp_dir().join(format!("ezer-doc-db-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let ctx = doctor_ctx_at(&base);
        // 없음 → OK
    ...
    fn doctor_hook_missing_then_fix() {
        let base = std::env::temp_dir().join(format!("ezer-doc-hook-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let ctx = doctor_ctx_at(&base);
        std::fs::create_dir_all(&ctx.pack_dir).unwrap();
    ...
    fn doctor_fix_then_rediag_ok() {
        ...
        std::env::set_var("EZER_DOCTOR_STAGING_MIN_IDLE_SECS", "0");
        let base = std::env::temp_dir().join(format!("ezer-doc-fix-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
        let ctx = doctor_ctx_at(&base);
        std::fs::create_dir_all(&ctx.pack_dir).unwrap();
    ...
    /// discover 스캔 밖(~/.ezer/claude 모사) 경로 + 같은 munge cwd의 foreign 프로필 공존 환경.
    #[test]
    fn w1_resume_gate_uses_recorded_config_dir_not_foreign_profile() {
        ...
        let recorded = base.join("acct").join(".ezer").join("claude");
    ...
    fn w1_restore_inlines_recorded_config_dir() {
        let template = "${EZER_ACCOUNT_DIR:-$HOME/.ezer/claude}";
        let recorded = "/home/x/acct/.ezer/claude";
    ...
    fn extract_tar_gz_rejects_symlink_entry() {
        let td = std::env::temp_dir().join(format!("ezer-xtar-sym-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::fs::create_dir_all(&td).unwrap();
    ...
    fn extract_tar_gz_rejects_parent_traversal() {
        let td = std::env::temp_dir().join(format!("ezer-xtar-dd-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::fs::create_dir_all(&td).unwrap();
        let tar_path = td.join("evil.tar.gz");
        let py = format!(
    ...
    fn extract_tar_gz_extracts_regular_files() {
        let td = std::env::temp_dir().join(format!("ezer-xtar-ok-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
    ...
    fn pack_update_from_dir_rejects_digest_mismatch() {
        ...
        let saved = std::env::var(ezer::pack::ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ezer::pack::ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("ezer-pu-digest-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        let pack_dir = td.join("pack");
        std::fs::create_dir_all(&pack_dir).unwrap();
        std::env::set_var(ezer::pack::ENV_PACK_DIR, &pack_dir);
