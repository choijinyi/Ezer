...
/// 서명된 prefix 승인 레코드. cmux SurfaceResumeApprovalRecord의 ezer 단순화(단일머신).
...
const ENV_SECRET_B64: &str = "EZER_APPROVAL_SECRET_B64";

/// 시크릿 파일 경로: ~/.ezer/.approval-secret — pack(~/.ezer/pack) 밖, ~/.ezer/ 직하.
...
/// 승인 레코드 영속 경로: ~/.ezer/approvals.json (0600).
...
pub fn signing_secret() -> Option<Vec<u8>> {
    ...
    // ② 0600 파일(pack 밖 ~/.ezer/ 하위) — Keychain crate 부재라 1차 경로.
