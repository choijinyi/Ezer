...
//! reviewer-*/planner 페인의 편집·커밋·write-shell을 ezerd 원장이 deny-by-default로 좁혀
...
//! enum(11종)·all-zero+mcp 이중 우회·CLJS 본문은 옮기지 않으며, ezer는 역할-기반 caps 집합 +
//! Rust 순수 정규화 함수로 재구현한다(MPL 파일전염 회피). 정직: penpot은 plugin permission을
//! 다루지 비-악의 에이전트의 내부 Edit 도구를 물리 차단하지 않는다 — ezer 기여는 프로세스 원장
//! 인-프로세스 가드(ezerd-매개 경로) + PreToolUse hook(에이전트 내부 도구의 실제 물리 enforcer).
//!
//! ★enforcement boundary (정직): caps 집합 자체는 *정책*이다. 이 정책을 강제하는 곳은 둘이다 —
//!   (1) ezerd-매개 경로(send/scoped run write-shell): handlers.rs가 직접 게이트.
//!   (2) 에이전트 내부 도구(Claude Code Edit/Write/Bash): ezerd는 *직접 못 막는다* —
//!       PreToolUse hook(ezer-pack/hooks/role-capability-gate.sh)이 실 enforcer.
//!   ezerd가 에이전트 내부 Edit 도구를 막는다고 주장하지 않는다(불가능).
