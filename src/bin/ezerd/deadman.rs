...
//! 홀더 pid가 ezerd로 확인될 때만) 홀더를 회수(SIGTERM→SIGKILL)하고 락을 인수한다.
//!
//! 안전 원칙:
//! - **fail-closed**: 홀더 pid 미상(구 락파일)·프로세스명 검증 실패·판정 애매 = 어떤 개입도 없이 exit.
//! - **무손실**: 건강한 홀더는 버전이 낮아도 절대 인수하지 않는다(인수=PTY 전멸). 오직 dead일 때만.
//! - **오살상 차단**: SIGTERM 전 pid의 프로세스명이 ezerd인지 확인(pid 재사용 방어 — channels.rs MED-4 원칙).
#![cfg(unix)]
...
/// pid의 프로세스명이 정확히 ezerd인가(kill 오살상 차단 — pid 재사용 방어).
/// `ps -p <pid> -o comm=` 출력의 basename이 정확히 "ezerd"인지 확인(부분일치 금지 —
/// `ezerd-<hash>` 테스트 바이너리 등 유사명 오살상 차단, fail-closed 강화).
/// macOS comm=실행 절대경로(basename=ezerd), Linux comm=15자 스레드명(ezerd, 미절단). 배포 sibling·앱
/// 번들 실행 바이너리 모두 basename이 정확히 "ezerd"다.
pub fn pid_is_ezerd(pid: u32) -> bool {
...
/// dead 판정된 홀더를 SIGTERM→유예→SIGKILL로 회수. `verify_ezerd`가 false면 kill하지 않는다
/// (오살상 차단 — main은 pid_is_ezerd를 주입, 테스트는 주입 가능). 반환: 실제로 회수를 시도했는가
/// (=락 재획득 재시도 가치가 있는가). false = 검증 실패로 개입 안 함.
pub fn reclaim_from_dead_holder(
    pid: u32,
    grace: Duration,
    verify_ezerd: impl Fn(u32) -> bool,
) -> bool {
    if !verify_ezerd(pid) {
        eprintln!("[ezerd] deadman: pid {pid} is not ezerd (stale pid reuse?) — refusing to kill");
        return false;
    }
    eprintln!(
        "[ezerd] deadman: holder pid {pid} appears hung (no socket response + stale heartbeat) — SIGTERM"
    );
    unsafe {
    ...
    while std::time::Instant::now() < deadline {
        if !pid_alive(pid) {
            eprintln!("[ezerd] deadman: holder pid {pid} exited after SIGTERM");
            return true;
        }
        std::thread::sleep(Duration::from_millis(100));
    }
    eprintln!("[ezerd] deadman: holder pid {pid} survived grace — SIGKILL");
    unsafe {
...
mod tests {
    ...
    #[test]
    fn pid_is_ezerd_refuses_non_ezerd() {
        // 안전 게이트: 테스트 바이너리·init(pid 1)은 ezerd가 아니므로 false → kill 거부.
        assert!(!pid_is_ezerd(std::process::id()), "테스트 프로세스≠ezerd");
        assert!(!pid_is_ezerd(1), "init/launchd≠ezerd");
        assert!(!pid_is_ezerd(0), "pid 0=false");
    }

    #[test]
    fn reclaim_refuses_when_not_ezerd() {
        // (오살상 차단) verify_ezerd=false면 kill하지 않고 false 반환 — 대상 프로세스 생존.
