...
//! context 사용량·rate limit 잔량을 결정론 산출한다. `ezer set-status` 자기보고(LLM 추론)의
...
/// 외부(비-pane) 세션 스윕 주기 초 기본값 — EZER_USAGE_EXTERNAL_SECS로 조정(0=끔)
...
fn poll_secs() -> u64 {
    ezer::env_compat("EZER_USAGE_POLL_SECS")
...
// ───────────────────────── 외부(비-pane) 세션 소비 수집 ─────────────────────────
// ezer pane 밖에서 도는 Claude Code 세션(예: 데스크톱 앱·직접 CLI)의 트랜스크립트도
...
fn external_sweep_secs() -> u64 {
    ezer::env_compat("EZER_USAGE_EXTERNAL_SECS")
        .and_then(|v| v.parse().ok())
...
/// claude 컨텍스트 윈도우 추정: 기본 200k, 1M 모델([1m])은 1M. EZER_CLAUDE_CTX_WINDOW로
/// 강제 가능 (passive 관측에선 서버 진실값이 없다 — Phase 2 statusline이 정밀값 제공).
pub fn claude_ctx_window(model: &str) -> u64 {
    if let Some(v) = ezer::env_compat("EZER_CLAUDE_CTX_WINDOW").and_then(|v| v.parse().ok()) {
        return v;
    }
...
/// 단일 소스는 ezer 라이브러리(resume 사전검증 게이트와 공유) — 여기선 위임만 한다(로직 중복 금지).
pub fn claude_project_component(cwd: &str) -> String {
    ezer::claude_project_component(cwd)
}
...
fn agy_poll_secs() -> u64 {
    ezer::env_compat("EZER_AGY_POLL_SECS")
        .and_then(|v| v.parse().ok())
        .filter(|v| *v >= 1)
        .unwrap_or(15)
}
...
async fn agy_quota_probe(port: u16) -> Option<Vec<RateWindow>> {
    ...
    let fut = tokio::process::Command::new("curl")
        ...
        // Windows: 주기 프로브가 콘솔 창을 반복 플래시하지 않게(콘솔 없는 ezerd의 콘솔 자식).
        .hide_console()
        .output();
...
mod tests {
    ...
    fn claude_window_default_and_1m_variant() {
        // ★테스트 격리: 런타임 환경(예: Claude Code 세션)이 EZER_CLAUDE_CTX_WINDOW(또는
        // EZER_/AITERM_ 호환 별칭)을 설정하면 env 오버라이드가 모델 기본값을 덮어 이 핀이
        // 거짓 실패한다. 모델 기반 분기만 검증하도록 해당 env를 제거 후 단언하고 복원한다.
        let keys = [
            "EZER_CLAUDE_CTX_WINDOW",
            "EZER_CLAUDE_CTX_WINDOW",
    ...
    fn read_new_lines_handles_partial_lines_and_truncation() {
        let dir = std::env::temp_dir().join(format!("ezer-usage-test-{}", std::process::id()));
    ...
    fn rollout_first_line_cwd_reads_session_meta() {
        let dir = std::env::temp_dir().join(format!("ezer-usage-meta-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
