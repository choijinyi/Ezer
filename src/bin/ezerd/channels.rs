//! C0 채널 계층 코어 — Slack·Discord 브리지를 ezerd(신뢰 경계)가 스폰·재조정하고,
//! 인바운드는 inbox-first 내구 퍼널로, 아웃바운드는 단조 상태기계(at-least-once)로 다룬다.
//! 정본 설계: `_research/openclaw-absorption/DESIGN-ko.md` §2 (2.0~2.8).
//!
//! 원리: **프로세스가 아니라 상태가 영속한다**(불사조와 동일). 브리지는 ezerd 스폰 자식이고,
//! 채널 상태(desired-state·inbox·원장)는 channels.db가 소유한다. ezerd/master가 정기 재시작되는
...
use ezer::{err_response, ok_response};
...
fn outbound_unknown_secs() -> f64 {
    std::env::var("EZER_CHANNEL_OUTBOUND_TIMEOUT_SECS")
        .ok()
...
fn channel_retain_secs() -> f64 {
    std::env::var("EZER_CHANNEL_RETAIN_DAYS")
        .ok()
...
/// 1회용 32바이트 토큰의 hex 문자열(env EZER_CHANNEL_TOKEN로 브리지에 주입).
...
/// 채널 전용 살균 — 공유 WriteReq::Inject·ezer send 신뢰 경로는 절대 건드리지 않는다(회귀 방지·§C1).
...
// ── 브리지 스폰(ezerd 스폰 자식·scoped 원장 등록·reaper 스레드로 zombie 회수) ────
...
fn spawn_bridge(
    ...
    cmd.env("EZER_CHANNEL_TOKEN", token)
        .env("EZER_CHANNEL", channel)
        .env(ezer::ENV_SOCKET, daemon.socket_path.to_string_lossy().as_ref());
    #[cfg(unix)]
    {
    ...
    #[cfg(windows)]
    {
        ...
        // 병용 불가. 없으면 콘솔 없는 ezerd가 띄우는 장수 브리지(cmd /C)마다 콘솔 창이 뜬다.
...
fn start(daemon: &Arc<Daemon>, conn: &mut Connection, params: &Value, id: &Value) -> Value {
    // H3: 채널은 메인 ezerd 단독 소유(§2.5) — 부서 데몬은 브리지 스폰 금지(동일 봇토큰 이중
    // 게이트웨이 연결 차단). 부서 데몬은 자기 channels.db만 검사해 교차 스폰을 못 막으므로,
    // 스폰 진입점(start·register)에서 구조적으로 거부한다.
    if ezer::is_dept_socket(&daemon.socket_path) {
        return err_response(
            id,
            "dept_channel_forbidden",
            "채널은 메인 ezerd 단독 소유 — 부서 데몬은 브리지 스폰 불가",
        );
    }
...
fn register(daemon: &Arc<Daemon>, conn: &mut Connection, params: &Value, id: &Value, caller_pid: Option<u32>) -> Value {
    ...
    if ezer::is_dept_socket(&daemon.socket_path) {
        return err_response(
            id,
            "dept_channel_forbidden",
            "채널은 메인 ezerd 단독 소유 — 부서 데몬은 브리지 등록 불가",
        );
    }
    let Some(channel) = p_str(params, "channel") else {
        return err_response(id, "invalid_params", "missing channel");
...
fn mirror_body(title: &str, summary: &str, feed_id: &str, until: f64) -> String {
    ...
    let note = if redacted { "\n(민감정보 가림 — 전문은 로컬 ezer 패널 확인)" } else { "" };
    let hhmm = {
        use chrono::TimeZone;
        chrono::Local
...
/// `ezer channel allow-remote-approve --for <기간>` → meta에 만료 ts 기록(기본 OFF에서 기간 열기).
...
/// `ezer channel unlock` → lockdown 플래그 해제(meta lockdown="0") + channel.unlocked 이벤트.
...
/// 되돌리지 않는다(enabled는 그대로 0) — 채널 재개는 `ezer channel start`로 명시한다(안전측).
...
/// ezerd 기동 시 enabled=1 채널마다: ①기록된 pgid 생존 시 고아 선-kill ②새 토큰으로 재스폰
/// ③channel.reconciled 발행. lockdown 중이면 재스폰 보류(안전측 — lockdown은 재부팅에도 유지).
pub fn reconcile(daemon: &Arc<Daemon>) {
    // LOW-3 H3 심층방어 — 채널 sweep/reconcile은 메인 ezerd 전용(부서 데몬 no-op).
    if ezer::is_dept_socket(&daemon.socket_path) {
        return;
    }
    let mut guard = daemon.channels.lock().unwrap();
    ...
    if lockdown_active(conn) {
        eprintln!("[ezerd] channels: lockdown active — reconcile skips respawn");
        return;
    }
    ...
    for (channel, bridge_cmd, old_pid, old_pgid) in rows {
        ...
        if let Some(pid) = old_pid {
            if pid_alive(pid as u32) {
                ...
                match (old_pgid, now_pgid) {
                    ...
                    _ => {
                        eprintln!("[ezerd] channels: '{channel}' pid {pid} pgid mismatch (stored {old_pgid:?}, now {now_pgid:?}) — stale pid reuse, skip kill");
                    }
                }
            }
        ...
        let Some(cmd) = bridge_cmd else {
            eprintln!("[ezerd] channels: '{channel}' enabled but no bridge_cmd — cannot respawn");
            continue;
        };
        let token = match random_token_hex() {
            Ok(t) => t,
            Err(e) => {
                eprintln!("[ezerd] channels: '{channel}' token gen failed, skip reconcile: {e}");
        ...
        match spawn_bridge(daemon, &channel, &cmd, &token) {
            ...
            Err(e) => eprintln!("[ezerd] channels: reconcile respawn '{channel}' failed: {e}"),
        }
    }
}
...
fn respawn_dead_bridges(daemon: &Arc<Daemon>, conn: &mut Connection) {
    ...
    for (channel, bridge_cmd, pid, last_spawn) in rows {
        ...
        let token = match random_token_hex() {
            Ok(t) => t,
            Err(e) => {
                eprintln!("[ezerd] channels: '{channel}' token gen failed, skip respawn: {e}");
...
mod tests {
    ...
    fn tmp_daemon(tag: &str) -> Arc<Daemon> {
        let dir = std::env::temp_dir().join(format!("ezer_chan_test_{}_{}", std::process::id(), tag));
    ...
    fn tmp_daemon_dept(tag: &str) -> Arc<Daemon> {
        let dir = std::env::temp_dir().join(format!("ezer-dept-chtest-{}-{}", std::process::id(), tag));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        Daemon::new(dir.join("ezer.sock"))
    }
