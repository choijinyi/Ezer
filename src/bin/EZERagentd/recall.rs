...
/// T4-19 보존 정리: EZERAGENT_RECALL_RETAIN_DAYS(기본 30, 0=비활성)보다 오래된 prefix를
...
fn maybe_prune(
    ...
    *last_prune = Some(std::time::Instant::now());
    let days: f64 = std::env::var("EZERAGENT_RECALL_RETAIN_DAYS")
        .ok()
...
mod tests {
    ...
    fn chain_matches_recomputation_and_detects_tamper() {
        let dir = std::env::temp_dir().join(format!("EZERagent_recall_test_{}", std::process::id()));
    ...
    /// EZERAGENT_RECALL_RETAIN_DAYS는 프로세스 전역 — maybe_prune 테스트가 cargo 병렬 러너에서
    ...
    fn maybe_prune_seals_anchor_consistent_with_full_chain() {
        ...
        let mut last_prune: Option<std::time::Instant> = None; // ★prune 발동(즉시 due·Instant 뺄셈 제거)
        {
            ...
            std::env::set_var("EZERAGENT_RECALL_RETAIN_DAYS", "30");
            maybe_prune(&conn, &mut chains, &mut last_prune);
            std::env::remove_var("EZERAGENT_RECALL_RETAIN_DAYS");
        }
    ...
    fn maybe_prune_disabled_when_retain_days_zero() {
        ...
        {
            let _g = RETAIN_ENV_LOCK.lock().unwrap();
            std::env::set_var("EZERAGENT_RECALL_RETAIN_DAYS", "0");
            maybe_prune(&conn, &mut chains, &mut last_prune);
            std::env::remove_var("EZERAGENT_RECALL_RETAIN_DAYS");
        }
        let cnt: i64 = conn
    ...
    fn max_surface_id_survives_full_surface_prune_via_chains() {
        ...
        let fake_socket = e2e_dir.join("EZERagent.sock");
        assert_eq!(
