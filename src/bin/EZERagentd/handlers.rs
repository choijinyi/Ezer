...
use EZERagent::{err_response, ok_response, parse_surface_ref, surface_ref, Request};
...
/// (EZERagent CLI 프로세스는 pane 셸의 자손이므로 조상 추적으로 소속 pane이 확정된다)
fn resolve_caller_surface(daemon: &Daemon, caller_pid: u32) -> Option<u64> {
    ...
    // 무한 성장 차단: EZERagent CLI는 매 호출이 단명 프로세스라 동일 pid가 사실상 재등장하지
...
/// T1-3 송신 ACL: ~/.EZERagent/pack/acl.json 의 role→role 정책 평가 + from 신원 검증.
...
fn check_send_acl(
    ...
    let acl_path = EZERagent::pack::pack_dir().join("acl.json");
...
/// T4-4/T6-P3 능력 가드 (EZERagentd-매개 변형 경로 — check_send_acl과 병렬·별 층위).
/// EZERagentd-인증 발신 surface(resolve_caller_surface, self-declared role 신뢰 금지)의 caps를
...
///
/// ★정직(enforcement boundary): 이 게이트는 *EZERagentd-매개* 변형(scoped run write-shell 등)만 막는다.
///   에이전트 *내부* 도구(Claude Code Edit/Write/Bash)는 EZERagentd가 직접 못 막는다 — 그건 PreToolUse
///   hook(role-capability-gate.sh)이 실 enforcer다. EZERagentd가 내부 Edit을 막는다고 주장하지 않는다.
///
...
fn typing_guard_secs() -> u64 {
    std::env::var("EZERAGENT_TYPING_GUARD_SECS")
        .ok()
...
/// EZERAGENT_CONTEXT_THRESHOLD_PCT로 조정 가능. 1~100 범위 밖·파싱 불가는 기본 60으로 폴백.
/// (usage.rs 관측 수집기도 같은 임계로 발화 — 자기보고/관측이 다른 임계를 쓰면 안 된다.)
pub(crate) fn context_threshold_pct() -> u8 {
    threshold_from(std::env::var("EZERAGENT_CONTEXT_THRESHOLD_PCT").ok())
}
...
pub(crate) fn maybe_fire_context_threshold(
    ...
    let threshold = pick_context_threshold(
        EZERagent::overrides::context_clear_pct(role.as_deref().unwrap_or("")),
    ...
    let mut payload = json!({
        ...
        "context_pct": pct,
        "threshold": threshold,
        "surface_ref": EZERagent::surface_ref(surface.id),
        "source": source,
...
/// RSI 학습 상태 디렉터리 — ★엔진(EZERagent_learn)의 EZERAGENT_ROUND_DIR/learn 규약과 일치시켜
/// 격리/테스트 정합을 보장한다(codex REVISE). 미설정 시 canonical = pack_dir()/round/learn
/// (pack_dir은 EZERAGENT_PACK_DIR 환경변수 우선). 데몬↔엔진이 동일 경로를 보게 한다.
...
fn learn_state_dir() -> std::path::PathBuf {
    if let Some(r) = EZERagent::env_compat("EZERAGENT_ROUND_DIR") {
        return std::path::PathBuf::from(r).join("learn");
    }
    EZERagent::pack::pack_dir().join("round").join("learn")
}
...
pub fn dispatch(daemon: &Arc<Daemon>, req: Request, caller_pid: Option<u32>) -> Reply {
    ...
    match req.method.as_str() {
        ...
        "surface.send_key" => {
            ...
            let Some(bytes) = EZERagent::key_to_bytes(&key) else {
                return Reply::Single(err_response(
                    &id,
                    "invalid_params",
        ...
        "system.claim_role" => {
            ...
            let master_after: Option<u64>;
            {
                ...
                *srole = Some(final_role.clone());
                ...
                // full=worker/master/cso, 그 외 deny-by-default). EZERagentd-매개 변형 게이트의 키 갱신.
        ...
        "ledger.register" => {
            ...
            // T4-4/T6-P3 능력 가드: scoped 실행 = EZERagentd-매개 write-shell 변형. reviewer-*/planner
            // surface가 scoped 셸을 원장에 등록(=EZERagentd가 생명주기를 책임지는 쓰기 셸 spawn)하려
        ...
        //     엔진(EZERagent_learn.py)이 CLI/트리거 경로에서만 수행(directive §4: 추천까지만 자율).
        "learn.propose" => {
            ...
            // feed 항목 등록(push_feed_notification·영속·이벤트 publish) → 사람이 feed 패널/EZERagent feed
            ...
            if !manual {
                ...
                let body = format!(
                    "{payload}\nfeed 패널 또는 'EZERagent feed reply <id> allow'로 승인 시에만 학습 ①~⑤ 착수(④저장·⑤채택은 rsi-gate 봉쇄 통과 필수). directive §4: 추천까지만 자율."
                );
            ...
            let (status, feed, note) = if manual {
                ...
                ("awaiting_approval", "created",
                 "pending feed approval item 등록 — feed 패널 또는 'EZERagent feed reply <id> allow'로 승인 시에만 ①~⑤ 착수(거부=무실행).")
            };
            Reply::Single(ok_response(
                &id,
                json!({
        ...
        // pack-update/reinject 컨트롤러(EZERagentd-매개 발신)가 surface_id·pack_version·directive_hash로
        ...
        "reinject.mark" => {
            ...
            // context 오염을 유발한다(설계 §7-⑪ step2 'self-declared 신뢰 금지 — EZERagentd-인증 발신만',
            // claim_role·set_meta·send ACL과 동일한 '임의 surface 무인증 쓰기' 부류). 발신 pane은
            // 커널 peer pid로만 확정한다. 발신이 surface로 해석되면(=노드 pane) 거부한다.
            // 정당 발신(EZERagent pack-update·EZERagent restore)은 일시적 CLI라 caller_pid가 surface로
            ...
            if let Some(cs) = caller_sid {
                ...
                return Reply::Single(err_response(
                    ...
                    "reinject_denied",
                    &format!(
                        "reinject.mark denied: node panes may not set reinject markers; only the EZERagentd-mediated controller (anonymous/non-pane caller) may (caller surface {cs})"
                    ),
                ));
            }
            *surface.pack_reinject.lock().unwrap() = Some(crate::state::PackReinject {
            ...
            // 마커를 즉시 topology에 영속 — EZERagentd 재기동/복원을 견뎌 동일 버전 일괄 재주입을 차단.
        ...
        "usage.register" => {
            ...
            *surface.registered_transcript.lock().unwrap() = Some(path.clone());
            daemon.bus.publish(
                ...
                "usage",
                Some(sid),
                json!({"transcript": path, "surface_ref": EZERagent::surface_ref(sid)}),
            );
        ...
        // statusline stdin JSON이다. settings의 EZERagent-statusline.sh 래퍼가 매 assistant 메시지마다
        ...
        "usage.report" => {
            ...
            *surface.observed_usage.lock().unwrap() = Some(crate::usage::ObservedUsage {
            ...
            daemon.bus.publish(
                ...
                Some(sid),
                json!({
                    "surface_ref": EZERagent::surface_ref(sid), "role": role, "agent": agent,
        ...
        // EZERagent-hook.sh 래퍼가 hook stdin을 EZERagent usage-event-stdin으로 흘려 이 RPC로 push. E3
        ...
        "usage.event" => {
            ...
            let session = param_str(&params, "session_id").unwrap_or_else(|| EZERagent::surface_ref(sid));
            ...
            //    master/reviewer가 `EZERagent events --category agent` 구독만으로 워커 hook 실시간 수신.
        ...
        "org.status" => {
            ...
            // 워커가 워크플로우 폴더 밖으로 cd해도 진행% 산출(EZERagent_report)이 실제 _round를 찾게 한다.
            let pids: Vec<sysinfo::Pid> = daemon
                .surfaces
                .lock()
                .unwrap()
            ...
            Reply::Single(ok_response(
                ...
                json!({
                    ...
                    "daemon": {"version": env!("CARGO_PKG_VERSION"),
                               "started_at": daemon.started_at,
                               "latest_seq": daemon.bus.latest_seq(),
                               // ★W1 identity(3중 대조): 폴백 EZERagent 가 이 데몬과 같은 빌드인지 python 이 교차대조.
        ...
        // 외장 Streamlit 대시보드 대신 EZERagentd가 직접 한 콜로 제공한다 — 플릿 상태·rate·
        // 시스템 CPU/MEM·소비통계·12h 스파크라인. EZERagent-app UI가 5초 폴링해 Control Center 패널을 그린다.
        "control.dashboard" => {
            let now = crate::state::now_epoch();
        ...
        "editor.action_catalog" => {
            Reply::Single(ok_response(&id, EZERagent::action_catalog::catalog_json()))
        }

        // ─── T4-3: on-demand 단건 상세 (전체 미주입 — penpot PenpotApiInfoTool 등가) ───
        "editor.action_info" => match param_str(&params, "name") {
            Some(name) => match EZERagent::action_catalog::action_info(&name) {
        ...
        "control.sessions" => {
            ...
            let since = crate::analytics::window_since(now, &window);
            // E9 RBAC: redact 파라미터 OR 환경변수 EZERAGENT_CONTROL_REDACT=1 → session_id(경로 PII) 가림(집계는 보존).
            let redact = params.get("redact").and_then(|v| v.as_bool()).unwrap_or(false)
                || std::env::var("EZERAGENT_CONTROL_REDACT").map(|v| v == "1").unwrap_or(false);
            let mut result = {
                let guard = daemon.analytics.lock().unwrap();
                match guard.as_ref() {
        ...
        // ─── T2-6 토폴로지: 영속 스냅샷 + 현재 라이브 역할 (EZERagent restore의 데이터 소스) ───
        "system.topology" => {
            ...
            let live: Vec<Value> = daemon
                ...
                .collect();
            // ★W2a: 묘비 집합을 동봉 — raw `EZERagent restore`(run_restore)가 의도 삭제 역할을 재스폰하지
...
mod tests {
    ...
    fn glob_multistar_matches_cli_semantics() {
        // EZERagent.rs의 재귀 cli_glob_match와 동일 의미를 regex판이 보장해야 한다
    ...
    /// EZERagent.rs `cli_glob_match`와 1:1 동일한 재귀 명세 (독립 오라클).
    ...
    // EZERAGENT_PACK_DIR는 프로세스 전역 env라 set/사용 윈도를 직렬화해야 cargo 병렬 러너에서
    ...
    /// 반환된 _guard가 살아있는 동안 EZERAGENT_PACK_DIR가 이 디렉터리를 가리킨다.
    fn daemon_with_acl(tag: &str, acl_json: &str) -> (Arc<Daemon>, std::path::PathBuf) {
        ...
        std::env::set_var(EZERagent::pack::ENV_PACK_DIR, &dir);
        let daemon = Daemon::new(dir.join("EZERagentd.sock"));
        (daemon, dir)
    }
    ...
    fn send_text_human_flag_does_not_bypass_acl() {
        ...
        std::env::remove_var(EZERagent::pack::ENV_PACK_DIR);
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn claim_daemon() -> Arc<Daemon> {
        ...
        let _ = std::fs::create_dir_all(&dir);
        Daemon::new(dir.join("EZERagentd.sock"))
    }
    ...
    /// 검사되는데 EZERagent CLI는 매 호출이 새 단명 프로세스라 동일 pid가 사실상 재등장하지 않아 TTL
    ...
    /// 프로세스 전역 env(EZERAGENT_MAX_ACTIVE_WORKERS)를 건드리지 않아 병렬 테스트 레이스가 없다.
    ...
    // ── T4-4/T6-P3 능력 가드: EZERagentd-매개 변형(scoped run write-shell) 차단 회귀 ──
    ...
    fn approval_sign_allowed_when_master_stable() {
        ...
        // 서명 부작용(secret·approvals.json)을 임시 HOME으로 격리 — 실제 ~/.EZERagent 오염 방지.
    ...
    /// system.topology RPC가 묘비를 노출해 raw `EZERagent restore` 심층방어(run_restore skip)의
    /// 데이터 소스가 된다.
    #[test]
