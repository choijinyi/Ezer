...
pub fn schedule_path() -> PathBuf {
    EZERagent::pack::pack_dir().join("schedule.json")
}
...
fn builtin_jobs() -> Vec<serde_json::Value> {
    vec![
        json!({
            ...
            "to": "master",
            "if_absent": "skip",
            "text_command": "printf '[heartbeat] phoenix 세대 스냅샷 정기화(6h·P2-4) — 손상 치유 소스 최신화.\\n'; python3 \"${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_state_snapshot.py\" snapshot 2>&1 | tail -3",
            "_builtin": "phoenix",
        ...
        json!({
            ...
            "to": "master",
            "if_absent": "skip",
            "text_command": "printf '[heartbeat] phoenix 주간 격리 드릴(원자성·중단내성 self-test·라이브 무접촉) — 실전이 첫 테스트인 상태 종료(축E E2).\\n'; python3 \"${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_state_snapshot.py\" self-test 2>&1 | tail -5",
            "_builtin": "phoenix",
            "_builtin_version": BUILTIN_JOBS_VERSION
        }),
    ]
...
/// user-owned(사용자 `EZERagent schedule add` 잡 보존)이라 팩 배달로는 built-in 잡을 갱신할 수 없다 — 코드가 upsert 한다.
...
pub fn ensure_builtin_jobs() {
    ...
    let mut root: serde_json::Value = match std::fs::read_to_string(&path) {
        Ok(c) => match serde_json::from_str(&c) {
            ...
            Err(e) => {
                ...
                eprintln!("[EZERagentd] ensure_builtin_jobs: schedule.json 파싱 실패({e}) — 무접촉(손상은 load_jobs 격리 소관)");
                return;
            }
        },
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => json!({"jobs": []}),
        Err(e) => {
            eprintln!("[EZERagentd] ensure_builtin_jobs: schedule.json 읽기 실패({e}) — 무접촉");
            return;
        }
    };
    if !root.is_object() {
        eprintln!("[EZERagentd] ensure_builtin_jobs: schedule.json 최상위가 object 아님 — 무접촉");
        return;
    }
    ...
    for id in &conflicts {
        eprintln!(
            "[EZERagentd] ensure_builtin_jobs: 사용자 잡이 예약 id '{id}' 를 선점 — built-in 갱신 skip(사용자 잡 보존). \
        ...
        match serde_json::to_string_pretty(&root) {
            Ok(s) => {
                ...
                if std::fs::write(&tmp, s).is_ok() && std::fs::rename(&tmp, &path).is_ok() {
                    eprintln!("[EZERagentd] ensure_builtin_jobs: built-in phoenix 잡 보장(생성/갱신) 완료");
                } else {
                    eprintln!("[EZERagentd] ensure_builtin_jobs: schedule.json 원자쓰기 실패");
                }
            }
            Err(e) => eprintln!("[EZERagentd] ensure_builtin_jobs: 직렬화 실패({e})"),
        }
    }
}
...
pub fn load_jobs() -> Vec<Job> {
    ...
    let content = match std::fs::read_to_string(&path) {
        ...
        Err(e) => {
            eprintln!(
                "[EZERagentd] schedule.json 읽기 실패({}): {e} — 빈 스케줄로 진행",
                path.display()
            );
            return Vec::new();
        }
    ...
    let root: serde_json::Value = match serde_json::from_str(&content) {
        ...
        Err(e) => {
            ...
            eprintln!("[EZERagentd] schedule.json 파싱 실패: {e} — {note}; 빈 스케줄로 진행");
            return Vec::new();
        }
    };
    match root.get("jobs") {
        ...
        Some(j) => match serde_json::from_value::<Vec<Job>>(j.clone()) {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "[EZERagentd] schedule.json 'jobs' 역직렬화 실패: {e} — 빈 스케줄(스키마 점검 필요)"
                );
                Vec::new()
            }
        },
...
fn load_state(daemon: &Daemon) -> ScheduleState {
    ...
    let content = match std::fs::read_to_string(&path) {
        ...
        Err(e) => {
            eprintln!(
                "[EZERagentd] schedule_state.json 읽기 실패({}): {e} — 기본 상태로 진행",
                path.display()
            );
    ...
    match serde_json::from_str::<ScheduleState>(&content) {
        ...
        Err(e) => {
            ...
            eprintln!("[EZERagentd] schedule_state.json 파싱 실패: {e} — {note}; 기본 상태로 진행");
...
async fn run_text_command(cmd: &str) -> Result<String, String> {
    ...
    // 성공(exit 0)이면 stdout만 push 텍스트로 쓴다. 보고 도구(EZERagent_report)는 진단·실패도
    // stdout 보고문에 담도록 설계됐으므로(예: "EZERagent status 수집 실패"), 성공 경로 stderr는
...
mod tests {
    ...
    fn builtin_jobs_ensure_idempotent_and_versioned() {
        // 사용자 잡 1개로 시작(EZERagent schedule add 시뮬).
    ...
    fn run_now_is_frozen_while_paused() {
        ...
        // 누구든 `EZERagent schedule run-now <id>`로 kill-switch를 우회해 정지된 에이전트 stdin에
    ...
    fn command_shell_actually_spawns_on_this_platform() {
        ...
        assert!(String::from_utf8_lossy(&out.stdout).contains("EZERagent"));
    }
