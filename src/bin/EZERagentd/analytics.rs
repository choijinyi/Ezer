//! T7 E1-3 영속 분석 저장소 — EZERagentd 내장 SQLite(`analytics.db`). recall.rs(transcripts.db)와
...
/// penpot의 Postgres MVCC 직렬화를 단일 EZERagentd writer + 한 SQLite 트랜잭션으로 재현한다.
...
/// tier 라우팅(R1) 도입 '전'에 EZERagent cost-baseline lock으로 박제 → 도입 후 동일 eval 재실행·diff로 회귀 판정.
...
mod tests {
    ...
    fn record_load_replay_roundtrip() {
        let dir = std::env::temp_dir().join(format!("EZERagent-analytics-{}", std::process::id()));
    ...
    fn derive_and_record_event() {
        ...
        let dir = std::env::temp_dir().join(format!("EZERagent-ev-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn record_usage_writes_role() {
        ...
        let dir = std::env::temp_dir().join(format!("EZERagent-role-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn redact_session_id_stable_and_pii_free() {
        ...
        assert!(!r.contains("EZERagent") && !r.contains("secret") && !r.contains("/"), "PII 노출: {r}");
    ...
    /// 테스트용: open()의 실제 스키마로 analytics.db 생성(socket_path=dir/EZERagent.sock → 부모 dir).
    /// tag로 테스트별 고유 디렉터리(병렬 실행 시 WAL 충돌 회피).
    fn open_change_db(tag: &str) -> (std::path::PathBuf, Connection) {
        let dir = std::env::temp_dir().join(format!("EZERagent-cl-{}-{}", std::process::id(), tag));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let socket = dir.join("EZERagent.sock");
    ...
    /// RevnConflict. 단일 EZERagentd writer 직렬화(한 트랜잭션 check+append)가 단조성을 보장하므로,
    ...
    fn concurrent_same_base_exactly_one_accepted() {
        ...
        // 두 writer가 둘 다 base_revn=1을 읽고(같은 base) 직렬로 EZERagentd writer에 도달:
