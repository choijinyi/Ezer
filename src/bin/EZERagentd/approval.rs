...
/// 서명된 prefix 승인 레코드. cmux SurfaceResumeApprovalRecord의 EZERagent 단순화(단일머신).
...
const ENV_SECRET_B64: &str = "EZERAGENT_APPROVAL_SECRET_B64";

/// 시크릿 파일 경로: ~/.EZERagent/.approval-secret — pack(~/.EZERagent/pack) 밖, ~/.EZERagent/ 직하.
...
/// 승인 레코드 영속 경로: ~/.EZERagent/approvals.json (0600).
...
pub fn signing_secret() -> Option<Vec<u8>> {
    ...
    // ② 0600 파일(pack 밖 ~/.EZERagent/ 하위) — Keychain crate 부재라 1차 경로.
