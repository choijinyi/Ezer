...
pub struct Surface {
    ...
    /// (데몬 env의 EZERAGENT_ACCOUNT_DIR 또는 $HOME/.EZERagent/claude, EZERagent::resolve_claude_config_dir). topology에
    /// 영속되고 restore가 이 값을 launch 문자열에 인라인 오버라이드해, 데몬 env가 바뀌어도 원 계정 dir로
    /// 정확히 재개한다. discover 스캔은 ~/.EZERagent/claude를 못 보므로 config_dir 권위는 오직 이 결정론 기록이다.
    ...
    /// 역할 변경(claim_role)과 동기 갱신 — EZERagentd-매개 변형 경로(send/scoped run)의 게이트 키.
    /// role과 함께 도출하되 self-declared role을 신뢰하지 않고 EZERagentd-인증 발신 surface를 키로 쓴다.
...
/// T1-1 에이전트 자기보고 상태 — 화면 파싱 없이 에이전트가 `EZERagent set-status`로 직접 신고.
...
/// (단일 write path — status.set 자기보고 경로로는 갱신 불가). topology에 영속되어 EZERagentd
...
impl Config {
    pub fn from_env() -> Self {
        ...
        Config {
            lang: detect_lang(),
            load_high_threshold: env_f64("EZERAGENT_LOAD_THRESHOLD", cores * 2.0),
            proc_count_threshold: env_f64("EZERAGENT_PROC_THRESHOLD", 50.0) as usize,
            duplicate_threshold: env_f64("EZERAGENT_DUP_THRESHOLD", 3.0) as usize,
            auto_kill_duplicates: EZERagent::env_compat("EZERAGENT_AUTOKILL_DUP")
                .map(|v| v == "1")
                .unwrap_or(false),
            idle_seconds: env_f64("EZERAGENT_IDLE_SECONDS", 300.0) as u64,
            max_active_workers: env_f64("EZERAGENT_MAX_ACTIVE_WORKERS", 8.0) as usize,
        }
    }
}
...
/// EZERAGENT_* 우선, 구 EZERAGENT_*/AITERM_* 폴백 — README가 약속한 EZERAGENT_* 이름이 실제로 동작하게 한다
fn env_f64(key: &str, default: f64) -> f64 {
    EZERagent::env_compat(key)
...
/// 실재, EZERagentd.log 누적 29회). 이 패닉이 reader 스레드를 죽이면 해당 pane의 PTY 배수가 정지해
...
/// `~/.EZERagent/policy.json`의 `deny_self_approve`(bool)를 읽는다. 파일이 없거나 파싱 실패하거나
...
pub fn deny_self_approve_policy() -> bool {
    let path = EZERagent::home_dir().join(".EZERagent").join("policy.json");
...
/// pid가 속한 프로세스 그룹 id(unix). 자기승인 판정의 pgid 격상(M4)에 쓴다 — `EZERagent feed push`와
...
///       → **fail-closed 차단**: `setsid EZERagent feed reply`로 새 세션/그룹을 만들어 pid·pgid 매칭을 탈출한
...
/// 기본 데몬 `\\.\pipe\EZERagent` → `"EZERagent"`(호출자가 %LOCALAPPDATA%\EZERagent 루트로 매핑·기존 호환 유지),
/// 부서 데몬 `\\.\pipe\EZERagent-dept-<n>` → `"EZERagent-dept-<n>"`(루트 하위 부서 고유 디렉토리).
...
/// 격리를 보장한다(구: 모든 부서가 단일 %LOCALAPPDATA%\EZERagent 공유 → SQLite 락 경합·부서간 오염).
/// 기본 데몬(`\\.\pipe\EZERagent`)은 %LOCALAPPDATA%\EZERagent 유지(호환 예외·마이그레이션 불요).
pub fn state_dir(socket_path: &std::path::Path) -> PathBuf {
    #[cfg(windows)]
    {
        let base = std::env::var("LOCALAPPDATA").unwrap_or_else(|_| ".".into());
        let root = PathBuf::from(base).join("EZERagent");
        let slug = pipe_slug(socket_path);
        if slug.is_empty() || slug == "EZERagent" {
...
/// 데몬과 같은 디렉터리에 놓인 형제 `EZERagent` CLI 경로.
/// Windows에서는 실행파일명이 `EZERagent.exe`이므로 플랫폼별 확장자를 붙인다
/// Windows: 데몬(EZERagentd)이 스폰하는 콘솔 자식(CLI·셸·taskkill 등)이 콘솔 창을 띄우지 않게
...
/// (EZERagent.rs `sibling_daemon_path`·main.rs `ensure_daemon`과 동일 패턴).
/// 형제 바이너리가 없으면 PATH 탐색용 파일명만 반환한다.
pub fn sibling_cli_path() -> PathBuf {
    let name = if cfg!(windows) { "EZERagent.exe" } else { "EZERagent" };
    std::env::current_exe()
        .ok()
...
impl Daemon {
    ...
    /// 무동작(셸 인라인 전개가 진실원). EZERAGENT_PACK_DIR·EZERAGENT_ACCOUNT_DIR 등 기존 주입과 동형.
    ...
    pub fn create_surface_with_env(
        ...
        if let Some(c) = &cmd {
            builder = CommandBuilder::new(&shell);
            // -Command는 PowerShell 전용 플래그다. EZERAGENT_SHELL로 cmd.exe를 지정하면
        ...
        // 팩 파일 읽기가 UnicodeDecodeError로 크래시. pane에서 도는 python(hooks·EZERagent_*.py)이 UTF-8을
        // 기본으로 쓰게 PYTHONUTF8=1 주입(unix 무영향·이미 UTF-8). EZERagent-dept는 자체 export로 보강.
        builder.env("PYTHONUTF8", "1");
        // 온보딩①: 데몬 옆 동봉 EZERagent CLI + (Windows)동봉 runtime을 pane PATH 선두 주입 —
        // 신규 머신(심링크 없음)에서도 pane 속 AI가 `EZERagent identify`·python3·bash를 즉시 쓴다.
        // RC-5: GUI 직스폰과 공유하는 공용 fn(EZERagent::runtime_prefixed_path) 사용 — 중복 구현 금지.
        if let Some(bin_dir) = std::env::current_exe()
            .ok()
            .and_then(|p| p.parent().map(|d| d.to_path_buf()))
        {
            let cur = std::env::var("PATH").unwrap_or_default();
            if let Some(newp) = EZERagent::runtime_prefixed_path(&bin_dir, &cur) {
                builder.env("PATH", newp);
            }
        }
        builder.env(EZERagent::ENV_SOCKET, self.socket_path.to_string_lossy().as_ref());
        // 부서 격리: 데몬 자신의 pack_dir(=EZERAGENT_PACK_DIR env, 미설정 시 기본 ~/.EZERagent/pack)을 자식 pane에
        // 전파한다. 이게 없으면 부서 데몬이 띄운 worker pane의 `EZERagent todo-path`/skill/memory가
        // 글로벌 pack으로 폴백해 부서 격리가 도구 레벨에서 깨진다(멀티마스터 정식화 F1).
        // 기본 데몬은 기본값을 전파하므로 단일 사용자 동작은 무변경.
        builder.env(
            EZERagent::pack::ENV_PACK_DIR,
            EZERagent::pack::pack_dir().to_string_lossy().as_ref(),
        );
        // 부서 계정 격리(＋부서 자동화): 데몬 자신의 EZERAGENT_ACCOUNT_DIR(EZERagent-dept create 가 주입)을 자식
        // pane 에 전파. agents.json claude.cmd 의 ${EZERAGENT_ACCOUNT_DIR:-...} 가 이 값으로 해석된다
        // (미설정=기본 계정 fail-safe). EZERAGENT_PACK_DIR 전파와 동형.
        if let Ok(acct) = std::env::var("EZERAGENT_ACCOUNT_DIR") {
            if !acct.is_empty() {
                builder.env("EZERAGENT_ACCOUNT_DIR", acct);
            }
        }
        builder.env(EZERagent::ENV_SURFACE_ID, id.to_string());
        builder.env(EZERagent::ENV_SURFACE_REF, EZERagent::surface_ref(id));
        if let Some(r) = &role {
            builder.env(EZERagent::ENV_ROLE, r);
        }
        ...
        self.bus.publish(
            ...
            "surface",
            Some(id),
            json!({"surface_ref": EZERagent::surface_ref(id), "pid": pid, "cwd": surface.cwd,
        ...
        let debug = EZERagent::env_compat("EZERAGENT_DEBUG")
            .map(|v| v == "1")
            .unwrap_or(false);
        std::thread::spawn(move || {
                        ...
                        let dsr_resp = {
                            ...
                            if panicked {
                                ...
                                *surf.last_parser_panic.lock().unwrap() = Some(now_epoch());
                                ...
                                eprintln!(
                                    "[EZERagentd] surface {} vt100 파서 패닉 격리 — 청크 {} 바이트 파싱 포기, \
                        ...
                        // 임계영역(위 :876-902) 밖이라 attach 중복배달 불변식과 직교한다.
                        {
                            ...
                            for (mut title, body) in extracted {
                                ...
                                daemon.bus.publish(
                                    ...
                                    "notify",
                                    Some(surf.id),
                                    json!({"surface_ref": EZERagent::surface_ref(surf.id), "title": title, "body": body}),
                                );
                            }
                        }
            ...
            *surf.exited_at.lock().unwrap() = Some(Instant::now());
            ...
            daemon.bus.publish(
                ...
                "surface",
                Some(surf.id),
                json!({"surface_ref": EZERagent::surface_ref(surf.id)}),
            );
        });
...
/// (기존 EZERagent.rs --clear-first의 클라측 sleep 값 계승). EZERAGENT_CLEAR_SETTLE_MS로 조정.
fn clear_settle_ms() -> u64 {
    std::env::var("EZERAGENT_CLEAR_SETTLE_MS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(150)
}
...
/// (default_shell이 EZERAGENT_SHELL로 셸을 바꿀 수 있으므로 플래그 하드코딩은 깨진다.)
#[cfg_attr(not(windows), allow(dead_code))]
...
fn default_shell() -> String {
    #[cfg(windows)]
    {
        EZERagent::env_compat("EZERAGENT_SHELL").unwrap_or_else(|| "powershell.exe".into())
    }
    #[cfg(not(windows))]
    {
        EZERagent::env_compat("EZERAGENT_SHELL")
...
/// 내부 `'`만 `'\''`로 닫고-이스케이프-열기. EZERagent 경로에 특수문자가 있어도 명령 주입 불가.
#[cfg(target_os = "macos")]
...
/// EZERagentd 자기 exe_dir(Contents/MacOS) 기준 runtime_bin_dirs와 단일화. runtime 부재(개발)여도 .local/bin은 주입.
#[cfg(target_os = "macos")]
fn mac_runtime_lc_prefix() -> Option<String> {
    let exe_dir = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.to_path_buf()))?;
    let mut dirs = EZERagent::runtime_bin_dirs(&exe_dir);
    dirs.push(EZERagent::home_dir().join(".local").join("bin"));
...
mod tests {
    ...
    fn mac_lc_path_prefix_single_quotes_special_chars() {
        use std::path::PathBuf;
        let dirs = vec![
            PathBuf::from("/Apps/EZERagent.app/Contents/Resources/runtime/python/bin"),
    ...
    // 버그: state_dir Windows 분기가 socket_path를 폐기하고 %LOCALAPPDATA%\EZERagent 고정 → 모든 부서가
    // 동일 transcripts.db·feed.jsonl 공유(SQLite 락 경합·부서간 오염). pipe_slug로 부서별 격리.
    #[test]
    fn pipe_slug_maps_base_and_dept_pipes() {
        // 기본 데몬 → "EZERagent"(호출자가 루트로 매핑)
        assert_eq!(pipe_slug(std::path::Path::new(r"\\.\pipe\EZERagent")), "EZERagent");
        // 부서 데몬 → 고유 슬러그
        assert_eq!(
            pipe_slug(std::path::Path::new(r"\\.\pipe\EZERagent-dept-3")),
            "EZERagent-dept-3"
        );
        assert_eq!(
            pipe_slug(std::path::Path::new(r"\\.\pipe\EZERagent-dept-future")),
            "EZERagent-dept-future"
        );
        // 방어적 sanitize: 마지막 컴포넌트에서 안전문자(영숫자·-·_)만 — `.`는 제거됨
        // (슬래시/역슬래시 모두에서 마지막 성분 추출: `EZERagent.sock` → `EZERagentsock`)
        assert_eq!(pipe_slug(std::path::Path::new("/tmp/EZERagent-dept-9/EZERagent.sock")), "EZERagentsock");
    }
    ...
    fn create_surface_with_env_records_env_injected_flag() {
        ...
        let s1 = daemon
            .create_surface_with_env(
                ...
                &[("CLAUDE_CONFIG_DIR".to_string(), "/x/.EZERagent/claude".to_string())],
                None,
            )
            .unwrap();
    ...
    fn rc15_dept_logdir_slug_matches_rc13_state_dir() {
        // D7 조건(정합 강제): EZERagent-dept dept_logdir(RC-15)의 Windows 폴더명과 state_dir(RC-13) 슬러그가
        // **동일 규약**이어야 로그(EZERagentd.log)+상태(transcripts.db·feed.jsonl)가 한 폴더로 모인다.
        // dept_logdir(Windows) = %LOCALAPPDATA%\EZERagent\EZERagent-dept-<name> (EZERagent-dept bash·스모크 검증).
        // state_dir(Windows)   = %LOCALAPPDATA%\EZERagent\<pipe_slug(\\.\pipe\EZERagent-dept-<name>)>.
        // 일치 조건: pipe_slug(dept pipe) == "EZERagent-dept-<name>". (2곳 slug 규약 갈라짐 방지 핀.)
        for name in ["dept-3", "dept-future", "dept-1"] {
            let pipe = format!(r"\\.\pipe\EZERagent-dept-{name}");
            assert_eq!(
                pipe_slug(std::path::Path::new(&pipe)),
                format!("EZERagent-dept-{name}"),
    ...
    fn pipe_slug_dept_differs_from_base_for_isolation() {
        // 핵심 불변식: 부서 슬러그 ≠ 기본("EZERagent") → state_dir가 서로 다른 디렉토리 파생(격리 보장).
        let base = pipe_slug(std::path::Path::new(r"\\.\pipe\EZERagent"));
        let d1 = pipe_slug(std::path::Path::new(r"\\.\pipe\EZERagent-dept-1"));
        let d2 = pipe_slug(std::path::Path::new(r"\\.\pipe\EZERagent-dept-2"));
        assert_ne!(d1, base);
        assert_ne!(d2, base);
    ...
    fn sibling_cli_path_uses_platform_extension() {
        ...
        // (버그였던 무확장자 "EZERagent" 하드코딩은 Windows에서 EZERagent.exe를 못 찾아
        //  node-recover·launch-agent 자동 기동이 전부 실패했다 — EZERagent.rs·main.rs와 동일 패턴이어야 함.)
        let p = sibling_cli_path();
        let want = if cfg!(windows) { "EZERagent.exe" } else { "EZERagent" };
        assert_eq!(
    ...
    fn windows_exec_flag_matches_shell_family() {
        ...
        // EZERAGENT_SHELL=cmd.exe일 때 `cmd.exe -Command <c>`가 되어 명령이 깨졌다.
