...
fn crash_window_secs() -> f64 {
    std::env::var("EZERAGENT_CRASH_WINDOW_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(10.0)
}
...
fn check_surface_crash(daemon: &Arc<Daemon>) {
    ...
    for s in surfaces {
        ...
        if let Some(capped) = EZERagent::wire::cap_response(&timeline) {
        ...
        daemon.bus.publish(
            ...
            "surface",
            Some(s.id),
            json!({"surface_ref": EZERagent::surface_ref(s.id), "role": role,
    ...
    for id in to_cleanup {
        ...
        if close_surface(daemon, id, CloseCause::Reap).is_ok() {
            ...
            daemon.bus.publish(
                ...
                "surface",
                Some(id),
                json!({"surface_ref": EZERagent::surface_ref(id),
...
fn check_agent_death(
    ...
    let auto_restart = std::env::var("EZERAGENT_AGENT_AUTORESTART")
        .map(|v| v == "1")
        .unwrap_or(false);
    let surfaces: Vec<Arc<crate::state::Surface>> =
        daemon.surfaces.lock().unwrap().values().cloned().collect();
    ...
    for s in surfaces {
        ...
        if alive {
            ...
            if s.agent_exit_notified.swap(false, Ordering::Relaxed) {
                ...
                daemon.bus.publish(
                    ...
                    "surface",
                    Some(s.id),
                    json!({"agent": agent, "surface_ref": EZERagent::surface_ref(s.id)}),
                );
            }
            continue;
        }
        ...
        daemon.bus.publish(
            ...
            "surface",
            Some(s.id),
            json!({"agent": agent, "role": role, "surface_ref": EZERagent::surface_ref(s.id),
                   "severity": crate::severity::Severity::Recoverable.as_str(),
...
fn check_learn_stuck(
    ...
    let threshold = env_u64("EZERAGENT_RSI_STUCK_RESTARTS", 3) as u32;
    let cooldown = env_u64("EZERAGENT_RSI_STUCK_DEBOUNCE_SECS", 3600) as f64;
    let now = now_epoch();
    ...
    for (sid, role) in roles {
        ...
        let body = format!(
            "{{\"event\":\"propose\",\"reason\":\"stuck\",\"topic\":\"{role} 막힘 돌파 방법론\",\"status\":\"awaiting_approval\",\"trigger\":\"watchdog restart>={threshold}\"}}\n\
             동일 노드 {threshold}회+ 재시작(막힘) 감지. 'EZERagent learn \"{role} 막힘 돌파\"'로 학습 착수(사람 승인). directive §4: 추천까지만 자율."
        );
...
fn check_feed_aging(daemon: &Arc<Daemon>, reminded: &mut HashMap<String, f64>) {
    let remind_secs = env_u64("EZERAGENT_FEED_REMIND_SECS", 300);
    if remind_secs == 0 {
        return;
    }
    let now = now_epoch();
...
fn check_master_deadman(daemon: &Arc<Daemon>, last_alert: &mut f64) {
    let secs = env_u64("EZERAGENT_MASTER_DEADMAN_SECS", 900);
    if secs == 0 {
        return;
    }
...
/// EZERAGENT_TODO_DIRS(PATH류 목록)를 플랫폼 규약대로 분해한다.
...
/// T3-9 todo 파일 워치: 각 surface cwd의 `_round/*_TODO.md` + EZERAGENT_TODO_DIRS 추가 루트.
...
fn check_todo(daemon: &Arc<Daemon>) {
    ...
    if let Ok(extra) = std::env::var("EZERAGENT_TODO_DIRS") {
...
fn check_approvals(daemon: &Arc<Daemon>, debounce: &mut HashMap<(u64, String), f64>) {
    let agents: serde_json::Value =
        match std::fs::read_to_string(EZERagent::pack::pack_dir().join("agents.json"))
            .ok()
    ...
    for s in surfaces {
        ...
        for p in patterns {
            ...
            daemon.push_feed_notification(
                "approval",
                &format!("{agent} 승인 대기 감지 ({})", EZERagent::surface_ref(s.id)),
                &excerpt,
                Some(s.id),
            );
...
/// (EZERAGENT_APPROVAL_STALL_SECS, 기본 300s)를 넘겨 pending이면 사람 개입 필요 신호
...
fn check_approval_stall(daemon: &Arc<Daemon>, fired: &mut std::collections::HashSet<String>) {
    let stall = env_u64("EZERAGENT_APPROVAL_STALL_SECS", 300);
    if stall == 0 {
        return;
    }
    let now = now_epoch();
    ...
    for (rid, title, age, sid) in stalled {
        ...
        daemon.bus.publish(
            ...
            json!({"request_id": rid, "title": title, "age_secs": age as u64,
                   "surface_ref": sid.map(EZERagent::surface_ref)}),
        );
    }
}
...
/// (EZERAGENT_FEED_BACKLOG_ALERT, 기본 25)를 넘으면 에지 1회 경보. 개별 항목 aging 재알림
...
fn check_feed_backlog(daemon: &Arc<Daemon>, alerted: &mut bool) {
    let threshold = env_u64("EZERAGENT_FEED_BACKLOG_ALERT", 25);
    let total = daemon
        .feed_items
        .lock()
        .unwrap()
...
/// T2-6 토폴로지 영속: role→agent→cwd 매핑을 디스크에 상시 기록 (EZERagent restore의 진실).
...
/// ★W2/P0-3: 세대 스냅샷(~/.EZERagent/state-generations/<gen>/topology.json)의 최신 tombstones 폴백.
/// 손상 topology 복구용 — best-effort(스냅샷 부재/없음=빈 집합).
fn tombstones_from_latest_generation() -> std::collections::HashSet<String> {
    let root = EZERagent::home_dir().join(".EZERagent").join("state-generations");
    ...
    match std::fs::read_to_string(&p) {
        Ok(s) => match serde_json::from_str::<serde_json::Value>(&s) {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "[EZERagentd] dept_tombstones.json 손상({e}) — {} isolate·빈 집합 폴백(부활 게이트 일시 해제 주의)",
                    corrupt.display()
                );
                std::collections::HashSet::new()
            }
...
pub fn load_tombstones_from_disk(socket_path: &std::path::Path) -> std::collections::HashSet<String> {
    ...
    match serde_json::from_str::<serde_json::Value>(&s) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "[EZERagentd] ★P0-3 topology.json 손상({e}) — {} isolate + 세대 스냅샷 tombstones 폴백({}개 복구)",
                corrupt.display(),
                recovered.len()
            );
            recovered
    ...
    for s in surfaces {
        ...
        if idle_for >= daemon.config.idle_seconds && !s.idle_notified.swap(true, Ordering::Relaxed)
        {
            daemon.bus.publish(
                ...
                "watchdog",
                Some(s.id),
                json!({"idle_seconds": idle_for, "surface_ref": EZERagent::surface_ref(s.id)}),
            );
        }
    }
}
...
/// reap 기능 on/off — 기본 on, `EZERAGENT_REAP_EXITED=0`으로만 비활성(다른 노브 컨벤션과 동일).
fn reap_exited_enabled() -> bool {
    std::env::var("EZERAGENT_REAP_EXITED")
        .map(|v| v != "0")
        .unwrap_or(true)
}
...
fn exited_surface_due(has_role: bool, elapsed_secs: u64) -> bool {
    let grace = if has_role {
        env_u64("EZERAGENT_REAP_EXITED_GRACE_SECS", 60)
    } else {
        env_u64("EZERAGENT_REAP_EXITED_NONROLE_GRACE_SECS", 10)
    };
    elapsed_secs >= grace
}
...
/// (windows에서) 데몬의 자식이 아니라 EZERagent CLI의 자식이므로 데몬 트리만 죽이는
...
pub fn close_surface(daemon: &Arc<Daemon>, id: u64, cause: CloseCause) -> Result<(), String> {
    ...
    daemon.bus.publish(
        ...
        "surface",
        Some(id),
        json!({"surface_ref": EZERagent::surface_ref(id), "descendants_killed": descendants.len()}),
    );
    persist_topology(daemon);
    Ok(())
}
...
/// 오래 막힐 수 있어 환경별 조정을 허용한다(EZERAGENT_QUEUE_QUIET_SECS).
fn queue_quiet_secs() -> u64 {
    std::env::var("EZERAGENT_QUEUE_QUIET_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(3)
}
...
/// (기본 5 · EZERAGENT_QUEUE_DEPTH_ALERT, 0=비활성). master가 working 중이라 조용해지지 않으면
/// 보고가 무음 적체된다(2026-06-12 실측 depth 9~12) — 침묵 대신 결정론 경보로 드러낸다.
fn queue_depth_alert_threshold() -> usize {
    std::env::var("EZERAGENT_QUEUE_DEPTH_ALERT")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(5)
}
...
/// 배달한다(EZERAGENT_QUEUE_HUMAN_QUIET_SECS로 조정).
fn queue_human_quiet_secs() -> u64 {
    std::env::var("EZERAGENT_QUEUE_HUMAN_QUIET_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(30)
}
...
fn alert_queue_depth_if_high(
    ...
    daemon.bus.publish(
        ...
        json!({"depth": depth, "threshold": threshold, "blocked_by": blocked_by,
               ...
               "surface_ref": EZERagent::surface_ref(s.id),
...
mod tests {
    ...
    fn daemon_approval_dedup_helpers_and_stale_clear() {
        let dir = std::env::temp_dir().join(format!("EZERagent_feed_dedup_{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let daemon = crate::state::Daemon::new(dir.join("EZERagentd.sock"));
    ...
    fn approval_stall_fires_once_per_item() {
        let dir = std::env::temp_dir().join(format!("EZERagent_stall_{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let daemon = crate::state::Daemon::new(dir.join("EZERagentd.sock"));
    ...
    /// reap 계열 테스트는 EZERAGENT_REAP_EXITED* env를 만지므로 직렬화(다른 env-터치 테스트와 충돌 방지).
    static REAP_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    /// EZERAGENT_REAP_EXITED* env를 테스트 종료 시(패닉 포함) 이전 값으로 원복하는 가드 —
    ...
    fn reap_exited_does_not_tombstone_role() {
        ...
        *s.exited_at.lock().unwrap() = Some(std::time::Instant::now());
        let _env = ReapEnvGuard::set(&[
            ("EZERAGENT_REAP_EXITED", "1"),
            ("EZERAGENT_REAP_EXITED_GRACE_SECS", "0"),
        ]);
        reap_exited_surfaces(&daemon);
