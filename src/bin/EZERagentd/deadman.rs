...
//! 홀더 pid가 EZERagentd로 확인될 때만) 홀더를 회수(SIGTERM→SIGKILL)하고 락을 인수한다.
//!
//! 안전 원칙:
//! - **fail-closed**: 홀더 pid 미상(구 락파일)·프로세스명 검증 실패·판정 애매 = 어떤 개입도 없이 exit.
//! - **무손실**: 건강한 홀더는 버전이 낮아도 절대 인수하지 않는다(인수=PTY 전멸). 오직 dead일 때만.
//! - **오살상 차단**: SIGTERM 전 pid의 프로세스명이 EZERagentd인지 확인(pid 재사용 방어 — channels.rs MED-4 원칙).
#![cfg(unix)]
...
/// pid의 프로세스명이 정확히 EZERagentd인가(kill 오살상 차단 — pid 재사용 방어).
/// `ps -p <pid> -o comm=` 출력의 basename이 정확히 "EZERagentd"인지 확인(부분일치 금지 —
/// `EZERagentd-<hash>` 테스트 바이너리 등 유사명 오살상 차단, fail-closed 강화).
/// macOS comm=실행 절대경로(basename=EZERagentd), Linux comm=15자 스레드명(EZERagentd, 미절단). 배포 sibling·앱
/// 번들 실행 바이너리 모두 basename이 정확히 "EZERagentd"다.
pub fn pid_is_EZERagentd(pid: u32) -> bool {
...
/// dead 판정된 홀더를 SIGTERM→유예→SIGKILL로 회수. `verify_EZERagentd`가 false면 kill하지 않는다
/// (오살상 차단 — main은 pid_is_EZERagentd를 주입, 테스트는 주입 가능). 반환: 실제로 회수를 시도했는가
/// (=락 재획득 재시도 가치가 있는가). false = 검증 실패로 개입 안 함.
pub fn reclaim_from_dead_holder(
    pid: u32,
    grace: Duration,
    verify_EZERagentd: impl Fn(u32) -> bool,
) -> bool {
    if !verify_EZERagentd(pid) {
        eprintln!("[EZERagentd] deadman: pid {pid} is not EZERagentd (stale pid reuse?) — refusing to kill");
        return false;
    }
    eprintln!(
        "[EZERagentd] deadman: holder pid {pid} appears hung (no socket response + stale heartbeat) — SIGTERM"
    );
    unsafe {
    ...
    while std::time::Instant::now() < deadline {
        if !pid_alive(pid) {
            eprintln!("[EZERagentd] deadman: holder pid {pid} exited after SIGTERM");
            return true;
        }
        std::thread::sleep(Duration::from_millis(100));
    }
    eprintln!("[EZERagentd] deadman: holder pid {pid} survived grace — SIGKILL");
    unsafe {
...
mod tests {
    ...
    fn tmp_dir() -> PathBuf {
        ...
        //   UnixListener::bind 가 패닉한다. 데몬 이름이 구제품(4B) → EZERagentd(10B) 로 길어지면서
    ...
    fn read_holder_pid_fail_closed_on_legacy_lockfile() {
        ...
        let d = tmp_dir();
        let lock = d.join("EZERagent.lock");
    ...
    #[test]
    fn pid_is_EZERagentd_refuses_non_EZERagentd() {
        // 안전 게이트: 테스트 바이너리·init(pid 1)은 EZERagentd가 아니므로 false → kill 거부.
        assert!(!pid_is_EZERagentd(std::process::id()), "테스트 프로세스≠EZERagentd");
        assert!(!pid_is_EZERagentd(1), "init/launchd≠EZERagentd");
        assert!(!pid_is_EZERagentd(0), "pid 0=false");
    }

    #[test]
    fn reclaim_refuses_when_not_EZERagentd() {
        // (오살상 차단) verify_EZERagentd=false면 kill하지 않고 false 반환 — 대상 프로세스 생존.
