...
/// (2026-06-13 실측: 데몬을 `EZERagent ping`으로 claude Bash에서 재기동하자 신규 노드 4종
...
fn scrub_claude_session_env() {
    ...
    for k in LEAKY {
        if std::env::var_os(k).is_some() {
            ...
            eprintln!("[EZERagentd] scrubbed leaky claude session env: {k}");
        }
    }
}
...
async fn main() {
    ...
    // <이름>.prev*(EZERagentd/EZERagent 고정 체인 + unlock-sweep의 <이름>.prev<rand> — msys-2.0.dll 등 세션이
    // 로드한 runtime 이미지)로 밀어두므로, 새 EZERagentd 기동 시 설치 트리를 재귀 순회하며 이름에
        ...
        if let Some(dir) = exe.parent() {
            fn sweep_prev(dir: &std::path::Path, depth: u8) {
                ...
                for e in entries.flatten() {
                    ...
                    {
                        eprintln!("[EZERagentd] stale update leftover removed: {}", p.display());
                    }
                }
            }
            sweep_prev(dir, 12);
    ...
    match EZERagent::pack::recover_pack_journal() {
        Ok(true) => eprintln!("[EZERagentd] pack-update orphan journal recovered (self-heal)"),
        Ok(false) => {}
        Err(e) => eprintln!("[EZERagentd] pack journal recovery skipped: {e}"),
    }
    ...
    // 손상+마커 무결 상태의 치유는 EZERagent init-pack/pack-update/doctor --fix 명시 경로가 담당한다
    // (매 부트 전량 치유는 seed-once 원복 사고(7-12)의 원인 기전 — 의도적 축소).
    if !EZERagent::pack::pack_current_for(env!("CARGO_PKG_VERSION")) {
        match EZERagent::pack::install(false) {
            Ok((written, _)) if written > 0 => eprintln!(
                "[EZERagentd] EZERagent Pack: {written} file(s) installed at {}",
                EZERagent::pack::pack_dir().display()
            ),
            Ok(_) => {}
    ...
    eprintln!(
        "EZERagentd (EZERagent terminal daemon) listening on {}",
        socket_path.display()
    );
...
async fn accept_loop(daemon: Arc<Daemon>, socket_path: &std::path::Path) {
    ...
    if socket_path.exists() {
        if std::os::unix::net::UnixStream::connect(socket_path).is_ok() {
            eprintln!(
                "error: another EZERagentd is already listening on {}",
                socket_path.display()
            );
            std::process::exit(1);
        }
    ...
    // raw `EZERagent restore`가 아니라 phoenix를 태워 desired_roster·묘비·회로차단기·저널을 경유한다.
...
fn acquire_startup_lock(
    ...
    match deadman::judge_holder(holder_pid, responded, hb_stale) {
        deadman::HolderVerdict::Dead => {
            // 홀더 hung 확정 → 회수(SIGTERM→SIGKILL, EZERagentd 검증 후) → 락 1회 재획득 시도.
            let pid = holder_pid.expect("Dead 판정은 pid 존재를 함의");
            if deadman::reclaim_from_dead_holder(pid, deadman::RECLAIM_GRACE, deadman::pid_is_EZERagentd)
                && try_flock(&file)
            {
                deadman::claim_lock(&mut file, state_dir);
                eprintln!("[EZERagentd] deadman: reclaimed startup lock from dead holder (pid {pid})");
                return Some(file);
            }
...
fn log_lock_loss(state_dir: &std::path::Path, lock_path: &std::path::Path, reason: &str) {
    ...
    if should_log {
        eprintln!(
            "error: another EZERagentd holds the startup lock ({}) — reason={reason}, occurrence #{count}",
            lock_path.display()
        );
    }
}
...
/// opt-out(EZERAGENT_NO_AUTORESTORE)이 아니면 항상 Ready — ★B1: phoenix 는 바이너리 임베드본이 권위이므로
/// 디스크 팩 phoenix 부재가 "미설치 skip"이 아니다(임베드 추출로 실행). args[0]=디스크 phoenix(폴백 후보).
#[derive(Debug, PartialEq)]
enum AutoRestore {
    /// EZERAGENT_NO_AUTORESTORE=1 — 사용자가 콜드부트 복원을 껐다.
    OptedOut,
    /// 스폰 대상: `python3 <phoenix> restore --auto`. args[0]=디스크 phoenix(B1 폴백 후보).
    /// ★W1/B3(§5-1): env 에 PHOENIX_EZERAGENT(exe 옆 EZERagent 절대경로)·PATH(runtime 선두주입)를 주입한다 —
    /// GUI/데몬 최소 PATH(/usr/bin:/bin:…)에서 phoenix 가 `EZERagent` 를 못 찾아 FileNotFoundError→exit 1
...
fn decide_auto_restore(
    ...
    let phoenix = pack_dir.join("bin").join("EZERagent_phoenix.py");
    let mut env: Vec<(String, String)> = Vec::new();
    // PHOENIX_EZERAGENT: 데몬 exe 옆 동봉 EZERagent 절대경로. 실존할 때만 주입한다(없으면 phoenix 의 which→표준경로
    // 폴백에 맡긴다 — 존재하지 않는 경로를 강제 주입해 재차 FileNotFoundError 를 만들지 않는다).
    let EZERagent_name = if cfg!(windows) { "EZERagent.exe" } else { "EZERagent" };
    let EZERagent_path = exe_dir.join(EZERagent_name);
    if EZERagent_path.is_file() {
        env.push((
            "PHOENIX_EZERAGENT".to_string(),
            EZERagent_path.to_string_lossy().into_owned(),
        ));
    }
    // PATH 재합성 — pane 자식(state.rs)과 동일 유틸 재사용(중복 구현 금지). 무변경이면 None(무주입).
    if let Some(newp) = EZERagent::runtime_prefixed_path(exe_dir, current_path) {
...
/// 메타버스 오피스 브리지(팩 EZERagent_hud_bridge.py · 127.0.0.1 한정) 자동기동 — CC "🏢 오피스" 탭이
/// 수동 python3 기동 없이 항상 열리게 한다. 단일 인스턴스 가드: HUD 포트가 이미 listen 중이면
/// (선행 EZERagentd·수동 기동) 스폰하지 않는다 — 동일 서버 누적이 구조적으로 0(자원 거버넌스 '누적·미종료' 차단).
/// 사망·부재는 60s 주기 재확인이 이어받고(KeepAlive), EZERagentd 정상 종료 시 kill_on_drop이 자식을 동반 정리한다.
/// EZERAGENT_NO_OFFICE_BRIDGE=1 opt-out · 팩에 브리지 부재(구팩)면 조용히 skip.
/// python 해석·PATH·EZERagent 주입은 auto-restore(★B3)와 동일 SOT(bundled_python3·runtime_prefixed_path).
fn spawn_office_bridge(state_dir: std::path::PathBuf) {
    if EZERagent::env_compat("EZERAGENT_NO_OFFICE_BRIDGE").map(|v| v == "1").unwrap_or(false) {
        eprintln!("[EZERagentd] office-bridge skipped (EZERAGENT_NO_OFFICE_BRIDGE=1)");
        return;
    }
    let script = EZERagent::pack::pack_dir().join("bin").join("EZERagent_hud_bridge.py");
    if !script.is_file() {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            cmd.arg(&script)
                ...
                .kill_on_drop(true);
            {
                // Windows: 콘솔 없는 EZERagentd가 콘솔 자식(python3.exe)을 그냥 스폰하면 새 콘솔 창이
            ...
            if let Some(newp) =
                EZERagent::runtime_prefixed_path(exe_dir_ref, &std::env::var("PATH").unwrap_or_default())
            {
                cmd.env("PATH", newp);
            }
            let EZERagent_name = if cfg!(windows) { "EZERagent.exe" } else { "EZERagent" };
            let EZERagent_path = exe_dir_ref.join(EZERagent_name);
            if EZERagent_path.is_file() {
                cmd.env("HUD_EZERAGENT_BIN", &EZERagent_path); // 사이드카 EZERagent 절대경로(PHOENIX_EZERAGENT 주입과 동일 패턴)
            }
            ...
            match cmd.spawn() {
                Ok(mut child) => {
                    eprintln!("[EZERagentd] office-bridge spawned (127.0.0.1:{port})");
                    let _ = child.wait().await; // 사망 감지 → 아래 백오프 후 루프가 재스폰 판단
                    eprintln!("[EZERagentd] office-bridge exited — 60s 후 재확인");
                }
                Err(e) => eprintln!("[EZERagentd] office-bridge spawn failed: {e}"),
            }
            tokio::time::sleep(std::time::Duration::from_secs(60)).await;
        }
    });
...
fn bundled_python3(exe_dir: &std::path::Path) -> Option<String> {
    ...
    for d in EZERagent::runtime_bin_dirs(exe_dir) {
        for n in names {
            let p = d.join(n);
            if p.is_file() {
...
/// state_dir 은 canonical 매핑(crate::state::state_dir — Windows LOCALAPPDATA/EZERagent/<slug>·unix 소켓 부모)으로
...
/// 콜드부트 auto-restore를 detached 스폰한다(env에 EZERAGENT_NO_AUTOSTART=1 — 자식 CLI가 라이벌
/// 데몬을 autostart하는 재귀를 차단). 대기 스레드가 자식을 reap해 좀비 잔존을 막는다.
/// ★W1: PHOENIX_EZERAGENT·PATH 주입(§5-1 침묵사 근원 수리) · stdout/stderr 를 null 대신 phoenix-restore.log 로
...
fn spawn_auto_restore(
    ...
    let opted_out = EZERagent::env_compat("EZERAGENT_NO_AUTORESTORE")
        .map(|v| v == "1")
        .unwrap_or(false);
    // exe_dir(데몬 바이너리 디렉터리) — PHOENIX_EZERAGENT·PATH 계산 기준.
    let exe_dir = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.to_path_buf()));
    ...
    match decide_auto_restore(&EZERagent::pack::pack_dir(), opted_out, exe_dir_ref, &current_path, &socket) {
        AutoRestore::OptedOut => {
            eprintln!("[EZERagentd] auto-restore skipped (EZERAGENT_NO_AUTORESTORE=1)");
        }
        AutoRestore::Ready { program, mut args, env } => {
                ...
                guard_restore_panic(&log_for_panic, || {
                    ...
                    match resolve_phoenix_source(&state_dir, &disk_phoenix, &program, &daemon) {
                        ...
                        PhoenixResolve::Failed(reason) => {
                            eprintln!("[EZERagentd] auto-restore ABORTED — 안전한 phoenix 없음: {reason}");
            ...
            eprintln!("[EZERagentd] auto-restore triggered (phoenix restore --auto · 임베드 추출 우선)");
        }
    }
}
...
/// PACK_ALL 에서 phoenix 실행에 필요한 bin/ 트리(EZERagent_phoenix.py + 형제 의존 EZERagent_state_snapshot.py 등)를 추린다.
fn phoenix_embed_files() -> Vec<(&'static str, &'static str)> {
    EZERagent::pack::PACK_ALL
        .iter()
        .copied()
...
fn extract_phoenix_embed(
    ...
    let write_all = || -> std::io::Result<()> {
        ...
        for (rel, content) in phoenix_embed_files() {
            ...
            if written == 1 && std::env::var("EZERAGENT_PHOENIX_EXTRACT_FAIL").is_ok() {
    ...
    match write_all() {
        Ok(()) => {
            let script = root.join("bin").join("EZERagent_phoenix.py");
            Ok((root, script))
        }
        Err(e) => {
...
fn resolve_phoenix_source(
    ...
    match extract_phoenix_embed(state_dir) {
        Ok((root, script)) => {
            ...
            eprintln!("[EZERagentd] phoenix 임베드 self-test 실패 — 디스크 폴백 시도");
            daemon.push_feed_notification(
                "warn",
        ...
        Err(e) => {
            eprintln!("[EZERagentd] phoenix 임베드 추출 실패({e}) — 디스크 폴백 시도");
            daemon.push_feed_notification(
                "warn",
                "phoenix 임베드 추출 실패",
    ...
    //    단일 소스 — 추출과 동일 목록)를 대조한다. EZERagent_phoenix.py 만 일치하고 형제 의존(EZERagent_state_snapshot.py)이
    //    부재/stale 인 디스크 팩이 통과하던 구멍을 막는다. 하나라도 불일치/부재=거부+어느 rel 인지 보고.
    match disk_fallback_verify(disk_phoenix) {
        Ok(()) => {
            eprintln!(
                "[EZERagentd] phoenix 디스크 폴백 채택(전 closure 해시 일치·verified): {}",
...
/// disk_phoenix = <pack>/bin/EZERagent_phoenix.py → pack_dir = 그 조부모(bin 의 부모).
fn disk_fallback_verify(disk_phoenix: &std::path::Path) -> Result<(), String> {
    ...
    for (rel, content) in files {
        ...
        match std::fs::read_to_string(&path) {
            Ok(disk) => {
                if EZERagent::pack::content_hash_pub(&disk) != EZERagent::pack::content_hash_pub(content) {
...
/// ★W1 재시도 지연(codex major test seam): 기본 60000ms. EZERAGENT_AUTORESTORE_RETRY_DELAY_MS 로 override —
/// 테스트가 sleep 0 으로 결정론 검증(1차 비0→2차 NOOP·중복 스폰 0, 5/6 무재시도)을 돌리게 한다.
fn autorestore_retry_delay() -> std::time::Duration {
    let ms = std::env::var("EZERAGENT_AUTORESTORE_RETRY_DELAY_MS")
        .ok()
    ...
    F: FnMut(u32) -> Option<i32>,
{
    ...
    loop {
        ...
        match code {
            Some(0) => {
                eprintln!("[EZERagentd] auto-restore finished (exit=0)");
                return attempt;
            }
            Some(5) => {
                eprintln!("[EZERagentd] auto-restore BREAKER_OPEN (exit=5) — 재시도 금지(크래시루프 정지·사람 승인 필요)");
                return attempt;
            }
            Some(6) => {
                eprintln!("[EZERagentd] auto-restore CORRUPT/identity (exit=6) — 재시도 금지(사람 개입 필요)");
                return attempt;
            }
            other => {
                if attempt >= 2 {
                    eprintln!(
                        "[EZERagentd] auto-restore finished (exit={other:?}) — 재시도 소진(1회). phoenix-restore.log 참조"
                    );
                    return attempt;
                }
                eprintln!(
                    "[EZERagentd] auto-restore non-zero (exit={other:?}) — {}ms 후 1회 재시도 (lease/liveness 재산정에 위임)",
...
fn open_restore_log(log_path: &std::path::Path) -> Option<std::fs::File> {
    ...
    let f = open(log_path).or_else(|| {
        let tmp = std::env::temp_dir().join("EZERagent-phoenix-restore.log");
        let alt = open(&tmp);
        if alt.is_some() {
            eprintln!(
                "[EZERagentd] auto-restore primary log 실패({}) — temp 폴백 {}",
                log_path.display(),
                tmp.display()
            );
        }
    ...
    if let Some(mut f) = f {
        ...
        let _ = writeln!(
            f,
            "\n===== phoenix auto-restore @ epoch={epoch} (pid EZERagentd={}) =====",
            std::process::id()
        );
        Some(f)
    } else {
        eprintln!(
            "[EZERagentd] auto-restore log(primary+temp) 모두 open 실패 — 자식 stdio 를 데몬 stderr 로 inherit(증거 소실 방지)"
        );
        None
    }
}
...
fn guard_restore_panic<F: FnOnce()>(log_path: &std::path::Path, body: F) -> bool {
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(body)) {
        ...
        Err(panic) => {
            ...
            eprintln!("[EZERagentd] ★auto-restore 스레드 panic 포착(P0-5 침묵사 차단·재스폰 안 함): {msg}");
            // phoenix-restore.log 에도 남겨 관측성 확보(빈 로그 → panic 기록으로 원인 직결).
            if let Some(mut f) = open_restore_log(log_path) {
                use std::io::Write;
                let _ = writeln!(f, "[EZERagentd] AUTO-RESTORE THREAD PANIC (P0-5 차단·재스폰 안 함): {msg}");
            }
            false
        }
    }
...
fn run_auto_restore_once(
    ...
    match open_restore_log(log_path) {
        Some(f) => {
            ...
            match f.try_clone() {
                ...
                Err(e) => {
                    eprintln!("[EZERagentd] auto-restore log stderr clone 실패({e}) — stderr inherit 폴백(null 금지)");
                    cmd.stdout(std::process::Stdio::from(f))
    ...
    let mut child = match cmd.spawn() {
        Ok(c) => c,
        Err(e) => {
            eprintln!("[EZERagentd] auto-restore spawn failed: {e}");
            return None;
        }
    };
    let pid = child.id();
    ...
    match child.wait() {
        ...
        Err(e) => {
            eprintln!("[EZERagentd] auto-restore wait failed: {e}");
            None
        }
    }
}
    ...
    loop {
        match server.connect().await {
            Ok(()) => {
                ...
                // 거부하므로, 미구현(None)이면 Windows에서 자기 surface 자가-claim('EZERagent claim-role
...
async fn handle_connection(daemon: Arc<Daemon>, stream: Stream, caller_pid: Option<u32>) {
    ...
    while let Ok(Some(line)) = next_line_capped(&mut reader, MAX_REQUEST_LINE).await {
        ...
        let req: Request = match serde_json::from_str(&line) {
            ...
            Err(e) => {
                let resp =
                    EZERagent::err_response(&serde_json::Value::Null, "parse_error", &e.to_string());
        ...
        match handlers::dispatch(&daemon, req, caller_pid) {
            ...
            Reply::FeedWait {
                ...
                let resp = match outcome {
                    Some(decision) => EZERagent::ok_response(
                        &id,
                    ...
                    None => {
                        ...
                        if let Some(s) = &snapshot {
                            ...
                            match decision {
                                Some(d) => EZERagent::ok_response(
                                    &id,
                                    json!({"request_id": request_id, "status": "resolved", "decision": d}),
                                ),
                                None => EZERagent::ok_response(
                                    &id,
                                    json!({"request_id": request_id, "status": "timeout", "decision": null}),
                                ),
                            }
...
/// T1-6: EZERagent↔EZERagentd ABI producer 자기검증 경계. 응답 `Value`를 `EZERagent::wire::frame_response`로
...
fn abi_severity(e: &EZERagent::wire::AbiError) -> severity::Severity {
    match e {
        EZERagent::wire::AbiError::Drift | EZERagent::wire::AbiError::LenMismatch => severity::Severity::Critical,
        EZERagent::wire::AbiError::VersionSkew { .. } => severity::Severity::Recoverable,
    }
}
...
async fn write_line<W: AsyncWrite + Unpin>(
    ...
    let capped = EZERagent::wire::cap_response(value);
    let value: &serde_json::Value = capped.as_ref().unwrap_or(value);
    let line = match EZERagent::wire::frame_response(value) {
        Ok(framed) => framed,
        Err(e) => {
            let sev = abi_severity(&e);
            eprintln!(
                "[EZERagentd] ABI producer self-verify {} ({:?}) — falling back to legacy serialization",
                sev.as_str(),
                e
            );
...
mod env_scrub_tests {
    ...
    fn scrub_removes_leaky_session_vars_only() {
        ...
        std::env::set_var("EZERAGENT_SCRUB_TEST_KEEP", "yes"); // 무관 변수 — 보존 확인용
        ...
        assert_eq!(
            std::env::var("EZERAGENT_SCRUB_TEST_KEEP").as_deref(),
            Ok("yes"),
            "무관 env까지 지우면 안 된다"
        );
        std::env::remove_var("EZERAGENT_SCRUB_TEST_KEEP");
    }
}
...
mod abi_severity_tests {
    ...
    fn abi_error_to_severity() {
        assert_eq!(super::abi_severity(&EZERagent::wire::AbiError::Drift), Severity::Critical);
        assert_eq!(
            super::abi_severity(&EZERagent::wire::AbiError::LenMismatch),
            Severity::Critical
        );
        assert_eq!(
            super::abi_severity(&EZERagent::wire::AbiError::VersionSkew {
                peer_pv: 2,
                local_pv: EZERagent::wire::PROTO_PV
            }),
            Severity::Recoverable
        );
        // 격리 술어와의 정합: Critical만 격리, Recoverable은 재시도.
        assert!(super::abi_severity(&EZERagent::wire::AbiError::Drift).is_critical());
        assert!(!super::abi_severity(&EZERagent::wire::AbiError::VersionSkew {
            peer_pv: 2,
            local_pv: EZERagent::wire::PROTO_PV
        })
        .is_critical());
...
mod feed_wait_disconnect_tests {
    ...
    async fn feed_wait_releases_waiter_when_client_disconnects_during_pause() {
        ...
        let tmp = dir.join("EZERagentd.sock");
        let daemon = Daemon::new(tmp.clone());
...
mod auto_restore_tests {
    ...
    fn guard_restore_panic_catches_and_logs_no_propagation() {
        let dir = std::env::temp_dir().join(format!("EZERagent-ar-panic-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    /// opt-out(EZERAGENT_NO_AUTORESTORE=1)이면 phoenix가 있어도 스폰하지 않는다.
    #[test]
    fn opted_out_never_spawns() {
        let dir = std::env::temp_dir().join(format!("EZERagent-ar-optout-{}", std::process::id()));
        let bin = dir.join("bin");
        std::fs::create_dir_all(&bin).unwrap();
        std::fs::write(bin.join("EZERagent_phoenix.py"), "#!/usr/bin/env python3\n").unwrap();
        assert_eq!(
    ...
    fn missing_disk_phoenix_still_ready_embed_authoritative() {
        let dir = std::env::temp_dir().join(format!("EZERagent-ar-missing-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        match decide_auto_restore(&dir, false, &dir, "/usr/bin:/bin", "sock:test") {
            AutoRestore::Ready { args, .. } => {
                assert!(args[0].ends_with("bin/EZERagent_phoenix.py"), "폴백 후보 경로: {}", args[0]);
    ...
    fn present_phoenix_builds_auto_restore_command() {
        let dir = std::env::temp_dir().join(format!("EZERagent-ar-ready-{}", std::process::id()));
        let bin = dir.join("bin");
        std::fs::create_dir_all(&bin).unwrap();
        let ph = bin.join("EZERagent_phoenix.py");
    ...
    fn ready_prefers_bundled_python_absolute_path() {
        let dir = std::env::temp_dir().join(format!("EZERagent-ar-bundlepy-{}", std::process::id()));
        let bin = dir.join("bin");
        std::fs::create_dir_all(&bin).unwrap();
        std::fs::write(bin.join("EZERagent_phoenix.py"), "#!/usr/bin/env python3\n").unwrap();
    ...
    /// ★W1/B3: exe 옆에 EZERagent 가 실존하면 PHOENIX_EZERAGENT 를 그 절대경로로 주입하고, exe_dir 가 PATH 에 없으면
    /// PATH 를 선두주입한다(GUI/데몬 최소 PATH 침묵사 근원 수리). "python3" 문자열 단언만으로는 불충분(D4).
    #[test]
    fn ready_injects_phoenix_EZERagent_and_path_env() {
        let dir = std::env::temp_dir().join(format!("EZERagent-ar-env-{}", std::process::id()));
        let bin = dir.join("bin");
        std::fs::create_dir_all(&bin).unwrap();
        std::fs::write(bin.join("EZERagent_phoenix.py"), "#!/usr/bin/env python3\n").unwrap();
        // exe_dir 에 실행가능 EZERagent 스텁을 둔다(PHOENIX_EZERAGENT 주입 조건 = 파일 실존).
        let EZERagent_name = if cfg!(windows) { "EZERagent.exe" } else { "EZERagent" };
        let EZERagent_path = bin.join(EZERagent_name);
        std::fs::write(&EZERagent_path, "#!/bin/sh\n").unwrap();
        ...
        match decide_auto_restore(&dir, false, &bin, "/usr/bin:/bin:/usr/sbin:/sbin", "sock:test") {
            AutoRestore::Ready { env, .. } => {
                let EZERagent_env = env
                    .iter()
                    .find(|(k, _)| k == "PHOENIX_EZERAGENT")
                    .map(|(_, v)| v.clone());
                assert_eq!(
                    EZERagent_env.as_deref(),
                    Some(EZERagent_path.to_string_lossy().as_ref()),
                    "PHOENIX_EZERAGENT 는 exe 옆 EZERagent 절대경로여야 한다"
                );
                let path_env = env
                    .iter()
                    .find(|(k, _)| k == "PATH")
    ...
    /// ★W1/B3: exe 옆에 EZERagent 가 없으면 PHOENIX_EZERAGENT 를 주입하지 않는다(존재하지 않는 경로 강제 주입으로
    /// 재차 FileNotFoundError 를 만들지 않는다 — phoenix 의 which→표준경로 폴백에 위임).
    #[test]
    fn ready_omits_phoenix_EZERagent_when_absent() {
        let dir = std::env::temp_dir().join(format!("EZERagent-ar-noEZERagent-{}", std::process::id()));
        let bin = dir.join("bin");
        std::fs::create_dir_all(&bin).unwrap();
        std::fs::write(bin.join("EZERagent_phoenix.py"), "#!/usr/bin/env python3\n").unwrap();
        match decide_auto_restore(&dir, false, &bin, "/usr/bin:/bin", "sock:test") {
            AutoRestore::Ready { env, .. } => {
                assert!(
                    !env.iter().any(|(k, _)| k == "PHOENIX_EZERAGENT"),
                    "EZERagent 부재 시 PHOENIX_EZERAGENT 무주입이어야 한다: {env:?}"
                );
            }
            other => panic!("expected Ready, got {other:?}"),
        }
    ...
    /// ★codex W4 fix1: 추출 중간 실패(seam EZERAGENT_PHOENIX_EXTRACT_FAIL) 시 partial root 즉시 정리 — phoenix-embed 잔여 0.
    #[test]
    fn b1_extract_mid_failure_leaves_no_partial_root() {
        let sd = std::env::temp_dir().join(format!("EZERagent-b1mf-{}", std::process::id()));
        std::fs::create_dir_all(&sd).unwrap();
        std::env::set_var("EZERAGENT_PHOENIX_EXTRACT_FAIL", "1");
        let res = extract_phoenix_embed(&sd);
        std::env::remove_var("EZERAGENT_PHOENIX_EXTRACT_FAIL");
    ...
    /// phoenix.py 는 일치해도 형제(EZERagent_state_snapshot.py) 부재/stale 이면 거부(어느 rel 인지 보고).
    #[test]
    fn b1_disk_fallback_full_tree_verify() {
        let pack = std::env::temp_dir().join(format!("EZERagent-b1ft-{}", std::process::id()));
        ...
        let disk_phoenix = bin.join("EZERagent_phoenix.py");
        assert!(disk_fallback_verify(&disk_phoenix).is_ok(), "전 closure 일치 → verified");
        // 형제 stale: snapshot.py 를 변조 → 거부(rel 명시).
        std::fs::write(bin.join("EZERagent_state_snapshot.py"), "STALE-SNAPSHOT-DRIFT").unwrap();
        let e = disk_fallback_verify(&disk_phoenix).unwrap_err();
        assert!(e.contains("EZERagent_state_snapshot.py"), "stale 형제 rel 미보고: {e}");
        // 형제 부재: snapshot.py 삭제 → 거부(부재 명시).
        std::fs::remove_file(bin.join("EZERagent_state_snapshot.py")).unwrap();
        let e2 = disk_fallback_verify(&disk_phoenix).unwrap_err();
        assert!(e2.contains("EZERagent_state_snapshot.py") && e2.contains("부재"), "부재 형제 미보고: {e2}");
        let _ = std::fs::remove_dir_all(&pack);
    }

    /// ★B1①: 임베드 추출이 phoenix.py + 형제 의존(EZERagent_state_snapshot.py)을 버전+uuid 격리 디렉터리에
    /// 임베드 내용 그대로 쓴다. temp 누수 0: 정리 후 디렉터리 소멸.
    #[test]
    fn b1_extract_writes_phoenix_and_deps() {
        let sd = std::env::temp_dir().join(format!("EZERagent-b1x-{}", std::process::id()));
        std::fs::create_dir_all(&sd).unwrap();
        let (root, script) = extract_phoenix_embed(&sd).expect("추출 성공");
        assert!(script.ends_with("bin/EZERagent_phoenix.py"));
        assert!(script.is_file(), "phoenix.py 추출 안됨");
        let snap = root.join("bin").join("EZERagent_state_snapshot.py");
        assert!(snap.is_file(), "형제 의존 EZERagent_state_snapshot.py 미추출");
        // 내용 == 임베드
        let embed_phoenix = phoenix_embed_files()
            .into_iter()
            .find(|(rel, _)| *rel == "bin/EZERagent_phoenix.py")
            .map(|(_, c)| c)
            .unwrap();
    ...
    fn b1_self_test_passes_on_real_embed() {
        ...
        let sd = std::env::temp_dir().join(format!("EZERagent-b1st-{}", std::process::id()));
        std::fs::create_dir_all(&sd).unwrap();
        let (root, script) = extract_phoenix_embed(&sd).expect("추출 성공");
    ...
    fn b1_prune_stale_embed_dirs() {
        ...
        let sd = std::env::temp_dir().join(format!("EZERagent-b1p-{}", std::process::id()));
        let root = sd.join("phoenix-embed");
        ...
        let empty = std::env::temp_dir().join(format!("EZERagent-b1p-empty-{}", std::process::id()));
    ...
    fn b3_bundled_python_absolute_path_preferred() {
        let dir = std::env::temp_dir().join(format!("EZERagent-b3-{}", std::process::id()));
        let bin = dir.join("bin");
        std::fs::create_dir_all(&bin).unwrap();
        std::fs::write(bin.join("EZERagent_phoenix.py"), "#!/usr/bin/env python3\n").unwrap();
    ...
    fn b3_no_bundled_python_falls_back_to_literal() {
        let dir = std::env::temp_dir().join(format!("EZERagent-b3-nolit-{}", std::process::id()));
        let bin = dir.join("bin");
        std::fs::create_dir_all(&bin).unwrap();
        std::fs::write(bin.join("EZERagent_phoenix.py"), "#!/usr/bin/env python3\n").unwrap();
    ...
    /// ★EZERAGENT_AUTORESTORE_RETRY_DELAY_MS override 파싱(기본 60000·override 반영).
    #[test]
    fn retry_delay_env_override() {
        // 기본값
        std::env::remove_var("EZERAGENT_AUTORESTORE_RETRY_DELAY_MS");
        assert_eq!(autorestore_retry_delay(), Duration::from_millis(60_000));
        // override — 이 테스트만 단일 스레드 실행 계약(--test-threads=1)이라 env 격리 안전.
        std::env::set_var("EZERAGENT_AUTORESTORE_RETRY_DELAY_MS", "0");
    ...
    fn restore_roots_cleared_on_all_paths_l1() {
        ...
        let _ = std::fs::create_dir_all(&dir);
        let daemon = Daemon::new(dir.join("EZERagentd.sock"));
