//! EZERagent Pack: EZERagent 터미널에 임베드된 멀티에이전트 운영체계 템플릿.

use std::path::{Path, PathBuf};

pub const ENV_PACK_DIR: &str = "EZERAGENT_PACK_DIR";
/// EZERagent 전용 CLAUDE_CONFIG_DIR 오버라이드(주로 테스트 격리용). 미설정 시 pack_dir 형제(~/.EZERagent/claude).
pub const ENV_CONFIG_DIR: &str = "EZERAGENT_CONFIG_DIR";
...
// EZERagent-pack의 git-추적 전체 트리는 build.rs가 `git ls-files EZERagent-pack` 소싱으로
// 컴파일 타임 자동 임베드한다(PACK_ALL — README·directives·bin·hooks·schemas·skills 등 전체). 새
// 파일은 EZERagent-pack/ 아래에 두고 git add 하면 재빌드 시 자동 통합 — 수동 목록 갱신 불필요. 추적
// 집합이 SOT이므로 gitignore(개인정보) 경계가 구조적으로 강제되고 untracked 개인파일은 임베드되지 않는다.
include!(concat!(env!("OUT_DIR"), "/pack_all.rs"));

/// 하위호환 별칭 — 전체 트리는 PACK_ALL 단일 소스다. 외부 호출처(src/bin/EZERagent.rs의 pack-manifest
...
/// ★W1 identity(3중 대조): phoenix ↔ EZERagentd/EZERagent 실행 신뢰원이 같은 빌드인지 교차대조하는 3필드 단일 SOT.
/// 폴백 EZERagent 채택 시 python 이 이 3필드를 self-report(EZERagent) vs daemon(EZERagentd status) 로 대조한다(§5-1②).
...
/// 빌드 식별자(git HEAD 짧은 SHA · build.rs 가 EZERAGENT_BUILD_ID 로 주입). 같은 빌드의 EZERagent·EZERagentd 동일.
pub fn build_id() -> &'static str {
    option_env!("EZERAGENT_BUILD_ID").unwrap_or("unknown")
}

/// 임베드 팩 매니페스트 해시 — PACK_ALL(rel+content, build.rs 가 이미 정렬)을 sha256 스트리밍 해시.
/// 같은 소스로 빌드된 EZERagent·EZERagentd 는 동일 값(둘 다 동일 PACK_ALL 임베드). 팩 내용이 다르면 값이 갈린다.
...
/// 설치 위치: $EZERAGENT_PACK_DIR 또는 ~/.EZERagent/pack
///
/// ★브랜드 잔재 청소(2026-08-11): 업스트림-시대 레거시 env 폴백 단은 전부 제거했다 —
/// EZERagent 는 신규 제품이라 그 키를 쓰던 사용자가 존재하지 않는다(죽은 분기).
...
/// EZERagent.rs::hook_command(init-pack 경로)와 setup_isolated_config_dir(격리 config dir 경로)가 **둘 다**
...
pub fn session_start_hook_command(pack_dir: &Path) -> String {
    ...
    if cfg!(windows) {
        // RC-2 잔여(T2.1·codex CONFIRMED): 공백 포함 경로(C:\Users\John Doe\.EZERagent\pack\...) 대응 — Windows만
        ...
        // 것(C:\Users\...→C:Users...→No such file) 방지. EZERagent_preflight._EZERagent_hook_cmd 의 Windows 형태와
...
/// EZERagent 전용 CLAUDE_CONFIG_DIR — 사용자 ~/.claude(외부 터미널 체계·구 지침 오염 가능)와 **격리**한다.
/// EZERagent가 띄우는 claude는 이 디렉터리만 읽으므로, 사용자 프로필이 오염돼 있어도 영향받지 않고
/// 사용자 프로필을 건드리지도(읽지도·지우지도) 않는다. macOS 인증은 계정 단위 Keychain이라
/// 격리해도 로그인이 유지된다(우리 DMG는 macOS 전용). pack_dir 형제(~/.EZERagent/claude).
pub fn config_dir() -> PathBuf {
    ...
    pack_dir()
        .parent()
        .map(|p| p.join("claude"))
        .unwrap_or_else(|| PathBuf::from(".EZERagent/claude"))
}

/// 격리 config dir 셋업: EZERagent 라우터(CLAUDE.md)와 SessionStart hook(settings.json)을 설치한다.
...
fn setup_isolated_config_dir() {
    ...
    // 라우터: 임베드 CLAUDE.md.template → <cfg>/CLAUDE.md (없을 때만 — 역할선언→~/.EZERagent/pack 라우팅)
                ...
                Ok(Some((channel, _, _))) if channel == "pro" => {
                    return Some(
                        "PACK_CHANNEL_PRESERVED state=free이나 accepted 기록=pro(거짓 free 의심) \
                         → 보존·내장 팩 미반영. 복구: EZERagent pack-repair-channel"
                            .to_string(),
                    )
                }
                Err(_) => {
...
/// ★B1: 외부(EZERagentd)가 디스크 폴백 phoenix 의 stale 여부(임베드 해시 대조)를 판정할 때 쓰는 공개 래퍼.
...
///   system(update)  = bin/*.py·hooks/*·skills·schemas·templates 등 그 외 전부(EZERagentd 소유·스큐 금지).
...
pub(crate) fn ownership(rel: &str) -> Ownership {
    ...
    if rel.ends_with("_DIRECTIVE.md")
        ...
        // ★B2-1(W3): schedule.json 은 사용자가 `EZERagent schedule add` 로 편집하는 혼합 파일 — 팩 강제갱신이 덮으면
        // 사용자 잡이 소실(비가역 데이터 손실)된다. user 소유로 보존하고, built-in 잡(phoenix-*)은 데몬 부트 시
        // 코드가 idempotent ensure 한다(EZERagentd schedule::ensure_builtin_jobs). 기본 잡 드리프트(복구 가능) < 사용자 잡 소실.
        || rel == "schedule.json"
    {
...
//     - `.merge-pending.json` = 병합 대기 원장 — `EZERagent pack-merge` 가 소비
...
/// 테스트 오버라이드: EZERAGENT_LOCAL_DIR. 기본 = pack_dir 형제 `local`(~/.EZERagent/local).
pub fn local_dir() -> PathBuf {
    if let Some(d) = crate::env_compat("EZERAGENT_LOCAL_DIR") {
        return PathBuf::from(d);
    }
    let pd = pack_dir();
    match pd.parent() {
...
/// install 드라이런 리포트(④ 투명성) — `EZERagent pack-plan` 이 설치 **전에** 사용자에게 보여준다.
...
/// 방향이 반대다. P4 `EZERagent pack-update`의 version_gates(반영 판정 축)가 호출한다.
...
/// 매니페스트가 실재하면 true(스윕 불요). 사용처 = **EZERagentd 부트**(EZERagentd/main.rs 온보딩②) 단독.
/// (v3에서 GUI 온보딩도 이 술어를 썼으나, EZERagentd가 GUI보다 먼저 돈 머신에서 게이트가 선점돼
/// hook 미설치 회귀(0.12.52 EZERagent-neo) → GUI는 자체 완료 마커(.gui-onboarded·main.rs)로 분리(v4).
...
/// pack_current_in의 실경로 래퍼 — 호출부(GUI setup·EZERagentd main)가 pack_dir 해석에 재결합하지 않게 한다.
/// ★게이트는 반드시 **부트 호출부**에 두고 install() 내부에 넣지 마라 — 내부에 넣으면 수동
/// `EZERagent init-pack`·pack-update·pack-downgrade(치유의 정식 경로)까지 게이트되어 치유가 불구가 된다.
...
/// (std::fs::write는 비원자라 부분 쓰기 노출). EZERagentd governance의 write_json_atomic과 동형.
...
pub fn install(force: bool) -> Result<(usize, usize), String> {
    ...
    install_from_iter(
        ...
        false, // embed/EZERagentd 경로(비트랜잭션): .pack-version 직접 기록 + 매니페스트 best-effort(외부 동작 불변).
    )
}
...
/// `transactional`: false면 embed/EZERagentd/init-pack 경로 — 종전대로 마지막에 `.pack-version`을
...
pub fn install_into<'a, I: IntoIterator<Item = (&'a str, &'a str)>>(
    ...
    .and_then(|s| serde_json::from_str(&s).ok())
    .unwrap_or_default();
    // 다운그레이드 차단: 디스크 팩 버전이 입력 버전(target_version)보다 새것이면(구버전 EZERagent로 롤백/오설치)
    ...
    if !pending.is_empty() {
        println!(
            "[init-pack] 커스터마이즈 병합 대기 {}건 — `EZERagent pack-merge` 로 검토 (신버전 .new 병치 / 치유 전 사용자본 .user 보존)",
            pending.len()
        );
    }
    ...
    // 매니페스트 영속:
    // - transactional=false(embed/EZERagentd/init-pack): 최선노력 — 직렬화·write 실패해도 설치 자체는
    ...
    // EZERagent 전용 CLAUDE_CONFIG_DIR 격리 셋업(오너 2026-06-15) — 사용자 ~/.claude 오염으로부터
    // EZERagent 마스터를 분리한다. best-effort·보존 모드라 깨끗한 환경에서도 회귀 0.
...
// all-or-nothing(부분적용 0)을 부여한다. ★install()/EZERagentd 자동설치·init-pack 경로는 이 트랜잭션을
...
/// 백업 저널 디렉터리(~/.EZERagent/.pack-journal) — pack_dir 형제(staging·lock·accepted와 동일 루트).
...
/// ★pack-update 착수 시·EZERagentd 기동 시(install(false) 전)에 호출해 부분적용을 선치유한다.
...
mod tests {
    ...
    fn pack_current_gate_only_closes_when_provably_current() {
        let td = std::env::temp_dir().join(format!("EZERagent-pack-current-test-{}", std::process::id()));
    ...
    fn pack_skills_embed_adopted_set_and_indexable() {
        ...
        // EZERagent-video-creator 영상 자동제작 스킬 32종(오너 제작 · preflight C26 VIDEO_SKILLS와
    ...
    /// 전부 설치된다 — "EZERagent 설치 = 기본 스킬 자동 설치" 계약의 기계 검증.
    #[test]
    fn install_writes_core_and_skills_to_fresh_dir() {
        ...
        let td = std::env::temp_dir().join(format!("EZERagent-pack-install-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        let cfgdir = td.join("EZERagentclaude"); // 격리 config dir(테스트 밀폐 — td와 함께 정리)
        ...
        // ★격리 config dir 셋업(오너 2026-06-15): EZERagent 라우터+hook이 전용 dir에 설치되고,
        // 사용자 ~/.claude 와 분리된다. 라우터는 ~/.EZERagent/pack 디렉티브로 라우팅해야 한다.
        let router = std::fs::read_to_string(cfgdir.join("CLAUDE.md")).expect("격리 CLAUDE.md 미설치");
        assert!(router.contains("~/.EZERagent/pack/directives"), "격리 라우터가 pack 디렉티브로 안 보냄");
        assert!(router.contains("EZERagent 터미널 전용"), "격리 라우터에 EZERagent 환경선언 부재");
    ...
    fn install_blocks_downgrade_when_disk_version_newer() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("EZERagent-pack-downgrade-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::env::set_var(ENV_PACK_DIR, &td);
        std::env::set_var(ENV_CONFIG_DIR, td.join("EZERagentclaude"));
        ...
        // 2) 디스크 .pack-version을 더 새 버전으로 위조(구버전 EZERagent 롤백/오설치 시뮬)
    ...
    fn install_threeway_sides_pristine_and_pending_lifecycle() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("EZERagent-pack-threeway-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::env::set_var(ENV_PACK_DIR, &td);
        std::env::set_var(ENV_CONFIG_DIR, td.join("EZERagentclaude"));
    ...
    fn plan_install_matches_actual_install_actions() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("EZERagent-pack-plan-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::env::set_var(ENV_PACK_DIR, &td);
        std::env::set_var(ENV_CONFIG_DIR, td.join("EZERagentclaude"));
    ...
    fn install_ownership_system_forced_user_preserved() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("EZERagent-pack-ownership-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::env::set_var(ENV_PACK_DIR, &td);
        std::env::set_var(ENV_CONFIG_DIR, td.join("EZERagentclaude")); // 격리(밀폐)
    ...
    fn install_force_preserves_user_schedule_jobs() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let td = std::env::temp_dir().join(format!("EZERagent-sched-owner-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::env::set_var(ENV_PACK_DIR, &td);
        std::env::set_var(ENV_CONFIG_DIR, td.join("EZERagentclaude"));
        std::fs::create_dir_all(&td).unwrap();

        // 사용자가 `EZERagent schedule add` 로 넣은 잡이 담긴 schedule.json(임베드와 상이).
    ...
    fn is_user_owned_classification() {
        ...
        for s in ["bin/EZERagent_phoenix.py", "hooks/session_start.sh", "README.md",
    ...
    fn is_seed_once_classification_and_behavior() {
        ...
        for s in ["round/TOOL_RESULT_VOCAB.md", "round/capability_catalog.json",
                  "round/video-archetypes/cinematic/workflow.json", "bin/EZERagent_memory.py"] {
    ...
    fn ownership_single_sot_priority_and_exclusivity() {
        ...
        assert_eq!(ownership("bin/EZERagent_phoenix.py"), Ownership::System);
        ...
        for rel in ["memory/CLAUDE.md", "memory/MEMORY.md", "soul.md", "CLAUDE.md",
                    ...
                    "schedule.json", "bin/EZERagent_phoenix.py"] {
    ...
    fn pack_dir_env_precedence_and_legacy_fallbacks() {
        // ★불변식 박제: pack_dir의 3단 폴백 우선순위.
        //   1) EZERAGENT_PACK_DIR (env_compat primary)
        //   2) AITERM_PACK_DIR   (env_compat이 primary 접두를 치환해 만드는 레거시 키)
        //   3) ~/.EZERagent/pack (기본)
        // 순서가 뒤집히면 명시 설정을 조용히 못 찾는다.
        // ★브랜드 잔재 청소(2026-08-11): 업스트림-시대 전용 폴백 단은 제거(pack_dir 주석 참조).
        let _g = PACK_ENV_LOCK.lock().unwrap();
        let keys = ["EZERAGENT_PACK_DIR", "AITERM_PACK_DIR"];
        ...
        // 셋 다 없으면 기본 ~/.EZERagent/pack (홈 끝 2요소가 .EZERagent/pack)
        let def = pack_dir();
        assert!(
            def.ends_with(".EZERagent/pack"),
            "기본 경로는 .EZERagent/pack: {def:?}"
        );

        // AITERM_PACK_DIR만 → env_compat 레거시 단으로 도달 (2순위)
    ...
    fn install_from_iter_equivalent_to_install() {
        ...
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let base =
            std::env::temp_dir().join(format!("EZERagent-pack-equiv-test-{}", std::process::id()));
    ...
    fn write_atomic_roundtrip_and_replace() {
        let td =
            std::env::temp_dir().join(format!("EZERagent-write-atomic-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&td);
        std::fs::create_dir_all(&td).unwrap();
    ...
    fn txn_prestate(tag: &str, files: &[(&str, &str)], version: &str) -> (PathBuf, PathBuf) {
        let base = std::env::temp_dir().join(format!("EZERagent-journal-{tag}-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
    ...
    /// ★대조(외부 동작 불변): embed/EZERagentd/init-pack 경로(transactional=false)는 .install-manifest.json이
    ...
    fn pack_state_read_three_way() {
        let base = std::env::temp_dir().join(format!("EZERagent-state3-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn embed_version_write_failure_creates_no_state_mismatch() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let base = std::env::temp_dir().join(format!("EZERagent-vfault-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        let pd = base.join("pack");
        std::fs::create_dir_all(&pd).unwrap();
        std::env::set_var(ENV_PACK_DIR, &pd);
    ...
    fn write_pack_state_failure_is_reported() {
        let base = std::env::temp_dir().join(format!("EZERagent-sfault-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn atomic_swap_success_creates_prev() {
        let base = std::env::temp_dir().join(format!("EZERagent-swap-ok-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn atomic_swap_reverses_on_failure_keeps_old_pack() {
        let base = std::env::temp_dir().join(format!("EZERagent-swap-rev-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        let dir = base.join("pack");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("a.txt"), "old").unwrap();
    ...
    fn verify_staging_detects_missing_file() {
        let base = std::env::temp_dir().join(format!("EZERagent-verify-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();
    ...
    fn install_staged_fresh_then_idempotent_with_prev() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let base = std::env::temp_dir().join(format!("EZERagent-staged-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        let pd = base.join("pack");
        std::env::set_var(ENV_PACK_DIR, &pd);
    ...
    fn install_staged_preserves_user_edit() {
        ...
        let saved = std::env::var(ENV_PACK_DIR).ok();
        let saved_cfg = std::env::var(ENV_CONFIG_DIR).ok();
        let base = std::env::temp_dir().join(format!("EZERagent-staged-pres-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        let pd = base.join("pack");
        std::env::set_var(ENV_PACK_DIR, &pd);
        std::env::set_var(ENV_CONFIG_DIR, base.join("claude"));
