//! EZERagent (EZERagent Terminal) — shared protocol types, socket path resolution, and key mapping.
...
pub const ENV_SOCKET: &str = "EZERAGENT_SOCKET";
pub const ENV_SURFACE_ID: &str = "EZERAGENT_SURFACE_ID";
...
/// Default socket path: ~/.local/state/EZERagent/EZERagent.sock (unix),
/// \\.\pipe\EZERagent (windows). Overridable via EZERAGENT_SOCKET (legacy EZERAGENT_/AITERM_ honored).
pub fn socket_path() -> PathBuf {
    ...
    #[cfg(not(windows))]
    {
        ...
        let dir = if base.ends_with(".local/state") || base.to_string_lossy().contains("state") {
            ...
            base.join(".local/state/EZERagent")
        };
        dir.join("EZERagent.sock")
    }
}

/// Windows named pipe busy-retry 정책 — CLI(EZERagent)·GUI(EZERagent-app) 클라이언트 공용 **단일 진실**
/// (이원 정의는 정책 변경 시 샷건 서저리). ERROR_PIPE_BUSY(os error 231, "모든 파이프
/// 인스턴스가 사용 중")는 데몬 다운이 아니라 listening 인스턴스 순간 소진(정상 혼잡)이다 —
/// 서버(EZERagentd 리스너 풀)는 accept 직후 인스턴스를 재생성하므로 잠깐 기다리면 열린다.
...
/// 동봉 runtime PATH 선두 주입(RC-5 · 공용 — EZERagentd PTY 자식·GUI 직스폰이 공유, 중복 구현 금지).
...
/// `~/.EZERagent/...` 경로를 CWD 상대경로로 붕괴시킨다(부서목록·프로파일·pending-restore 오지정). dirs::home_dir()
/// (Windows=USERPROFILE/HOMEDRIVE 기반·unix=$HOME)로 OS중립 해소. 코어(EZERagent)·GUI(src-tauri) 공유.
pub fn home_dir() -> PathBuf {
    dirs::home_dir().unwrap_or_else(|| PathBuf::from("."))
}

/// (W1) claude CLAUDE_CONFIG_DIR 결정론 해소 — agents.json의 `${EZERAGENT_ACCOUNT_DIR:-$HOME/.EZERagent/claude}`와
/// 동일 규칙을 **현재 프로세스 env**로 전개한다. pane 셸(=데몬 자식)이 실제로 해소하는 값과 일치하려면
/// 실제 전개 주체인 **데몬 프로세스에서 호출**하는 것이 권위다(state.rs의 EZERAGENT_ACCOUNT_DIR 전파와 정합).
/// discover 스캔(usage.rs)이 ~/.EZERagent/claude를 원리적으로 못 보므로, config_dir 권위는 이 결정론 해소뿐이다.
pub fn resolve_claude_config_dir() -> String {
    std::env::var("EZERAGENT_ACCOUNT_DIR")
        .ok()
        .filter(|v| !v.is_empty())
        .unwrap_or_else(|| home_dir().join(".EZERagent").join("claude").to_string_lossy().into_owned())
}

/// Claude Code projects/ 디렉터리명 munge — 실측: '/'와 특수문자가 '-'로 치환된다.
/// ASCII 영숫자·'-'만 보존하는 보수 구현. resume 사전검증 게이트(EZERagent.rs)와 usage 휴리스틱이 공유한다.
...
/// 부서 데몬 소켓/파이프 경로(RC-4 · 공용 — GUI(src-tauri)·EZERagent fleet가 공유, 규약 단일화).
/// Windows: named pipe `\\.\pipe\EZERagent-dept-<name>`(기본 데몬 `\\.\pipe\EZERagent`와 대칭 · RC-13 state_dir
/// 슬러그 `EZERagent-dept-<name>`과 정합). unix: `~/.local/state/EZERagent-dept-<name>/EZERagent.sock`(EZERagent-dept 규약).
/// HOME 미설정 함정(RC-7) 회피 — unix도 dirs::home_dir() 사용.
pub fn dept_socket_path(name: &str) -> PathBuf {
    #[cfg(windows)]
    {
        PathBuf::from(format!(r"\\.\pipe\EZERagent-dept-{name}"))
    }
    #[cfg(not(windows))]
    {
        dirs::home_dir()
            ...
            .join(".local/state")
            .join(format!("EZERagent-dept-{name}"))
            .join("EZERagent.sock")
    }
}

/// 이 소켓/파이프 경로가 부서(dept) 데몬의 것인가 — 부서 규약 `EZERagent-dept-<name>`(dept_socket_path와 정합).
/// 채널은 메인 EZERagentd 단독 소유(DESIGN §2.5)이므로 부서 데몬의 브리지 스폰을 구조적으로 거부하는 데 쓴다.
/// 판별: 경로 컴포넌트(unix 부모 디렉토리 `EZERagent-dept-<name>` / windows 파이프명 `EZERagent-dept-<name>`) 중
/// `EZERagent-dept-` 접두 이름이 있으면 부서. 메인 데몬(슬러그 `EZERagent`·`EZERagent.sock`)은 오판하지 않는다
/// (`EZERagent`는 `EZERagent-dept-` 접두가 아님 — 오탐 시 채널 전면 불능이라 접두를 정확히 요구).
pub fn is_dept_socket(socket_path: &std::path::Path) -> bool {
    socket_path
        .to_string_lossy()
        .split(|c| c == '/' || c == '\\')
        .any(|comp| comp.starts_with("EZERagent-dept-"))
}
...
mod tests {
    ...
    fn dept_socket_path_os_convention() {
        ...
        #[cfg(windows)]
        assert_eq!(s, r"\\.\pipe\EZERagent-dept-dept-3", "windows named pipe");
        #[cfg(not(windows))]
        {
            assert!(s.ends_with(".local/state/EZERagent-dept-dept-3/EZERagent.sock"), "unix .sock: {s}");
    ...
    fn is_dept_socket_detects_dept_not_main() {
        ...
        assert!(is_dept_socket(Path::new("/x/.local/state/EZERagent-dept-future/EZERagent.sock")));
        assert!(is_dept_socket(Path::new(r"\\.\pipe\EZERagent-dept-3")), "windows 파이프");
        // 메인 데몬 — 오판 금지.
        assert!(!is_dept_socket(Path::new("/x/.local/state/EZERagent/EZERagent.sock")), "메인 unix");
        assert!(!is_dept_socket(Path::new(r"\\.\pipe\EZERagent")), "메인 windows");
        assert!(!is_dept_socket(Path::new("/tmp/EZERagent_chan_test_1_tag/EZERagentd.sock")), "테스트 임시");
    }
    ...
    fn runtime_prefixed_path_prepends_exe_dir_and_dedups() {
        ...
        let sep = if cfg!(windows) { ';' } else { ':' };
        let exe = Path::new("/opt/EZERagentapp/bin");
        let cur = format!("/usr/bin{sep}/bin");
        let got = runtime_prefixed_path(exe, &cur).expect("exe_dir 미포함이면 Some");
        assert!(got.starts_with("/opt/EZERagentapp/bin"), "exe_dir 선두 주입: {got}");
        #[cfg(windows)]
        {
            ...
            let already = format!("/opt/EZERagentapp/bin{sep}/usr/bin");
        ...
        #[cfg(not(windows))]
        {
            ...
            let already = format!("/opt/EZERagentapp/bin{sep}/usr/bin{sep}{local}");
            assert_eq!(runtime_prefixed_path(exe, &already), None, "중복이면 무변경");
        }
    }
    ...
    fn runtime_bin_dirs_macos_resolves_bundle_resources_layout() {
        ...
        use std::fs;
        let base = std::env::temp_dir().join(format!("EZERagentrt-t6b-{}", std::process::id()));
    ...
    fn expand_windows_env_cases() {
        ...
        let lk = |k: &str| match k {
            "USERPROFILE" => Some(r"C:\Users\EZERagent".to_string()),
            "APPDATA" => Some(r"C:\Users\EZERagent\AppData\Roaming".to_string()),
            _ => None,
        };
        // 전개.
        assert_eq!(
            expand_windows_env(r"%USERPROFILE%\.local\bin", lk),
            r"C:\Users\EZERagent\.local\bin"
        );
        // 미지 변수는 원문 유지.
        ...
        assert_eq!(
            ...
            r"C:\Users\EZERagentC:\Users\EZERagent\AppData\Roaming"
        );
    ...
    fn windows_user_bin_dirs_composition() {
        let home = Path::new(r"C:\Users\EZERagent");
        let appdata = PathBuf::from(r"C:\Users\EZERagent\AppData\Roaming");
    ...
    fn compose_pane_path_rules() {
        ...
        let user_bins = vec![r"C:\Users\EZERagent\.local\bin".to_string(), r"C:\Users\EZERagent\AppData\Roaming\npm".to_string()];
        // ① 낡은 프로세스 PATH(.local\bin 없음) + 신선 레지스트리(.local\bin 있음) →
        //    새 순서: prefixes ; process ; fresh 신규분 ; user_bins 신규분. .local\bin 정확히 1회.
        let fresh = r"C:\Windows\System32;C:\Users\EZERagent\.local\bin";
        ...
        let local_count = parts.iter().filter(|&&p| p == r"C:\Users\EZERagent\.local\bin").count();
        ...
        let local_idx = parts.iter().position(|&p| p == r"C:\Users\EZERagent\.local\bin").unwrap();
        ...
        assert!(parts2.contains(&r"C:\Users\EZERagent\.local\bin"));
    ...
    fn resolve_claude_config_dir_is_deterministic_env_not_scan() {
        ...
        // 참조하지 않는다. EZERAGENT_ACCOUNT_DIR 설정 시 그 값 그대로, 미설정 시 $HOME/.EZERagent/claude.
        let prev = std::env::var("EZERAGENT_ACCOUNT_DIR").ok();
        // (a) 명시 계정 dir → 그 절대경로 그대로 (foreign ~/.claude-* 존재 여부와 무관 = 스캔 안 함)
        std::env::set_var("EZERAGENT_ACCOUNT_DIR", "/tmp/zz-acct/.EZERagent/claude");
        assert_eq!(resolve_claude_config_dir(), "/tmp/zz-acct/.EZERagent/claude");
        // (b) 빈 문자열 = 미설정 취급 → 기본 $HOME/.EZERagent/claude
        std::env::set_var("EZERAGENT_ACCOUNT_DIR", "");
        let def = resolve_claude_config_dir();
        assert!(def.ends_with("/.EZERagent/claude"), "기본 해소: {def}");
        assert!(
            def.starts_with(&home_dir().to_string_lossy().into_owned()),
            "HOME 기반: {def}"
        );
        // (c) 미설정도 동일 기본
        std::env::remove_var("EZERAGENT_ACCOUNT_DIR");
        assert_eq!(resolve_claude_config_dir(), def);
        // 원복
        match prev {
            Some(v) => std::env::set_var("EZERAGENT_ACCOUNT_DIR", v),
            None => std::env::remove_var("EZERAGENT_ACCOUNT_DIR"),
        }
    }
}
