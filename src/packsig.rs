...
// build.rs 자동 생성 키링(tauri.conf.json pubkey + EZERagent-pack/trusted-keys.json 병합).
...
/// 마지막으로 수용한 팩 기록(~/.EZERagent/.pack-accepted.json) — replay 단조 게이트의 기준선.
...
/// embed 키링(TRUSTED_KEYS_JSON) 파싱 — 프로덕션 검증 경로(verify_manifest)와 P4 `EZERagent pack-update`가
...
mod tests {
    ...
    fn embedded_keyring_parses_with_bootstrap_pubkey() {
        ...
        let boot = kr
            .keys
            .iter()
            .find(|k| k.key_id == "1703EEA014899763")
    ...
    fn tmp_accepted(tag: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "EZERagent-packsig-{tag}-{}-accepted.json",
            std::process::id()
        ))
    }
    ...
    fn verify_files_match_and_mismatch() {
        use sha2::{Digest, Sha256};
        let root = std::env::temp_dir().join(format!("EZERagent-packsig-files-{}", std::process::id()));
    ...
    fn verify_no_extra_files_rejects_unlisted() {
        let root =
            std::env::temp_dir().join(format!("EZERagent-packsig-extra-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn verify_no_extra_files_rejects_symlink() {
        ...
        let root = std::env::temp_dir().join(format!("EZERagent-vnef-sym-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
