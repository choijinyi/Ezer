...
# 예:    sh scripts/make-update-manifest.sh 0.2.0 EZERagentfuturist EZERagent
set -e
cd "$(dirname "$0")/.."
...
REPO="${3:-EZERagent}"
NOTES="${UPDATE_NOTES:-EZERagent $VERSION}"

BUNDLE="target/release/bundle/macos"
SIG_FILE="$BUNDLE/EZERagent.app.tar.gz.sig"
TARBALL="$BUNDLE/EZERagent.app.tar.gz"

if [ ! -f "$SIG_FILE" ]; then
  echo "error: $SIG_FILE 없음 — 먼저 서명 키로 tauri build를 실행하라:" >&2
  echo "  TAURI_SIGNING_PRIVATE_KEY=\$(cat ~/.tauri/EZERagent-updater.key) bun x @tauri-apps/cli build" >&2
  exit 1
fi

SIGNATURE="$(cat "$SIG_FILE")"
# 업로드 자산 이름(릴리스에 올릴 표준 이름) — latest.json의 url과 일치해야 한다
ASSET="EZERagent-${VERSION}-macos-aarch64.app.tar.gz"
...
echo "    dist-update/latest.json \\"
echo "    dist-update/${ASSET} \\"
echo "    dist-mac/EZERagent-${VERSION}-macos-arm64.dmg"
echo ""
