#!/usr/bin/env bash
# sync-pack.sh — 배포본(~/.EZERagent/pack) → 정본(EZERagent-pack) 정제 동기화 (오너 2026-06-14).
...
SRC="${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}"
DST="$ROOT/EZERagent-pack"
APPLY="${1:-}"
...
# 텍스트 파일만, 동기화된 EZERagent-pack 안에서만.
