...
#
# 판정 규칙:
#   PACK-ONLY  — 마지막 태그 이후 변경이 전부 EZERagent-pack/ 내부 (인테리어만 교체)
...
NONPACK=$(echo "$CHANGED" | grep -v "^EZERagent-pack/" || true)
...
if [ -z "$NONPACK" ]; then
  echo "판정: ★PACK-ONLY 레인 (전 변경이 EZERagent-pack/ 내부 — 인테리어만 교체)"
