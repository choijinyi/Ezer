...
11. [업데이트](#11-업데이트)
12. [EZERagent 팩 운용](#12-EZERagent-팩-운용)
...
| 용어 | 뜻 |
|---|---|
| **EZERagentd** | 헤드리스 코어 데몬. PTY(세션)·소켓 서버·이벤트·관제 데이터의 소유자 |
| **EZERagent** | CLI. pane 안의 AI(그리고 사람)가 쓰는 동등 노드 클라이언트 |
| **EZERagent.app** | Tauri 데스크톱 앱. 터미널 UI + Control Center — 데몬의 thin client |
...
| **팩(pack)** | EZERagent 멀티에이전트 운영체계 — 역할별 절대지침·운영 도구·훅·스킬. `~/.EZERagent/pack`에 설치 |
...
[Releases](https://github.com/choijinyi/Ezer/releases/latest)에서 받습니다.
**데몬을 따로 설치할 필요가 없습니다** — 앱이 자동 기동하고 팩도 자동 설치됩니다.

### 2.1 macOS (Apple Silicon)

1. `EZERagent_<버전>_aarch64.dmg`를 열고 `EZERagent.app`을 Applications로 드래그.
2. 첫 실행에서 Gatekeeper 경고가 뜨면: 공증된 빌드는 그대로 열리고, 아니면 우클릭 → "열기".
3. 앱이 데몬(EZERagentd)을 자동 기동하고 launchd에 등록합니다(재부팅 후에도 유지).

### 2.2 Windows (x64)

1. `EZERagent_<버전>_x64-setup.exe`(NSIS) 실행 — **자기완결 설치**: 데몬·CLI·런타임(Git Bash·
   Python)이 동봉되어 별도 준비물이 없습니다.
2. 앱을 1회 실행하면 온보딩이 자동으로 팩 설치·훅 등록·데몬 자동 기동(작업 스케줄러
   ONLOGON)을 마칩니다.
3. 확인: `dir %USERPROFILE%\.EZERagent\pack` · `schtasks /Query /TN EZERagentd`
4. 상세(비기술자용 안내 포함): [INSTALL-Windows-KR.md](docs/INSTALL-Windows-KR.md)

### 2.3 데몬 상시 가동 (24/365, 선택)

```bash
EZERagent daemon install      # macOS launchd KeepAlive / Windows 작업 스케줄러 등록
EZERagent daemon status
EZERagent daemon uninstall
```

이미 데몬이 돌고 있으면 `install`은 안전하게 거부됩니다.

### 2.4 외부 터미널에서 `EZERagent` 쓰기 (셸 설치)

- **권장(macOS)**: 앱 Control Center 헤더 → **"셸에 EZERagent 설치"** 1클릭(관리자 승인 1회) —
  `/usr/local/bin/EZERagent`·`EZERagentd` 심볼릭 링크가 생기고, 앱 업데이트에도 자동 추종합니다.
- Windows는 설치기가 PATH를 구성합니다.

### 2.5 설치 확인

```bash
EZERagent ping            # 데몬 응답 확인
EZERagent identify        # 데몬·내 주소 확인
EZERagent status          # 전 노드 관제 보드
EZERagent doctor          # 자기진단 (문제 시 --fix)
```

### 2.6 제거

1. `EZERagent daemon uninstall`
2. 앱 삭제(macOS: Applications에서 제거 + 심링크 제거 / Windows: 제어판 제거)
3. 선택 — 데이터까지 완전 삭제(비가역): `~/.EZERagent`(팩·설정)와 `~/.local/state/EZERagent`(소켓·
...
- 데몬 자동 기동(끄려면 `EZERAGENT_NO_AUTOSTART=1`) 및 상시 가동 등록
- 팩 설치(`~/.EZERagent/pack`) — **이미 사용자가 수정한 파일(soul.md·디렉티브·CLAUDE.md·
  schedule.json)은 보존**되고, 수정하지 않은 파일만 갱신됩니다
- Claude Code SessionStart 훅 등록(역할 지침 자동 주입용 — `EZERAGENT_ROLE` 세션에서만 발동)
- pane 프로세스에 `EZERAGENT_SURFACE_ID`·`EZERAGENT_SURFACE_REF`·`EZERAGENT_SOCKET` 자동 주입
...
`＋부서` 버튼으로 부서 워크스페이스를 만들면 **별도 EZERagentd 데몬(별도 소켓)**이 뜹니다 —
프로젝트 간 장애·자원·통신이 격리됩니다. Control Center "작업" 탭과 `EZERagent fleet`은 모든
부서를 집계해 보여줍니다.
...
```bash
EZERagent identify                          # 나는 누구인가 (surface ref)
EZERagent claim-role worker                 # launch-agent 없이 시작한 세션을 역할로 등록
EZERagent surface-role                      # 데몬이 알고 있는 내 역할 1단어 출력
```

### 5.2 노드 기동

```bash
EZERagent launch-agent --role worker --agent claude   # surface 생성 + CLI 기동 + 절대지침 자동 주입 + 역할 등록
EZERagent boot                                        # 표준 노드 세트 일괄 기동(설치된 CLI 자동 감지)
```

`launch-agent`는 ①surface 생성(EZERAGENT_ROLE 주입) ②에이전트 CLI 기동 ③역할 절대지침 stdin
...
```bash
EZERagent send --to worker "상태 보고해줘"    # 대상 PTY stdin에 직접 주입 (타이핑만)
EZERagent send-key --to worker Return        # 전송 확정 (send 후 필수)
EZERagent send --queued --to worker "..."    # followup 큐: 대상이 조용해지면 자동 배달(Return 불필요)
```
...
```bash
EZERagent status --json                     # 전 노드 1콜 스냅샷 (폴링 대체)
EZERagent fleet                             # 모든 부서×노드의 현재 업무
EZERagent events --reconnect                # 이벤트 푸시 구독 (seq 이어받기)
EZERagent read-screen --surface surface:3   # 화면 읽기 (vt100 정확) — 보조 수단
EZERagent watch --surface surface:3 --until "DONE"   # scrollback이 regex에 맞을 때까지 대기
```
...
```bash
EZERagent set-status --state working --context 57 --task "리팩터링 중"
```
...
```bash
EZERagent cycle-agent --role worker          # 저장 지시 → 파일 게이트 → clear → 지침 재주입 → 재개
EZERagent node-recover --role worker         # 죽은 에이전트를 같은 surface에서 재기동+재주입
EZERagent restore [--include-master]         # 토폴로지 스냅샷의 죽은 역할 일괄 복원
EZERagent reinject --role worker [--check]   # 디렉티브 재주입 (--check: 드리프트 감지 후 필요 시에만)
```

에이전트 사망은 즉시 감지되어 `agent.exited/recovered` 이벤트가 흐르고, 옵션으로 자동
재기동(`EZERAGENT_AGENT_AUTORESTART=1`, 3회 상한·인증 오류 시 차단)이 가능합니다.

### 5.7 역할별 TODO 경로

```bash
EZERagent todo-path        # 이 surface 역할 전용 TODO 파일 경로를 결정론으로 산출(없으면 생성)
```
...
```bash
EZERagent feed push --wait --title "git push 승인" --body "..."   # 결정까지 블록. exit 0=allow, 2=deny, 3=timeout
EZERagent feed list --status pending
EZERagent feed reply <request_id> allow                            # CLI로 응답 (UI Allow/Deny 버튼과 동일)
```

- 에이전트 훅 연동 예: PreToolUse 훅에서 `EZERagent feed push --wait ...`를 호출하고 exit code로
  결정을 반영.
...
```bash
EZERagent approval sign   # (master 전용) 위험 명령 prefix를 HMAC 서명 — 이후 guard 훅 자동 통과
EZERagent approval check  # 서명 유효성 확인
```
...
```bash
EZERagent run --scoped -- python -m http.server   # 새 프로세스 그룹+원장 등록. 종료 시 그룹째 강제 정리
EZERagent ps                                      # 프로세스 원장
EZERagent kill <pid>                              # 원장 등록 프로세스(그룹) 종료
EZERagent add-health-rule relogin "Not logged in" # 출력 라인 헬스룰 추가 → health.alert
EZERagent health-rules
```

- **watchdog**(5초 주기): load 폭주·프로세스 수·중복 명령·idle(기본 300초 무출력)·에이전트
  사망·좀비를 감시해 이벤트를 발행합니다. 중복 프로세스 자동 kill은 opt-in
  (`EZERAGENT_AUTOKILL_DUP=1`, 최고(最古) 프로세스 보존).
...
```bash
EZERagent pause        # 큐 배달·스케줄 발화 동결 (직접 send는 통과 — '신경 차단')
EZERagent resume
EZERagent gate-check   # exit 0=running, 4=paused (자율주행이 매 action 전 확인)
EZERagent queue list / clear   # 미배달 큐 검사·철회
```
...
```bash
EZERagent schedule add --id wake --in 20m --text "[wakeup] 다음 액션 착수" --to master   # 원샷(발화 후 자동 삭제)
EZERagent schedule list / remove <id> / run <id>
```
...
- PII 가림: `EZERAGENT_CONTROL_REDACT=1` — 세션 식별자를 가리고 집계는 보존.
...
```bash
EZERagent skill list / show <name> / run <name> / new   # 경험을 스킬로 영속·재사용
```

팩에는 스킬 102종이 실려 있고, 스킬 보안은 정적 스캐너(`EZERagent_skillscan.py`)와 벤더링
해시 매니페스트로 게이트됩니다.
...
- 재설치(rename-swap) 후 "디스크는 새 버전·프로세스는 구 데몬" 스큐가 남으면 상단바에
  **"데몬 vX · 앱 vY — 세션 보존 중"** 배지가 뜹니다. 클릭해 교대하거나, 라이브 세션이
  0이 되면 자동 교대됩니다(무손실).
- 수동 팩 업데이트: `EZERagent pack-update --from <디렉터리>` (pack.tar.gz + pack-manifest.json +
  .minisig). 서명·신선도·replay 검증은 전건 fail-closed입니다.
- **업데이트 전 미리보기**: `EZERagent pack-plan` — 무엇이 갱신/보존/치유/병합대기/정리되는지
  설치 전에 표시합니다(쓰기 0). 팩을 커스터마이즈해 쓰고 있다면 §12.7을 꼭 읽으세요 —
  수정본은 파괴되지 않고 `.new`(신버전 병치)/`.user`(보존본)로 관리되며 `EZERagent pack-merge`로
  병합합니다.
- 진단·수리: `EZERagent doctor [--fix]` — 팩 스큐·stale lock·고아 소켓·훅 등록을 진단하고,
...
```bash
EZERagent init-pack                     # ~/.EZERagent/pack 설치 (사용자 수정 파일 보존)
EZERagent init-pack --install-hook --claude-settings ~/.claude/settings.json   # (선택) Claude Code 훅 강화 주입
```
...
```bash
python3 "${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin/EZERagent_preflight.py" --fix
```
...
```bash
P=${EZERAGENT_PACK_DIR:-$HOME/.EZERagent/pack}/bin
python3 $P/EZERagent_orchestra.py check           # 필수 노드 생존 결정론 확인
python3 $P/EZERagent_orchestra.py task-prompt --task "<T>" --scope "<범위>" --success "<기준>"   # 위임 티켓 생성
python3 $P/EZERagent_orchestra.py gate-status --task "<T>"   # 게이트 수렴 판정 (CONVERGED=다음 단계)
python3 $P/EZERagent_orchestra.py next-action                # 다음 액션 큐 결정론 추출
```

### 12.4 보조 결정론 도구

```bash
python3 $P/EZERagent_route.py --request "<요청>"          # fast/deliberate/slow 3단 사고 라우팅
python3 $P/EZERagent_report.py                            # 진행% 결정론 산출 (todo 체크박스 산술)
python3 $P/EZERagent_memory.py add --type <t> --name <slug> --desc "..." --body "..."   # 장기기억 증류(색인 원자 동기)
python3 $P/EZERagent_task.py checkout <id> --owner <역할>  # 원자적 태스크 체크아웃 (충돌=exit 9)
python3 $P/EZERagent_resource_gate.py check               # 착수 전 자원 게이트 (0 allow / 1 soft / 2 hard)
python3 $P/EZERagent_event.py emit <type> ...             # 닫힌 enum 이벤트 방출 (미지 타입 거부)
python3 $P/EZERagent_wakeup.py enqueue --to <역할> --task <key> --reason "..."   # 코얼레싱 웨이크업 큐
```

### 12.5 RSI 학습·페르소나

```bash
EZERagent learn                 # RSI 학습 루프 — 제안 생성·라운드 상태 (Control Center 학습 탭과 연동)
EZERagent persona list-params   # 노드 페르소나·운영 노브 (안전핵은 잠김)
EZERagent persona show / set / reset
```

### 12.6 pro 채널 (선택)

```bash
EZERagent license install / status    # 서명 라이선스 설치·진단 (검증 전용)
EZERagent pack-repair-channel         # 채널 상태 진단·복구
EZERagent pack-downgrade-to-free      # pro → free 강등의 유일한 경로 (명시적)
```
...
**사용자 전용 오버레이 `~/.EZERagent/local/`** (업데이트·치유·정리가 존재 자체를 모르는 영역):
...
```bash
EZERagent pack-plan                 # 업데이트 전 드라이런 — 갱신/보존/치유/병합대기/정리를 미리 표시
EZERagent pack-merge                # 병합 대기 목록
EZERagent pack-merge --file <경로> --take-new    # vendor 신버전 채택
EZERagent pack-merge --file <경로> --keep-mine   # 내 수정 유지 (이번 신버전 소화)
EZERagent pack-merge --file <경로>               # diff3 3-way 자동 병합 (조상=.pristine)
EZERagent pack-merge --file <경로> --ai          # AI 3-way 병합 — 내 수정 "의도"를 신버전 위에 재적용
EZERagent pack-merge --file skills/<이름>/SKILL.md --to-local   # 스킬 수정본을 오버레이로 승격(권장)
```
...
```bash
EZERagent channel --json <액션>   # start·stop·register·inbound·outbound·receipt·ack·
...
```bash
EZERagent recall "<검색어>"      # 모든 에이전트 터미널 활동의 영속 전사 전문검색(FTS)
EZERagent attest pin            # 전사 해시체인을 외부 보관 (평가자 분리)
EZERagent attest verify         # 사후 변조 대조
EZERagent cost-baseline lock / diff   # 비용·효율 baseline 잠금·전후 비교
```

전사 보존 기간은 `EZERAGENT_RECALL_RETAIN_DAYS`(기본 30일, 0=무제한)로 제어합니다 — 무한 성장
차단.
...
`EZERagent actions`를 실행하면 기계가 읽는 자기기술 카탈로그가 나옵니다(clap 정의가 단일
진실원). 아래는 사람용 요약입니다.
...
### 코어(EZERagentd·EZERagent)가 읽는 변수 — 주요

| 변수 | 기본 | 뜻 |
|---|---|---|
| `EZERAGENT_SOCKET` | `~/.local/state/EZERagent/EZERagent.sock` / win `\\.\pipe\EZERagent` | 소켓 경로 |
| `EZERAGENT_SHELL` | `$SHELL`→zsh | pane 셸 |
| `EZERAGENT_PACK_DIR` | `~/.EZERagent/pack` | 팩 위치 |
| `EZERAGENT_LOAD_THRESHOLD` | 코어수×2 | watchdog load 임계 |
| `EZERAGENT_PROC_THRESHOLD` / `EZERAGENT_DUP_THRESHOLD` | 50 / 3 | 프로세스 수·중복 임계 |
| `EZERAGENT_AUTOKILL_DUP` | 0 | 중복 프로세스 자동 kill (opt-in) |
| `EZERAGENT_IDLE_SECONDS` | 300 | idle 감지 |
| `EZERAGENT_TYPING_GUARD_SECS` | 3 (0=off) | 사람 타이핑 보호 |
| `EZERAGENT_CONTEXT_THRESHOLD_PCT` | 60 | 컨텍스트 통보 임계 |
| `EZERAGENT_MAX_ACTIVE_WORKERS` | 8 | 워커 동시 상한 |
| `EZERAGENT_QUEUE_QUIET_SECS` / `EZERAGENT_QUEUE_DEPTH_ALERT` | 3 / 5 | followup 배달 조건·큐 깊이 경보 |
| `EZERAGENT_FEED_REMIND_SECS` | 300 (0=off) | 승인 적체 재알림 |
| `EZERAGENT_MASTER_DEADMAN_SECS` | 900 (0=off) | 오케스트레이터 무반응 감지 |
| `EZERAGENT_AGENT_AUTORESTART` | 0 | 죽은 에이전트 자동 재기동 (3회 상한) |
| `EZERAGENT_RECALL_RETAIN_DAYS` | 30 (0=무제한) | 전사 보존 |
...
(이 밖에 진단·튜닝용 변수 다수 — `EZERAGENT_DEBUG`, `EZERAGENT_USAGE_POLL_SECS`, `EZERAGENT_REAP_EXITED*`,
`EZERAGENT_CRASH_WINDOW_SECS`, `EZERAGENT_MAX_RESPONSE_BYTES`, `EZERAGENT_ABI_VERIFY` 등. 소스 grep
...
| 변수 | 뜻 |
|---|---|
| `EZERAGENT_URL_ALLOW_HOSTS` | 외부 URL 허용 도메인 확장(또는 `~/.EZERagent/url-allow-hosts` 파일) |
| `EZERAGENT_WORKER_PROFILE_DIR` | 워커 프로필 경로(또는 `~/.EZERagent/worker-profile-dir` 파일) |
...
| `EZERagent ping` 실패 | 앱 실행(데몬 자동 기동) 또는 `EZERagentd` 직접 기동. `EZERagent doctor --fix` |
| 데몬이 두 개 뜬 것 같음 | 실제로는 불가(중복 기동 거부). 업데이트 후 스큐 배지가 떠 있으면 클릭해 교대 |
| 팩 업데이트가 거부됨 | 정상일 수 있음 — 서명·신선도·replay 검증은 fail-closed. `EZERagent pack-update --dry-run`·`EZERagent doctor`로 원인 확인 |
| 노드에 메시지가 안 들어감 | 타이핑 가드(사람 입력 직후 3초)·ACL(`acl.denied` 이벤트)·kill-switch(`EZERagent gate-check`) 순서로 확인 |
...
- `EZERagent run` 중 Ctrl-C로 CLI가 죽으면 그룹 정리가 watchdog 주기(5초)로 넘어감.
