...
> 오너 2026-06-18 지시: "재귀적 자기개선을 EZERagent 기본 기능으로 탑재."
...
- 5단계 학습 루프를 결정론 엔진(`EZERagent_learn.py`)으로 박제
- 사람 명령(`EZERagent learn`) + master 자율추천(3트리거→사람 승인) **이중 트리거**
- Control Center '학습' 탭으로 가시화

**재사용(이미 제품에 있음)**: ③평가=`EZERagent_rsi.py`(producer≠evaluator·retention·rollback) · ④저장=`EZERagent_memory.py`(원자적 add·증류 게이트)
...
| ③ 객관·근거 평가 | 직전(baseline) 대비 **우리 실측 eval에서** 더 나은지 판정 | `verdict` = improved\|regressed\|flat (EZERagent_rsi 주입) |
| ④ 문서·지침 저장 | 통과 시에만 연관 memory/directive에 영속 | `EZERagent_memory add` 결과(원자적) |
...
| 안전장치 | 함정(막는 것) | 배선 |
|---|---|---|
| **평가자 분리** | producer=evaluator → 자기채점 reward-hack | ③ score 산출자 = master locked-eval launcher(워커≠평가자). EZERagent_rsi는 주입만 |
| **출처+실측 게이트** | 검색 환각·"최신=더 나음" 거짓 | ①은 citation 필수. "더 낫다"는 외부 주장 아닌 **§9 benchmark eval 실측 우위**로만 확정 |
| **baseline+rollback** | 발전이 개악(회귀)·skill bloat | ⑤는 retention gate(이겨야 채택)·실패 시 `EZERagent_rsi rollback`(현 HEAD를 rsi-abandoned 브랜치 박제 후 reset) |
...
| **메모리 무결성** | store 누적으로 의미론적 오염=나쁜 방법론 참값화(gemini 3.4) | store는 retention 품질 게이트 통과분만 **confirmed**, 나머지 **provisional**(참조 우선순위 낮춤). 주기 `EZERagent_memory audit`로 오염 후보 격리 |
...
| 트리거 | 신호원 | 발동 |
|---|---|---|
| 막힘·반복실패 | `governance.rs` watchdog / 워커 동일문제 N회 실패 | `EZERagent_learn propose --reason stuck` |
| 작업 종료 게이트 | `EZERagent_orchestra` slow 종료 게이트에 학습점검 1단 추가 | `propose --reason gate` |
| eval ceiling | `EZERagent_rsi progress` verdict=flat N연속 | `propose --reason ceiling` |

`propose`는 후보+근거만 만들어 **pending feed approval item**으로 등록(자율추천만). 사람이 **`EZERagent feed reply <id> allow`**(또는 feed 패널)로 승인할 때만 ①~⑤ 착수. 거부=무실행. manual(`EZERagent learn`)은 사람 직접 명령이라 즉시 착수(게이트 없음). (오너 구상의 "자율 추천→사람 승인"과 정합)

---

## 4. `EZERagent_learn.py` — 7서브명령 계약

`EZERagent-pack/bin/EZERagent_learn.py`. 기존 도구에 위임(중복 구현 금지): ③→EZERagent_rsi, ④→EZERagent_memory.

```
propose  --reason <stuck|gate|ceiling> --topic <S> [--json]
    트리거 신호를 받아 학습 후보·근거를 산출 → pending feed approval item 등록(EZERagent feed reply <id> allow로 승인).
...
evaluate --round <id> --pattern <p.json> --score F   # ③ — EZERagent_rsi에 위임
    score는 §9 locked-eval launcher가 산출(이 명령은 받기만). 내부적으로:
      EZERagent_rsi checkpoint(첫 회 baseline) / progress(비교) 호출.
    출력: verdict. ★score 자체생성 금지(EZERagent_rsi 불변 계승).

store    --round <id> --pattern <p.json> --type <feedback|reference|project>   # ④
    verdict=improved AND 사람승인 플래그일 때만 EZERagent_memory add 위임. 아니면 거부.

harness  --round <id> --pattern <p.json> [--evolve <skill_name>]   # ⑤
    skill/harness 신설 또는 기존 발전. retention: 새 harness가 baseline eval 못 이기면
      EZERagent_rsi rollback(dry-run 먼저). 채택은 사람 승인.

status   [--json]                          # UI 데이터원 — 라운드·verdict·채택/rollback·발견 누적
```

상태 파일: `_round/learn/state.json` + `ledger.jsonl` (EZERagent_rsi 패턴 답습 — os.replace 원자교체).

---

## 5. `rsi-gate.sh` — 빠진 강제자

`EZERagent-pack/bin/rsi-gate.sh`. eval-driven 스킬이 참조하나 미구현이던 Tier-2 강제자.
...
**RPC** (`src/bin/EZERagentd/handlers.rs` dispatch에 분기 추가):
- `learn.propose` {reason, topic} → EZERagent_learn propose 결과
- `learn.status` → EZERagent_learn status (UI 폴링용)
- `learn.history` {round} → ledger 조회

**CLI** (`src/bin/EZERagent.rs` Command enum):
- `EZERagent learn <topic>` — 사람 직접 명령. RPC learn.propose(reason=manual 우회=즉시착수) 또는 워커에 학습 티켓 디스패치.
- `EZERagent learn --status` — 현재 학습 라운드 상태.

자율추천 배선: 3트리거가 pending feed approval item을 등록 → 사람이 `EZERagent feed reply <id> allow`(또는 feed 패널)로 승인(기존 feed 패널 재사용·non-wait).
...
5번째 directive. `EZERagent-pack/directives/` + `pack.rs` PACK 배열 + role 매핑(master·worker 주입).
내용: §1 5단계 조작적 정의 · §2 4안전장치 · §3 자율추천 3트리거 · §4 "자율추천은 추천까지, 저장·채택은 사람 승인" 경계 · §5 도구 사용법(EZERagent_learn 7명령).

---

## 9. ★설계 난제 — producer≠evaluator로 "방법론"을 어떻게 평가하나

EZERagent_rsi는 score를 주입만 받는다(자체생성 금지). 그럼 "더 나은 방법론인가"의 score는 누가/어떻게?
...
3. score 산출자 = master의 **locked-eval launcher**(검색·추출한 워커가 아님 = 평가자 분리). EZERagent_rsi에 주입.
...
| 4 RPC/CLI | learn.* 응답·`EZERagent learn` 동작·3트리거 발동 실측 | 로컬 |
