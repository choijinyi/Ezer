...
> ⚠ DRAFT — 오너 승인 전. 승인 시 `EZERagent-pack/directives/RSI_LEARNING_DIRECTIVE.md`로 이동 + `pack.rs` 배선.
...
- **③ 객관·근거 평가** — **benchmark eval 실측 우위로만** '더 낫다' 판정. **점수 산출자 ≠ 후보 생산자**(EZERagent_rsi 주입).
...
- **baseline·rollback**: 발전이 baseline 못 이기면 `EZERagent_rsi rollback`(현 HEAD를 복구 브랜치 박제 후 reset).
...
- **메모리 무결성**: `provisional`/`confirmed` 구분·주기 `EZERagent_memory audit`로 오염 격리.
...
- **eval ceiling**: `EZERagent_rsi progress` verdict=flat N연속.

추천 → `EZERagent_learn propose` → `feed.push --wait` 승인요청. **승인(0)에서만** ①~⑤ 착수(거부·타임아웃=무실행).
...
`EZERagent_learn {propose|search|extract|evaluate|store|harness|status}` (EZERagent-pack/bin/).
- ③평가 → `EZERagent_rsi` 위임(producer≠evaluator·rollback) · ④저장 → `EZERagent_memory` 위임(원자적·audit)
...
- **의미·논리 검증은 독립 모델 다양성**: 결정론으로 못 잡는 사실·논리·맥락은 **생산자와 다른 모델 패밀리**(EZERagent=agy·codex 등)의 adversarial 검증 + ledger 박제. "자기보고 불신"=생산자 자기채점 불신이지 독립 모델 판단 배제 아님. 단일 모델 신뢰 금지.
...
도구: `EZERagent_learn`이 각 단계에서 5차원 봉쇄 게이트 호출(citation-gate 스킬 확장 + 5차원 결정론 검증 스크립트). 실패 시 단계 차단.
