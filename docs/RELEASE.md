...
출력이 비어 있으면(=`ezer-pack/`·docs만 변경) 팩-온리 대상. 한 줄이라도 있으면 바이너리 릴리스.
...
pack_version은 빌드 시점 `CARGO_PKG_VERSION`에 용접돼 있어(`ezer.rs build_pack_manifest_value`)
팩만 발행해도 **버전 범프 + ezer 재빌드**가 필요하다(§0 전 위치 갱신 — version-check.sh 통과).

1. 버전 범프(§0) → `cargo build --release --bin ezer` (Tauri 빌드 불요 — ezer 단독).
2. pack 3종 생성 — release.yml `pack-artifacts` 잡과 동일 파라미터(스캔 게이트 2종 선행 포함):
   `ezer pack-manifest --key-id … --signed-at … --expires-at … --min-binary-version 0.12.48 > pack-manifest.json`
...
- 팩-온리 적용 후 "디스크 팩 > 바이너리 임베드 팩" 상태가 일상화된다 — 이때 부트 스윕은
  ...
  (2026-07-12 도입 — 종전의 "스윕 실행 → 다운그레이드 가드 no-op" 소음 제거). 수동 `ezer init-pack`은
...
- ~~`dist-win/ezer.wxs` / `dist-win/ezer-x64.wxs`~~ (legacy MSI — NSIS 전환으로 폐기)
...
#       서명 키: ~/.tauri/ezer-updater.key (최초 1회 `bun x @tauri-apps/cli signer generate`로 생성, 분실 시 자동업데이트 영구 불가)
export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/ezer-updater.key)"
export TAURI_SIGNING_PRIVATE_KEY_PASSWORD=""   # 키에 암호를 걸었다면 그 값

bun x @tauri-apps/cli build
#  → target/release/bundle/dmg/ezer_0.2.0_aarch64.dmg
#  → target/release/bundle/macos/ezer.app             (ezerd·ezer externalBin 동봉)
#  → target/release/bundle/macos/ezer.app.tar.gz(.sig) (자동 업데이트용 — 서명 키 있을 때만)

# 배포본으로 정리 (아키텍처 접미사 표준화)
cp target/release/bundle/dmg/ezer_0.2.0_aarch64.dmg dist-mac/ezer-0.2.0-macos-arm64.dmg

# 업데이트 manifest(latest.json) + 자산 생성
sh scripts/make-update-manifest.sh 0.2.0 <OWNER> ezer-agent
#  → dist-update/latest.json, dist-update/ezer-0.2.0-macos-aarch64.app.tar.gz
```

`beforeBuildCommand`(scripts/bundle-prep.sh)가 UI 번들 + ezer/ezerd 릴리스 빌드 + `externalBin` 배치를
...
- 배선: `tauri.conf.json > bundle.macOS.entitlements = entitlements.plist`(hardened runtime +
  사이드카 ezerd·ezer 로드 허용). Tauri가 빌드 중 Developer ID codesign + notarytool 제출 +
  staple 을 자동 수행한다(별도 `codesign`/`notarytool` 수동 호출 불요).
- **검증 통과 기준**: `spctl -a -vv ezer.app` = **accepted**. (rejected면 공증 실패 — 빌드
...
> /Applications/ezer.app` 로만 우회 가능(배포용 아님).

### ★비기술자(청중) 배포 전 게이트 체크리스트 (D6 제품 모드)
오너 대표 산출물을 제3자에게 패키징해 내보내기 전, 아래를 **모두** 확인한다.
- [ ] **공증 빌드**(`spctl -a -vv ezer.app` = accepted) — 미공증은 비기술자 배포 금지(다른 맥에서 "손상됨" 차단).
- [ ] **신뢰선 라벨 활성** — 스킬 보드 산출물에 "🔒 AI 보조 생성 · 오너 검수 전"이 부착되는지(과대약속 "80~90%" 금지).
- [ ] **외부발행은 master 승인 경유** — 제3자 공유/전송은 자율주행 denylist의 "외부발행(비가역)"에 해당. `ezer feed push --wait`(master 승인)를 거친다. 임의 전송 금지(§4 외부발행 원칙 계승).
- [ ] **HITL 미리보기 보존** — 제품 모드도 입력 모달·validate_ir 게이트·미리보기 확인을 우회하지 않는다("1클릭"이라도 게이트 제거는 REJECT).
- [ ] **청중 프로파일 확인** — `~/.ezer/profile.json` audience가 대상 청중과 일치(민감 스킬은 카탈로그 미포함=암묵 차단).
...
cargo build --release --bin ezer --bin ezerd --target x86_64-pc-windows-msvc
cargo build --release --bin ezer --bin ezerd --target aarch64-pc-windows-msvc

# WiX(candle/light)로 MSI 생성 — dist-win/ezer.wxs(arm64)·ezer-x64.wxs(x64) 사용
#   ProgramFiles에 ezer.exe·ezerd.exe 설치 + PATH 등록
candle dist-win\ezer-x64.wxs -o ezer-x64.wixobj
light  ezer-x64.wixobj -o dist-win\ezer-0.2.0-windows-x64.msi
candle dist-win\ezer.wxs    -o ezer.wixobj
light  ezer.wixobj    -o dist-win\ezer-0.2.0-windows-arm64.msi

# ZIP (설치 없이)
Compress-Archive target\x86_64-pc-windows-msvc\release\ezer.exe,ezerd.exe `
  dist-win\ezer-0.2.0-windows-x64.zip
```
...
WiX 대신 **msitools(wixl)**로 MSI를 만든다. (ezer.wxs는 표준 WiX v3라 wixl이 그대로 읽는다.)
...
# 바이너리 크로스컴파일 (GUI 없이 ezer+ezerd만)
cargo zigbuild --release --target x86_64-pc-windows-gnu      --bin ezer --bin ezerd
cargo zigbuild --release --target aarch64-pc-windows-gnullvm --bin ezer --bin ezerd

# MSI (wixl — wxs Source 상대경로가 ../target/... 이므로 dist-win에서 실행)
cd dist-win
wixl -o ezer-0.2.1-windows-x64.msi   ezer-x64.wxs
wixl -o ezer-0.2.1-windows-arm64.msi ezer.wxs
cd ..
# ZIP
zip -j dist-win/ezer-0.2.1-windows-x64.zip   target/x86_64-pc-windows-gnu/release/ezer.exe target/x86_64-pc-windows-gnu/release/ezerd.exe
zip -j dist-win/ezer-0.2.1-windows-arm64.zip target/aarch64-pc-windows-gnullvm/release/ezer.exe target/aarch64-pc-windows-gnullvm/release/ezerd.exe
```

⚠ **한계(정직)**: 크로스빌드 산출물은 PE 포맷·아키텍처는 검증되나(`file`로 PE32+ x86-64 /
Aarch64 확인) **실제 Windows에서 실행 검증은 불가**하다. 광범위 배포 전 Windows 머신에서
스모크테스트(설치→`ezer status`) 권장.
...
```sh
# 1) GitHub에 공개 repo 생성 (이름은 ezer-agent 권장 — endpoint와 일치)
gh repo create <OWNER>/ezer-agent --public --source . --remote origin

# 2) tauri.conf.json의 updater.endpoints에서 OWNER를 실제 GitHub 사용자명으로 치환
#    "https://github.com/<OWNER>/ezer-agent/releases/latest/download/latest.json"
...
```sh
# 태그
git tag -a v0.2.0 -m "ezer 0.2.0 — 에제르 네이티브 기능 19건 + zero-setup 온보딩 + 자동 업데이트"

# gh CLI 릴리스 (드래프트로 먼저 검토 권장)
gh release create v0.2.0 --draft --title "ezer 0.2.0" --notes-file docs/RELEASE_NOTES_0.2.0.md \
  dist-update/latest.json \
  dist-update/ezer-0.2.0-macos-aarch64.app.tar.gz \
  dist-mac/ezer-0.2.0-macos-arm64.dmg \
  dist-win/ezer-0.2.0-windows-x64.msi \
  dist-win/ezer-0.2.0-windows-arm64.msi \
  dist-win/ezer-0.2.0-windows-x64.zip
```
...
- 설치 = 새 `.app` 교체 + 구 데몬 SIGTERM + 앱 재시작(새 ezerd 자동 기동). **재설치 불필요.**
...
`~/.tauri/ezer-updater.key`(private)를 **분실하면 이후 버전에 서명할 수 없어 자동 업데이트가 영구 중단**됩니다.
...
- [ ] 신규 머신 시뮬레이션: 빈 HOME에서 `ezer list` → 데몬 자동기동 + pack 자동설치 확인
- [ ] DMG에서 설치 → 앱 실행 → `ezer status` 동작
- [ ] 버전 문자열 4곳(+wxs 2곳) 일치
