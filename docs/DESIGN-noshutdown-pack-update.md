# DESIGN — EZERagent 무중단(재시작 0) 팩 업데이트 채널

> **목표**: 오너이 새 기능(디렉티브·스킬·스크립트·워크플로우 = "팩")을 개발하면, EZERagent을
> **끄지 않고·재시작 없이·24/365 가동 중에** 그 새 기능을 살아있는 노드에 적용한다.
>
> **R0 = 설계 전용.** 본 문서는 코드 0줄이다. 모든 단정은 코드 실측(file:line)에 근거하며,
> repo = `~/dev/EZERagent`, 브랜치 = `feat/multi-master-formalization`.
>
...
현재 팩은 **EZERagent 바이너리에 `include_str!`로 embed**(`src/pack.rs:10-358`)되어, 새 디렉티브/스킬을
...
아티팩트**(`pack.tar.gz` + `pack-manifest.json`)로도 배포하고, **이미 실행 중인 EZERagent 바이너리**가
그것을 받아 검증→디스크 반영(`install`의 preserve-gate 재사용)→**살아있는 노드 reinject**까지
하는 경로를 추가한다. EZERagentd·EZERagent-app·세션은 단 한 번도 죽지 않는다.
...
| C | EZERagentd 첫 기동 자동 설치 `install(false)` | `src/bin/EZERagentd/main.rs:59` | 데몬 기동 시 1회 |
| D | `EZERagent init-pack` CLI | `src/bin/EZERagent.rs:1372-1373`, `run_init_pack` `src/bin/EZERagent.rs:2270-2321` | 없음 |
| E | Tauri 업데이트 재시작 후 `EZERagent init-pack --no-install-hook` | `src-tauri/src/main.rs:489-494`(`maybe_apply_pending_update`) | **있음** (`app.restart()` `main.rs:1126` 직후) |
| F | 살아있는 노드에 디렉티브 재주입 `EZERagent reinject` | `src/bin/EZERagent.rs:1215-1217`, `run_reinject` `src/bin/EZERagent.rs:3889-3937` | 없음 |
...
3. `reinject`(F)는 **디스크의 디렉티브를 `compose_directive`로 다시 읽어 주입**한다
   (`src/bin/EZERagent.rs:3922`). 즉 B로 디스크만 갱신하면 F로 살아있는 노드를 각성시킬 수 있다 —
...
- **embed는 불가침**: `src/pack.rs:10-358`의 `include_str!`, `build.rs` skills walk(`src/pack.rs:362`),
  `install(false)`의 EZERagentd 첫 기동 자동설치(`src/bin/EZERagentd/main.rs:59`)는 **그대로 둔다**. 이는
  ⓐ신규 머신 첫 설치 ⓑ오프라인 ⓒ다운로드 실패 시의 **바닥(floor)**이다.
- **추가 아티팩트(릴리스에 동봉)**:
  - `pack.tar.gz` — `EZERagent-pack/` 트리(=embed 입력원과 **동일 소스**)의 tarball.
  - `pack-manifest.json` — `{ pack_version, min_binary_version, key_id, signed_at, expires_at,
    files: { "<rel>": "<sha256>" } }`.
    - `pack_version` = 릴리스 태그(§3에서 단일 버전선 유지).
    - `min_binary_version` = 이 팩이 의존하는 **최소 EZERagent/EZERagentd 바이너리 버전**(새 RPC 의존 시 상향).
  ...
  - **로컬 replay 상태 파일**(다운로드 아티팩트 아님 · 클라이언트 측 on-disk): `~/.EZERagent/.pack-accepted.json`
...
- **동일성 보증**: 같은 태그의 embed 팩과 standalone 팩은 **같은 `EZERagent-pack/` 소스에서**
...
[가드]   parse 성공한 remote.min_binary_version ≤ 실행 EZERagent --version 이어야 진행, 초과면 무중단 거부→바이너리(재시작) 경로(§3)
[다운로드] pack.tar.gz + pack-manifest.json + pack-manifest.json.minisig → 임시 스테이징(~/.EZERagent/.pack-staging/)
...
[각성]   디렉티브 해시가 바뀐 노드만, idle 게이트 통과 후 강제 주입 ← §7-② pack-update 전용 게이트(기존 EZERagent reinject --check 단독 사용 금지)
```

EZERagentd·EZERagent-app·세션 프로세스는 **그 무엇도 종료/재시작되지 않는다**. `app.restart()`(`main.rs:1126`)는
**호출되지 않는다**.

- **트리거 주체**: UI "업데이트 버튼"이 `EZERagent` 사이드카(`resolve_sidecar` `src-tauri/src/main.rs:460-466`)의
  신규 서브커맨드(가칭 `EZERagent pack-update`)를 spawn. 오케스트레이션은 **Tauri 계층이 아니라 `EZERagent`(Rust)에**
...
- **UI 갱신 브리지(중요 — EZERagent CLI엔 AppHandle 없음)**: 오케스트레이션이 `EZERagent` 사이드카에 있으므로
  CLI는 Tauri `app.emit`을 직접 못 한다. → **Tauri command가 사이드카 실행을 래핑**한다:
  `install_pack_update` Tauri command(가칭)가 `EZERagent pack-update`를 spawn→종료코드/새 pack_version을
...
| 릴리스 종류 | 판정 | 경로 |
|---|---|---|
| **팩만 변경** (디렉티브·스킬·스크립트, 새 RPC 불요) | remote.pack_version > 디스크 .pack-version **그리고** min_binary_version ≤ 실행 EZERagent | **무중단 채널**(§2-②) |
| **바이너리 변경** (EZERagentd/EZERagent/EZERagent-app, 새 RPC 포함) | Tauri updater가 latest.json로 신버전 감지(기존 `check_update` `main.rs:1050`) | **기존 Tauri 경로**(재시작, 불가침) |
...
4. **기존 불변식이 무중단 팩을 보호**: 무중단으로 새 팩(vX > 실행 바이너리 버전)을 깐 뒤
   구 바이너리 EZERagentd가 재기동되면, `install(false)`의 **다운그레이드 차단**(`src/pack.rs:480-492`,
...
- **무엇을 reinject하나(범위 최소화)**:
  - **디렉티브/soul/메모리 색인 변경** → reinject 필요(노드 컨텍스트에 박혀 있어 디스크만 바꿔선
    안 깨어남). `compose_directive`(`src/bin/EZERagent.rs:2533`,3922)가 디스크에서 재조립.
...
- **안전 경계(언제)**: 생성/툴콜 도중 주입 금지. 기존 `reinject --check`의 ACK-핑은
  **단독으로는 위험**하다 — Busy(추론 중) 노드가 핑에 응답 못 하면 timeout을 드리프트로 **오판**해
  실행 중 컨텍스트에 전문 디렉티브를 강제 주입, 대화 이력을 파괴한다(`src/bin/EZERagent.rs:3920`,
  agy major). → **Busy 가드 + 디렉티브 해시 선검사를 ACK 핑 앞에 둔다**(상세 §7-②).
- **중복주입 회피(dedup)**: `boot_agent_on_surface`의 resume 분기 패턴 차용
  (`src/bin/EZERagent.rs:2665-2675` — resume 노드엔 전문 디렉티브 **재주입 안 함**, 짧은 복귀 가드만).
...
- **컨텍스트 오염 완화**: 전문 디렉티브 재주입은 토큰 2배·컨텍스트 임계(clear) 유발 위험이
  명시돼 있다(`src/bin/EZERagent.rs:2666`). 완화: 디렉티브가 **실제로 바뀐 경우에만** 전문 재주입,
  그 외엔 짧은 델타 통지("팩 vX 반영됨 — `~/.EZERagent/pack/directives/<역할>` 재숙지")만. 스킬/스크립트
  변경은 통지조차 불요(위 범위 규칙).

### ⑤ 무중단 검증 기준 (EZERagent-app·EZERagentd·세션 전부 생존 실측)

결정론 검증 스크립트가 **반영 전/후 스냅샷 동등성**을 비교(브리프의 "어떻게 실측하나" 답):

| 생존 대상 | 측정 | 합격 |
|---|---|---|
| **EZERagentd** | `EZERagent identify` → `daemon_pid`(`src/bin/EZERagent.rs` identify; 실측 예 `daemon_pid:92226`) | 전/후 **동일 pid** |
| **EZERagent-app** | Tauri 앱 프로세스 pid | 전/후 **동일 pid** (`app.restart()` 미호출) |
...
| **노드 각성** | §7-② pack-update 전용 게이트: idle 확인(`control.dashboard` state) **후 강제 주입** → 주입 **이후** 새 디렉티브에만 있는 사실 인용 확인 (★`EZERagent reinject --check`는 ACK 수신 시 주입을 **생략**하므로 `src/bin/EZERagent.rs:3916` 각성검증으로 부적합) | idle에서 주입됨 + 인용 성공 |
...
신규 코드는 전부 **가산적(additive)**: `install_from_iter`(입력만 (rel,content) 이터레이터), `EZERagent pack-update`
...
| 리스크 | 완화책 | 근거 |
|---|---|---|
| **embed 첫설치 보존** | embed·`install`·EZERagentd 자동설치 불가침. 무중단 채널은 가산·온라인 옵트인. 다운로드 실패 시 embed가 바닥. | `src/pack.rs:10-358`, `src/bin/EZERagentd/main.rs:59` |
...
| **reinject 컨텍스트 오염** | 디렉티브 변경 시에만 전문 재주입·그 외 델타통지/무통지. ACK 게이트. surface별 버전 마커로 반복 차단. | `src/bin/EZERagent.rs:2666,2665-2675,3902-3920` |
...
2. **무중단 실측 1회**: 살아있는 노드 1개 상태에서 `EZERagent pack-update` →
   §2-⑤ 표의 daemon_pid·app pid·surface 집합 **전부 불변** + `.pack-version` 범프 +
   노드 각성은 §7-② 게이트(idle 확인 후 강제 주입 → **주입 이후** 새 디렉티브 사실 인용)로 확인
   (★`reinject --check` 단독 사용 금지 — ACK 수신 시 주입 생략 `EZERagent.rs:3916`).
3. **min_binary_version·서명 만료 폴백**: `min_binary_version` > 실행 EZERagent면 무중단 **거부**→바이너리
   경로 안내 emit 확인. ★만료된 서명(`expires_at` 경과)·만료 key_id 팩 주입 시 **검증 거부**(§7-⑩) 확인.
4. **다운그레이드 견딤**: 무중단 vX 반영 후 구 바이너리 EZERagentd 재기동 → `install(false)`가
...
6. **마커 영속**: pack-update 후 EZERagentd 재기동 → surface별 `pack_reinject` 마커가 topology에서 복원되어
...
1. **버전선 정책 → 단일 버전선**(태그 범프). min_binary: **embed≥standalone**(동일 태그 임베드 팩이 standalone과 동일). `EZERagent pack-manifest`/version_gates(`src/bin/EZERagent.rs`)·release.yml 동일성 게이트로 보장.
...
> pack-update 전용 게이트로 교체(`reinject --check`는 ACK 수신 시 주입 생략 `EZERagent.rs:3916`이라 부적합)
...
> ⑧multi-file 집합 원자성(per-file atomic만으론 reader가 신/구 혼재 — apply-lock+epoch+apply후emit, `EZERagent.rs:2533-2602`·`main.rs:521`)
...
> 백업이라 codex BLOCK 백업바이트 요구가 더 견고히 해소. live `round/`(`EZERagent.rs:3628-3643`) carve-out +
...
> ⑧reinject 마커를 **메모리 아닌 on-disk topology 영속**으로 명문화(EZERagentd 기동 시 로드 복원·마커 유실로 인한
...
**결함**(agy blocking, `Cargo.toml:23`): EZERagent `Cargo.toml` 의존성은 `sha2`까지뿐, **minisign 검증
...
우회해 `EZERagent` CLI가 직접 검증해야 하므로, **현 상태로는 CLI 서명검증이 물리적으로 불가능**.
...
- **공개키 단일 SOT**: 키를 두 곳에 두면 드리프트(검증 실패·우회)의 원천. `src-tauri/tauri.conf.json:44`의
  `pubkey`를 **빌드타임에 EZERagent 바이너리로 embed**한다 — `build.rs`가 `tauri.conf.json`을 읽어
...
**결함**(agy major, `src/bin/EZERagent.rs:3920`): ACK-핑은 Busy(추론 중) 노드를 idle과 구분 못 해
timeout→드리프트 오판→전문 주입으로 **대화 컨텍스트 파괴**. 또 매 업데이트마다 핑 텍스트가
**토큰 영구 누적**.

**실측 — 이미 있는 부품**:
- 노드 상태 판정 `derive_node_state`(`src/bin/EZERagentd/handlers.rs:436-463`): 스크롤백 키워드
...
- 출력 정지 시간 `last_output.elapsed()`(`src/bin/EZERagentd/governance.rs:496,971`).
- 노드 자기보고 상태 `status.set`(`handlers.rs:1879-1973`, working/done/waiting/blocked) →
  `control.dashboard`가 병합 노출(`handlers.rs:2221`).
- 합성 디렉티브는 `compose_directive`(`src/bin/EZERagent.rs:2533`)로 결정론 산출 → 해시 가능.
...
2. **Busy 가드(★진짜 idle — §7-⑨와 통합)**: `last_output` quiet 단독은 **turn-boundary가 아니다**
   ...
   - ★**어댑터별 prompt-ready predicate(codex MEDIUM — 영구 deferral 방지)**: ⓒ는 `agents.json`의
     ...
     `ready_marker`가 없다**(`EZERagent-pack/agents.json:46-52` — cmd·resume_arg만). 마커 미정의 어댑터는
...
| 소비자 | 캐시 성격 | 리로드 트리거 |
|---|---|---|
| **Tauri UI** | 보드 카탈로그·설정 메모리 캐시 | ★EZERagent CLI엔 AppHandle 없음 → **Tauri command가 `EZERagent pack-update` 사이드카를 래핑**해 종료 후 **자신이** `app.emit("pack-updated", {pack_version})`(브리지, §2-② "UI 갱신 브리지"). 프런트가 `read_board_catalog`(`main.rs:521`) 등 재호출. 대안: EZERagentd event bus로 push 후 UI 구독(emit 패턴 `main.rs:498,1095` 동형) |
...
| **MCP 서버**(serena 등 scoped 데몬) | 프로세스 메모리 | idle 시점 graceful: `EZERagent run --scoped`(`src/bin/EZERagent.rs:3942`)로 띄운 것은 생명주기 강제종료가 보장되므로, 팩이 MCP 설정/바이너리를 바꾼 경우 **해당 scoped 프로세스만** idle에서 재기동(전체 무중단 유지). 설정만 바뀌고 서버가 핫리로드 지원하면 시그널, 아니면 idle graceful restart. |

원칙: **무캐시 자산(스크립트·스킬)은 트리거 불요, 캐시 자산만 명시 리로드**. 어느 소비자도
EZERagent-app/EZERagentd 재시작을 요구하지 않는다(§5 무중단 불변).
...
| **호환 게이트**(remote→running) | `remote.min_binary_version` vs 실행 `EZERagent --version`(`CARGO_PKG_VERSION`) | parse 성공 후 `min_binary ≤ running` 이어야 반영 | 파싱 실패·초과 = 무중단 **거부**→바이너리(재시작) 경로 안내(§3) |
...
1. **`install_from_iter(items)`**: `install`의 코어(preserve-gate·prune·버전 판정 `src/pack.rs:496-589`)를
   **`(rel, content)` 이터레이터**를 받는 순수 함수로 추출 — embed `PACK` iter(기존 경로)와 staged-tree
   ...
   얇은 래퍼로 남김(하위호환·§3).
2. **staging build**: 다음 상태 콘텐츠 트리 **전체**를 `~/.EZERagent/.pack-next/`에 조립 — ⓐ보존 대상
...
4. **atomic swap(전체 일괄)**: `pack_dir` 콘텐츠를 새 트리로 **단일 atomic 연산**(버전 디렉토리
   ...
   all-new**(혼재 0·multi-file 부분반영 0). **직전 트리를 `~/.EZERagent/.pack-prev`로 보존 = rollback 백업**
   → codex BLOCK#1 해소(백업 바이트 = prev 트리 통째, sha256만 기록하는 문제 원천 제거).
5. **swap 저널**: 어느 트리가 authoritative인지 1줄(`~/.EZERagent/.pack-swap.json`, 원자 쓰기). crash가
...
6. **rollback / commit**: 검증·swap 실패 = 포인터를 `.pack-prev`로 atomic 되돌림. commit 성공 =
   ...
   시점에 `~/.EZERagent/.pack-prev`·`.pack-next`·잔존 staging의 옛 트리를 **명시적 일괄 퍼지**(디스크 누적
   방지) — 단 직전 1개(롤백용)는 보존.
- **live 상태 carve-out(★필수)**: `round/`(노드 TODO·SESSION_STATE 실시간 기록 — cycle 저장 타깃
  `src/bin/EZERagent.rs:3628-3643`)·사용자 오버라이드는 **스왑 트리에서 제외**(스왑이 라이브 쓰기를 덮지
  않게). → `round/`는 안정 실디렉토리로 두고 **콘텐츠 트리만 버전닝**. 이를 위해 pack_dir을 버전
  심링크로 두는 **1회 마이그레이션** 필요 → §6 오너 결정.
- **시작 시 회수(순서 고정)**: EZERagentd 기동 시 swap 저널 잔존이면 **EZERagentd `install(false)`
  (`src/bin/EZERagentd/main.rs:59`)보다 먼저** 포인터를 확정(half-swap 자가 치유).
...
|---|---|---|---|
| EZERagentd 생존 | `system.identify`(`handlers.rs:485`) | `daemon_pid` | 전/후 동일 |
| EZERagent-app 생존 | OS pid (Tauri 앱) | 프로세스 pid | 전/후 동일 (`app.restart()` 미호출) |
...
| ACK | `surface.read_text`(`handlers.rs:976`)+`surface.wait_for` | `latest_cursor`·`matched` | `DIRECTIVE-ACK-<pid>` 수신(`src/bin/EZERagent.rs:3905-3916`) |
| 팩 반영 | 파일 | `.pack-version`·파일 sha256 | 새 버전·manifest 일치 |

검증 스크립트 = (반영 전 스냅샷) → `EZERagent pack-update` → (반영 후 스냅샷) **동등성 diff**.
...
- **하위호환**: 기존 `init-pack --force`(같은/상위 버전 재설치) 동작은 불변 — 다운그레이드일 때만
  거동이 바뀐다(사일런트 롤백 → 명시 거부). EZERagentd 자동설치(`install(false)`)는 영향 없음.
...
- `compose_directive`(`src/bin/EZERagent.rs:2533-2602`)는 한 번 호출에 **디렉티브+RSI_DIRECTIVE+soul.md+
  MEMORY.md+각 SKILL.md**를 차례로 읽는다(`EZERagent.rs:2540,2548,2554,2561,2572`) → 새 디렉티브 + 옛 soul
...
**폴백(심링크 마이그레이션 거부 시)**: `~/.EZERagent/.pack-apply.lock`(flock) 임계영역 + `.pack-version`
epoch(seqlock: reader가 version→파일→version 재확인) + apply 후 emit. 단 "부분반영 0"이 락 보유 중
reader 차단으로만 보장돼 약하다 — **우선안은 스왑**(§7-⑤).

> ⚠**구현 현황(정직 명시 · R2)**: 현재 코드(`with_apply_lock`, `src/bin/EZERagent.rs`)는 §6-4 심링크
> 마이그레이션(우선안=디렉터리 일괄 atomic 스왑)이 보류돼 이 §7-⑧ 폴백을 ship하되, **flock을
> writer(pack-update)만 취득**한다. load-bearing 리더(`compose_directive` `EZERagent.rs` · `read_board_catalog`
...
`last_output` quiet ≥ 3초**(`src/bin/EZERagentd/governance.rs:1341-1370`, `quiet_for < queue_quiet_secs()`)로만
...
게이트를 통과한 직접 주입으로 한다(큐의 quiet-3초 오배달 회피). 향후 결정론 강화: EZERagentd가 prompt-ready
...
- **(필수2) persisted accepted-pack replay 계약**: 마지막으로 **수용한** 팩의 `{pack_version, signed_at}`을
  디스크(예 `~/.EZERagent/.pack-accepted.json`, 원자 쓰기)에 기록하고, 이후 검증에서 **`signed_at ≤ 수용본.signed_at`
...
**설계 보강 B — 키링 + 회전 + 키 만료**:
- **단일 pubkey → 신뢰 키링**: EZERagent에 embed하는 것을 단일 키가 아니라 **trusted-keys 목록**
...
마커가 **어디에 사는지·재기동을 견디는지** 미설계. 메모리에만 두면 EZERagentd 재기동 시 전 마커 소실 →
...
1. **Surface 신규 필드**: `pack_reinject: Mutex<Option<PackReinject>>`에 `PackReinject{pack_version:String,
   directive_hash:String}`. 위치 = `src/bin/EZERagentd/state.rs:40-105`의 Surface 구조체, `agent_session_id`
   (`state.rs:85-87`) 옆. **초기화**: surface 생성 시 `Mutex::new(None)`(미주입).
2. **단일 write path = `reinject.mark` RPC(EZERagentd 매개)**: `status.set` 확장이 아니라 **전용 RPC로 확정**한다
   (혼선 제거). 호출 주체는 **pack-update/reinject 컨트롤러뿐**이고, **노드 주입이 성공한 직후에만** 호출
   (`reinject.mark {surface_id, pack_version, directive_hash}`) → EZERagentd가 해당 Surface의 `pack_reinject`를
   set. self-declared 신뢰 금지(`state.rs:97`)에 맞춰 노드 자기보고로는 갱신 불가(EZERagentd-인증 발신만).
3. **persist 직렬화**: `persist_topology`(`src/bin/EZERagentd/governance.rs:703` 부근 — 현재 entry는
...
4. **load/restore seeding**: `load_topology`가 topology.json에서 `pack_reinject`를 읽어, **restore/launch가
   ...
   EZERagentd 재기동·노드 복원 후에도 마커 생존.
...
| # | 파일 | 변경(설계) |
|---|---|---|
| ①⑩ | `Cargo.toml`, `build.rs` | `minisign-verify` 의존 + **trusted-keys 키링 embed**(`{key_id, pubkey, not_after(필수)}` + **`revoked_key_ids[]`**) + manifest `signed_at`/`expires_at` **전건 필수 검증** + **`~/.EZERagent/.pack-accepted.json` 단조 수용 게이트** |
| ②③⑨ | `src/bin/EZERagent.rs`(신규 `pack-update`·reinject 3단 게이트·진짜 idle 다중신호·**어댑터별 ready predicate**), `src-tauri/src/main.rs`(신규 `install_pack_update` command — `pack-updated` emit 브리지) | Busy 가드·해시 선검사·UI 리로드·codex deferral 방지 |
...
| ⑪ | `src/bin/EZERagentd/state.rs`(신규 `pack_reinject` 필드+init)·`governance.rs:703`(persist_topology에 `pack_reinject` 키 추가)·`load_topology`+restore seeding·**신규 `reinject.mark` RPC**(주입 성공 후에만 호출) | reinject 마커 단일 write path·재기동 복원 |
...
- EZERagentd 첫 기동 자동설치: `src/bin/EZERagentd/main.rs:59`
- `run_init_pack`: `src/bin/EZERagent.rs:2270-2321`
- `run_reinject`(ACK-핑·재조립): `src/bin/EZERagent.rs:3889-3937` · `compose_directive`: `src/bin/EZERagent.rs:2533`
- `boot_agent_on_surface` resume 중복주입 분기: `src/bin/EZERagent.rs:2665-2675`
...
- `resolve_sidecar`(UI→EZERagent shell-out): `src-tauri/src/main.rs:460-466`
- 릴리스 2-repo·minisign·아티팩트: `.github/workflows/release.yml:4-9,22,56-105`
- **R1 보강 근거**: EZERagent deps(minisign 부재): `Cargo.toml:23-38` · Tauri updater 플러그인: `src-tauri/Cargo.toml:13` · minisign pubkey SOT: `src-tauri/tauri.conf.json:44`
- 노드 상태 판정 `derive_node_state`: `src/bin/EZERagentd/handlers.rs:436-463`(키워드 438) · `control.dashboard`: `handlers.rs:2317-2355`(상태병합 2221)
- idle 출력정지 `last_output.elapsed`: `src/bin/EZERagentd/governance.rs:496,971` · `pane.idle` push: `governance.rs:975`
...
- UI 카탈로그 읽기 `read_board_catalog`: `src-tauri/src/main.rs:521-527` · UI emit 패턴: `main.rs:498,1095` · scoped 생명주기 `run_scoped`: `src/bin/EZERagent.rs:3942`
- **R7 보강 근거(codex VERDICT3)**: `agent_session_id` Surface 필드: `src/bin/EZERagentd/state.rs:85-87` · `persist_topology` 직렬화(session_id만): `src/bin/EZERagentd/governance.rs:703` 부근 · `write_json_atomic`(실측 atomic helper temp+fsync+rename+fsync dir): `governance.rs`(persist_topology 직후 정의)
- **어댑터 `ready_marker`**: claude `❯` `EZERagent-pack/agents.json:7` · gemini `? for shortcuts` `:27` · **codex 부재** `:46-52`(cmd·resume_arg만)
