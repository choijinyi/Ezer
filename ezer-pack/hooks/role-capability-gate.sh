...
#
# ★두 hook 클래스 (ezer-hook.sh:6 불변 narrowing — 위반이 아니라 정밀화):
#   (a) OBSERVABILITY hook (ezer-hook.sh) = **절대 차단 금지·항상 exit 0** — 텔레메트리가
#       에이전트를 깨뜨려선 안 된다(관측은 무해 통과가 불변).
#   (b) GATE hook (appbuild-gate.sh, role-capability-gate.sh) = **설계상 deny 가능**(deny-by-default
#       act tier) — 이 클래스는 차단이 목적이다. ezer-hook.sh:6의 "막지 않는다"는 (a) 관측 전용
...
#   ezerd는 이 내부 도구를 직접 못 막는다(에이전트 프로세스 내부) — ezerd는 ezerd-매개 경로
#   (send/scoped run write-shell)만 게이트한다(handlers.rs check_caps_gate). 이 둘이 합쳐 가드다.
#
# 신원 = ezerd 권위: 자기 surface 역할은 `ezer surface-role`(EZER_SURFACE_ID→데몬 roles 맵)로 읽는다.
#   self-declared가 아니라 claim_role/launch-agent가 신원검증 후 등록한 값. EZER_SURFACE_ID는
...
#   write-shell의 *대표* 위험 동사(파일 리다이렉트·tee·rm·mv·cp·설치자)만 deny. ② ezerd 인증
...
if [ "${1:-}" = "--self-test" ]; then
  ...
  # 자기 surface 역할 조회(ezerd 권위). 실패 시 빈 문자열 → '무역할'로 통과(사람/일반 셸 과차단 방지).
  ...
  CAPGATE_CACHE="${TMPDIR:-/tmp}/ezer-capgate-role-${EZER_SURFACE_ID:-none}"
  if [ -z "${EZER_SURFACE_ROLE:-}" ] && command -v ezer >/dev/null 2>&1; then
    EZER_SURFACE_ROLE="$(ezer surface-role 2>/dev/null | head -n1)"
    if [ -n "${EZER_SURFACE_ID:-}" ]; then
      case "$EZER_SURFACE_ROLE" in
        reviewer*|planner|planner-*) printf '%s\n' "$EZER_SURFACE_ROLE" > "$CAPGATE_CACHE" 2>/dev/null ;;
        "") [ -f "$CAPGATE_CACHE" ] && EZER_SURFACE_ROLE="$(head -n1 "$CAPGATE_CACHE" 2>/dev/null)" ;;
      esac
    fi
  fi
  export EZER_SURFACE_ROLE
fi
...
ALLOW_PATH_SUBSTRS = ("/.ezer/", "/logs/", "/log/", "/_round/")
...
def main():
    ...
    role = os.environ.get("EZER_SURFACE_ROLE", "")
    try:
        data = json.loads(raw)
    except ValueError:
...
def self_test():
    ...
    cases_allow = [
        ...
        ("reviewer-codex", "Edit", {"file_path": "/Users/x/.ezer/scratch.txt"}),
