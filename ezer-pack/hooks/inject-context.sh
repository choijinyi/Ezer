#!/bin/bash
# ezer 영속성 — SessionStart 컨텍스트 주입 hook
...
case "$EZER_PACK_DIR" in */pack-dept-dept-*) DEPT_CTX=1 ;; esac
case "$EZER_SOCKET"   in */ezer-dept-dept-*)  DEPT_CTX=1 ;; esac
if [ -n "$DEPT_CTX" ]; then
  case "$EZER_PACK_DIR" in
    */pack-dept-dept-*) DEPT_ROUND="$EZER_PACK_DIR/round" ;;
    *)                  DEPT_NO_STATE=1 ;;
  esac
...
if [ -z "$STATE" ] && [ -n "$DEPT_NO_STATE" ]; then
  OUT="${OUT}⚠ 부서 pack round SESSION_STATE 부재/이상환경 — 작업기억 주입 생략(메인 레인 미참조·R2/R3). 부서 정본 생성: ezer todo-path.
"
fi
if [ -n "$STATE" ]; then
...
# 출력은 ezer_checklist가 6줄·1KB 이내로 자체 절단, 머리에 P0.2 방어 문구 내장.
if [ "$SOURCE" != "compact" ] && [ -n "$STATE" ]; then
  CHK="${EZER_PACK_DIR:-$HOME/.ezer/pack}/bin/ezer_checklist.py"
  ...
  if [ -f "$CHK" ]; then
    if command -v timeout >/dev/null 2>&1; then
      ...
      CHK_OUT=$(timeout 40 "$EZER_PY" "$CHK" --state "$STATE" --round-dir "$(dirname "$STATE")" 2>/dev/null)
    else
      CHK_OUT=$("$EZER_PY" "$CHK" --state "$STATE" --round-dir "$(dirname "$STATE")" 2>/dev/null)
    fi
