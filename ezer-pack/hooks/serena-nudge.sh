...
#
# ezer 불변(절대 위반 금지):
#   - ALWAYS exit 0 (never exit 2) — ezer hook-never-block 불변.
#   - additionalContext 만 출력 — NEVER updatedInput (RTK ezer-rewrite.sh 가 PreToolUse:Bash
...
#
# 설치(human-hold/denylist): 노드 settings.json PreToolUse 에 RTK ezer-rewrite tuple **뒤** 등록.
#   {"matcher":"","hooks":[{"type":"command","command":"$HOME/.ezer/pack/hooks/serena-nudge.sh"}]}
...
state_dir = os.path.expanduser("~/.ezer/state")
