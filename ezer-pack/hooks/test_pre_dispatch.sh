...
#     (3) ezer-hook 부수효과(사용량 이벤트) 발생 동일 — ★비-편집 도구(Read/Task)에서도 존속
...
REAL_GUARD=${REAL_GUARD:-$HOME/dev/ezer-arp/_round/autopilot/guard.sh}

for f in "$REAL_HOOKS/pre-dispatch.sh" "$REAL_HOOKS/ezer-hook.sh" "$REAL_HOOKS/appbuild-gate.sh" "$REAL_HOOKS/grill-gate.sh"; do
...
export EZER_LOG="$T/ezer.log"
export REAL_HOOKS
export REAL_GUARD

# ---- ezer 스텁: usage-event 만 기록, approval check 는 미서명(exit 1)으로 guard LOOSE deny 유지 ----
mkdir -p "$T/bin"
cat >"$T/bin/ezer" <<'STUB'
#!/bin/sh
case "$1" in
  usage-event-stdin) cat >/dev/null 2>&1; echo usage-event >> "$EZER_LOG"; exit 0 ;;
  approval) exit 1 ;;
  *) exit 0 ;;
esac
STUB
...
for name in ezer-hook appbuild-gate grill-gate; do
...
run_chain() {  # $1=tool  $2=json
  ...
  if match "$_tool" Bash Write Edit MultiEdit NotebookEdit; then
    printf '%s' "$_json" | EZER_GUARD_HOOK="$T/guard-wrap.sh" "$T/guard-wrap.sh" >/dev/null 2>&1
    [ $? -eq 2 ] && CHAIN_BLOCK=1
  fi
  printf '%s' "$_json" | sh "$T/hooks/ezer-hook.sh" >/dev/null 2>&1   # 전 도구
  ...
  CHAIN_INVOKE=$(tr '\n' ',' < "$INVOKE_LOG")
  CHAIN_USAGE=$(wc -l < "$EZER_LOG" | tr -d ' ')
}
...
run_dispatch() {  # $1=json
  _json=$1
  : > "$INVOKE_LOG"; : > "$EZER_LOG"
  DISP_OUT=$(printf '%s' "$_json" | EZER_GUARD_HOOK="$T/guard-wrap.sh" sh "$T/hooks/pre-dispatch.sh" 2>"$T/derr")
  DISP_RC=$?
  DISP_BLOCK=0; [ "$DISP_RC" -eq 2 ] && DISP_BLOCK=1
  DISP_ERR=$(cat "$T/derr")
  DISP_INVOKE=$(tr '\n' ',' < "$INVOKE_LOG")
  DISP_USAGE=$(wc -l < "$EZER_LOG" | tr -d ' ')
}
...
case_run() {  # $1=name $2=tool $3=json $4=exp_invoke $5=exp_block
  ...
  check "ezer-hook 사용량 동치(chain=$CHAIN_USAGE)" "$CHAIN_USAGE" "$DISP_USAGE"
...
case_run "C3 Edit 정상" Edit \
  ...
  "guard,ezer-hook,appbuild-gate,grill-gate," 0
...
case_run "C4 Write appbuild차단(.ts)" Write \
  ...
  "guard,ezer-hook,appbuild-gate,grill-gate," 1
contains "디스패처 stderr=appbuild BLOCKED" "$DISP_ERR" "appbuild-gate BLOCKED"

# C5 Read — ★비-편집 도구에서 ezer-hook 만 실행(matcher 소실 검출)
case_run "C5 Read(비편집)" Read \
  '{"tool_name":"Read","tool_input":{"file_path":"/etc/hosts"}}' \
  "ezer-hook," 0

# C6 Task — 비-편집 도구에서 ezer-hook 만
...
# ================ CASE-G: EZER_GUARD_HOOK 미설정 시 guard skip(exit0 유지) ================
printf '\n[CASE-G] EZER_GUARD_HOOK 미설정 — guard skip·나머지 실행·exit0\n'
: > "$INVOKE_LOG"; : > "$EZER_LOG"
G_OUT=$(printf '%s' '{"tool_name":"Bash","tool_input":{"command":"git push origin main"}}' \
  | env -u EZER_GUARD_HOOK sh "$T/hooks/pre-dispatch.sh" 2>"$T/gerr")
G_RC=$?
G_INVOKE=$(tr '\n' ',' < "$INVOKE_LOG")
check "guard 미설정 시 exit 0(차단 안 함)" "0" "$G_RC"
check "guard 미호출·ezer-hook 만 실행" "ezer-hook," "$G_INVOKE"
contains "stderr 경고 존재" "$(cat "$T/gerr")" "EZER_GUARD_HOOK 미설정"

# ================ 시간 실측 before/after ================
printf '\n================ 시간 실측 (N=8, 실제 pack 서브훅 직접·stub ezer) ================\n'
...
genv={"EZER_GUARD_HOOK":RG}
# 기존 체인 = 각 서브훅을 별도 프로세스로(Claude Code 항목별 실행 근사)
def chain_bash():
    run([RG], bash, genv); run(["sh",f"{RH}/ezer-hook.sh"], bash)
def chain_edit():
    run([RG], edit, genv); run(["sh",f"{RH}/ezer-hook.sh"], edit)
