...
#
# 목적: 현행 settings.json 은 도구 호출마다 PreToolUse 항목을 최대 4개(guard·ezer-hook·
...
#   ③ deny-wins: 서브훅을 등록 순서(guard→ezer-hook→appbuild→grill)로 '전부' 실행하되
#      (차단 발생해도 나머지 실행 — ezer-hook 사용량 기록 무손실), 최종 exit 는 어느 하나라도
#      exit 2 면 2. 차단 서브훅의 stdout(JSON)·stderr 를 최종 출력으로 채택.
#   ④ 경로 주입: guard 경로 하드코딩 금지 — 환경변수 EZER_GUARD_HOOK 로 주입(master 가
#      settings.json 배선). 미설정/실행불가 시 stderr 경고 후 guard 단계 skip(=exit 0 유지,
#      나머지 서브훅은 정상 실행). ★한계: 이 경우 autopilot 안전게이트가 빠지므로 master 는
#      반드시 EZER_GUARD_HOOK 을 배선해야 한다(회귀 테스트 CASE-G 로 명시).
#
...
run_hook() {
  ...
  # exit 0: 정상 통과. ezer-hook 등은 stdout 없음(있어도 allow 경로 stdout 은 채택 안 함).
}

# ================= dispatch (등록 순서 보존: guard → ezer-hook → appbuild → grill) =================

# ① guard (matcher: Bash Write Edit MultiEdit NotebookEdit) — 경로 주입(EZER_GUARD_HOOK)
if in_set "$TOOL" Bash Write Edit MultiEdit NotebookEdit; then
  if [ -n "${EZER_GUARD_HOOK:-}" ] && [ -x "${EZER_GUARD_HOOK}" ]; then
    run_hook guard "$EZER_GUARD_HOOK"
  elif [ -n "${EZER_GUARD_HOOK:-}" ]; then
    printf '[pre-dispatch] EZER_GUARD_HOOK 지정됐으나 실행 불가(%s) — guard 단계 skip(exit 0 유지)\n' "$EZER_GUARD_HOOK" >>"$TMPD/agg_err"
  else
    printf '[pre-dispatch] EZER_GUARD_HOOK 미설정 — autopilot guard 단계 skip(exit 0 유지). master 배선 필요.\n' >>"$TMPD/agg_err"
  fi
fi

# ② ezer-hook (matcher: 전 도구 "") — OBSERVABILITY, 무조건 실행(사용량 기록 무손실)
run_hook ezer-hook sh "$HOOK_DIR/ezer-hook.sh"
