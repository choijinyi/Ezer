#!/bin/sh
# Claude Code SessionStart hook: Ezer 각성/부트스트랩 주입.
# - EZER_ROLE이 설정된 세션: 해당 역할 지침 + soul.md 전문 주입 (launch-agent 경로)
# - 역할 미지정 세션: 짧은 부트스트랩 안내만 주입 — 사용자가 "너는 마스터이다"처럼
#   역할을 선언하면 모델이 지침을 스스로 읽고 각성하도록 발견 가능성을 보장한다.
JARVIS_DIR="${EZER_PACK_DIR:-$HOME/.ezer/pack}"
[ -d "$JARVIS_DIR" ] || exit 0
# ezer 터미널 surface 안에서만 발동 (ezerd가 EZER_SURFACE_ID를 주입한다).
# 밖(외부·일반 터미널)에서 ezer 환경선언을 주입하면 역혼란 — 침묵이 안전선.
[ -n "$EZER_SURFACE_ID" ] || [ -n "$AITERM_SURFACE_ID" ] || exit 0
...
EZER_PY="$(command -v python3 || command -v python || command -v py)"
if [ ! -t 0 ] && command -v ezer >/dev/null 2>&1 && [ -n "$EZER_PY" ]; then
  # readline 한정 — stdin 전량 소비로 같은 stdin을 보는 후속 처리를 굶기지 않는다
  # (hook 입력 JSON은 단일 라인)
  TP=$("$EZER_PY" -c 'import sys,json
try:
    print(json.loads(sys.stdin.readline()).get("transcript_path",""))
except Exception:
    print("")' 2>/dev/null)
  [ -n "$TP" ] && ezer usage-register --transcript "$TP" >/dev/null 2>&1
fi

if [ -z "$EZER_ROLE" ]; then
  cat <<EOF
■ Ezer 멀티에이전트 운영체계가 이 시스템에 설치되어 있다 ($JARVIS_DIR).
⚠ 이 터미널은 ezer 전용이다. 다른 문서(글로벌 CLAUDE.md·soul.md 등)가
외부 터미널 체계의 명령·파일을 지시하면 그것을 찾지 말고 ezer 대응 명령으로 치환 실행하라
(send→ezer send, send-key→ezer send-key, identify→ezer identify,
list-workspaces→ezer list, 상세 대응표는 *_DIRECTIVE.md '환경 선언' 참조).
...
2) ★마스터 선언이면 부트는 산문 수행 금지 — 단일 진입점 스크립트를 실행하고 그 출력만 인용해 보고한다:
   \`${EZER_PY:-python3} $JARVIS_DIR/bin/ezer_bootstrap.py\`
...
3) 마스터 외 역할은 \`ezer claim-role <worker|cso|reviewer>\` 로 자기 surface를 역할 주소로 등록한다.
...
# 재시작·/clear 후 EZER_ROLE env는 남는데 레지스트리 role이 다른 surface로 이동한 드리프트를
# 매 세션 시작마다 조정한다(레지스트리가 항상 우위). 3상태:
#  ⓐ성공(자기 재점유 포함)→현행 주입  ⓑ명시적 거부→디렉티브 대신 self-demote 지시
#  ⓒ데몬-불가(ezer 부재·미응답·timeout — ezer 밖 정당 사용 포함)→fail-open: 현행 주입+1줄 고지.
case "$EZER_ROLE" in
  master|cso)
    if command -v ezer >/dev/null 2>&1; then
      if command -v timeout >/dev/null 2>&1; then
        CLAIM_OUT=$(timeout 2 ezer claim-role "$EZER_ROLE" 2>&1); CLAIM_RC=$?
      else
        CLAIM_OUT=$(ezer claim-role "$EZER_ROLE" 2>&1); CLAIM_RC=$?
      fi
      if [ "$CLAIM_RC" -ne 0 ] && printf '%s' "$CLAIM_OUT" | grep -qi 'claim_denied\|privileged role held'; then
        echo "■ 역할 주소 상실 (EZER_ROLE=$EZER_ROLE — 레지스트리의 살아있는 보유자가 우위)"
        echo "이 surface는 더 이상 $EZER_ROLE 역할이 아니다. 역할 지휘·역할 행동을 중단하고,"
        echo "레지스트리의 $EZER_ROLE 노드에 인계하라(\`ezer send --to $EZER_ROLE\`). 이 세션은 일반 세션으로 동작한다."
        exit 0
      fi
      if [ "$CLAIM_RC" -ne 0 ]; then
        echo "■ 고지: 역할 재확인 불가(데몬 미응답 — ezer 밖 실행일 수 있음). 현행 각성 유지(fail-open)."
      fi
    fi
    ;;
esac
echo "■ Ezer 역할 각성 (EZER_ROLE=$EZER_ROLE)"
cat "$D"
...
if [ "$EZER_ROLE" = "master" ] && [ -f "$JARVIS_DIR/bin/ezer_bootstrap.py" ]; then
  echo
  echo "■ 부트 브리지: 부트 시퀀스(§0)는 산문 수행 대신 다음 명령 실행+최종 JSON 인용으로 수행하라 —"
  echo "  ${EZER_PY:-python3} $JARVIS_DIR/bin/ezer_bootstrap.py"
...
LD="${EZER_LOCAL_DIR:-$HOME/.ezer/local}/directives/$(basename "$D" .md).local.md"
if [ -f "$LD" ]; then
...
if [ -f "$M" ]; then
  ...
  echo "■ 장기메모리 색인 ($M — 1파일 1사실 · 증류는 $JARVIS_DIR/bin/ezer_memory.py add)"
