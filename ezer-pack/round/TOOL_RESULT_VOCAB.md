...
| `retryable` | 일시 실패(5xx·네트워크·타임아웃) — 유한 재시도 허용(ezer_retry 분류와 동일) | 5 |
...
| 도구 | 기존 exit | 어휘 병기 |
|---|---|---|
| ezer_task checkout | 0 / 9(충돌) / 4(blocker) | ok / rejected / dependency_missing |
| ezer_resource_gate check | 0 / 1(soft) / 2(hard) | ok / rejected(soft=경고 후 진행 관례 유지) / rejected |
| ezer_retry classify (A1) | 0 / 1 | retryable / fatal |
| B2 check_manifest check | 0 / 1 | skipped / (생성 필요 — 어휘 없음, 판정 전 상태) |
| ezer feed push --wait | 0 / 2 / 3 | ok / rejected / retryable(timeout) |
