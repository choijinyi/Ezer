...
import ezer_hud_bridge as B  # noqa: E402
import ezer_event as E       # noqa: E402
...
class EventSurfaceValidation(unittest.TestCase):
    """§4d ezer_event --surface 검증 확장 — bare·정식 키 허용, 비정합 거부."""
...
class EventSurfaceAuto(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._saved = {k: os.environ.get(k)
                       for k in ("EZER_SOCKET", "EZER_DEPTS_JSON", "HUD_STATE_DIR")}
        os.environ["EZER_DEPTS_JSON"] = depts
        os.environ.pop("EZER_SOCKET", None)
        self._orig_ref = E._resolve_surface_ref
        E._resolve_surface_ref = lambda: "surface:7"   # ezer identify 스텁
    ...
    def test_emit_auto_unresolved_omits_key(self):
        os.environ["EZER_SOCKET"] = "/tmp/unknown.sock"   # 미매칭 → None
        sd = tempfile.mkdtemp()
        os.environ["HUD_STATE_DIR"] = sd
        out = io.StringIO()
        with contextlib.redirect_stdout(out):
