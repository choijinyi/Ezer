#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_skillscan — 스킬 보안·품질 결정론 게이트 (stdlib 전용·SkillSpector 규칙 포트).
...
langgraph/langchain/openai/yara 비의존 — 다른 ezer_*.py와 동일하게 네트워크/LLM 호출 0·
...
시맨틱(LLM)층은 무-API 제약상 여기서 하지 않는다 — ezer 워커 skillscan-semantic이 담당.

사용:
    ezer_skillscan.py scan <path> [--json]            # 단일 스킬/repo 스캔 → verdict
    ezer_skillscan.py all [--roots D1 D2 ...] [--json] # 다수 스킬 전수 베이스라인
    ezer_skillscan.py --self-test                      # 결정론 자기검증
...
# 규칙 사이드카 로드 (ezer_route.py + route_triggers.json 선례) — 있으면 시드 대체.
...
def parse_frontmatter(text):
    """SKILL.md YAML frontmatter 최소 파서 (stdlib — PyYAML 미사용, ezer_registry 정합).
...
def memory_poison_scan(text, label="memory"):
    """메모리 포이즌 WARN용 재사용 API (ezer_memory.py가 호출) — KO + 메모리관련 영어 규칙.
...
def cmd_card(path, as_json):
    ...
    verdict는 enum(score 아님 — ezer_registry frontmatter no-score 불변 정합, 연구보고서 §4)."""
    if not os.path.exists(path):
        print(json.dumps({"error": "경로 없음: %s" % path}, ensure_ascii=False), file=sys.stderr)
        return 2
    r = scan_skill(path)
...
def main():
    ...
    a.add_argument("--roots", nargs="+", default=[os.path.expanduser("~/.ezer/pack/skills")])
