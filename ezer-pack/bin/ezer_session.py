...
사용:
    ezer_session.py ensure [--file PATH]   # exit 0=ensure 완료(생성·무변형·복구), 2=I/O오류
    ezer_session.py verify [--file PATH] [--json]
                                            # exit 0=두 예약 필드 정상, 1=불변식 위반, 2=I/O
    ezer_session.py --self-test            # exit 0=배터리 ok, 1=fail (JSON 출력)
"""
import argparse
import json
import os
...
RESERVED_SENTINEL = "__EZER__RESERVED__"
FIELDS = ("restore_pointer", "open_gates")

# (open_marker, default_body, close_marker) — penpot hidden-theme-name sentinel 등가.
BLOCKS = {
    "restore_pointer": (
        "<!-- EZER:RESERVED:restore_pointer %s -->" % RESERVED_SENTINEL,
        "- 복원 포인터: (없음)",
        "<!-- /EZER:RESERVED:restore_pointer -->",
    ),
    "open_gates": (
        "<!-- EZER:RESERVED:open_gates %s -->" % RESERVED_SENTINEL,
        "## 미해결 게이트\n- (없음)",
        "<!-- /EZER:RESERVED:open_gates -->",
    ),
}
...
def _open_re(field):
    ...
    return re.compile(
        r"<!--\s*EZER:RESERVED:%s\s+%s\s*-->" % (re.escape(field), re.escape(RESERVED_SENTINEL))
    )


def _close_re(field):
    return re.compile(r"<!--\s*/EZER:RESERVED:%s\s*-->" % re.escape(field))
...
def self_test():
    ...
    broken = t.replace("<!-- /EZER:RESERVED:open_gates -->", "", 1)
    ...
    orphan = "<!-- EZER:RESERVED:restore_pointer %s -->\n없는 close" % RESERVED_SENTINEL
