...
#                      # · 크기 채점 안 함(무결성만; 채점=ezer_rsi injected-only)
#   },
...
HIGHRISK_INFRA = ("evaluator", "benchmark", "ledger", "locked-eval", "rsi-gate", "rsi_rsi", "ezer_rsi")
...
def gate(inp):
    ...
    # (작은 양의 절감도 인정; 크기 채점=ezer_rsi injected-only). '내용 삭제로 효율 보이기' 차단.
...
def self_test():
    ...
    d = copy.deepcopy(base); d["target_paths"] = ["ezer-pack/bin/ezer_rsi.py"]
