#!/usr/bin/env python3
"""ezer_resource_gate.py — P0-3 자원 사전 게이트 (getInvocationBlock의 정액제 번안)
...
사용 예: python3 ezer_resource_gate.py check --context 42 --json
...
DEPT_SOCKET_GLOB = "~/.local/state/ezer-dept-*/ezer.sock"


def _active_dept_count():
    """부서 소켓(ezer-dept-*/ezer.sock) 존재 개수 = 활성 부서 수(소켓 파일 존재=기동중)."""
    import glob
...
def cmd_enforce(a):
    ...
    if a.kill:
        ...
        for v in victims:  # 좀비 인지 집계 — kill(v,0) 프로브는 좀비에 성공해 잔존으로 오판(G5 동형)
            ...
            if not st or st.startswith("Z"):
                killed += 1
    ledger = os.path.join(os.environ.get("EZER_ROOT") or os.getcwd(),
    ...
    if a.kill and killed and getattr(a, "notify", None):
        try:  # 실사건에만 push — 무사건 스케줄 주기는 침묵(스팸 0)
            subprocess.run(["ezer", "send", "--queued", "--to", a.notify,
