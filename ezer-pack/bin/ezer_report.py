...
def ezer_status():
    """`ezer status --json` 수집. ezer 부재·실패 시 None(=todo 직접 스캔만)."""
...
def discover_todo_files(status, extra_dirs):
    ...
    ① pack/round  ② status의 각 surface cwd/_round  ③ --extra-dir/_round ④ EZER_TODO_DIRS."""
    ...
    for d in extra_dirs or []:
        ...
        roots.append(d)
    for d in os.environ.get("EZER_TODO_DIRS", "").split(os.pathsep):
        if d:
            roots.append(d)
    found = set()
    for root in roots:
...
def render_text(rep):
    ...
    if not rep["status_available"]:
        lines.append("  • 노드 현황: ezer status 수집 실패(데몬 미가동?) — todo 스캔만 반영")
    else:
        if rep["paused"]:
...
def main():
    ...
    rep = build_report(ezer_status(), args.extra_dir)
    if args.json:
