#!/usr/bin/env python3
"""ezer_memory_inject.py — 트리거 선택 주입 (W2-4 · OMC OPP-17 클린룸 포트)
...
def memory_dir():
    v = os.environ.get("EZER_MEMORY_DIR")
    if v:
        return v
...
def state_path(session_id):
    d = os.path.join(os.environ.get("EZER_STATE_DIR")
                     or os.path.expanduser("~/.ezer/state"), "guards")
...
def poison_scan_fn():
    ...
    try:
        import ezer_skillscan
        return ezer_skillscan.memory_poison_scan
    except Exception:
        return None


def main():
    if os.environ.get("EZER_DISABLE_GUARDS") == "1":
        return 0
    if "memory-inject" in (os.environ.get("EZER_SKIP_HOOKS") or ""):
        return 0
    raw = read_stdin(5.0)
    if not raw:
        return 0
    ...
    scan = poison_scan_fn()
    if scan is None:
        sys.stderr.write("ezer_memory_inject: poison scanner unavailable — "
    ...
    for cnt, fn, body in found:
        try:
            ...
            sys.stderr.write("ezer_memory_inject: scan error on %s — dropping (fail-safe)\n" % fn)
            continue
        if hits:
            sev = ",".join(sorted({h.get("severity", "?") for h in hits}))
            sys.stderr.write("ezer_memory_inject: dropped poisoned memo %s (%d hit, %s)\n"
                             % (fn, len(hits), sev))
            continue
