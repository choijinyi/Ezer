#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_timeline — 편집-결정 IR(edit_decisions.json) 위의 스냅샷-교체 편집 커맨드 + 언두/재현 저널.

에이전트가 타임라인을 *프로그램으로 편집*하는 ezer-native API(소켓버스 CLI — 네트워크 MCP 아님).
...
사용:
    python3 ezer_timeline.py init   --file F --fps N --render-runtime R
    python3 ezer_timeline.py insert --file F --track KIND --id ID --in T --out T \
            [--intended T] [--mode M] [--transition X] [--source S]
    python3 ezer_timeline.py move   --file F --id ID --to T              # in=T, 길이 보존(시프트)
    python3 ezer_timeline.py trim   --file F --id ID [--in T] [--out T]  # in/out 직접 설정
    python3 ezer_timeline.py split  --file F --id ID --at T              # at에서 분할(스팬 보존·snap-once)
    python3 ezer_timeline.py remove --file F --id ID
    python3 ezer_timeline.py undo   --file F                             # 마지막 편집 스냅샷 복원
    python3 ezer_timeline.py show   --file F [--json]
    python3 ezer_timeline.py --self-test
...
def self_test():
    ...
    with tempfile.TemporaryDirectory(prefix="ezer-timeline-st-") as td:
