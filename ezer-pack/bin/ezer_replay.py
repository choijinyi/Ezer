#!/usr/bin/env python3
"""ezer_replay.py — 이벤트 원장 리플레이 타임라인 (읽기 전용)

원장: $EZER_ROOT/_round/events/events-YYYYMMDD.jsonl (+ .1 로테이트본)
ezer_event.py `emit --log`가 남긴 JSONL({"ts","wire"})을 시간순으로 읽어
...
import ezer_event  # 동일 bin — parse_wire(타입·스키마 검증 포함) 재사용

EXIT_OK, EXIT_USAGE = 0, 2


def _ledger_root():
    # EZER_ROOT env 또는 CWD (ezer_task.py·ezer_event.py와 동일 규약)
    return os.environ.get("EZER_ROOT") or os.getcwd()
        ...
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    evt_type, payload = ezer_event.parse_wire(rec["wire"])
