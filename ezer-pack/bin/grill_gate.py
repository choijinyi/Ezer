...
서브커맨드:
  begin  : 복잡도를 ezer_route.py(+보조 토큰)로 판정 → floor 20/30 → 마커 생성.
           ★surface 격리 불가(EZER_SURFACE_ID 부재)면 마커를 만들지 않고 fail-open
...
마커: <EZER_ROOT>/_round/.grill_session.<surface>.json (노드별 격리).
...
def _surface_id():
    return re.sub(r"[^0-9A-Za-z_-]", "", os.environ.get("EZER_SURFACE_ID", "").strip())
...
def _marker_path():
    ...
    if v:
        return v
    # ★노드별 격리: 마커를 surface(PTY 상속 EZER_SURFACE_ID)별로 둔다.
    sid = _surface_id()
    suffix = (".grill_session.%s.json" % sid) if sid else ".grill_session.json"
    return os.path.join(_ezer_root(), "_round", suffix)


def _route_py():
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "ezer_route.py")
...
def cmd_begin(request_text):
    ...
    if not sid and not has_override:
        print(json.dumps({"gate": "disabled",
                          "reason": "EZER_SURFACE_ID 부재 — surface 격리 불가, "
...
def self_test():
    ...
    try:
        ...
        os.environ.pop("EZER_SURFACE_ID", None)
        os.environ["EZER_ROOT"] = os.path.join(td, "noiso")
