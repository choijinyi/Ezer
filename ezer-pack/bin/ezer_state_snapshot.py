#!/usr/bin/env python3
# ezer_state_snapshot.py — 무손실 복원 Phase 0: 상태 파일 세대(generation) 보관기
#
...
#
# 이 도구가 하는 일 / 안 하는 일:
#   - 한다: ~/.local/state/ezer/topology.json 등 소형 L1 선언 상태 파일을 읽어
#           ~/.ezer/state-generations/<timestamp>/ 에 원자적으로 세대 보관한다.
...
#
# 사용:
#   ezer_state_snapshot.py snapshot [--dry-run]   # 세대 1건 생성 + GC
#   ezer_state_snapshot.py list                   # 세대 목록
#   ezer_state_snapshot.py verify [--gen <이름>]  # manifest 해시 무결성 검증
#   ezer_state_snapshot.py gc [--dry-run]         # 보관정책 적용(삭제 대상 산출/실행)
#   ezer_state_snapshot.py self-test              # 원자성/중단내성 실증(격리 임시폴더)
...
#   부서는 손 배선·부서명 하드코딩 없이 glob(ezer-dept-*) ∪ depts.json 레지스트리로 발견된다
...
# ── Windows named pipe → 상태 디렉터리 매핑 (Rust src/bin/ezerd/state.rs::pipe_slug/state_dir 규칙의 단일 소스) ──
# ★ezer_phoenix.py 도 이 두 함수를 재사용한다(중복 구현 금지) — 규칙은 여기 한 곳에만 둔다.
#   unix 는 소켓 부모가 곧 상태 dir 이지만 Windows 는 파이프에 파일시스템 부모가 없어 슬러그로 파생해야 한다:
#   기본 데몬(`\\.\pipe\ezer`)=%LOCALAPPDATA%\ezer · 부서(`\\.\pipe\ezer-dept-<n>`)=그 하위 슬러그 디렉터리.
...
def _win_state_dir_for_socket(socket, localappdata=None, home=HOME):
    ...
    root = os.path.join(base, "ezer")
    slug = _win_pipe_slug(socket)
    if not slug or slug == "ezer":
...
def _dept_state_dirs(state_root, depts_json=None, windows=None, localappdata=None):
    """부서 상태 디렉터리를 동적 발견한다 — 파일시스템 glob(ezer-dept-*) ∪ depts.json 소켓 매핑.
    ...
    ★Windows(권고#1): 부서 디렉터리는 %LOCALAPPDATA%\\ezer 하위(ezer-dept-*)이고 registry 소켓은 named pipe 라
    ...
    try:
        ...
        pass
    dj = depts_json or os.path.join(os.path.dirname(os.path.dirname(state_root)), ".ezer", "depts.json")
...
def default_sources(home=HOME, state_root=None, depts_json=None, windows=None, localappdata=None):
    ...
    ★Windows(권고#1): 메인 상태=%LOCALAPPDATA%\\ezer, 부서=그 하위 ezer-dept-*(state.rs 레이아웃). unix 는 종전대로
    <state_root>/ezer + <state_root>/ezer-dept-*. windows/localappdata 주입으로 mac 에서도 win 분기 단위검증 가능."""
    win = IS_WINDOWS if windows is None else windows
    if win:
        base = localappdata or os.environ.get("LOCALAPPDATA") or os.path.join(home, "AppData", "Local")
        main_state = os.path.join(base, "ezer")
        dept_scan_root = main_state            # win: 부서는 ezer 하위(state.rs 슬러그 레이아웃)
    else:
        state_root = state_root or os.path.join(home, ".local", "state")
        main_state = os.path.join(state_root, "ezer")
        dept_scan_root = state_root             # unix: 부서는 state_root 형제(ezer-dept-*)
    srcs = [os.path.join(main_state, b) for b in DECLARATIVE_BASENAMES]
    depts_json = depts_json or os.path.join(home, ".ezer", "depts.json")
    ...
    #   개인경로 하드코딩 금지(pack scan gate): 프로젝트는 $EZER_ROOT(env 또는 CWD),
    #   장기기억은 HOME glob 파생만(ezer_wakeup.py 등과 동일 관례).
    proj_round = os.path.join(os.environ.get("EZER_ROOT") or os.getcwd(), "_round")
...
def _now_utc():
    ...
    override = os.environ.get("_EZER_SNAPSHOT_NOW")
    if override:
...
def do_snapshot(sources=None, gen_root=GEN_ROOT, dry_run=False, crash_hook=None):
    ...
    manifest = {
        ...
        "generator": "ezer_state_snapshot.py",
        "files": [],
    }
    try:
        for src in present:
        ...
        if crash_hook == "before_rename" or os.environ.get("_EZER_SNAPSHOT_CRASH_BEFORE_RENAME"):
...
def do_self_test():
    ...
    print("=== ezer_state_snapshot self-test (격리 임시폴더) ===")
    ...
    workdir = tempfile.mkdtemp(prefix="ezer-snaptest-")
    try:
        ...
        main6 = os.path.join(state_root6, "ezer")
        os.makedirs(main6)
        with open(os.path.join(main6, "topology.json"), "w") as f:
            f.write('{"entries":[{"role":"worker"}]}')
        os.makedirs(os.path.join(home6, ".ezer"))
        # registry 는 dept-A 만 등록(stale) — 하지만 디스크엔 dept-A·dept-B 둘 다 존재
        with open(os.path.join(home6, ".ezer", "depts.json"), "w") as f:
            json.dump({"depts": {"dept-A": {"socket": os.path.join(state_root6, "ezer-dept-dept-A", "ezer.sock")}}}, f)
        for dep in ("ezer-dept-dept-A", "ezer-dept-dept-B"):
        ...
        assert any("ezer-dept-dept-A" in s and s.endswith("schedule_state.json") for s in srcs6), "T6: dept-A 선언상태 누락"
        assert any("ezer-dept-dept-B" in s and s.endswith("schedule_state.json") for s in srcs6), "T6: dept-B(미등록) 누락 — 파일시스템 truth 실패"
        ...
        assert any("ezer-dept-dept-B" in s for s in stored_srcs), "T6: 스냅샷에 부서 선언상태 미포함"
...
def main():
    ...
    ap = argparse.ArgumentParser(description="ezer 상태 파일 세대 보관기 (무손실 복원 Phase 0)")
