#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_params — 능력 스킬 튜너블의 타입드 ParamDefinition + 결정론적 coerce 바닥(가치 게이트).
...
ezer 가치: 우리 스킬은 자유텍스트 의도를 받아 각자 산문으로 입력 검증을 재유도한다 — 기계 바닥이
...
사용:
    python3 ezer_params.py validate-defs <DEFS.json> [--json]        # ParamDefinition 스키마 검증
    python3 ezer_params.py defaults      <DEFS.json> [--json]        # {key: default} 산출
    python3 ezer_params.py coerce        <DEFS.json> --values <V.json> [--json]
    python3 ezer_params.py --self-test
...
def self_test():
    ...
    import tempfile
    sink = io.StringIO()
    with tempfile.TemporaryDirectory(prefix="ezer-params-st-") as td:
