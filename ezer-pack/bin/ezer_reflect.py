#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_reflect — 반복신호 light scan (자기교정 ① Capture 자동화).

외부 /reflect shadow mode의 ezer판. Stop·SessionEnd hook이 세션 transcript를
...
사용:
    python3 ezer_reflect.py scan --transcript <path.jsonl> --ledger <RSI_LEDGER.md> \
        [--threshold 3] [--json]      # 1회 스캔 (hook이 호출)
    python3 ezer_reflect.py --self-test                  # 결정론 자기검증 (preflight C28)
...
SYSTEM_PREFIXES = ("<command-", "<local-command", "Caveat:", "<system-reminder",
                   "■ Ezer", "# MASTER ABSOLUTE DIRECTIVE", "# WORKER ABSOLUTE DIRECTIVE",
...
def self_test():
    ...
    failures = []
    with tempfile.TemporaryDirectory(prefix="ezer-reflect-selftest-") as td:
        ...
        open(tpath3, "w", encoding="utf-8").write("\n".join([
            ...
            urow3("■ Ezer 역할 각성 (EZER_ROLE=worker)\n하지 마 시켰잖"),
