#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_orchestra — LLM 오케스트레이션의 결정론 도구 (절대지침 4차: LLM orchestrating 앵커).
...
서브커맨드:
  check                         4종 의무 노드(cso·worker·reviewer-gemini·reviewer-codex)
                                생존을 ezer status로 판정. exit 0=4종 생존, 1=부재 존재.
  ...
  phase-plan   --task T --phases "p1;p2;p3" --scope S [--success X] [--to ROLE] [--dont D]
                                ...
                                pipeline·ezer 워커 순차 위임으로 실행(스킬 안내).
...
의존성: 파이썬 표준 라이브러리 + PATH의 ezer(check만 필요).
"""
...
def ezer_status():
    ...
    if not ezer:
        return None
    try:
        r = subprocess.run([ezer, "status", "--json"], capture_output=True, timeout=10)
        if r.returncode != 0:
            return None
...
def live_roles(status):
    ...
    어디까지나 set-status ack 다. 프로세스 탐침은 stuck pane 회수 판단(ezer_boot_node.py --reclaim)
    에만 쓴다."""
    out = {}
...
def _quiet_alive_roles(status, roles):
    ...
    ★생존 술어는 ezer_boot_node.quiet_but_alive 단일 정의를 공유한다(codex R2 결함2 — cmd_check 와
    reclaim 이 같은 상태를 반대로 해석하던 중복 로직 제거). status 를 surface_ref 에 결박해
    litter/exited row·과거이력 오인을 차단한다."""
    out = {}
    try:
        import ezer_boot_node as _bn
    except Exception:
        return out
    for role in roles:
...
def cmd_check(args):
    status = ezer_status()
    if status is None:
        print("[orchestra check] ezer status 수집 실패(데몬 미가동?) — `ezer ping` 확인 후 재실행")
        return 2
    ...
    if missing:
        ...
        howto = ("ezer_orchestra.py boot-reviewers (리뷰어 감지·자동 폴백)" if only_rev
                 else "ezer boot")
...
def _boot_one_node(role, agent, timeout=130):
    """ezer_boot_node.py 로 단일 노드 결정론 부트. rc==0(각성확정) → True."""
    bn = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ezer_boot_node.py")
    try:
...
def cmd_review_prompt(args):
    ...
    _root = os.environ.get("EZER_ROOT") or os.getcwd()
    ...
    lines.append("회신: `ezer send --queued --to master \"[리뷰] ...\"` (자동 Return 배달 — "
...
FALLBACK_RULES = [
    ...
    "전담 sub-skill(`ezer skill show hallucination-guard`)을 반드시 사용해 검증 엄밀성·평가 "
    ...
    "(`ezer skill show grill-me`) 등으로 의뢰자(master)와 합의에 이를 때까지 질문을 반복한다.",
...
# ezer_registry와 byte-동일 규칙(preflight는 orchestra를 import 안 함 → C39는 registry verify에 위임).
...
def normalize_slug(ref):
    """ref → 표준 슬러그(ezer_registry.normalize_slug와 byte-동일): lower·.md 제거·타입접두 제거."""
...
def resolve_prereq_block(requires_skills, related_memory, memory_dir):
    ...
    for name in skills:
        if name in seen_sk:  # 중복 skill은 순서 보존하며 1회만 방출(노이즈 차단)
            continue
        seen_sk.add(name)
        lines.append("  [skill] %s — ezer skill show %s" % (name, name))
    seen_mem = set()
    for ref in mems:
...
def resolve_manifest_phase(manifest, phase_id):
    """타입드 워크플로우 매니페스트(D4) 단계 계약 해소 — ezer_manifest phase에 위임.
    ...
    if not manifest or not phase_id:
        return None, []
    tool = os.path.join(pack_dir(), "bin", "ezer_manifest.py")
...
def build_task_ticket(task, scope, success, to_role, rules, output_format=None, prereq_block="", dont=None, tier_hint=None):
    ...
    lines.extend("  " + b for b in bullets)
    lines.append("")
    # 경로는 pack 앵커 절대경로 — ezer_report의 todo 스캔 루트(pack/round)와 일치해야
    # 진행% 집계에 잡힌다(상대경로 'round/'는 워커 cwd에 따라 집계 누락 — 적대 검증 R1).
    lines.append("todo 영속: 이 작업을 \"${EZER_PACK_DIR:-$HOME/.ezer/pack}/round/%s_TODO.md\"에 "
                 "분해하고 세부 완료마다 체크박스를 갱신하라(진행%% 집계 원천)."
                 % to_role.upper().replace("-", "_"))
    lines.append("보고 채널: 완료·질문·충돌·막힘은 `ezer send --queued --to master \"[보고] ...\"` "
                 "로 직접 push하라(--queued는 자동 Return 배달 — send-key 불필요·타이핑 가드 "
                 "안전). 즉시 끼어들어야 할 긴급 보고만 직접 send 후 `ezer send-key --to master "
...
def cmd_task_prompt(args):
    ...
    # — 이 확인은 눈대중이 아니라 ezer status의 agent_alive로만 확정한다.
    ...
    if not getattr(args, "no_survival_gate", False):
        status = ezer_status()
        if status is None:
            print("[task-prompt] ezer status 수집 실패(데몬 미가동?) — `ezer ping` 확인 후 재실행. "
                  "대상 생존 미확인 상태로는 티켓을 내지 않는다.", file=sys.stderr)
            return 2
        if not live_roles(status).get(args.to):
            print("[task-prompt] 대상 '%s' 미기동 — 티켓 미출력. `ezer boot`(4종 의무 기동) 또는 "
                  "`ezer launch-agent --role %s --agent claude`로 기동 후 재실행하라."
                  % (args.to, args.to), file=sys.stderr)
            return 1
        # '정상 작동' 보조 신호: 장기 idle(기본 5분 — EZER_IDLE_SECONDS와 동기)은 hang일 수
        # 있다 — 차단은 아니고 경고만(지시 대기 중인 워커도 idle이므로 alive가 결정 기준,
        # idle은 §5 능동 점검 트리거). 같은 role의 죽은 stale surface는 건너뛴다.
        try:
            idle_thr = int(os.environ.get("EZER_IDLE_SECONDS", "300"))
        except ValueError:
            idle_thr = 300
        for s in status.get("surfaces", []):
...
# 철학·자원 거버넌스 충돌 회피) — 스킬이 Phase 티켓을 Workflow pipeline 또는 ezer 워커 순차
# 위임으로 실행하도록 안내한다.
...
def cmd_phase_plan(args):
    ...
    for i, name in enumerate(phases, start=1):
        ...
        prev = ("직전 Phase(%s) 산출물과 docs-diff(ezer_docsdiff.py 변경 줄)를 참조하라."
                % ("P%d" % (i - 1)) if i > 1 else
    ...
    blocks.append("실행: 코드는 claude -p를 띄우지 않는다. 아래 Phase 티켓을 Workflow "
                  "pipeline 또는 ezer 워커로 순차 위임하라(각 Phase 독립 세션·메인 컨텍스트 보존).")
...
def cmd_round_log(args):
    ...
    if getattr(args, "from_cmd", None):
        ...
        #   verdict JSON이 ezer_verdict 스키마(enum·evidence·score 금지)를 통과할 때만 기록.
        ...
        try:
            ...
            schema_errors, _lint, verdict_out = ezer_verdict.validate_verdict(obj)
...
# ── 무음실패 카탈로그(OpenMontage D5 2부): ezer엔 guard.sh가 없다 — denylist는 CLAUDE.md §6
...
SILENT_FAILURES = [
    {"id": "SF-GATE-SCORE-FIELD",
     ...
     "detection": "verdict/round-log 레코드에 score 키 또는 0-100·0-1 수치 등급 값이 있으면 위반 — ezer_verdict.py 스키마 게이트가 차단",
     "kind": "deterministic"},
...
CATALOG_BANNER = (
    "<!-- 생성됨: `ezer_orchestra.py silent-failure-catalog` 가 SILENT_FAILURES에서 결정론 파생.\n"
    "     손편집 금지 — 재생성: `ezer_orchestra.py silent-failure-catalog`. "
...
def render_catalog():
    ...
    lines = [
        ...
        CATALOG_BANNER,
        "",
        "> ezer엔 guard.sh가 없다 — denylist는 CLAUDE.md §6 산문이다. 이 표는 그 산문을 각 항목의 "
        "**탐지절(위반 증명 아티팩트)**로 큐레이션한 것이다. 무점수(수치 등급 없음). source-of-record"
        "=ezer_orchestra.py 내부 `SILENT_FAILURES`.",
        "",
...
def _recommend_learn_once(reason, topic, marker_key):
    ...
    directive §4). ezer 부재·데몬 미가동·오류·중복 마커는 무시(추천은 비핵심·핵심 판정 불간섭)."""
    import shutil
    learn_dir = os.path.join(pack_dir(), "round", "learn")
    marker = os.path.join(learn_dir, ".rec_" + marker_key)
    if os.path.exists(marker) or not shutil.which("ezer"):
        return
    body = ('{"reason":"%s","topic":"%s","status":"awaiting_approval"} — '
            "feed 패널 또는 'ezer feed reply <id> allow'로 승인 시에만 학습 착수. directive §4: 추천까지만 자율." % (reason, topic))
    try:
        os.makedirs(learn_dir, exist_ok=True)
        r = subprocess.run(["ezer", "feed", "push", "--kind", "learn_proposal",
                            # 제목 포맷은 ezerd RPC 생산자(handlers.rs learn_proposal)와 동일 규격 유지.
...
def cmd_channel_health(args):
    ...
    ezer_channels.py 를 subprocess 호출(사람판=silence-first·기계판=--json). check 가
    부트 노드(cso/worker/agy/codex) 생존을, channel-health 가 콘텐츠 채널 도달성을 본다."""
    tool = os.path.join(pack_dir(), "bin", "ezer_channels.py")
    if not os.path.isfile(tool):
        print("[channel-health] ezer_channels.py 부재 — `ezer init-pack`", file=sys.stderr)
        return 2
...
# 데몬 내부의 privileged-role 점유 차단(ezerd handlers.rs)이 1차 방어, 이 명령이 2차(부트 전 선검사).
def _surface_id_env():
    """내 surface id 문자열 반환(없으면 None). ezer::env_compat 우선순위(AITERM_*→EZER_*→EZER_*)와 정합 —
    AITERM_SURFACE_ID를 먼저 보되, ezerd가 실제 주입하는 EZER_SURFACE_ID(src/lib.rs ENV_SURFACE_ID)도 인식한다.
    셋 다 미설정(외부 셸 세션)이면 None → 호출부가 PASS(부팅 차단 회귀 방지·gemini D2)."""
    for k in ("AITERM_SURFACE_ID", "EZER_SURFACE_ID", "EZER_SURFACE_ID"):
...
def _ezer_list_masters():
    """현재 데몬(상속 EZER_SOCKET)의 live(미exited) role=master surface id 리스트. ezer list 실패 시 None.
    ezer list 라인 형식: '{surface_ref}\\trole={role}\\tpid={pid}\\texited={bool}\\t{title}\\t{cwd}'."""
    ezer = shutil.which("ezer")
    if not ezer:
        return None
    try:
        r = subprocess.run([ezer, "list"], capture_output=True, timeout=10)
    except Exception:
        return None
    if r.returncode != 0:
        return None
...
def guard_master_verdict(my_env, masters):
    """순수 판정(ezer 의존 없음·self-test 핀). 반환 (code, kind).
    my_env=내 surface id 문자열(None=미설정) · masters=live master id 리스트(None=ezer list 실패).
...
def cmd_guard_master_claim(args):
    """claim-role master 직전 결정론 선검사. AITERM/EZER_SURFACE_ID 미설정(외부 셸)→PASS(0).
    설정 시 내 surface 와 다른 live master 보유자가 있으면 MISROUTED_MASTER + exit 9.
    보유자=나(멱등)·master 부재→PASS(0)."""
    my_env = _surface_id_env()
    masters = _ezer_list_masters() if (my_env is not None and _parse_ref(my_env) is not None) else None
    ...
    if kind == "unset":
        ...
        print("[guard-master-claim] ezer list 미수집(데몬 미응답?) — PASS(부팅 차단 회귀 방지)")
    elif kind == "misrouted":
        my_id = _parse_ref(my_env)
...
def cmd_self_test(args):
    """순수 로직 자기검증 (ezer 의존 없음) — preflight C19가 호출. assert 실패는 exit 1."""
    try:
        ...
        _prev_root = os.environ.get("EZER_ROOT")
        try:
            with _tf.TemporaryDirectory() as _td:
                os.environ["EZER_ROOT"] = _td
                # (a) 소스 부재 = 주입 0 (회귀 0)
                _b0 = io.StringIO()
            ...
            if _prev_root is None:
                os.environ.pop("EZER_ROOT", None)
            else:
                os.environ["EZER_ROOT"] = _prev_root
        # live_roles 파싱
        ...
        for must in ("절대 강조 4규칙", "품질 절대우선", "할루시네이션 방지",
                     ...
                     "성공 기준", "WORKER_TODO.md", "${EZER_PACK_DIR", "보고 채널",
                     "--queued"):
        ...
        # 같은 소실을 검출할 수 있어야 진단 루프가 닫힌다. ezer_preflight가 같은 bin에
        ...
        # 쓰기·--check 왕복(격리 tempdir — 라이브 pack 미접촉; EZER_PACK_DIR 재지정·복원)
        import tempfile as _tf
        _saved_pd = os.environ.get("EZER_PACK_DIR")
        with _tf.TemporaryDirectory(prefix="ezer-orch-sfc-") as _td:
            os.environ["EZER_PACK_DIR"] = _td
            try:
                _sink = io.StringIO()
                ...
                if _saved_pd is None:
                    os.environ.pop("EZER_PACK_DIR", None)
                else:
                    os.environ["EZER_PACK_DIR"] = _saved_pd

        # ── 전제지식 자동주입 배터리 (OpenMontage D6 — normalize_slug 핀·resolver·티켓 byte-동일) ──
        assert normalize_slug("feedback_decision-consult-ezer-sot.md") == "decision-consult-ezer-sot", \
        ...
        with _tf2.TemporaryDirectory(prefix="ezer-orch-d6-") as _td2:
            ...
            assert "[skill] grill-me — ezer skill show grill-me" in _blk, "skill 읽기명령 누락"
            ...
            assert _dup.count("[skill] grill-me — ezer skill show grill-me") == 1, "중복 skill 방출"
        ...
        with _tf3.TemporaryDirectory(prefix="ezer-orch-d4-") as _td3:
            ...
            if _su is not None:  # ezer_manifest 배포 시에만 해소(환경 독립 — 부재 시 (None,[]) 계약은 위에서 핀)
        ...
        # ★Fix2' guard-master-claim 순수 판정 배터리(ezer 의존 없음·결정론):
        ...
        assert guard_master_verdict("31", None) == (0, "list_fail"), "ezer list 실패인데 PASS/list_fail 아님(부팅 차단 회귀)"
        assert guard_master_verdict("notanumber", [99]) == (0, "unparsed"), "파싱불가 env가 false-block(회귀)"
    except AssertionError as e:
        print("ezer_orchestra self-test FAIL: %s" % e, file=sys.stderr)
        return 1
    print("ezer_orchestra self-test OK (4종 노드·라운드 상한·경로 탈출방지·제약 주입·"
...
def main():
    ...
    tp.add_argument("--tier", default=None,
                    help="권장 실행 등급 정보 1줄 주입(trivial/standard/heavy — 강제 아님·R2 1단계·"
                         "ezer_route suggested_node와 정합). 미지정 시 티켓 byte-동일")
    ...
    rl.add_argument("--verdict-json", dest="verdict_json", default=None,
                    help="★G8 리뷰어(gemini/agy/codex) 행 필수 — ezer_verdict 스키마 통과 "
