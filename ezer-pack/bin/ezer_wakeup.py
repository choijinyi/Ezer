#!/usr/bin/env python3
"""ezer_wakeup.py — P0-2 wakeup 큐: 코얼레싱 + 멱등키 + zombie 가드 (클린룸 파일 포트)
...
상태 저장: $EZER_ROOT/_round/wakeups/pending/<safe(target)>__<safe(task_key)>.json
원장:      $EZER_ROOT/_round/wakeups/queue.jsonl (append-only)
enqueue 직렬화: pending 파일별 read-modify-write를 .lock 디렉터리(mkdir 원자성)로 보호.

대상 생존 판정(zombie 가드) 우선순위:
  1) 테스트/강제 주입: EZER_WAKEUP_LIVENESS=alive|dead
  2) `ezer list` 출력에 target 문자열 존재 여부 (ezer 없으면 unknown→배달 보류 아닌 경고 배달)
배달은 기본 드라이런(ezer send --queued 명령을 출력만). --deliver 시 실제 실행.
...
import ezer_scrub  # ★G2: 원장 기록 직전 비밀 마스킹(같은 폴더 형제 모듈 — 부재 시 즉시 실패=fail-closed)
...
def _ledger_append(event):
    ...
    event = ezer_scrub.scrub_obj(event)
...
def _target_alive(target):
    ...
    override = os.environ.get("EZER_WAKEUP_LIVENESS")
    ...
    if not ezer:
        return "unknown"
    try:
        out = subprocess.run([ezer, "list"], capture_output=True, text=True, timeout=10).stdout
...
def cmd_drain(a):
    fastfail_max = int(os.environ.get("EZER_FASTFAIL_MAX", "3"))
    ...
    for path, rec in pending:
        ...
        cmd = ["ezer", "send", "--queued", "--to", rec["target"], msg]
        if a.deliver:
            if alive == "unknown":
...
def main(argv=None):
    ...
    c.add_argument("--deliver", action="store_true", help="실제 ezer send 실행(기본 드라이런)")
