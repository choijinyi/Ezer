#!/usr/bin/env python3
"""ezer_transcript_stats — 세션 전사(JSONL)에서 결정론 노력·검증 통계 추출.

출처: Steinberger 강연(2026-06-25 SF) agent-transcript 스킬 증류 —
"전사가 길수록 그 사람이 실제로 이해하려 했다는 신뢰가 올라간다"를
ezer 철학(환각0·산문 신고 대신 결정론 수치)으로 포트.
OpenClaw는 전사 '본문'을 redaction 후 첨부하지만, 우리는 '통계만' 추출한다
— 본문 미노출 = sanitize 실패 리스크 자체가 없다(fail-closed by design).

사용:
  python3 ezer_transcript_stats.py --latest            # 이 프로젝트 최신 세션
  python3 ezer_transcript_stats.py --session <path>    # 특정 JSONL
  python3 ezer_transcript_stats.py --latest --oneline  # evidence 한 줄용

evidence 접합:
  ezer_task.py set-status <id> done --evidence "$(... --latest --oneline)"
...
TEST_CMD_RE = re.compile(
    ...
    r"|preflight|ezer_.*eval|validate)\b"
)
...
def resolve_project_dir(explicit=None):
    """유도 순서: ①명시 인자 ②env EZER_SESSION_PROJECT_DIR ③CWD 슬러그(최신 세션 보유 프로필).
    실패 시 None — 호출부가 exit 2(연성 의존 계약: 배선부는 이를 '통계 생략'으로 처리)."""
    if explicit:
        return explicit
    env = os.environ.get("EZER_SESSION_PROJECT_DIR")
    if env:
        return env
...
def main():
    ...
    ap.add_argument(
        ...
        help="세션 JSONL 디렉토리(생략 시 env EZER_SESSION_PROJECT_DIR → CWD 슬러그 유도)",
    )
