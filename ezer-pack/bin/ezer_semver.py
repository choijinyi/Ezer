#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_semver — strictly-newer 버전 비교 결정론 advisory (다운그레이드 유도 방지).
...
- Agent Reach `_is_newer_version`(PHIL-07) 계약(strictly-newer + main-ahead 거부 + fail-safe)만
  ...
  pip 미사용=의존성0 원칙). ezer 실사용 버전은 0.2.7·0.42.4 류 단순 release뿐(실측) — 프리릴리스/
...
사용:
    python3 ezer_semver.py compare --local <V> --remote <V> [--json]
    ...
    python3 ezer_semver.py parse <V> [--json]              # ParsedVersion 상태 방출(디버그)
    python3 ezer_semver.py gate --local-file <PATH> --remote <V> [--field version] [--json]
        # Cargo.toml/tauri.conf.json/manifest의 version 필드를 정규식으로 읽어 compare 위임
    python3 ezer_semver.py --self-test                    # 결정론 자기검증 (preflight C45)
