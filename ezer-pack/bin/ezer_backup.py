...
# -*- coding: utf-8 -*-
"""
ezer_backup.py — 불사조 Phase 8: 일반 백업 능력 + 정직한 보호 상태
...
★제품 계층(개인정보 무첨가·모든 사용자 동일): 이 도구는 "무엇을 어떻게 백업하는가"의 능력·기본값·정직한
  ...
  - 암호키는 --key-file 또는 env EZER_BACKUP_KEY 로 사용자가 제공한다(코드에 키 없음).
  - 오프사이트 목적지는 env EZER_BACKUP_OFFSITE(앵커)로 사용자가 댄다. 없어도 상태 보고는 정직하게 동작한다.

3계층 분류(G5):
  · Tier1 정체성(재생성 불가): ~/.ezer/pack 트리(soul.md·directives·skills·memory·_round). → 암호화하여 오프사이트.
...
def default_roots(home=HOME):
    ...
    return [os.path.join(home, ".ezer", "pack")]
...
def get_passphrase(key_file=None):
    """사용자 제공 키 — --key-file 우선, 없으면 env EZER_BACKUP_KEY, 둘 다 없으면 None(평문). ★코드 하드코딩 0."""
    if key_file and os.path.isfile(key_file):
        return open(key_file).read().strip()
    env = os.environ.get("EZER_BACKUP_KEY")
...
def do_restore(in_dir, dest, passphrase=None):
    ...
    if manifest.get("encrypted"):
        if not passphrase:
            die("암호화 백업 복원에 키 필요(--key-file 또는 EZER_BACKUP_KEY).")
...
def protection_status(home=HOME, backup_dir=None, offsite_anchor=None, recent_days=7):
    ...
    backup_dir = backup_dir or os.environ.get("EZER_BACKUP_DIR") or os.path.join(home, ".ezer", "backups")
    if offsite_anchor is None:
        offsite_anchor = os.environ.get("EZER_BACKUP_OFFSITE")
    ...
    if encrypted and anchored:
        ...
        if not anchored:
            reasons.append("오프사이트 앵커 미무장(env EZER_BACKUP_OFFSITE 미설정) — 단일 사이트")
...
def _build_synth_home(root):
    pack = os.path.join(root, ".ezer", "pack")
    ...
    open(os.path.join(pack, "ezerd.log"), "w").write("log line\n" * 100)
    return root


def do_self_test():
    print("=== ezer_backup self-test (격리 합성 home) ===")
    work = tempfile.mkdtemp(prefix="ezer-backuptest-")
    try:
        ...
        assert any("ezerd.log" in f for f in t3r), "T1: log 가 Tier3 아님"
        ...
        # 복원본에도 토큰 없어야
        assert not os.path.exists(os.path.join(dest, ".ezer", "pack", "session.token")), "T4: 토큰이 복원됨(제외 실패)"
