#!/usr/bin/env python3
"""ezer_event.py — P0-4 이벤트 타입 enum 계약 검증기/방출기 (EVENT_CONTRACT v2 구현)
...
import ezer_scrub  # ★G2: 기록·전파 직전 비밀 마스킹(같은 폴더 형제 모듈 — 부재 시 즉시 실패=fail-closed)
...
EZER_BIN = os.environ.get("EZER_BIN", "ezer")


def _resolve_slug_from_socket():
    """EZER_SOCKET env → 부서 slug (P2-4). 부재=본부 main. depts.json socket 매칭.

    매칭 실패 시 None(미귀속 폴백 — fail-open 금지: main 으로 오귀속하지 않는다).
    """
    sock = os.environ.get("EZER_SOCKET")
    if not sock:
        return "main"   # 본부 기본(EZER_SOCKET 미설정 노드 = 본부)
    reg = os.environ.get("EZER_DEPTS_JSON") or os.path.expanduser("~/.ezer/depts.json")
    try:
        with open(reg) as f:
...
def _resolve_surface_ref():
    """`ezer identify` → caller.surface_ref (P2-4). 실패·surface 밖이면 None."""
    try:
        out = subprocess.run([EZER_BIN, "identify"], capture_output=True, text=True, timeout=5)
    except Exception:
        return None
    if out.returncode != 0:
        return None
...
def resolve_auto_surface():
    ...
    slug(EZER_SOCKET→depts.json)·surface_ref(ezer identify) 중 하나라도 해석 실패 시 None
...
def to_wire(evt_type, payload):
    ...
    payload = ezer_scrub.scrub_obj(payload)
...
def _spool_append(evt_type, payload, surface):
    ...
    entry = {"ts": time.time(), "type": evt_type, "payload": ezer_scrub.scrub_obj(payload)}
    if surface:
        entry["key"] = surface
