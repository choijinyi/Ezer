...
# -*- coding: utf-8 -*-
"""
ezer_boot_node.py — 결정론 단일 노드 부트 헬퍼 (부트스트랩 시행착오 재발방지)

배경(2026-06-13 실측 부트스트랩 시행착오 · MASTER_DIRECTIVE §0 등재 · codex R1·R2 적대검증 반영):
  F1 typing_guard 콜드스타트 레이스 — `ezer launch-agent`는 pane 생성 직후 *즉시* 디렉티브를
     주입한다. 그러나 갓 뜬 CLI 의 시작 애니메이션이 idle_secs=0 을 유지해 데몬 typing_guard 가
     "사람 입력 중"으로 오탐 → 주입 send 차단(EZER_TYPING_GUARD_SECS=0 무효 — 활동기반).
  ...
  F4 헌법 가드가 ezer send 본문의 "*_DIRECTIVE.md"·"soul.md" 패턴을 헌법쓰기로 오탐·차단.
  F5 막힌 privileged-role surface 회수에 kill -9 필요(surface.close self-only).

★상태 계약 3분리(codex R1 핵심 권고) — 절대 섞지 마라:
  surface_occupied : role 을 가진 비종료 surface 존재(ezer list).
...
프로토콜(idle-then-inject):
  ...
  2) LAUNCH     없을 때만 launch-agent 1회. 실패 텍스트 무시·ezer list 재조회(F2).
  3) POLL-IDLE  idle_secs>=IDLE 안착까지 폴링(F1). 폴링 중 awake_ready 잡히면 즉시 종료.
  4) INJECT     주입 직전 t_inject 기록. 자연어(확장자 없음·F4) 각성 지침을 `ezer send --queued`
...
사용:
  python3 ezer_boot_node.py --role cso --agent claude [--cwd D] [--idle 4] [--timeout 90] [--json]
  python3 ezer_boot_node.py --reclaim --role cso          # 막힌(죽은) 미각성 surface 결정론 회수
  python3 ezer_boot_node.py --self-test                   # 순수함수 회귀 배터리
...
PACK_DIR = os.environ.get("EZER_PACK_DIR") or os.path.expanduser("~/.ezer/pack")
...
def ezer_status():
    rc, out, _ = run(["ezer", "status", "--json"], timeout=12)
    if rc != 0:
        return None
    try:
...
def ezer_list_rows():
    """ezer list 의 모든 행을 {surface_ref, role, pid, exited} 로 파싱.
    ★key=value 컬럼을 위치가정 없이 전부 훑는다(codex R1 논쟁점: 컬럼순서 변동 견고화)."""
    rc, out, _ = run(["ezer", "list"], timeout=12)
    rows = []
    if rc != 0:
        return rows
...
def awaken_message(role):
    ...
    return (
        "너는 이 ezer 워크스페이스의 %s 노드다. 즉시 각성하라: "
        "① pack 디렉티브 폴더의 %s 문서와 soul 헌장을 읽고 정체를 확정 "
        "② ezer claim-role %s 로 역할 확인(이미 보유 시 ezer list 로 확인만) "
        "③ ezer set-status --state %s --context 5 로 생존 신호 발신 "
        "④ 너의 TODO 파일 확인·복원 "
        "⑤ 각성 완료를 'ezer send --to master' 후 'ezer send-key --to master Return' 으로 master 에게 push 보고하라."
        % (role, name, role, state)
    )


def inject(role, msg, attempts=4):
    """★`ezer send --queued` 단일경로(codex R2 결함4): 큐는 대상이 조용해질 때 메시지+자동 Return 을
    원자적으로 배달한다 → typing_guard 우회(F1)·Return 분리 실패로 인한 중복 입력 위험 제거.
    (idle-then-inject 로 이미 안착했으므로 큐는 즉시 배달된다.)"""
    for i in range(attempts):
        rcq, _, errq = run(["ezer", "send", "--queued", "--to", role, msg], timeout=12)
        if rcq == 0:
...
def reclaim(role, emit):
    ...
    st = ezer_status()
    if st is None:
        emit("reclaim", "ezer status 실패 — 회수 보류")
        return 2
...
def self_test():
    ...
    chk("ezer claim-role reviewer-codex" in awaken_message("reviewer-codex"), "reviewer-codex claim 풀네임 누락")
    chk("ezer claim-role reviewer-gemini" in awaken_message("reviewer-gemini"), "reviewer-gemini claim 풀네임 누락")
    ...
    chk("ezer claim-role reviewer-claude-1" in awaken_message("reviewer-claude-1"), "reviewer-claude-1 claim 풀네임 누락")
...
def main():
    ...
    status = ezer_status()
    if status is None:
        emit("fatal", "ezer status 수집 실패 — 데몬 미가동? `ezer ping` 확인")
    ...
    row = role_surface_row(a.role)
    if row is None:
        cmd = ["ezer", "launch-agent", "--role", a.role, "--agent", a.agent]
        if a.cwd:
            cmd += ["--cwd", a.cwd]
        rc, _, _ = run(cmd, timeout=80)
        emit("launch", "launch-agent rc=%d (실패 텍스트 무시·ezer list 재조회)" % rc)
        for _ in range(3):
            time.sleep(2)
            row = role_surface_row(a.role)
            if row is not None:
