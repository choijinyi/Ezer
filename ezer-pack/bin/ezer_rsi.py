#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_rsi — RSI(재귀적 자기개선) 라운드 무결성의 결정론 도구 (T7 E7).
...
사용: python3 ezer_rsi.py <cmd> ... · 의존성: 표준 라이브러리 + PATH의 git.
"""
import argparse
import json
import os
...
def _learn_state_dir():
    """ezerd learn.status가 읽는 위치(handlers.rs learn_state_dir와 동일 규칙) —
    CC 학습 탭 미러링용(B-10: 프로젝트 _round/rsi와 데몬 읽기 경로의 단절 해소)."""
    root = os.environ.get("EZER_ROUND_DIR")
    if root:
        return os.path.join(root, "learn")
    pack = os.environ.get("EZER_PACK_DIR") or os.path.expanduser("~/.ezer/pack")
...
def _recommend_learn(reason, topic):
    ...
    승인·directive §4). ezer 부재·데몬 미가동·오류는 무시(추천은 비핵심 부가 신호 — 핵심 판정 불간섭)."""
    import shutil
    if not shutil.which("ezer"):
        return
    ...
    try:
        subprocess.run(["ezer", "feed", "push", "--kind", "learn_proposal",
...
def cmd_progress(a):
    ...
    _append_ledger(entry)
    print(json.dumps(entry, ensure_ascii=False))
    if r["flat_streak"] >= int(os.environ.get("EZER_RSI_CEILING_FLATS", "3")):
