#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ezer_cleanroom — _research 흡수 보고서의 MPL/라이선스 클린룸 4원칙 헤더 검증·삽입.
...
서브커맨드:
    ezer_cleanroom.py --self-test               # → exit 0, {"self_test":"ok"} (검증로직 자기검증)
    ezer_cleanroom.py check  --root <_research>  # → {"ok":bool,"missing":[...],"broken":[...]}  exit 1 if any
    ezer_cleanroom.py fix    --root <_research>  # → 누락 헤더 삽입(라이선스 자동탐지), {"fixed":[...]}

AGENTREACH OPP-19/OPP-20 통합(새 도구 안 만듦 — 본 도구에 1-체크 통합):
    ezer_cleanroom.py vendor-check    --skills <skills_dir>  # 벤더링 스냅샷 해시핀 드리프트 감지 (NEVER-modify-upstream 자동강제)
    ezer_cleanroom.py vendor-snapshot --skills <skills_dir>  # baseline 해시 재생성 (owner 승인 게이트 — denylist ②)
    ezer_cleanroom.py license-check   --skills <skills_dir>  # THIRD_PARTY/NOTICE 라이선스 추적 + AGPL copyleft 게이트

종료 코드: 0 = 정합/수리 완료, 1 = missing/broken/drifted/license 위반(check) 또는 self-test 실패,
            3 = vendor-snapshot 승인대기(EZER_VENDOR_SNAPSHOT_OWNER_APPROVED 미설정).
"""
...
def cmd_vendor_snapshot(skills_dir):
    if os.environ.get("EZER_VENDOR_SNAPSHOT_OWNER_APPROVED", "") not in ("1", "true", "yes"):
        sys.stderr.write("[CLEANROOM] baseline 갱신 = 변조 정상승인 = 오너/owner 승인 필요"
                         "(denylist ②). EZER_VENDOR_SNAPSHOT_OWNER_APPROVED=1 후 재실행.\n")
        return 3
    try:
        m = load_manifest(skills_dir)
    except ManifestError as e:
...
def render_header(spdx, lic_ref):
    ...
    return "\n".join([
        ...
        "> ① **코드복사 0** — 대상 repo 소스 텍스트를 ezer 트리에 복사하지 않는다"
        ...
        "> ④ **복사 아님 명시** — 본 보고서의 모든 ezer 매핑은 재구현이며 복제가 아님.",
...
def _self_test_vendor():
    ...
    try:
        ...
        old = os.environ.pop("EZER_VENDOR_SNAPSHOT_OWNER_APPROVED", None)
        _stderr = sys.stderr
        try:
            ...
            sys.stderr = _stderr
            if old is not None:
                os.environ["EZER_VENDOR_SNAPSHOT_OWNER_APPROVED"] = old
        if rc_snap != 3:
...
def main():
    ...
    # OPP-19/20 — 벤더링 무결성 + 라이선스 추적(--skills = ezer-pack/skills).
