...
격리 ezerd 로 실측: exited=true 잔재를 Reap 사유(ezer close-surface --reap → CloseCause::Reap)로만 회수 —
라이브 오회수 0·회수분 묘비 0(desired·topology 불변). 바이너리 미발견 시 skip(exit 0). 라이브 무접촉.

실행: python3 ezer-pack/bin/tests/test_phoenix_c6_reap.py
"""
...
EZERD, EZER = _find("ezerd"), _find("ezer")
if not (EZERD and EZER):
    print("SKIP: ezerd/ezer 미발견(빌드 필요) — CI 게이트는 빌드 후 실행. skip(exit 0).")
    sys.exit(0)

os.environ["PHOENIX_HARNESS_EZERD"] = EZERD
os.environ["PATH"] = DBG + ":" + os.environ.get("PATH", "")
os.environ["PHOENIX_EZER"] = EZER
spec = importlib.util.spec_from_file_location("h", os.path.join(HERE, "..", "ezer_phoenix_harness.py"))
h = importlib.util.module_from_spec(spec); spec.loader.exec_module(h)
h.EZER = EZER; h.EZERD = EZERD
h.guard_isolation()

PH = os.path.join(HERE, "..", "ezer_phoenix.py")
results = []
def check(n, c, d=""):
...
def _run_phoenix(args, timeout=60):
    import subprocess
    env = dict(os.environ); env["PHOENIX_EZER"] = EZER
...
def main():
    ...
    try:
        ...
        live = h.ezer("new-surface").stdout or ""
        live_ref = (re.search(r"(surface:\d+)", live) or [None])[0] if re.search(r"(surface:\d+)", live) else None
        if live_ref:
            h.ezer("send", "--surface", live_ref, "sleep 3600"); h.ezer("send-key", "--surface", live_ref, "Return")
        stale = h.ezer("new-surface").stdout or ""
        stale_ref = re.search(r"(surface:\d+)", stale)
        stale_ref = stale_ref.group(1) if stale_ref else None
        if stale_ref:
            h.ezer("send", "--surface", stale_ref, "exit"); h.ezer("send-key", "--surface", stale_ref, "Return")
            time.sleep(2.0)
        lst_before = h.ezer("list").stdout or ""
        ...
        lst_after = h.ezer("list").stdout or ""
