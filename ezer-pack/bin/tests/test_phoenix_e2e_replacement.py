...
격리 상태 디렉터리에서 실 ezerd 를 기동해 auto-restore(임베드 추출→phoenix restore --auto)가 데몬 자신의
소켓/상태를 타는지(W6 --socket enabler), 그리고 세 게이트 시나리오를 실측한다:
  ① no-target-roles → NOOP(fresh live)
  ② PHOENIX_STRICT_EZER=1 → Rust PHOENIX_EZER/PATH 주입으로 phoenix 해석 성공(폴백 아님·exit6 아님)
  ③ 재기동 후 seeded dead role → 부활 대상 타겟팅(kill→재기동→부활 경로 발화)

바이너리(ezerd/ezer)는 PHOENIX_HARNESS_EZERD·PATH 또는 <repo>/target/debug 에서 찾는다. 없으면 SKIP(exit 0) —
CI 는 빌드 후 실행(게이트), 로컬 무빌드 실행은 skip. 라이브 무접촉(guard_isolation·격리 소켓 전용).

실행: python3 ezer-pack/bin/tests/test_phoenix_e2e_replacement.py
"""
...
if not (EZERD and EZER):
    print("SKIP: ezerd/ezer 바이너리 미발견(빌드 필요) — CI 게이트는 빌드 후 실행. skip(exit 0).")
    sys.exit(0)

os.environ["PHOENIX_HARNESS_EZERD"] = EZERD
os.environ["PATH"] = DBG + ":" + os.environ.get("PATH", "")
os.environ["PHOENIX_EZER"] = EZER  # 하네스 self 대조용(ezerd 는 자체 주입)
os.environ["EZER_NO_AUTORESTORE"] = "0"  # ★E1 은 ezerd auto-restore 를 검증하므로 명시 활성화(하네스 기본=비활성)

HARNMOD = os.path.join(HERE, "..", "ezer_phoenix_harness.py")
spec = importlib.util.spec_from_file_location("h", HARNMOD)
h = importlib.util.module_from_spec(spec); spec.loader.exec_module(h)
h.EZER = EZER; h.EZERD = EZERD
h.guard_isolation()
...
def _boot_and_capture(wait=20.0, want="티켓="):
    """★교체 시뮬레이션: 기존 데몬 kill → 새 ezerd 기동(=교체) → auto-restore 가 phoenix-restore.log 에 판정을
...
def main():
    ...
    # ── ② PHOENIX_STRICT_EZER=1 → Rust 주입으로 해석 성공(폴백 차단인데도 phoenix 가 돎·exit6/FATAL 없음) ──
    _wipe_state()
