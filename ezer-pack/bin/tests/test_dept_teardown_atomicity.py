...
기능시험(가짜 HOME·스텁 ezer·스텁 phoenix — 실 데몬/실 phoenix 무접촉):
...
정적 트립와이어 핀(배선 소실 검출 — 기능시험이 무거운 생성 경로용):
  launch/allocate/create의 dept_tombstone_remove 배선 · rotate의 EZER_DEPT_ROTATE=1 가드 ·
  helper의 --remove 플래그·rotate 가드. (launch/allocate/create 기능시험은 실 ezerd 필요 — CI 통합 영역.)
"""
import json
import os
import shutil
...
DEPT = os.path.join(SELF, "..", "ezer-dept")
fails = []
...
def setup(tmp, reg_depts):
    ...
    reg = os.path.join(home, ".ezer", "depts.json")
    os.makedirs(os.path.dirname(reg), exist_ok=True)
    with open(reg, "w", encoding="utf-8") as f:
        json.dump({"depts": reg_depts}, f)
    # 스텁 ezer: identify 실패(pid 빈값 → kill 생략)·기타 성공·전 호출 기록(A4 데몬 묘비 핀용)
    with open(os.path.join(bindir, "ezer"), "w", encoding="utf-8", newline="\n") as f:
        f.write("#!/bin/sh\necho \"ezer $@\" >> \"%s/calls.log\"\n"
                "case \"$1\" in identify) exit 1;; esac\nexit 0\n" % tmp)
    os.chmod(os.path.join(bindir, "ezer"), 0o755)
    # 스텁 phoenix: 인자 기록만
    with open(os.path.join(fakepack, "ezer_phoenix.py"), "w", encoding="utf-8", newline="\n") as f:
        f.write("import sys\nopen(%r, 'a').write(' '.join(sys.argv[1:]) + '\\n')\n"
                % os.path.join(tmp, "phoenix.log"))
    env = dict(os.environ)
    env.update({"HOME": home, "EZER_DEPTS_JSON": reg,
                "EZER_PACK_DIR": os.path.join(tmp, "fakepack"),
                "PATH": bindir + os.pathsep + env.get("PATH", "")})
    for k in ("EZER_ROLE", "EZER_SOCKET"):
        env.pop(k, None)
    return env, home, reg
...
sockA = "/tmp/x/ezer-dept-dept-3/ezer.sock"
...
code, out = run(env, "down-sock", "/tmp/x/ezer-dept-dept-7/ezer.sock")
...
code, out = run(env, "down-sock", r"\\.\pipe\ezer-dept-dept-9")
...
check("W2 rotate 가드(export)", "EZER_DEPT_ROTATE=1 bash \"$0\" launch" in src)
check("W3 helper rotate 가드", '[ "${EZER_DEPT_ROTATE:-}" = "1" ] && return 0' in src)
check("W4 helper --remove", "--dept --remove" in src)
check("W5 D8 파생 로직", "ezer-dept-[^/]*" in src)
# ★D-IMPL-2 대칭 핀: phoenix 묘비와 데몬 묘비는 set/remove가 항상 쌍으로 — 한쪽만 있으면
# "삭제→재생성→재시작 시 새 부서 살해"(데몬 묘비 잔존) 또는 부활 구멍(데몬 묘비 미기록).
check("W6 데몬 묘비 set 병행", '"$EZER" tombstone "$1" --dept' in src)
check("W7 데몬 묘비 remove 병행", '"$EZER" tombstone "$1" --dept --remove' in src)
