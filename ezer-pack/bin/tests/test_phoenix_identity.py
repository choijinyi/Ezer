...
실행: python3 ezer-pack/bin/tests/test_phoenix_identity.py  (0=전건 PASS)
"""
import contextlib
...
PH = os.path.normpath(os.path.join(HERE, "..", "ezer_phoenix.py"))

spec = importlib.util.spec_from_file_location("ezer_phoenix", PH)
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)

SELF = {"build_id": "abc123", "embedded_pack_hash": "H_SELF", "protocol_version": "1", "version": "0.12.20"}
FAKE_EZER = os.path.join(tempfile.gettempdir(), "fake-ezer-identity-test")
...
def _isolated_socket(tmp):
    ...
    return os.path.join(tmp, "ezer.sock")


def _resolve_expect_exit6(socket, phoenix_ezer, self_id, daemon_id):
    """PHOENIX_EZER 경로로 _resolve_ezer 호출 → SystemExit(6) 기대. (code, stderr) 반환."""
    m._EZER_IDENTITY = None
    err = io.StringIO()
    old = os.environ.get("PHOENIX_EZER")
    os.environ["PHOENIX_EZER"] = phoenix_ezer
    # X_OK/isfile 통과 강제(실 파일 없이도).
    oi, oa = m.os.path.isfile, m.os.access
    m.os.path.isfile = lambda p: True if p == phoenix_ezer else oi(p)
    m.os.access = lambda p, mode: True if p == phoenix_ezer else oa(p, mode)
    # 재시도 sleep 0(테스트 속도).
    m._IDENTITY_RETRY_SLEEP = 0.0
    code = None
    try:
        with _mock_subprocess(self_id, daemon_id), contextlib.redirect_stderr(err):
            m._resolve_ezer(socket)
    except SystemExit as e:
        code = e.code
    finally:
        m.os.path.isfile, m.os.access = oi, oa
        if old is None:
            os.environ.pop("PHOENIX_EZER", None)
        else:
            os.environ["PHOENIX_EZER"] = old
...
def main():
    ...
    # ── A. _ezer_identity_check 필드별 mismatch(순수 대조) ──
    with _mock_subprocess(SELF, dict(SELF)):
        st, fld, _ = m._ezer_identity_check(FAKE_EZER, socket)
    ...
    with _mock_subprocess(None, dict(SELF)):
        st, _, _ = m._ezer_identity_check(FAKE_EZER, socket)
    ...
    # ── B. _resolve_ezer 필드별 mismatch → exit 6 + 필드명 stderr ──
    for field, bad in [("build_id", "DIFF"), ("embedded_pack_hash", "H_OTHER"), ("protocol_version", "9")]:
        dmn = dict(SELF); dmn[field] = bad
        code, err = _resolve_expect_exit6(socket, FAKE_EZER, SELF, dmn)
    ...
    code, err = _resolve_expect_exit6(socket, FAKE_EZER, SELF, {"build_id": "abc123", "protocol_version": "1"})
    check("B legacy 필드부재 → exit 6", code == 6 and "embedded_pack_hash" in err,
          "code=%s" % code)

    # ── C. inconclusive → degraded 채택(exit 없음) + ezer_identity + 저널 기록 ──
    m._EZER_IDENTITY = None
    m._IDENTITY_RETRY_SLEEP = 0.0
    oi, oa = m.os.path.isfile, m.os.access
    m.os.path.isfile = lambda p: True if p == FAKE_EZER else oi(p)
    m.os.access = lambda p, mode: True if p == FAKE_EZER else oa(p, mode)
    os.environ["PHOENIX_EZER"] = FAKE_EZER
    err = io.StringIO()
    got = None
    try:
        with _mock_subprocess(SELF, None), contextlib.redirect_stderr(err):  # 데몬 미도달=inconclusive
            got = m._resolve_ezer(socket)
    except SystemExit as e:
        got = "EXIT:%s" % e.code
    finally:
        m.os.path.isfile, m.os.access = oi, oa
        os.environ.pop("PHOENIX_EZER", None)
    check("C inconclusive → 채택(exit 없음)", got == FAKE_EZER, "got=%r" % got)
    check("C ezer_identity=degraded-unverified", m._EZER_IDENTITY == "degraded-unverified", str(m._EZER_IDENTITY))
    ...
    if os.path.exists(rj):
        try:
            ...
            recorded = any(e.get("stage") == "resolve_ezer" and e.get("status") == "degraded" for e in ev)
        except Exception:
            recorded = False
    check("C degraded 저널 기록", recorded, "journal-resolve.json resolve_ezer/degraded")

    # ── D. gate2 BLOCKING: _which('ezer') PATH 후보도 identity 게이트를 우회하지 않는다 ──
    #    PHOENIX_EZER 없이 which 가 후보를 잡을 때, mismatch → exit 6(필드명). (과거엔 곧바로 return 으로 우회했다)
    os.environ.pop("PHOENIX_EZER", None)
    ow = m._which
    m._which = lambda name: FAKE_EZER if name == "ezer" else ow(name)
    m._IDENTITY_RETRY_SLEEP = 0.0
    ...
    try:
        ...
        m._which = ow
    check("D PATH(which) 후보 match → 채택+verified", got == FAKE_EZER and m._EZER_IDENTITY == "verified",
          "got=%r identity=%s" % (got, m._EZER_IDENTITY))

    # ── E. W4 STRICT 하위케이스(codex W4 fix3): PHOENIX_STRICT_EZER=1 positive(주입 정상=성공)·
    #    negative(주입 부재=표준경로 폴백 차단 exit 6). B1 임베드 실행이 의존하는 ezer 해석을 STRICT 로 봉인 —
    #    B1 폴백이 Rust PHOENIX_EZER/PATH 주입 누락을 가리는 false-green 차단.
    os.environ["PHOENIX_STRICT_EZER"] = "1"
    # positive: STRICT + PHOENIX_EZER(X_OK·주입 정상) → 채택(폴백 불필요·성공, exit 없음).
    m._EZER_IDENTITY = None; m._IDENTITY_RETRY_SLEEP = 0.0
    os.environ["PHOENIX_EZER"] = FAKE_EZER
    oi, oa = m.os.path.isfile, m.os.access
    m.os.path.isfile = lambda p: True if p == FAKE_EZER else oi(p)
    m.os.access = lambda p, mode: True if p == FAKE_EZER else oa(p, mode)
    ...
    try:
        ...
        m.os.path.isfile, m.os.access = oi, oa
        os.environ.pop("PHOENIX_EZER", None)
    check("E STRICT + PHOENIX_EZER 주입 정상 → 성공(채택·exit 없음)", got == FAKE_EZER,
          "got=%r identity=%s" % (got, m._EZER_IDENTITY))
    # negative: STRICT + PHOENIX_EZER 부재 + which=None → 표준경로 폴백 강제 차단 exit 6.
    ow = m._which
    ...
    try:
        ...
        m._which = ow
        os.environ.pop("PHOENIX_STRICT_EZER", None)
