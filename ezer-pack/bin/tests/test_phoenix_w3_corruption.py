...
데몬 불요: EZER 를 더미로 두거나 m.ezer/헬퍼를 몽키패치해 파일·순수 로직만 검증한다(라이브 무접촉).

실행: python3 ezer-pack/bin/tests/test_phoenix_w3_corruption.py  (0=전건 PASS)
"""
...
def _ph_home(td, name):
    ...
    return os.path.join(sd, "ezer.sock"), home
...
def c2_bak_fallback(td):
    ...
    m.EZER = os.path.join(td, "nonexistent-ezer")  # ezer 전부 rc127 강등 → live 없음
...
def c2_snapshot_fallback(td):
    ...
    os.makedirs(home, exist_ok=True)
    sock = os.path.join(live, "ezer.sock")
    dp = os.path.join(home, "desired_roster.json")
    _write(dp, "]]] corrupt no bak")
    gen = os.path.join(td, ".ezer", "state-generations", "20260706T120000Z")
    ...
    try:
        m.LIVE_STATE = live
        m.HOME = td
        m.EZER = os.path.join(td, "nonexistent-ezer")
...
def c5_liveness_from_status_json(td):
    """live_role_surfaces 가 `ezer status --json`.surfaces(구조화)에서 role/exited 를 읽는다."""
    saved = m.ezer
    status = {"surfaces": [
...
def c5_liveness_fallback_to_list(td):
    """status --json 미도달(rc≠0) 시 `ezer list` 정규식 폴백(가용성 하한)."""
    saved = m.ezer
    def fake_ezer(*args, **kw):
        if args[:2] == ("status", "--json"):
...
def c5_harness_allow_live_gate():
    """하네스 guard_isolation: LIVE 타깃은 EZER_PHOENIX_ALLOW_LIVE=1 없으면 거부, 있으면 통과."""
    HPH = os.path.normpath(os.path.join(HERE, "..", "ezer_phoenix_harness.py"))
    hspec = importlib.util.spec_from_file_location("ezer_phoenix_harness", HPH)
    h = importlib.util.module_from_spec(hspec)
    hspec.loader.exec_module(h)
    saved_hd, saved_sock = h.HARN_DIR, h.HARN_SOCK
    saved_env = os.environ.get("EZER_PHOENIX_ALLOW_LIVE")
    try:
        h.HARN_DIR = h.LIVE_STATE          # 강제 LIVE 타깃(사고/오설정 시뮬)
        h.HARN_SOCK = h.LIVE_SOCK
        os.environ.pop("EZER_PHOENIX_ALLOW_LIVE", None)
        refused = False
        try:
            h.guard_isolation()
        except SystemExit:
        ...
        os.environ["EZER_PHOENIX_ALLOW_LIVE"] = "1"
        allowed = True
        try:
        ...
        if saved_env is None:
            ...
            os.environ["EZER_PHOENIX_ALLOW_LIVE"] = saved_env
...
def c2_degraded_persists_across_retry(td):
    """★codex BLOCKING(1): 손상 복구→DEGRADED 후, ezerd auto-retry 상당 2차 실행에서도 보류 유지(휘발 금지).
