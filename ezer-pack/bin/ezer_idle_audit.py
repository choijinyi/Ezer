...
판정 4조건(전부 충족해야 "정리 후보"):
  ① ezer 노드 레지스트리 미등록  — ezer list 등록 pid(부모 체인 포함)와 무관
  ② 라이브 surface 미매핑        — ezer status --json 라이브 surface에도 미매핑
  ③ CPU 임계 미만                — 기본 1.0% 미만
...
사용:
  python3 ezer_idle_audit.py [--idle-hours 6] [--cpu-max 1.0] [--json]
  python3 ezer_idle_audit.py --self-test
"""
import argparse
import json
import os
...
EZER_TIMEOUT = 10          # 초 — ezer 서브프로세스 타임아웃
...
def collect_procs():
    ...
    try:
        out = subprocess.run(
            ...
            capture_output=True, text=True, timeout=EZER_TIMEOUT,
        ).stdout
...
def _run_ezer(args):
    """ezer 서브커맨드 실행. (ok, stdout). 실패·부재면 (False, '')."""
    try:
        r = subprocess.run(['ezer'] + args, capture_output=True,
                           text=True, timeout=EZER_TIMEOUT)
    except (OSError, subprocess.TimeoutExpired):
        return False, ''
    if r.returncode != 0:
        return False, ''
...
def parse_ezer_list(text):
    """`ezer list` 출력을 {pid: surface_ref}로. `pid=NNN`·`surface:NN` 파싱."""
    reg = {}
...
def parse_ezer_status_refs(text):
    """`ezer status --json`에서 exited=false 라이브 surface_ref 집합. 실패면 None."""
    try:
...
def load_registry():
    ...
    반환 dict:
      available   — ezer list 성공 여부(False면 ①②는 판정 불가)
      pid_to_ref  — {등록 pid: surface_ref}
      live_refs   — 라이브 surface_ref 집합 (status 실패 시 None)
    """
    ok, out = _run_ezer(['list'])
    pid_to_ref = parse_ezer_list(out) if ok else {}
    ok_st, out_st = _run_ezer(['status', '--json'])
    live_refs = parse_ezer_status_refs(out_st) if ok_st else None
...
def judge(proc, all_by_pid, registry, idle_hours, cpu_max):
    ...
    if not registry['available']:
        ...
        c1 = {'met': None, 'why': 'ezer list 조회 실패 — 등록 여부 불명'}
        c2 = {'met': None, 'why': 'ezer 레지스트리 없음 — surface 매핑 불명'}
    else:
        c1 = {'met': anc is None,
        ...
        if anc is None:
            ...
            ref = reg_pids[anc]
            # status 성공 시 라이브 교차확인, 실패 시 ezer list를 라이브로 간주
...
def render_text(result):
    ...
    out.append(f"  임계: 유휴 > {result['idle_hours']}h · CPU < {result['cpu_max']}%"
               f"  | ezer 레지스트리={'OK' if result['registry_available'] else '조회실패'}"
               f" · 라이브 surface={'OK' if result['live_refs_available'] else 'N/A'}")
    if not result['registry_available']:
        out.append('  ⚠ ezer list 조회 실패 → 등록 여부 판정 불가 → 전 대상 보류(unknown) 처리')
    out.append('')
