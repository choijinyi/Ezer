...
> "본 레포를 ezer/ezer-agent 평가 게이트에 통합할 때, **'코드가 통과한다'≠'수치가 검증됐다'**를
...
`eval-driven-self-improvement` 스킬(`../../../../ezer-rsi-skills/eval-driven-self-improvement/SKILL.md`)
...
Rule 1(SHA256 manifest pin + evaluator-owned launcher 가 locked git ref 에서 추출)은 ezer-agent
...
ezer 철학 ⑤(deny-by-default)·적대검증 "자가보고 우위 신뢰 금지" 의 결정론 구현:
...
- exit code 가 사실(ezer 철학 ②): `gate_pass=True`→exit 0, gate fail→exit 1, 입력/런타임
...
- **순수 stdlib·numpy 의존 없음**: 지표 산술을 list 수학으로 구현. ezer 철학 ③(stdlib 우선,
...
- LOCKED-ref 암호학적 핀(SHA256 manifest + evaluator-owned launcher): ezer-agent PACK 배포층
