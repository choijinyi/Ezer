...
description: "도메인 한 문장을 검증된·비용통제된·재개가능한 풀스택 Claude 프리미티브 하네스(orchestrator skill + .claude/agents + Sub-agents + Agent Teams + Hooks + .claude/skills + 장기기억)로 변환하는 메타스킬(EZER Harness Creator). '하네스 만들어줘/구성/설계', '에이전트 팀 설계', '이 도메인 자동화', '하네스 점검·감사·확장·진화·동기화' 요청 시 사용. 후속: 재실행·수정·보완·부분재생성·이전결과개선·드리프트수정·유지보수 요청 시에도 반드시 이 스킬을 사용."
---

# Harness Creator (EZER) — 메타스킬
...
## 워크플로우 — 4 스테이지 (AWF Research→Planning→Implementation→Evolution ⊃ choijinyi 단계 ⊃ EZER 게이트)
...
- **R1 상태감사** — `audit_harness.py <TARGET>` → `.harness/audit.json {branch ∈ new/extend/maintain, drift[]}`. drift = 디스크 agents/skills ↔ graph 계약의 결정론 set-diff(choijinyi는 산문, EZER는 검증 가능한 사실).
...
- **P5 ⛔ 사람 승인 게이트 (AWF + EZER)** — 계획(토폴로지·에이전트·`skill_authoring`·R1 드리프트) + 비용밴드를 제시 → 사용자 승인 대기. **Implementation은 승인 전 실행 금지.**
...
### STAGE 4 — EVOLUTION (실행 + 학습; choijinyi Phase 7 + EZER 측정)
