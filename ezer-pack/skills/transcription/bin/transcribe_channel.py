...
분리 · probe 자연폴백)만 흡수해 ezer 인프라로 재구현한다(AR 소스 복붙 0):

  - provider 랭킹 = 기존 **ezer_select**(새 선택엔진 0). transcription capability의
    `ytdlp_subs`(무키 free 자막)가 `transcriber_whisperx`(무키 로컬 ASR) 위에 있되,
    자막 부재/단어 타임스탬프 필요 시 **자연 폴백**(if문 분기 0, ezer_select가 결정).
...
def _pack_dir() -> str:
    env = os.environ.get("EZER_PACK_DIR")
    if env:
...
def _import_deps():
    pack = _pack_dir()
    ezer_select = _load("ezer_select", os.path.join(pack, "bin", "ezer_select.py"))
...
def transcribe(source, *, intent="", want_words=False, prefer=None,
    ...
    """URL/path → transcript.json dict. provider 분기는 ezer_select가 결정한다.

    deps=(ezer_select, phase0) 주입 가능(테스트 모킹). whisperx=콜러블 주입 가능
    ...
    ezer_select, phase0 = deps if deps else _import_deps()
    catalog = ezer_select.load_catalog(catalog_path or _default_catalog())
    ...
    ranked, unavailable, _forced = ezer_select.rank(
    ...
    for prov in ranked:                      # PHIL-02 ordered candidate list (dict, prov["id"])
        ...
        if pid == "transcriber_whisperx":
            env = _utf8_env(ezer_select, phase0)
            if src_kind == "file":
                media = source
                tmp = None
            else:
