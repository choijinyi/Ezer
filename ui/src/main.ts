// EZERagent UI — xterm.js panes over the EZERagentd socket (thin client).
...
interface Workspace {
  ...
  pending?: boolean;
  // 06: 소속 그룹 id(undefined=ungrouped). 부서 ws도 그룹에 들어가면 set. 진실원=localStorage(EZERagent-layout-v2).
  groupId?: number;
}

// 06: 워크스페이스 그룹 메타데이터. 진실원=localStorage(EZERagent-layout-v2). 데몬은 모름(그룹=UI/solution 층).
...
const LAYOUT_KEY = "EZERagent-layout-v2";
...
let ccDensity: CcDensity =
  (localStorage.getItem("EZERagent-cc-density") as CcDensity) === "glance" ? "glance" : "ops";
// Tasks Control Center: Glance 모드 안에서 보여줄 면(Live=시스템부하 ↔ tasks=부서 업무) — 오너 선택.
let ccGlanceFace: "live" | "tasks" =
  localStorage.getItem("EZERagent-cc-glance-face") === "tasks" ? "tasks" : "live";
...
function applyCcDensity(mode: CcDensity) {
  ...
  localStorage.setItem("EZERagent-cc-density", mode);
...
function applyGlanceFace(face: "live" | "tasks") {
  ccGlanceFace = face;
  localStorage.setItem("EZERagent-cc-glance-face", face);
...
async function refreshBoard() {
  ...
  outHost.innerHTML =
    !dirs || dirs.length === 0
      ? `<div class="cc-empty">산출물 없음 (~/.EZERagent/_round/skill-out)</div>`
      : dirs
          .map((x: any) => {
            const p = x.path ?? "";
            const nm = x.name ?? p;
...
function taskRow(s: any, deptKey: string): string {
  ...
  const trust = selfReport
    ? `<span class="cc-trust-badge self" title="노드가 EZERagent set-status로 직접 보고한 상태">📍자기보고</span>`
...
async function runSkillButton(s: any) {
  ...
  try {
    ...
    // ★무계약 차단: task-prompt 티켓을 먼저 생성(EZERagent_orchestra 경유). UI는 ticket 텍스트만 받는다.
...
async function refreshLearn() {
  ...
  document.getElementById("cc-learn-timeline")!.innerHTML = keys.length
    ? keys
        ...
        .join("")
    : `<div class="cc-empty">학습 라운드 기록 없음 — RSI 라운드(EZERagent_rsi.py checkpoint)가 기록을 남기면 여기 표시됩니다</div>`;
...
function renderControlCenter(d: any) {
  ...
  document.getElementById("cc-footer")!.textContent =
    `EZERagent Control Center · v${d.version ?? ""} · 대시보드 5초 · 하드웨어 2초 갱신`;
}
...
let fontSize = Number(localStorage.getItem("EZERagent-font-size") || 13);
function applyZoom(delta: number | null) {
  fontSize = delta === null ? 13 : Math.min(32, Math.max(8, fontSize + delta));
  localStorage.setItem("EZERagent-font-size", String(fontSize));
...
// 터미널 폰트 선택(EZERagent-font-face · 오너 요청 2026-07-12) — 선택 폰트를 기본 스택 앞에 합성
// (composeFontFamily · CJK 폴백 보존), null=기본. 폰트 메트릭 변화 → 셀 재계산(applyZoom과 동일 패턴).
let fontFace: string | null = localStorage.getItem("EZERagent-font-face");
function applyFontFace(face: string | null) {
  fontFace = face && face.trim() ? face : null;
  if (fontFace === null) localStorage.removeItem("EZERagent-font-face");
  else localStorage.setItem("EZERagent-font-face", fontFace);
  for (const rt of panes.values()) {
...
let panelZoom = Math.min(2, Math.max(0.6, Number(localStorage.getItem("EZERagent-panel-zoom")) || 1)); // NaN·범위밖 방어
...
function applyPanelZoom(delta: number | null) {
  ...
  localStorage.setItem("EZERagent-panel-zoom", String(panelZoom));
  applyPanelZoomVar();
}
...
let bgColor: string | null = localStorage.getItem("EZERagent-bg-color");
...
function applyBgColor(color: string | null): void {
  ...
  if (color === null) localStorage.removeItem("EZERagent-bg-color");
  else localStorage.setItem("EZERagent-bg-color", color);
}
...
// + 외부(CLI launch-agent·EZERagent boot)에서 생성된 역할 노드 surface를 pane으로 자동 입양 —
...
async function makePane(sid: number, title: string, socket?: string): Promise<PaneRuntime> {
  ...
  // 여기서는 DOM 이벤트를 리듀서에 배선만 한다. 계측: localStorage.EZERagentImeDebug="1" 또는 파일
  // 게이트(~/.EZERagent/ime-debug)/EZERAGENT_IME_DEBUG=1 시 이벤트 시퀀스를 log_ime로 기록(유실 경로를
  // 결정론으로 확정하는 채널 — 릴리스 빌드엔 devtools가 없어 파일 게이트가 최종 사용자 진단 경로). 평시 비용 0.
  let imeDbg = localStorage.getItem("EZERagentImeDebug") === "1";
  ...
  // ★프로필 D 유출 감지(EZERagent-neo, macOS 26.5.1 WKWebView): xterm의 Terminal._inputEvent가
...
// 부서 socket 경로에서 원래 부서명 역산 — unix(~/.local/state/EZERagent-dept-<name>/EZERagent.sock)와
// ★Windows named pipe(\\.\pipe\EZERagent-dept-<name> — RC-4 규약·dept_socket_path 정합) 양쪽 지원(2026-07-10).
...
function deptNameFromSocket(sock: string | undefined): string | null {
  const m = /\/EZERagent-dept-(.+?)\/EZERagent\.sock$/.exec(sock ?? "");
  if (m) return m[1];
  const w = /^\\\\\.\\pipe\\EZERagent-dept-(.+)$/.exec(sock ?? "");
  return w ? w[1] : null;
}

// 멀티마스터 F4: 새 '부서 workspace' 런칭 = 새 부서 데몬 spawn(EZERagent-dept launch 단일 진입점).
// 첫 부서가 생기면 백엔드(EZERagent-dept)가 기본 데몬을 CEO로 자동 승격한다.
// ① 표시 지연(안 C): 무거운 launch await(최대 ~12s) '전에' placeholder 탭을 즉시 render — 체감 지연 0.
// ② 고아 방지(안 A): 빈 newSurface를 만들지 않는다. EZERagent-dept가 띄우는 role=master surface가
...
async function addDeptWorkspace(catalogKey?: string): Promise<Workspace> {
  ...
  try {
    ...
    // 안 A(C4 더블 surface 해소): EZERagent-dept(create·allocate 모두 role=master '빈 셸' — WP-11 일원화)가 부서장
...
async function promptBinaryPatch() {
  ...
  const ok = await confirmModal(
    ...
    `새 본체(앱) ${v}을 패치 방식으로 설치합니다: 저장(drain) 신호 후 다운로드·서명 검증·교체하고 앱을 ` +
      ...
      `있습니다.\n\n지금 설치하시겠습니까? (수동 설치는 홈페이지 www.kaea.ai.kr)`,
    "설치",
  );
  if (!ok) return;
  try {
...
// (rotate_dept_daemon=EZERagent-dept rotate=데몬 프로세스만 재기동·레지스트리·묘비·CEO 불변).
...
// 흡수: 팔레트 메커니즘(모달·fuzzy·키 라우팅)=webview primitive. 액션 큐레이션(역할 점프·재기동·60% cycle·feed 승인)=EZERagent 처방 solution.
...
async function buildPaletteItems(): Promise<PaletteItem[]> {
  ...
  const RESTART: Record<string, string> = {
    cso: "EZERagent launch-agent --role cso --agent claude",
    worker: "EZERagent launch-agent --role worker --agent claude",
  ...
  // ── (4-b) ★R8(WP-2): CEO 승격 대기 해소 — EZERagent-dept PENDING(부트 게이트 보류)의 즉시 경로.
...
async function start() {
  ...
  await new Promise<void>((resolve) => {
    ...
    listen("daemon-retry-hint", () => {
      stickyToast(
        ...
        "health",
        "데몬 시작 대기 중",
        "백그라운드 서비스(EZERagentd) 시작을 기다리고 있습니다. 계속 이 상태면: 시스템 설정 → 일반 → 로그인 항목에서 EZERagent 관련 항목을 허용해 주세요. 허용 즉시 자동으로 연결됩니다.",
      );
    });
  ...
  await listen("perm-warning", (e) => {
    ...
    stickyToast(
      ...
      "health",
      `⚠ macOS ${f} 폴더 접근 차단`,
      `pane 안의 claude 등이 EPERM으로 꺼질 수 있습니다 — 시스템 설정 → 개인정보 보호 및 보안 → 파일 및 폴더(또는 전체 디스크 접근 권한)에서 EZERagent를 허용한 뒤 앱을 재시작하세요.`,
    );
  });
  ...
  // 부서 데몬 확보를 list 대조보다 선행 — 미가동이면 EZERagent-dept launch. 실패해도(등록된) ws는 보존.
...
document.getElementById("btn-install-cli")?.addEventListener("click", async () => {
  try {
    const r = (await invoke("install_cli_to_path")) as {
      EZERagent_link: string; EZERagentd_link: string; shadowed_by: string | null; warnings: string[];
    };
    // B-11: alert()는 WKWebView에서 억제될 수 있음(confirm() 무동작 실측과 동계열) — toast로 통일
    let msg = `${r.EZERagent_link} · ${r.EZERagentd_link} — 새 터미널에서 'EZERagent' 사용 가능`;
...
// 새 ws를 master로 선언 시 공유 데몬 claim 충돌은 데몬 레벨 claim_denied(EZERagentd handlers.rs·kill 없음)가
...
// ★WP-1 결정 e(BOOTSTRAP_HARDENING v1.1): "마스터 시작" — EZERagent launch-agent --role master 배선.
// worker/cso 기동과 동일 메커니즘(앵커: 시스템은 노드만 띄우고 지휘하지 않는다). 초보를 "올바른
// surface에서 마법 문구 입력"이라는 취약한 산문 계약에서 해방. 명령 자체가 base 데몬 고정
// (EZERAGENT_SOCKET 제거 — start_master)이라 어느 탭에서 눌러도 부서 오염 불가. 생성 surface는 자동입양.
...
let wsbarW = clampWsbarWidth(Number(localStorage.getItem("EZERagent-wsbar-w")) || WSBAR_W_DEFAULT);
let wsbarFont = clampWsbarFont(Number(localStorage.getItem("EZERagent-wsbar-font")) || 1);
...
function applyWsbarFontStep(dir: number) {
  ...
  applyWsbarVars();
  localStorage.setItem("EZERagent-wsbar-font", String(wsbarFont));
}
